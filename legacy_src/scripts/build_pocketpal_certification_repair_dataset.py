...
def _payload(action: str, content: str, metadata: dict[str, Any] | None = None) -> str:
    proposal = {"task_type": "certification_repair"}
    ...
    rows.append(
        {
            ...
            "split": split,
            "source_type": "pocketpal_certification_repair",
            "source_id": task_type,
            "task_type": task_type,
            "encoder_text": prompt,
...
def _rewrite_content(name: str, verb: str, item: str, deadline: str, reason: str) -> str:
    return f"Hi {name}, could you please {verb} the {item} by {deadline}? {reason}. Thank you."
...
def _build_rows(split: str, repeats: int) -> list[dict[str, Any]]:
    ...
    idx = 0
    rewrite_cases = [
        ("Lena", "send", "budget draft", "June 3", "Finance is waiting", "yo lena send the budget draft by june 3 because finance is waiting"),
        ("Ava", "send", "invoice", "tomorrow morning", "Finance needs it", "ava please send the invoice by tomorrow morning because finance needs it"),
        ("Devon", "prepare", "roadmap update", "next Tuesday", "Planning needs it", "devon prepare the roadmap update by next tuesday because planning needs it"),
        ("Marco", "send", "contract", "end of day", "Legal needs it", "marco send the contract by end of day because legal needs it"),
        ("Nora", "share", "badge list", "Friday", "Security needs it", "nora share the badge list by friday because security needs it"),
        ("Sam", "finish", "slide deck", "Thursday at 2 PM", "The client review moved up", "sam finish the slide deck by thursday at 2 pm because the client review moved up"),
        ("Priya", "send", "launch notes", "Monday", "The team is waiting", "priya send the launch notes by monday because the team is waiting"),
        ("Eli", "update", "release checklist", "Wednesday", "QA is blocked", "eli update the release checklist by wednesday because qa is blocked"),
    ]
    classify_cases = [
    ...
    fixed_replay = [
        (ev._runtime_plain_chat_prompt("Hi how are you?"), "respond", "I'm doing well. How can I help?", "casual", None),
        (
            ev._agent_prompt(
    ...
    for _ in range(int(repeats)):
        for name, verb, item, deadline, reason, user_text in rewrite_cases:
            ...
            _add(
                rows,
                split=split,
                task_type="cert_rewrite_value_copy",
                prompt=prompt,
                action="respond",
                content=_rewrite_content(name, verb, item, deadline, reason),
                idx=idx,
                weight=9.0,
                intent="rewrite",
        ...
        for text, label in classify_cases:
            ...
            _add(
                rows,
                split=split,
                task_type="cert_classifier_label_only",
                prompt=prompt,
                action="respond",
                content=label,
                idx=idx,
        ...
        for query in web_queries:
            metadata = dict(web_metadata)
            metadata["query"] = query
            _add(
                rows,
                split=split,
                task_type="cert_web_metadata",
            ...
            idx += 1
        for prompt, action, content, intent, metadata in fixed_replay:
            _add(
                rows,
                split=split,
                task_type="cert_fixed_gate_replay",
                prompt=prompt,
                action=action,
                content=content,
                metadata=metadata,
...
def main() -> None:
    ...
    manifest = {
        ...
        "objective": "pocketpal_certification_repair",
