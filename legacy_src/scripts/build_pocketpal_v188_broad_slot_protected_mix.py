...
def main() -> None:
    ...
    for path_key, limit, dest, split_name in [
        ...
        for index, row in enumerate(selected[:limit]):
            dest.append(_normalize(row, f"v188_broad_{split_name}_{index:06d}", 5.0, 16.0, "v188_broad"))

    for repeat in range(int(args.slot_repeat)):
        for row in _iter_jsonl(Path(slot_manifest["train_dataset_path"])):
            train_rows.append(_normalize(row, f"v188_slot_{repeat:02d}", 14.0, 42.0, "v188_slot_protect"))
        for row in _iter_jsonl(Path(slot_manifest["eval_dataset_path"])):
            eval_rows.append(_normalize(row, f"v188_slot_{repeat:02d}", 14.0, 42.0, "v188_slot_protect"))

    for repeat in range(int(args.rewrite_repeat)):
        for row in _iter_jsonl(Path(rewrite_manifest["train_dataset_path"])):
            train_rows.append(_normalize(row, f"v188_rewrite_{repeat:02d}", 18.0, 44.0, "v188_rewrite_protect"))
        for row in _iter_jsonl(Path(rewrite_manifest["eval_dataset_path"])):
            eval_rows.append(_normalize(row, f"v188_rewrite_{repeat:02d}", 18.0, 44.0, "v188_rewrite_protect"))
    ...
    for row in _iter_jsonl(Path(retrieval_manifest["train_dataset_path"])):
        ...
        if str(row.get("retrieval_doc_text") or "").strip():
            protected = dict(row)
            protected["example_id"] = f"{row.get('example_id')}_v188_retrieval"
            protected["source_type"] = "v182_retrieval_protection_v188"
    ...
    manifest = {
        ...
        "objective": "pocketpal_v188_broad_slot_protected_mix",
