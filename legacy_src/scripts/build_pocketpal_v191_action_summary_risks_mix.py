...
def _normalize(row: dict[str, Any], suffix: str, floor: float, cap: float, source: str) -> dict[str, Any]:
    ...
    if not str(out.get("negative_decoder_text") or "").strip():
        if task == "active_agent_action_items":
            negative = _payload("respond", "Hello, I hope you are well.", "negative_greeting_attractor")
        elif task == "active_agent_risks":
            negative = _payload("respond", "Source text: [[SOURCE_TEXT]]", "negative_source_copy_attractor")
        elif task == "active_agent_summary":
            negative = _payload(
                "respond",
                "Hi [[NAME]], could you please send the [[ITEM]] by [[DEADLINE]]? [[REASON]]. Thank you.",
                "negative_rewrite_slot_attractor",
            )
        else:
            negative = ""
        if negative:
            out["negative_decoder_text"] = negative
            out["negative_loss_weight"] = 0.45
    return out
...
def main() -> None:
    ...
    parser.add_argument("--max-focus-train", type=int, default=42000)
    parser.add_argument("--max-focus-eval", type=int, default=4500)
    ...
    for path_key, limit, dest, split in [
        ...
        rows = [row for row in _iter_jsonl(Path(broad_manifest[path_key])) if str(row.get("task_type") or "") in FOCUS_TASKS]
        rows.sort(key=lambda row: _hash_float(str(row.get("example_id") or row.get("source_id") or row.get("encoder_text"))))
        for index, row in enumerate(rows[:limit]):
            dest.append(_normalize(row, f"v191_focus_{split}_{index:06d}", 7.0, 18.0, "v191_action_summary_risks"))

    for repeat in range(int(args.slot_repeat)):
        for row in _iter_jsonl(Path(slot_manifest["train_dataset_path"])):
            train_rows.append(_normalize(row, f"v191_slot_{repeat:02d}", 14.0, 38.0, "v191_slot_protect"))
        for row in _iter_jsonl(Path(slot_manifest["eval_dataset_path"])):
            eval_rows.append(_normalize(row, f"v191_slot_{repeat:02d}", 14.0, 38.0, "v191_slot_protect"))

    for repeat in range(int(args.rewrite_repeat)):
        for row in _iter_jsonl(Path(rewrite_manifest["train_dataset_path"])):
            train_rows.append(_normalize(row, f"v191_rewrite_{repeat:02d}", 18.0, 46.0, "v191_rewrite_protect"))
        for row in _iter_jsonl(Path(rewrite_manifest["eval_dataset_path"])):
            eval_rows.append(_normalize(row, f"v191_rewrite_{repeat:02d}", 18.0, 46.0, "v191_rewrite_protect"))
    ...
    for row in _iter_jsonl(Path(retrieval_manifest["train_dataset_path"])):
        ...
        if str(row.get("retrieval_doc_text") or "").strip():
            out = dict(row)
            out["example_id"] = f"{row.get('example_id')}_v191_retrieval"
            out["source_type"] = "v182_retrieval_protection_v191"
            out["weight"] = 0.0
    ...
    manifest = {
        ...
        "objective": "pocketpal_v191_action_summary_risks_mix",
