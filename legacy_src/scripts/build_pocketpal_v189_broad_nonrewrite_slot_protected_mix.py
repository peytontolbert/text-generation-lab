...
def main() -> None:
    ...
    parser.add_argument("--max-broad-train", type=int, default=50000)
    parser.add_argument("--max-broad-eval", type=int, default=6000)
    parser.add_argument("--slot-repeat", type=int, default=3)
    parser.add_argument("--rewrite-repeat", type=int, default=4)
    ...
    for path_key, limit, dest, split_name in [
        ("train_dataset_path", int(args.max_broad_train), train_rows, "train"),
        ("eval_dataset_path", int(args.max_broad_eval), eval_rows, "eval"),
    ]:
        selected = [row for row in _iter_jsonl(Path(broad_manifest[path_key])) if str(row.get("task_type") or "").startswith(FOCUS_PREFIXES)]
        ...
        for index, row in enumerate(selected[:limit]):
            dest.append(_normalize(row, f"v189_broad_{split_name}_{index:06d}", 5.0, 16.0, "v189_broad"))

    for repeat in range(int(args.slot_repeat)):
        for row in _iter_jsonl(Path(slot_manifest["train_dataset_path"])):
            train_rows.append(_normalize(row, f"v189_slot_{repeat:02d}", 14.0, 42.0, "v189_slot_protect"))
        for row in _iter_jsonl(Path(slot_manifest["eval_dataset_path"])):
            eval_rows.append(_normalize(row, f"v189_slot_{repeat:02d}", 14.0, 42.0, "v189_slot_protect"))

    for repeat in range(int(args.rewrite_repeat)):
        for row in _iter_jsonl(Path(rewrite_manifest["train_dataset_path"])):
            train_rows.append(_normalize(row, f"v189_rewrite_{repeat:02d}", 18.0, 44.0, "v189_rewrite_protect"))
        for row in _iter_jsonl(Path(rewrite_manifest["eval_dataset_path"])):
            eval_rows.append(_normalize(row, f"v189_rewrite_{repeat:02d}", 18.0, 44.0, "v189_rewrite_protect"))
    ...
    for row in _iter_jsonl(Path(retrieval_manifest["train_dataset_path"])):
        ...
        if str(row.get("retrieval_doc_text") or "").strip():
            protected = dict(row)
            protected["example_id"] = f"{row.get('example_id')}_v189_retrieval"
            protected["source_type"] = "v182_retrieval_protection_v189"
    ...
    manifest = {
        ...
        "objective": "pocketpal_v189_broad_nonrewrite_slot_protected_mix",
