...
STAGE = 9745
NAME = "stage9745_edit_localization_target_only_language_slice_audit"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "edit_localization_target_only_language_slice_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "EDIT_LOCALIZATION_TARGET_ONLY_LANGUAGE_SLICE_AUDIT_STAGE9745.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9744_multilingual_edit_localization_target_only_execution_audit.json"
BASELINE_AUDIT = ROOT / "runs/local/artifacts/stage9734_edit_localization_label_aligned_language_slice_audit/edit_localization_label_aligned_language_slice_audit.json"
...
LOGITS = ROOT / "runs/local/artifacts/stage9744_multilingual_edit_localization_target_only_exec/row_field_logits.jsonl"
    ...
    for lang in LANGS:
        ...
        for split in SPLITS:
            ...
            language_slices[lang][split] = {
                ...
                "improved_over_stage9734": None if baseline_exact is None or exact is None else bool(exact > float(baseline_exact)),
                "pred_counts": pred_counts,
            }
    if source.get("passed") is not True:
        failures.append("stage9744_not_passed")
    if baseline.get("passed") is not True:
        failures.append("stage9734_not_passed")
    return {
        "passed": not failures,
        "failures": failures,
        "language_slices": language_slices,
        "uniform_improvement_over_stage9734": uniform_improvement,
...
def main() -> None:
    ...
    next_step = "Use the target-only edit-localization result as the current strongest standalone multilingual 100M surface, or port the same target-space simplification idea to other structured surfaces."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        "passed": audit["passed"],
        "authority": dict(AUTHORITY_CLOSED),
        "metrics": {**dict(AUTHORITY_CLOSED), "uniform_improvement_over_stage9734": audit["uniform_improvement_over_stage9734"]},
        "artifacts": {"audit": str(AUDIT.relative_to(ROOT)), "doc": str(DOC.relative_to(ROOT))},
        "decision": "Recorded the per-language effect of the target-only edit-localization intervention against the prior full-label language-slice baseline.",
    ...
    DOC.write_text("\n".join([
        ...
        f"Uniform improvement over Stage9734: `{audit['uniform_improvement_over_stage9734']}`",
    ...
    print(json.dumps({"stage": STAGE, "passed": summary["passed"], "uniform_improvement_over_stage9734": audit["uniform_improvement_over_stage9734"], "failures": audit["failures"], "next_best_step": next_step}, indent=2, sort_keys=True))
    if audit["failures"]:
        raise SystemExit(1)
