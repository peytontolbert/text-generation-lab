#!/usr/bin/env python3
"""Materialize codex-rmcp-client Rust selected-verifier support rows."""
...
STAGE = 11833
NAME = "stage11833_codex_rmcp_client_rust_support_rows"
OUT = ART / NAME
SUMMARY = OUT / "codex_rmcp_client_rust_support_rows.json"
ROWS = OUT / "codex_rmcp_client_rust_support_rows.jsonl"
FEASIBILITY = ART / "stage11832_codex_rmcp_client_rust_feasibility/codex_rmcp_client_rust_feasibility_results.jsonl"
...
def file_evidence(repo: Path, eid: str, kind: str, rel_path: str, summary: str, start: int = 1, end: int = 180) -> dict[str, Any]:
...
def main() -> None:
    ...
    if not probes:
        raise SystemExit("no materializable codex-rmcp-client Rust probe found")
    probe = probes[0]
    ...
    root_id = "stage11833::codex_rmcp_client::rust::program_resolver_support"
    evidence = [
        file_evidence(repo, "E01", "candidate_change_surface", source_path, "RMCP client program resolver handles platform-specific executable path behavior and has Unix/resolved execution unit tests."),
        file_evidence(repo, "E02", "nearby_source_distractor", distractor_source_path, "Stdio server launcher is adjacent RMCP process logic but not the selected resolver verifier target."),
    ...
    artifact = {"stage": STAGE, "stage_name": NAME, "created_at_utc": now(), "decision": "seventh_rust_verifier_support_root_materialized", "passed": True, "row_count": len(rows), "root_count": 1, "repo_family": repo_family, "source_artifacts": {"feasibility": rel(FEASIBILITY)}, "outputs": {"summary": rel(SUMMARY), "rows": rel(ROWS)}, "claim_boundary": ["Rows are train-support-only.", "This expands Rust verifier support with a non-tokenizers Codex RMCP client repo family."]}
