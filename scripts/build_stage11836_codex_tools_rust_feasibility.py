#!/usr/bin/env python3
"""Capture codex-tools Rust verifier feasibility."""
...
STAGE = 11836
NAME = "stage11836_codex_tools_rust_feasibility"
OUT = ART / NAME
LOGS = OUT / "logs"
SUMMARY = OUT / "codex_tools_rust_feasibility.json"
RESULTS = OUT / "codex_tools_rust_feasibility_results.jsonl"
REPO = Path("/data/agentkernel/other_repos/codex/codex-rs")
SELECTED_TEST = "request_user_input_tool_includes_questions_schema"
SIBLING_TEST = "request_user_input_tool_description_mentions_available_modes"
...
def run_cargo(test_filter: str, name: str) -> dict[str, Any]:
    ...
    command = ["cargo", "test", "-p", "codex-tools", test_filter, "--", "--nocapture"]
    env = os.environ.copy()
    env.update(
        {
...
def main() -> None:
    selected = run_cargo(SELECTED_TEST, "selected_request_user_input_schema")
    sibling = run_cargo(SIBLING_TEST, "sibling_request_user_input_description")
    passed = bool(selected["passed"] and sibling["passed"])
    row = {
        "stage": STAGE,
        "stage_name": NAME,
        "created_at_utc": now(),
        "repo_family": "codex_tools",
        "repo_path": str(REPO),
        ...
        "source_path": "tools/src/request_user_input_tool.rs",
        "distractor_source_path": "tools/src/tool_config.rs",
        "passed": passed,
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "codex_tools_rust_feasibility_recorded",
        "passed": passed,
