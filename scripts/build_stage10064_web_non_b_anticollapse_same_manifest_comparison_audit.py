...
STAGE = 10064
NAME = "stage10064_web_non_b_anticollapse_same_manifest_comparison_audit"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "web_non_b_anticollapse_same_manifest_comparison_audit.json"
ROWS = OUT_DIR / "web_non_b_anticollapse_same_manifest_comparison_rows.jsonl"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "WEB_NON_B_ANTICOLLAPSE_SAME_MANIFEST_COMPARISON_AUDIT_STAGE10064.md"
...
SUCCESSOR_HUNDRED_M = ROOT / "runs/local/artifacts/stage10063_web_non_b_anticollapse_target100m_probe/edit_localization_probe/row_field_logits.jsonl"
...
def main() -> None:
    ...
    next_step = "Use this audit to decide whether the web non-B anti-collapse packet finally turns the stage10058 3-win/1-tie frontier into a clean 4-language win over Gemma."
    summary = {"stage": STAGE, "stage_name": NAME, "name": NAME, "passed": built["passed"], "metrics": {**built["metrics"], "failures": built["failures"]}, "artifacts": {"audit": display(AUDIT), "rows": display(ROWS), "doc": display(DOC)}, "decision": "Materialized the same-manifest comparison audit for the web non-B anti-collapse successor run so the resulting 100M outputs can be judged directly against stage10040 and the fixed Gemma comparator.", "next_best_step": next_step, "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage10064 Web Non-B Anticollapse Same Manifest Comparison Audit", "", f"Passed: `{summary['passed']}`", "", summary["decision"], "", f"Next: {next_step}", ""]), encoding="utf-8")
