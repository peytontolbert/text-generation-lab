...
STAGE = 10782
NAME = "stage10782_targeted_residual_support_probe_request"
OUT_DIR = ARTIFACTS / NAME
REQUEST_JSON = OUT_DIR / "targeted_residual_support_probe_request.json"
COMMAND_JSON = OUT_DIR / "targeted_residual_support_probe_command.json"
MANIFEST_JSONL = OUT_DIR / "targeted_residual_support_probe_manifest.jsonl"

PACKAGE_JSON = ARTIFACTS / "stage10781_targeted_residual_support_package" / "targeted_residual_support_package.json"
TRAIN_ROWS = ARTIFACTS / "stage10781_targeted_residual_support_package" / "agentkernel_lite_encdec_train.jsonl"
VALIDATION_ROWS = ARTIFACTS / "stage10781_targeted_residual_support_package" / "agentkernel_lite_encdec_validation.jsonl"
STRICT_ROWS = ARTIFACTS / "stage10781_targeted_residual_support_package" / "agentkernel_lite_encdec_strict_eval.jsonl"
STRESS_ROWS = ARTIFACTS / "stage10781_targeted_residual_support_package" / "agentkernel_lite_encdec_stress_eval.jsonl"
...
RUN_ID = "stage10783_targeted_residual_support_probe"
OUTPUT_DIR = "runs/local/artifacts/stage10783_targeted_residual_support_probe/bounded_decoder_probe"
RUNTIME_MODEL_DIR = "runs/local/artifacts/stage10783_targeted_residual_support_probe/runtime_model"
...
def main() -> None:
    ...
    request = {
        ...
        "decision": "targeted_residual_support_probe_ready",
        ...
        "support_snapshot": {
            "support_rows_added_from_stage10781": len(support_rows),
            "support_languages": dict(sorted(Counter(str(row.get("language_family") or "unknown") for row in support_rows).items())),
            "support_tasks": dict(sorted(Counter(str(row.get("task_type") or "unknown") for row in support_rows).items())),
        },
        "stress_rows_excluded_from_manifest": len(stress_rows),
        "claim_scope": [
            "Run a standalone support-only probe that keeps the reviewed v2.7 strict frontier fixed while adding only Python verifier and Rust citation support rows.",
        ...
        "required_honesty_gates": [
            ...
            "all stage10781 rows remain train_support_only and strict_eval_eligible=false",
            "all stage10781 rows are targeted residual support, not direct copies of the strict miss rows",
        ...
        "next_best_step": "run the probe, then audit whether the targeted residual support rows flip either live miss without regressions",
    }
