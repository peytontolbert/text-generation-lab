#!/usr/bin/env python3
"""Capture bounded PyTorch C++ sample build/verifier feasibility."""
...
STAGE = 11782
NAME = "stage11782_cpp_pytorch_sample_build_feasibility"
OUT = ART / NAME
LOGS = OUT / "logs"
SUMMARY = OUT / "cpp_pytorch_sample_build_feasibility.json"
RESULTS = OUT / "cpp_pytorch_sample_build_feasibility_results.jsonl"

REPO = Path("/data/parametergolf/helpful_repos/pytorch")
SAMPLE = REPO / ".ci/pytorch/test_example_code"
BUILD_DIR = Path("/data/tmp/stage11782_pytorch_cpp_sample_build")
...
def main() -> None:
    ...
    build = run(["cmake", "--build", str(BUILD_DIR), "--target", "simple-torch-test", "--parallel", "2"], "build", timeout=240) if configure["passed"] else None
    executable = BUILD_DIR / "simple-torch-test"
    execute = run([str(executable)], "execute", timeout=60) if build and build["passed"] else None
    passed = bool(configure["passed"] and build and build["passed"] and execute and execute["passed"] and "Simple test passed" in (execute["stderr_tail"] + execute["stdout_tail"]))
    result = {
        "repo_family": "pytorch",
        "repo_path": str(REPO),
        ...
        "selected_verifier": "cmake_configure_build_and_run_simple_torch_test",
    ...
    artifact: dict[str, Any] = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "cpp_pytorch_sample_build_feasibility_complete",
        "passed": True,
        "candidate_count": 1,
        ...
        "interpretation": [
            "The small PyTorch C++ example configures against the trellis Torch CMake package, builds, and executes.",
            "This provides a third executable C/C++ build/run verifier root for abstain-attractor support.",
        ],
        "claim_boundary": [
