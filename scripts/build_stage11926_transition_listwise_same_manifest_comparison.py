#!/usr/bin/env python3
"""Compare Stage11924 routed transition listwise scorer against Stage11916 Gemma outputs."""
...
STAGE = 11926
NAME = "stage11926_transition_listwise_same_manifest_comparison"
OUT = ROOT / "runs/local/artifacts" / NAME
SUMMARY = OUT / "transition_listwise_same_manifest_comparison.json"
ROWS_OUT = OUT / "transition_listwise_same_manifest_rows.jsonl"

ROWS = ROOT / "runs/local/artifacts/stage11897_transition_record_projection_rows/transition_projection_rows.jsonl"
GEMMA_ROWS = ROOT / "runs/local/artifacts/stage11916_transition_projection_same_manifest_gemma_comparison/transition_projection_same_manifest_gemma_rows.jsonl"
HUNDRED_CARD = ROOT / "runs/local/artifacts/stage11925_transition_listwise_routed_postrun_audit/stage11924_transition_listwise_head_only/bounded_choice_eval_audit_transition_projection__semantic_candidate_head.json"
...
def verdicts(hundred: dict[str, Any], gemma: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in sorted(set(hundred) | set(gemma)):
        ...
        if h is None or g is None:
            ...
            verdict = "tie"
            summary["ties"] += 1
        out[key] = {"hundred_m_accuracy": h, "gemma12b_accuracy": g, "delta": None if h is None or g is None else h - g, "verdict": verdict, "rows": (hundred.get(key) or gemma.get(key) or {}).get("rows")}
...
def main() -> None:
    ...
    gemma_rows = {str(row.get("row_id")): row for row in read_jsonl(GEMMA_ROWS)}
    hundred_rows = {str(row.get("row_id")): row for row in (read_json(HUNDRED_CARD).get("row_cards") or [])}
    comparison: list[dict[str, Any]] = []
    for row in sorted(rows, key=lambda item: str(item.get("row_id") or "")):
        row_id = str(row.get("row_id") or "")
        hundred = hundred_rows[row_id]
        ...
        comparison.append(
            {
                ...
                "hundred_m_runtime": rel(ROOT / "runs/local/artifacts/stage11924_transition_listwise_head_only_probe/runtime_model/runtime_model_bundle.json"),
    ...
    summary = {
        ...
        "source_artifacts": {"rows": rel(ROWS), "gemma_rows": rel(GEMMA_ROWS), "hundred_m_card": rel(HUNDRED_CARD)},
