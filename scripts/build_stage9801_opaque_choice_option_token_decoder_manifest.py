...
STAGE = 9801
NAME = "stage9801_opaque_choice_option_token_decoder_manifest"
...
MANIFEST = OUT_DIR / "opaque_choice_option_token_decoder_manifest.jsonl"
AUDIT = OUT_DIR / "opaque_choice_option_token_decoder_manifest_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "OPAQUE_CHOICE_OPTION_TOKEN_DECODER_MANIFEST_STAGE9801.md"
...
def build_rows() -> list[dict[str, Any]]:
    ...
    for source in load_jsonl(SOURCE):
        ...
        label = str(target.get("decoder_text") or "").strip()
        decoder_text = f"option {label}"
        row = dict(source)
        ...
        row["surface"] = "EDIT_LOCALIZATION_OPAQUE_CHOICE_OPTION_TOKEN_DECODER"
        row["target"] = {
            "decoder_text": decoder_text,
            "decoder_label": label,
            "target_shape": "OPAQUE_CHOICE_OPTION_TOKEN",
            "target_ref": label,
        }
        ...
        row["anti_cheat"] = {
            **anti,
            ...
            "decoder_target_format": "option_token",
        }
        rows.append(row)
    return rows
...
def audit_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    decoder_text_counts = Counter(str(((row.get("target") if isinstance(row.get("target"), dict) else {}).get("decoder_text") or "")) for row in rows)
    failures: list[str] = []
    unsafe_loss_rows = []
    malformed_rows = []
    for row in rows:
        ...
        if not decoder_text.startswith("option ") or len(decoder_text.split()) != 2:
    ...
    if set(decoder_text_counts) != {f"option {label}" for label in ['A','B','C','D','E']}:
        failures.append("unexpected_decoder_text_vocab")
    if malformed_rows:
        failures.append("malformed_decoder_rows")
    if unsafe_loss_rows:
    ...
    return {
        ...
        "decoder_text_counts": dict(sorted(decoder_text_counts.items())),
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Reformatted the opaque-choice decoder targets from bare single-letter labels to `option X` tokens so the decoder path can learn a less collapse-prone output surface while staying aligned with the visible task framing.",
        "next_best_step": "Run a bounded decoder CE preexecution and one real probe on the Stage9801 manifest, then compare it against the Stage9799 single-letter decoder path and the Stage9794 structured baseline.",
    ...
    DOC.write_text("\n".join([
        ...
        f"Decoder texts: `{audit['decoder_text_counts']}`",
        "",
    ...
    print(json.dumps({"stage": STAGE, "passed": summary["passed"], "metrics": {"rows": audit["rows"], "decoder_text_counts": audit["decoder_text_counts"]}, "failures": audit["failures"]}, indent=2, sort_keys=True))
    if audit["failures"]:
        raise SystemExit(1)
