...
STAGE = 10076
NAME = "stage10076_canonical_v27_finish_gate"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
GATE = OUT_DIR / "canonical_v27_finish_gate.json"
...
def build_gate() -> dict[str, Any]:
    ...
    frontier_rows = (frontier.get("metrics") or {}).get("per_language") or {}
    ...
    for lang in LANGS:
        ...
        gate_rows.append({
            ...
            "standalone_same_surface_win": bool(frontier_row.get("verdict") == "100m_better"),
            "standalone_exact_100m": frontier_row.get("hundred_m_exact"),
            "standalone_exact_gemma": frontier_row.get("gemma_exact"),
...
def main() -> None:
    ...
    next_step = "Complete the 8 human signoff tasks in the Stage10074 workbook, then hand the 4 Stage9931 backend bundles to the external full-product runtime to satisfy the remaining harness requirement."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Materialized a refreshed v2.7 finish gate that proves the canonical four-language standalone win is present, the live signoff queue is still exactly 8 human tasks, and the harness side remains packaged through external-backend handoff but not complete.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage10076 Canonical V27 Finish Gate",
        "",
