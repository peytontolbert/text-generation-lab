...
STAGE = 9383
NAME = "stage9383_bounded_decoder_failure_denoise_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9382_bounded_decoder_failure_to_denoise_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9382_bounded_decoder_failure_to_denoise_manifest/bounded_decoder_failure_to_denoise_manifest.jsonl"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9384_bounded_decoder_failure_denoise_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "bounded_decoder_failure_denoise_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9384_bounded_decoder_failure_denoise_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "BOUNDED_DECODER_FAILURE_DENOISE_PREEXECUTION_STAGE9383.md"
...
COMMAND = [
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9384_bounded_decoder_failure_denoise_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9382_not_passed")
    if len(rows) != 23:
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared a 23-row denoise repair probe for Stage9381 bounded decoder generation failures; decoder CE remains closed.",
        "next_best_step": "Execute Stage9384 denoise repair probe under trellis and audit whether bounded decoder failure rows are repaired in isolation.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9383 Bounded Decoder Failure Denoise Preexecution", "", f"Passed: `{card['passed']}`", f"Rows: `{card['manifest_rows']}`", f"Splits: `{card['split_counts']}`", f"Routes: `{card['route_counts']}`", "", "Only the Stage9384 denoise probe is authorized. Decoder CE and external authority remain closed.", ""]), encoding="utf-8")
