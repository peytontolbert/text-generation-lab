#!/usr/bin/env python3
"""Materialize codex-utils-path Rust selected-verifier support rows."""
...
STAGE = 11853
NAME = "stage11853_codex_utils_path_rust_support_rows"
OUT = ART / NAME
SUMMARY = OUT / "codex_utils_path_rust_support_rows.json"
ROWS = OUT / "codex_utils_path_rust_support_rows.jsonl"
FEASIBILITY = ART / "stage11852_codex_utils_path_rust_feasibility/codex_utils_path_rust_feasibility_results.jsonl"
...
def main() -> None:
    ...
    if not probes:
        raise SystemExit("no materializable codex-utils-path Rust probe found")
    probe = probes[0]
    ...
    root_id = "stage11853::codex_utils_path::rust::wsl_path_normalization_support"
    evidence = [
        file_evidence(repo, "E01", "candidate_change_surface", source_path, "Codex path utilities source normalizes WSL mount paths and native workdir path comparisons."),
        file_evidence(repo, "E02", "nearby_source_distractor", distractor_source_path, "Path utility tests are verifier context but not the implementation surface."),
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "twelfth_rust_verifier_support_root_materialized",
        "passed": True,
        "row_count": len(rows),
        "root_count": 1,
        "repo_family": repo_family,
        ...
        "claim_boundary": ["Rows are train-support-only.", "This completes the 12-root Rust verifier support quota with a non-tokenizers Codex path utilities repo family."],
    }
