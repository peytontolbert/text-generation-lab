...
STAGE = 10067
NAME = "stage10067_cpp_rust_regression_counterbalance_same_manifest_comparison_audit"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "cpp_rust_regression_counterbalance_same_manifest_comparison_audit.json"
ROWS = OUT_DIR / "cpp_rust_regression_counterbalance_same_manifest_comparison_rows.jsonl"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "CPP_RUST_REGRESSION_COUNTERBALANCE_SAME_MANIFEST_COMPARISON_AUDIT_STAGE10067.md"
...
SUCCESSOR_HUNDRED_M = ROOT / "runs/local/artifacts/stage10066_cpp_rust_regression_counterbalance_target100m_probe/edit_localization_probe/row_field_logits.jsonl"
...
def main() -> None:
    ...
    next_step = "Use this audit to decide whether the c_cpp/rust counter-balance packet keeps the stage10064 4-language Gemma win while narrowing or eliminating the remaining baseline gap."
    summary = {"stage": STAGE, "stage_name": NAME, "name": NAME, "passed": built["passed"], "metrics": {**built["metrics"], "failures": built["failures"]}, "artifacts": {"audit": display(AUDIT), "rows": display(ROWS), "doc": display(DOC)}, "decision": "Materialized the same-manifest comparison audit for the c_cpp/rust counter-balance successor run so the resulting 100M outputs can be judged directly against stage10040 and the fixed Gemma comparator.", "next_best_step": next_step, "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage10067 Cpp Rust Regression Counterbalance Same Manifest Comparison Audit", "", f"Passed: `{summary['passed']}`", "", summary["decision"], "", f"Next: {next_step}", ""]), encoding="utf-8")
    if summary["passed"]:
