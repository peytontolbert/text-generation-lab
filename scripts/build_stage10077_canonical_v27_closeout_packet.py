...
STAGE = 10077
NAME = "stage10077_canonical_v27_closeout_packet"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
PACKET = OUT_DIR / "canonical_v27_closeout_packet.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "CANONICAL_V27_CLOSEOUT_PACKET_STAGE10077.md"
...
def build_packet() -> dict[str, Any]:
    ...
    for lang in LANGS:
        ...
        packet_rows.append({
            ...
            "standalone_win_summary": {
                "exact_100m": finish_row.get("standalone_exact_100m"),
                "exact_gemma": finish_row.get("standalone_exact_gemma"),
    ...
    return {"passed": not failures, "failures": failures, "metrics": metrics, "language_packets": packet_rows, "authority": dict(AUTHORITY_CLOSED)}
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Materialized a canonical language-by-language closeout packet that combines the current standalone win evidence, the live human signoff tasks, and the existing external harness handoff bundle for each required language family.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage10077 Canonical V27 Closeout Packet",
        "",
