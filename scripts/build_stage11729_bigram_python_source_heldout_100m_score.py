#!/usr/bin/env python3
"""Score admitted bigram Python source-heldout smoke rows with Stage11507."""
...
STAGE = 11729
NAME = "stage11729_bigram_python_source_heldout_100m_score"
OUT = ART / NAME
SUMMARY = OUT / "bigram_python_source_heldout_100m_score.json"
RENDERED_ROWS = OUT / "bigram_python_rendered_score_rows.jsonl"
SCORED_ROWS = OUT / "bigram_python_100m_score_rows.jsonl"

STAGE11722 = ROOT / "scripts/build_stage11722_sentencepiece_cpp_source_heldout_100m_score.py"
spec = importlib.util.spec_from_file_location("stage11722_for_11729", STAGE11722)
...
ROWS = ART / "stage11728_bigram_python_no_train_overlap_and_admission/bigram_python_admitted_smoke_rows.jsonl"
...
def render_prompt(row: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "OBSERVED_STATE",
        "A compact Python maintainer root has concrete implementation, test, nearby data, integration, and config evidence.",
...
def main() -> None:
    ...
    if admission.get("passed") is not True:
        raise SystemExit("Stage11728 admission did not pass")
    ...
    card = stage11722._write_bounded_choice_eval_audit(
        ...
        split_name="bigram_python_source_heldout_smoke",
    ...
    gates = {
        "stage11728_admitted": admission.get("passed") is True,
    ...
    summary = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "bigram_python_source_heldout_100m_score_complete",
        ...
        "claim_boundary": [
            "This scores a 4-row Python bigram_language_model compact source-heldout smoke packet.",
        ...
        "outputs": {
            ...
            "bounded_choice_audit": rel(OUT / "bounded_choice_eval_audit_bigram_python_source_heldout_smoke.json"),
        },
    }
