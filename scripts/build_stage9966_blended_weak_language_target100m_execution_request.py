...
STAGE = 9966
NAME = "stage9966_blended_weak_language_target100m_execution_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
REQUEST = OUT_DIR / "blended_weak_language_target100m_execution_request.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "BLENDED_WEAK_LANGUAGE_TARGET100M_EXECUTION_REQUEST_STAGE9966.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
TICKET = ROOT / "runs/local/artifacts/stage9964_blended_weak_language_execution_review/blended_weak_language_execution_review_inactive.json"
AUDIT_9963 = ROOT / "runs/local/artifacts/stage9963_blended_weak_language_target100m_contract_preflight/blended_weak_language_target100m_contract_preflight_audit.json"
...
def _surface_request(card: dict[str, Any], command: list[str], stage9963_row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "required_contract_invariants": {
            "source_stage9963_contract_passed": bool(stage9963_row.get("contract_passed") is True),
            "source_stage9963_model_execution_attempted": bool(stage9963_row.get("model_execution_attempted")),
            "source_stage9963_unsafe_loss_rows": stage9963_row.get("unsafe_loss_rows"),
            "source_stage9963_missing_required_contract_artifacts": list(stage9963_row.get("missing_required_contract_artifacts") or []),
        },
        "authority": dict(AUTHORITY_CLOSED),
    }
...
def build_request() -> dict[str, Any]:
    ...
    audit_9963 = load_json(AUDIT_9963)
    failures: list[str] = []

    if ticket.get("ticket_status") != "DESIGN_ONLY_INACTIVE":
        failures.append("stage9964_ticket_not_inactive")
    if ticket.get("execution_authorized_now") is not False:
        failures.append("stage9964_execution_authorized_now")
    if audit_9963.get("passed") is not True:
    ...
    stage9963_rows = {
        ...
        for row in (audit_9963.get("surface_results") or [])
    ...
    for card in surface_cards:
        ...
        stage9963_row = stage9963_rows.get(surface)
        if not isinstance(command, list):
            failures.append(f"missing_surface_command:{surface}")
            continue
        if not isinstance(stage9963_row, dict):
            failures.append(f"missing_stage9963_surface:{surface}")
            continue
        req = _surface_request(card, [str(item) for item in command], stage9963_row)
    ...
    if metrics["edit_localization_rows"] != 120:
        failures.append("edit_localization_rows_not_120")
    if metrics["edit_localization_python_rows"] != 24:
        failures.append("edit_localization_python_rows_not_24")
    if metrics["edit_localization_c_cpp_rows"] != 30:
    ...
    return {
        ...
        "source_artifacts": {
            ...
            "contract_preflight_audit": display(AUDIT_9963),
        },
        "execution_constraints": {
            ...
            "requires_stage9964_ticket": True,
            "requires_stage9963_contract_artifacts": True,
        },
...
def main() -> None:
    ...
    next_step = "Use the four surface request files from Stage9966 when explicitly authorizing one successor blended target-100M structured probe series, so execution stays pinned to the Stage9962 weak-language successor mix and Stage9963 contract-validated manifests."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Materialized the concrete next-run request for the successor blended multilingual target-100M structured probe, with per-surface commands, required artifacts, and invariants that preserve the stage9961 weak-language recovery rows.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9966 Blended Weak-Language Target100M Execution Request",
        "",
