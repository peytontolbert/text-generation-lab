#!/usr/bin/env python3
"""Combined admission audit for Rust verifier support including codex-utils-path."""
...
STAGE = 11854
NAME = "stage11854_rust_verifier_support_combined_audit_v11"
OUT = ART / NAME
SUMMARY = OUT / "rust_verifier_support_combined_audit_v11.json"
ADMITTED_ROWS = OUT / "rust_verifier_support_rows_v11.jsonl"
...
ROW_SOURCES = [
    ...
    ART / "stage11841_codex_protocol_rust_support_rows/codex_protocol_rust_support_rows.jsonl",
    ART / "stage11845_codex_config_rust_support_rows/codex_config_rust_support_rows.jsonl",
    ART / "stage11849_codex_utils_string_rust_support_rows/codex_utils_string_rust_support_rows.jsonl",
    ART / "stage11853_codex_utils_path_rust_support_rows/codex_utils_path_rust_support_rows.jsonl",
]
...
def main() -> None:
    ...
    for row in rows:
        ...
        record = {"row_id": row.get("row_id"), "root_id": row.get("root_id"), "repo_family": row.get("repo_family"), "task_type": row.get("task_type"), "admitted": not failures, "failures": failures}
    ...
    artifact = {
        ...
        "quota_status": {"support_roots_ready": len(admitted_roots), "support_roots_remaining": max(0, 12 - len(admitted_roots)), "repo_families_ready": len(admitted_families)},
        "claim_boundary": ["Rows are Rust verifier_outcome selected-inline-test train support only.", "Rust support quota is satisfied, but full training remains blocked until Python and C/C++ quotas are satisfied."],
        "outputs": {"summary": rel(SUMMARY), "admitted_rows": rel(ADMITTED_ROWS)},
    }
    write_json(SUMMARY, artifact)
    SUMMARIES.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(SUMMARY, SUMMARIES / f"{NAME}.json")
    print(json.dumps({"decision": artifact["decision"], "passed": artifact["passed"], "admitted_rows": artifact["admitted_rows"], "admitted_root_count": artifact["admitted_root_count"], "admitted_repo_families": artifact["admitted_repo_families"], "blocked_rows": artifact["blocked_rows"]}, indent=2, sort_keys=True))
