...
STAGE = 10999
NAME = "stage10999_fresh_evidence_candidate_runtime_audit"
OUT_DIR = ARTIFACTS / NAME
SUMMARY_JSON = OUT_DIR / "fresh_evidence_candidate_runtime_audit.json"
...
def main() -> None:
    ...
    candidate_card = _write_bounded_choice_eval_audit(
        ...
        split_name="fresh_candidate_strict",
        bounded_choice_aux_source=source,
        eval_batch_size=8,
    )
    overlay_val = _write_bounded_choice_eval_audit(
        OUT_DIR,
        model=model,
        rows=load_jsonl(OVERLAY_VAL),
        tokenizer=tokenizer,
        max_encoder_tokens=768,
        max_decoder_tokens=8,
        split_name="overlay_eval_reference",
        bounded_choice_aux_source=source,
        eval_batch_size=8,
    )
    overlay_strict = _write_bounded_choice_eval_audit(
        OUT_DIR,
        model=model,
        rows=load_jsonl(OVERLAY_STRICT),
        tokenizer=tokenizer,
        max_encoder_tokens=768,
        max_decoder_tokens=8,
        split_name="overlay_strict_reference",
        bounded_choice_aux_source=source,
        eval_batch_size=8,
    )
    payload = {
        ...
        "passed": True,
        "claim_scope": [
            "Measure the current best standalone runtime on the fresh immediate-replenishment evidence candidate slice.",
            "Keep the overlay reference side-by-side so fresh-root behavior is interpreted against the stable canary rather than in isolation.",
        ],
        ...
        "next_best_step": "Use this fresh-root baseline to decide whether the 12 replenishment support rows deserve a clean training probe or should be held for a larger non-overlap package.",
    }
