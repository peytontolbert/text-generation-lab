#!/usr/bin/env python3
"""Roll Stage12017 NOT_EXERCISED rows into transition support v16."""
...
ROOT = Path("runs/local/artifacts/stage12018_transition_support_rollup_v16")
BASE = Path("runs/local/artifacts/stage12016_transition_support_rollup_v15/transition_support_rows_v15.jsonl")
ADDED = Path("runs/local/artifacts/stage12017_not_exercised_expansion_rows/not_exercised_expansion_rows.jsonl")
ROWS_OUT = ROOT / "transition_support_rows_v16.jsonl"
SUMMARY = ROOT / "transition_support_rollup_v16.json"
SUMMARY_MIRROR = Path("runs/summaries/stage12018_transition_support_rollup_v16.json")
...
def main() -> None:
    ...
    duplicates = 0
    for source, path in [("stage12016", BASE), ("stage12017", ADDED)]:
        for row in read_jsonl(path):
            out = dict(row)
            out["stage12018_source"] = source
            out["split"] = "train"
            out["split_role"] = "stage12018_transition_support_v16_train_support_only"
    ...
    summary = {
        "stage": "stage12018_transition_support_rollup_v16",
        ...
        "decision": "support_inventory_v16_not_train_ready",
        "reason": "NOT_EXERCISED supply improved, but PASS_TO_PASS and several language floors remain far short.",
        "claim_boundary": "Support inventory only. No model promotion or Gemma comparison should be claimed from v16.",
        "next_stage_recommendation": {"stage": "stage12019_transition_root_250_positive_supply_expansion", "action": "Return to independent executable PASS_TO_PASS roots and C++/Rust/Web language floors before training."},
    }
