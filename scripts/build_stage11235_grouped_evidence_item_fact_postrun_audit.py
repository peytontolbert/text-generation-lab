...
STAGE = 11235
NAME = "stage11235_grouped_evidence_item_fact_postrun_audit"
OUT_DIR = ARTIFACTS / NAME
SUMMARY_JSON = OUT_DIR / "grouped_evidence_item_fact_postrun_audit.json"

RUNTIME_BUNDLE = ARTIFACTS / "stage11234_grouped_evidence_item_fact_probe/runtime_model/runtime_model_bundle.json"
...
def enrich(card: dict[str, Any], source_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in card.get("row_cards") or []:
        ...
        merged = dict(row)
        for key in ["language_family", "repo_family", "task_type", "root_id", "source_root_id", "source_row_id", "residual_successor_source"]:
            merged[key] = source.get(key)
        merged["gold_value"] = projection.get("source_gold_value") or projection.get("gold_value") or source.get("gold_value")
        out.append(merged)
    return out
...
def main() -> None:
    ...
    grouped_diag_rows = load_jsonl(GROUPED_DIAG)
    ...
    grouped_diag = score_rows(model, tokenizer, grouped_diag_rows, f"grouped_evidence_item_fact_diag_{SCORER}")

    strict_summary = summarize(strict)
    validation_summary = summarize(validation)
    residual_summary = summarize(residual)
    grouped_summary = summarize(grouped_diag)
    ...
    gates = {
        ...
        "grouped_diag_nonzero": nested(grouped_summary, ["row_metric", "correct"], 0) > 0,
        "stage11233_complete_package": bool(package_summary.get("passed")),
    }
    ...
    summary = {
        ...
        "key_metrics": {
            ...
            "grouped_diag_accuracy": nested(grouped_summary, ["row_metric", "exact_accuracy"]),
        ...
        "grouped_diag": grouped_summary,
        "verifier_and_test_constraint_residual": {"row_metric": verifier_metric, "misses": misses(verifier_rows)},
