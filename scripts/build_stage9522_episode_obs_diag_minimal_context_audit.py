...
STAGE = 9522
NAME = "stage9522_episode_obs_diag_minimal_context_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9521_episode_obs_diag_minimal_context_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9521_episode_obs_diag_minimal_context_manifest/episode_obs_diag_minimal_context_manifest.jsonl"
CARD = ROOT / "runs/local/artifacts/stage9521_episode_obs_diag_minimal_context_manifest/episode_obs_diag_minimal_context_manifest_card.json"
OUT_DIR = ROOT / "runs/local/artifacts/stage9522_episode_obs_diag_minimal_context_audit"
AUDIT = OUT_DIR / "episode_obs_diag_minimal_context_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "EPISODE_OBS_DIAG_MINIMAL_CONTEXT_AUDIT_STAGE9522.md"
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9521_summary_not_passed")
    if card.get("passed") is not True:
        failures.append("stage9521_card_not_passed")
    if len(rows) != 58:
    ...
    for row in rows:
        ...
        if state.get("active_generation_prefix_span") != "redacted_literal_prefix_for_observation_diagnosis":
            literal_prefix_rows.append(row_id)
        required = [
            model_input.get("context_kept_language_family"),
            model_input.get("context_kept_bridge_error_family"),
            model_input.get("context_kept_prefix_token_bucket"),
            model_input.get("context_kept_action_type"),
    ...
    audit = {
        ...
        "decision": "Minimal-context diagnosis manifest is ready for contract-only preflight.",
    ...
    summary = {
        ...
        "next_best_step": "Run contract-only target-100M preflight for Stage9521 with max rows 46/6/6 and no decoder, denoise, runtime, checkpoint export, or promotion authority.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    DOC.write_text("\n".join([
        "# Stage9522 Episode Observation Diagnosis Minimal-Context Audit",
        "",
