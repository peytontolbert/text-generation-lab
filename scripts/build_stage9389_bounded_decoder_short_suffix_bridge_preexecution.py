...
STAGE = 9389
NAME = "stage9389_bounded_decoder_short_suffix_bridge_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9388_bounded_decoder_short_suffix_bridge_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9388_bounded_decoder_short_suffix_bridge_manifest/bounded_decoder_short_suffix_bridge_manifest.jsonl"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9390_bounded_decoder_short_suffix_bridge_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "bounded_decoder_short_suffix_bridge_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9390_bounded_decoder_short_suffix_bridge_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "BOUNDED_DECODER_SHORT_SUFFIX_BRIDGE_PREEXECUTION_STAGE9389.md"
...
COMMAND = [
    ...
    str(OUTPUT_DIR),
    "--run-id",
    "stage9390_bounded_decoder_short_suffix_bridge_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9388_not_passed")
    if len(rows) != 23:
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared a 23-row short-suffix denoise bridge probe. The probe trains denoise CE only on short continuation targets and leaves decoder CE closed.",
        "next_best_step": "Execute Stage9390 bounded decoder short-suffix bridge probe under trellis and audit exact short-suffix recovery.",
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage9389 Bounded Decoder Short-Suffix Bridge Preexecution",
                "",
                f"Passed: `{card['passed']}`",
                f"Rows: `{card['manifest_rows']}`",
                f"Splits: `{card['split_counts']}`",
                f"Routes: `{card['route_counts']}`",
                "",
                "Only the Stage9390 short-suffix denoise probe is authorized. Decoder CE and external authority remain closed.",
                "",
            ]
        ),
        encoding="utf-8",
