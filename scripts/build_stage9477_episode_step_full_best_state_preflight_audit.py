...
STAGE = 9477
NAME = "stage9477_episode_step_full_best_state_preflight_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9476_episode_obs_diag_best_state_probe_audit.json"
PREFLIGHT_DIR = ROOT / "runs/local/artifacts/stage9477_episode_step_full_best_state_target_100m_contract_preflight"
CONTRACT = PREFLIGHT_DIR / "probe_contract_audit.json"
AUDIT = PREFLIGHT_DIR / "stage9477_preflight_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "EPISODE_STEP_FULL_BEST_STATE_PREFLIGHT_AUDIT_STAGE9477.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
ENABLED = {"episode_boundary_match_ce", "episode_failure_type_ce", "episode_repair_outcome_ce", "episode_step_value_mse", "episode_target_prefix_match_ce"}
DISABLED = {"decoder_ce", "denoise_ce", "runtime_reward"}
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9476_not_passed")
    ...
    if contract.get("split_counts") != {"eval": 1, "other": 0, "strict_eval": 1, "train": 48}:
    ...
    if weights.get("eval_interval") != 8 or weights.get("restore_best_structured_state") is not True:
    ...
    audit = {
        "passed": not failures,
        "failures": failures,
        "source_stage": "stage9476_episode_obs_diag_best_state_probe_audit",
    ...
    summary = {
        ...
        "decision": "Authorized only the next target-100M full five-head episode-step structured probe with in-memory best-state restore if this preflight passes.",
        "next_best_step": "Run Stage9478 full five-head episode-step best-state-restored probe under trellis, then audit final eval/strict exactness and best-state telemetry.",
    ...
    DOC.write_text("\n".join([
        ...
        f"Enabled losses: `{sorted(ENABLED)}`",
        "",
