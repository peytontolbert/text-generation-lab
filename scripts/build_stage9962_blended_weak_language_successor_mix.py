...
STAGE = 9962
NAME = "stage9962_blended_weak_language_successor_mix"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
BLEND = OUT_DIR / "blended_weak_language_successor_structured_state.jsonl"
AUDIT = OUT_DIR / "blended_weak_language_successor_mix_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "BLENDED_WEAK_LANGUAGE_SUCCESSOR_MIX_STAGE9962.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"

BASE_STRUCTURED = ROOT / "runs/local/artifacts/stage9945_web_targeted_blended_structured_mix/web_targeted_blended_structured_state.jsonl"
...
BASE_AUDIT = ROOT / "runs/local/artifacts/stage9945_web_targeted_blended_structured_mix/web_targeted_blended_structured_mix_audit.json"
...
def build_blend() -> dict[str, Any]:
    ...
    if metrics["base_structured_rows"] != 404:
        failures.append("base_structured_rows_not_404")
    if metrics["recovery_rows_added"] != 48:
        failures.append("recovery_rows_added_not_48")
    if metrics["blended_structured_rows"] != 452:
        failures.append("blended_structured_rows_not_452")
    if metrics["edit_localization_rows_before"] != 72:
        failures.append("edit_localization_rows_before_not_72")
    if metrics["edit_localization_rows_after"] != 120:
        failures.append("edit_localization_rows_after_not_120")
    if metrics["python_edit_rows_after"] != 24:
        failures.append("python_edit_rows_after_not_24")
    if metrics["c_cpp_edit_rows_after"] != 30:
    ...
    if metrics["loss_counts"].get("edit_localization_ce") != 120:
        failures.append("edit_localization_ce_not_120")
    ...
    return {
        ...
        "training_recommendation": {
            ...
            "blend_strategy": "append the stage9961 weak-language recovery packet to the existing web-recovered successor mix and preserve curriculum_priority_weight during sampling",
            "expected_edit_localization_row_delta": metrics["edit_localization_rows_after"] - metrics["edit_localization_rows_before"],
            "why": "this preserves the web-recovery gains while increasing python and c_cpp edit-localization coverage on the exact weak roots that lost to Gemma or the task itself.",
        },
        "authority": dict(AUTHORITY_CLOSED),
    }
...
def main() -> None:
    ...
    next_step = (
        "Use this successor blended structured-state mix as the next 100M edit-localization training input so python, c_cpp, and web_js_ts_html weak roots are all replayed together while rust winning roots remain anchored."
    )
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        ...
        "decision": "Built a successor structured-state mix that appends the stage9961 weak-language recovery packet to the existing web-recovered multilingual blend, materially increasing the exact weak-root edit-localization coverage for python, c_cpp, and web_js_ts_html.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9962 Blended Weak-Language Successor Mix",
        "",
