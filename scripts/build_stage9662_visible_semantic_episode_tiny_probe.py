...
STAGE = 9662
NAME = "stage9662_visible_semantic_episode_tiny_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9661_visible_semantic_episode_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9661_visible_semantic_episode_manifest/visible_semantic_episode_manifest.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
RUN_DIR = OUT_DIR / "episode_step_probe"
AUDIT = OUT_DIR / "visible_semantic_episode_tiny_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "VISIBLE_SEMANTIC_EPISODE_TINY_PROBE_STAGE9662.md"
...
EXPECTED_FIELDS = {"episode_failure_type", "episode_repair_outcome", "episode_step_value"}
...
def command() -> list[str]:
    return ["env", f"TMPDIR={TMPDIR}", f"TEMP={TMPDIR}", f"TMP={TMPDIR}", "conda", "run", "-n", "trellis", "python", str(TRAINER),
        ...
        "--output-dir", str(RUN_DIR), "--run-id", "stage9662_visible_semantic_episode_probe", "--execution-authorized-for-recovery-probe"]
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        safety_failures.append("stage9661_not_passed")
    if int(source_metrics.get("rows") or 0) != 66:
        safety_failures.append("stage9661_unexpected_row_count")
