...
STAGE = 11495
NAME = "stage11495_residual50_candidate_set_postrun_audit"
OUT = ART / NAME
SUMMARY = OUT / "residual50_candidate_set_postrun_audit.json"

RUNTIME = ART / "stage11494_residual50_candidate_set_probe/runtime_model/runtime_model_bundle.json"
...
STAGE11483 = ART / "stage11484_residual50_controlled_probe_postrun_audit/residual50_controlled_probe_postrun_audit.json"
STAGE11487 = ART / "stage11488_residual50_sampler_ablation_postrun_audit/residual50_sampler_ablation_postrun_audit.json"
...
def metric(card: dict[str, Any], split: str) -> Any:
    return (((card.get("scored") or {}).get(PRODUCT_SCORER) or {}).get(split) or {}).get("correct")
...
def main() -> None:
    ...
    stage11483 = load_json_opt(STAGE11483)
    stage11487 = load_json_opt(STAGE11487)
    rows = {
    ...
    summary = {
        ...
        "created_at_utc": now(),
        "passed": True,
        "decision": "residual50_candidate_set_probe_promoted" if promoted else "residual50_candidate_set_probe_rejected_or_diagnostic_only",
        ...
        "comparison_to_prior_residual50_runs": {
            "stage11483_filtered_strict": metric(stage11483, "filtered_strict"),
            "stage11483_old_canary_strict": metric(stage11483, "old_canary_strict"),
            "stage11483_residual_bank": metric(stage11483, "residual_bank"),
            "stage11487_filtered_strict": metric(stage11487, "filtered_strict"),
            "stage11487_old_canary_strict": metric(stage11487, "old_canary_strict"),
            "stage11487_residual_bank": metric(stage11487, "residual_bank"),
            "stage11494_filtered_strict": product["filtered_strict"]["correct"],
            "stage11494_old_canary_strict": product["old_canary_strict"]["correct"],
            "stage11494_residual_bank": product["residual_bank"]["correct"],
        },
        "decision_basis": [
            "Stage11494 changed Residual-50 data geometry by training on deconflicted candidate-set rows from Stage11492.",
            "Init runtime, product scorer, loss weights, residual-family sampler, and protected splits match the Stage11483 contract.",
            "Promotion requires product scorer encoder_option_retrieval to preserve old/filtered strict and validation while improving residual bank above 5/10.",
        ],
        "source_artifacts": {
            "runtime_bundle": rel(RUNTIME),
            "request": rel(REQUEST),
            "package": rel(PACKAGE),
            "stage11483_reference": rel(STAGE11483),
            "stage11487_reference": rel(STAGE11487),
    ...
    print(json.dumps({"decision": summary["decision"], "promotion_gates": gates, "comparison_to_prior_residual50_runs": summary["comparison_to_prior_residual50_runs"], "product": product}, indent=2, sort_keys=True))
