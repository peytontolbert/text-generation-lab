...
STAGE = 9330
NAME = "stage9330_phrase_router_mixture_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9329_phrase_router_probe_preexecution.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9328_phrase_router_mixture_manifest/phrase_router_mixture_manifest.jsonl"
RUN_DIR = ROOT / "runs/local/artifacts/stage9330_phrase_router_mixture_probe"
AUDIT = RUN_DIR / "stage9330_phrase_router_mixture_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PHRASE_ROUTER_MIXTURE_PROBE_AUDIT_STAGE9330.md"
...
def audit_run() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9329_not_passed")
    ...
    return {
        ...
        "by_route": _bucket(samples, manifest_by_id, "route"),
        "by_split": _bucket(samples, manifest_by_id, "split"),
        "diagnosis": "route_conditioned_semantic_discriminators_solve_phrase_family_interference_for_tiny_probe",
        "next_patch_target": "merge_route_conditioned_phrase_rows_back_with_stage9310_prefix_ladder_and_rerun_combined_suffix_probe",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Route-conditioned phrase mixture passed exactly and repaired the Stage9321 phrase-family interference.",
        "next_best_step": "Merge route-conditioned phrase rows back with the Stage9310 prefix ladder and rerun the combined suffix probe.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9330 Phrase Router Mixture Probe Audit",
        "",
        ...
        "Opaque route IDs plus non-label semantic discriminators solved the tiny phrase-family interference probe.",
