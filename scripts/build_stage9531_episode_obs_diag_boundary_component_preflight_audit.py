...
STAGE = 9531
NAME = "stage9531_episode_obs_diag_boundary_component_preflight_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9530_episode_obs_diag_boundary_component_audit.json"
RUN_DIR = ROOT / "runs/local/artifacts/stage9531_episode_obs_diag_boundary_component_target_100m_contract_preflight"
CONTRACT = RUN_DIR / "probe_contract_audit.json"
AUDIT = RUN_DIR / "stage9531_preflight_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "EPISODE_OBS_DIAG_BOUNDARY_COMPONENT_PREFLIGHT_STAGE9531.md"
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9530_not_passed")
    ...
    if losses.get("episode_failure_type_ce") != 58:
    ...
    audit = {
        "passed": not failures,
        "failures": failures,
        "source_stage": "stage9530_episode_obs_diag_boundary_component_audit",
    ...
    summary = {
        ...
        "decision": "Contract-only target-100M preflight passed for the boundary-component failure-type-only manifest.",
        "next_best_step": "Run Stage9532 target-100M boundary-component failure-type-only probe under trellis, then audit whether eval and strict are both exact.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    DOC.write_text("\n".join([
        "# Stage9531 Episode Observation Diagnosis Boundary-Component Preflight",
        "",
