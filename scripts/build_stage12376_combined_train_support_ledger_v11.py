...
STAGE = "stage12376_combined_train_support_ledger_v11"
OUT = ROOT / "runs/local/artifacts" / STAGE
SUMMARY = ROOT / "runs/summaries" / f"{STAGE}.json"
BASE_SCRIPT = ROOT / "scripts/build_stage12375_combined_train_support_ledger_v10.py"
BASE_SUMMARY = ROOT / "runs/summaries/stage12375_combined_train_support_ledger_v10.json"
BASE_ROWS = ROOT / "runs/local/artifacts/stage12375_combined_train_support_ledger_v10/combined_selected_test_rows_v10.jsonl"
PY_RERENDER = ROOT / "runs/local/artifacts/stage12374_python_task_specific_selected_test_rerender/python_task_specific_selected_test_rows.jsonl"
SUPERSEDED_ROOTS = {
...
def main() -> None:
    ...
    py_rows = read_jsonl(PY_RERENDER)
    ...
    selected_rows = kept_base_rows + py_rows
    ...
    for row in selected_rows:
        ...
        if row.get("stage") == "stage12374_python_task_specific_selected_test_rerender":
            semantic_by_root.setdefault(row.get("root_id") or row.get("row_id"), set()).add(target_semantic(row))
            source_refs = row.get("source_refs") or {}
            if not source_refs.get("commit_sha"):
                empty_commit_rows += 1
    ...
    source_counts["stage12326_direct_python_selected_test_superseded"] = 0
    source_counts["stage12374_python_task_specific_rerender"] = len(py_rows)
    total = NON_SELECTED_BASE_ROWS + len(selected_rows)
    ledger = dict(base)
    ledger.update({
        "stage": STAGE,
        "decision": "combined_train_support_ledger_v11_ready_500_not_reached" if not anti_collapse_failures and not risky_counts(selected_rows) and empty_commit_rows == 0 else "combined_train_support_ledger_v11_needs_review",
        ...
        "supersedes": [
            str(BASE_SUMMARY),
            str(ROOT / "runs/summaries/stage12374_python_task_specific_selected_test_rerender.json"),
        ],
        ...
        "selected_test_rows_superseded_from_v10": len(superseded_rows),
        ...
        "stage12374_target_semantic_values_by_root": {root: sorted(values) for root, values in semantic_by_root.items()},
        "stage12374_empty_commit_rows_should_be_zero": empty_commit_rows,
        "anti_collapse_failures_should_be_empty": anti_collapse_failures,
        "training_blockers": [
            "500_train_support_target_not_reached",
            "Web fallback re-render pending",
            "Open-SWE/MCP transformation rows not yet admitted under Stage12364 policy",
        ],
    })
    OUT.mkdir(parents=True, exist_ok=True)
    write_jsonl(OUT / "combined_selected_test_rows_v11.jsonl", selected_rows)
    write_jsonl(OUT / "superseded_selected_test_rows_v11.jsonl", superseded_rows)
    (OUT / "combined_train_support_ledger_v11.json").write_text(json.dumps(ledger, indent=2, sort_keys=True) + "\n", encoding="utf-8")
