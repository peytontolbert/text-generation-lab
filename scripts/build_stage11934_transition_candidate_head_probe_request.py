#!/usr/bin/env python3
"""Emit a transition-projection candidate-head probe with contrast plus listwise losses."""
...
STAGE = 11934
NAME = "stage11934_transition_candidate_head_probe_request"
OUT = ART / NAME
SUMMARY = OUT / "transition_candidate_head_probe_request.json"
COMMAND_JSON = OUT / "transition_candidate_head_command.json"
MANIFEST = OUT / "transition_candidate_head_manifest.jsonl"
...
RUN_DIR = ART / "stage11935_transition_candidate_head_probe"
...
def train_copy(row: dict[str, Any]) -> dict[str, Any]:
    out = dict(row)
    out["row_id"] = f"{row.get('row_id')}::stage11934_transition_candidate_head"
    out["split"] = "train"
    out["package_split"] = "train"
    out["train_support_only"] = True
    out["strict_eval_eligible"] = False
    out["stage11934_replay_kind"] = "transition_projection_candidate_head"
...
def main() -> None:
    ...
    gates = {
        ...
        "transition_candidate_head_enabled": "encoder_option_retrieval_transition_candidate_head" in command,
        ...
        "no_protected_rows_in_train": all(str(row.get("stage11934_replay_kind")) == "transition_projection_candidate_head" for row in train_rows),
    }
    artifact = {
        "stage": STAGE,
        "stage_name": NAME,
        ...
        "decision": "transition_candidate_head_probe_request_ready" if all(gates.values()) else "transition_candidate_head_probe_request_blocked",
        "command": command,
        ...
        "hypothesis": "Stage11924 trails Gemma because the semantic candidate head under-represents transition task, verifier status, language, and artifact metadata.",
        "intervention": "Continue from Stage11924, add/train only a transition-specific candidate head, retain transition contrast, and retain same-role plus verifier-value listwise losses.",
        "promotion_gate": {
