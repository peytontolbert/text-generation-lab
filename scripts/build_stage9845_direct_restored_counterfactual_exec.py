...
STAGE = 9845
NAME = "stage9845_direct_restored_counterfactual_exec"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
RUN_DIR = OUT_DIR
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "DIRECT_RESTORED_COUNTERFACTUAL_EXEC_STAGE9845.md"
AUDIT = OUT_DIR / "direct_restored_counterfactual_exec_audit.json"
...
def build_runner() -> None:
    runner_text = """import json\nimport sys\nfrom pathlib import Path\nroot=Path('/data/agentkernel-seq2seq-text-lab')\nlegacy=root/'legacy_src'\nsys.path.insert(0, str(legacy))\nfrom agentkernel_lite.training_loop import run_structured_aux_probe\nrows=[json.loads(line) for line in (root/'runs/local/artifacts/stage9844_restored_counterfactual_probe_manifest/restored_counterfactual_probe_manifest.jsonl').read_text().splitlines() if line.strip()]\nout=root/'runs/local/artifacts/stage9845_direct_restored_counterfactual_exec'\nres=run_structured_aux_probe(rows=rows, output_dir=out, run_id='stage9845_direct_restored_counterfactual_exec', mode='edit_localization_probe', max_train_rows=20, max_eval_rows=20, max_strict_rows=20, max_steps=64, batch_size=2, max_encoder_tokens=512, max_decoder_tokens=4, learning_rate=5e-5, implementation='transformer', structured_trainable_profile='full_non_decoder', probe_scale='target_100m', model_config=root/'configs/model/agentkernel_100m_seq2seq_recovered_target.json', tokenizer_json=root/'configs/tokenizer/agentkernel_bpe_1506/tokenizer.json', tokenizer_config=root/'configs/tokenizer/agentkernel_bpe_1506/tokenizer_config.json')\nprint(json.dumps(res, indent=2, sort_keys=True))\n"""
...
def main() -> None:
    ...
    next_step = "Compare these restored ablation-style and contradictory-evidence 100M outputs against same-surface Gemma outputs to see whether the multilingual win survives the harder probe families."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Executed the restored evidence-removed and contradictory-evidence multilingual probe directly through the structured training loop so the harder anti-cheat packet emits real row-level 100M artifacts.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9845 Direct Restored Counterfactual Exec",
        "",
        f"Passed: `{summary['passed']}`",
        f"Runtime executed: `{audit['runtime_executed']}`",
        f"Row artifacts present: `{audit['required_row_artifacts_present']}`",
        "",
        "This stage bypasses the outer trainer wrapper and runs the restored counterfactual manifest directly so the harder anti-cheat packet emits usable row-level artifacts.",
        "",
        f"Next: {next_step}",
        "",
    ]), encoding="utf-8")
