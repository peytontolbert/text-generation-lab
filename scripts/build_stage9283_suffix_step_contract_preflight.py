...
STAGE = 9283
NAME = "stage9283_suffix_step_contract_preflight"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9282_suffix_step_micro_overfit_manifest.json"
...
AUDIT = OUT_DIR / "suffix_step_contract_preflight_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "SUFFIX_STEP_CONTRACT_PREFLIGHT_STAGE9283.md"
...
def _denoise_runtime_support() -> dict[str, Any]:
    ...
    prefix_contract = "validate_generation_prefix_contract" in trainer_text and "generation_prefix_field" in trainer_text
    return {
        "has_native_denoise_runner": has_native_runner,
        "trainer_dispatches_native_denoise_runner": dispatches_native,
        "generation_prefix_contract_supported": prefix_contract,
        "execution_supported": has_native_runner and dispatches_native and prefix_contract,
    }
...
def audit_preflight() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9282_not_passed")
    if not rows:
    ...
    prefix_target_mismatch_rows = 0
    over_decoder_token_cap_rows = 0
    ...
    for row in rows:
        ...
        # Byte tokenizer upper bound: chars <= bytes <= tokens approximately for ASCII-heavy rows.
        if len(target.encode("utf-8")) > 160:
            over_decoder_token_cap_rows += 1
        visible = json.dumps({"input_state": row.get("input_state"), "model_input": row.get("model_input"), "corrupted_output": row.get("corrupted_output")}, sort_keys=True)
        full_hash = str((row.get("input_state") or {}).get("full_target_sha256") or "")
        if full_hash and target and target in visible:
    ...
    return {
        ...
        "suffix_step_execution_ready": execution_ready,
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Stage9282 suffix-step manifest passes trainer contract preflight with generation-prefix enforcement and denoise-only loss masks. Execution remains closed pending a one-run review.",
        "next_best_step": "Build a suffix-step one-run execution review card, then run a final pre-execution audit before any tiny target-100M denoise execution.",
    ...
    DOC.write_text(
        "\n".join([
            ...
            f"Suffix-step execution ready: {audit['suffix_step_execution_ready']}",
