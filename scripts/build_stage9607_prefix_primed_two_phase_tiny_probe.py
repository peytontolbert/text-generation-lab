...
STAGE = 9607
NAME = "stage9607_prefix_primed_two_phase_tiny_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9606_prefix_primed_two_phase_contract_preflight.json"
...
AUDIT = OUT_DIR / "prefix_primed_two_phase_tiny_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PREFIX_PRIMED_TWO_PHASE_TINY_PROBE_STAGE9607.md"
...
def command() -> list[str]:
    return [
        ...
        str(RUN_DIR),
        "--run-id",
        "stage9607_prefix_primed_two_phase_probe",
...
def main() -> None:
    ...
    next_step = "Audit Stage9607 generation samples and token losses; if quality passes, design a slightly wider prefix-primed residual denoise probe, otherwise patch the remaining failure bucket."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Executed the prefix-primed in-memory two-phase suffix-choice plus residual-denoise tiny probe under closed runtime/export authority.",
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage9607 Prefix-Primed Two-Phase Tiny Probe",
                "",
