...
STAGE = 9326
NAME = "stage9326_phrase_b_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9325_phrase_b_probe_preexecution.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9322_phrase_family_split_manifests/phrase_b_patch_operator_manifest.jsonl"
RUN_DIR = ROOT / "runs/local/artifacts/stage9326_phrase_b_patch_operator_probe"
AUDIT = RUN_DIR / "stage9326_phrase_b_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PHRASE_B_PROBE_AUDIT_STAGE9326.md"
...
def audit_run() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9325_not_passed")
    ...
    return {
        ...
        "diagnosis": "phrase_b_patch_operator_is_individually_learnable_and_fails_only_under_mixed_phrase_curriculum",
        "next_patch_target": "design_phrase_router_or_source_balanced_mixture_before_merging_phrase_a_and_b",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Phrase B passed in isolation; patch-operator failures are caused by mixed-curriculum interference, not local unlearnability.",
        "next_best_step": "Design an explicit phrase router or source-balanced mixture before recombining phrase A and B.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9326 Phrase B Probe Audit",
        "",
        ...
        "Phrase B (`patch operator`) is individually learnable. Authority is closed after the audit.",
        "",
    ]), encoding="utf-8")
