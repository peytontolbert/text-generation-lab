#!/usr/bin/env python3
"""Combined C/C++ abstain-attractor support admission audit."""
...
STAGE = 11768
NAME = "stage11768_cpp_abstain_support_combined_audit_v2"
OUT = ART / NAME
SUMMARY = OUT / "cpp_abstain_support_combined_audit_v2.json"
COMBINED_ROWS = OUT / "cpp_abstain_support_rows_v2.jsonl"
...
def failures_for(row: dict[str, Any]) -> list[str]:
    ...
    for key in ["deterministic_option_shuffle", "opaque_labels", "target_label_not_visible_before_options", "target_value_not_visible_before_options", "source_backed_snippets", "paired_answerable_and_abstain_variants"]:
        if not anti.get(key):
...
def main() -> None:
    ...
    artifact: dict[str, Any] = {
        ...
        "quota_status": {"support_roots_ready": len(roots), "support_roots_remaining": max(0, 16 - len(roots)), "strict_analogue_roots_ready": 0, "strict_analogue_roots_remaining": 4},
        "audited_rows": audited,
        "claim_boundary": ["Rows are train-support-only for C/C++ abstain-attractor repair.", "The Stage11742 C/C++ quota remains incomplete."],
        "outputs": {"summary": rel(SUMMARY), "combined_rows": rel(COMBINED_ROWS)},
    }
    write_json(SUMMARY, artifact)
    SUMMARIES.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(SUMMARY, SUMMARIES / f"{NAME}.json")
    print(json.dumps({"decision": artifact["decision"], "admitted_rows": artifact["admitted_rows"], "admitted_root_count": artifact["admitted_root_count"], "support_roots_remaining": artifact["quota_status"]["support_roots_remaining"]}, indent=2, sort_keys=True))
