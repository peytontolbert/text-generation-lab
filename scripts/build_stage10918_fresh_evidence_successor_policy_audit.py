...
STAGE = 10918
NAME = "stage10918_fresh_evidence_successor_policy_audit"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
OUT_JSON = OUT_DIR / "fresh_evidence_successor_policy_audit.json"
...
ROWS_JSONL = ROOT / "runs/local/artifacts/stage10915_evidence_successor_strict_candidates/strict_candidate_rows.jsonl"
...
def single_token_id(tokenizer: AgentKernelBPETokenizer, text: str) -> int | None:
    ids = [
        idx
        for idx in tokenizer.encode(text, max_length=8)
        if idx not in {int(getattr(tokenizer, "bos_id", 1)), int(getattr(tokenizer, "eos_id", 2))}
    ]
    if len(ids) != 1:
        return None
    return int(ids[0])
...
def policy_prediction(
    ...
    return {
        ...
        "target_value": next((value for label, value in option_pairs(row) if label == str(row["target_text"])), None),
        "raw_pred": raw_pred,
        "role_pred": role_pred,
...
def summarize(cards: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for card in cards:
        by_repo.setdefault(card["repo_id"], []).append(card)
    return {
        "rows": rows,
        "correct": correct,
        ...
        "by_repo": {
            ...
            for key, items in sorted(by_repo.items())
        },
...
def main() -> None:
    ...
    payload = {
        ...
        "claim_scope": [
            "Compare raw retrieval, evidence-role mapped retrieval, decoder-label, and hybrid decoder-on-evidence scoring on the fresh Python/C++ evidence successor slice.",
            "Determine whether any evidence-citation interface candidate is strong enough on the fresh slice to justify a broader multilingual retry once a web control exists.",
        ],
        "policies": {},
    }
