#!/usr/bin/env python3
"""Score a loadable 100M bundle plus the learned Stage996 pair comparator."""
...
def main() -> None:
    ...
    parser.add_argument("--output-json", type=Path, default=Path("runs/local/artifacts/stage997_100m_learned_pair_comparator_hybrid_summary.json"))
    parser.add_argument("--predictions-jsonl", type=Path, default=Path("runs/local/artifacts/stage997_100m_learned_pair_comparator_hybrid_predictions.jsonl"))
    ...
    pair_docs = {split: stage995._unique_docs(split_rows) for split, split_rows in rows_by_split.items()}
    pair_doc_to_index = {
        ...
        split: [
            {
                ...
                "suffixes": [str(item) for item in (row.get("query_bridge", {}) or {}).get("qpair", []) or []],
            }
    ...
    policy_by_operation = {
        "atomic_fact": "base",
        "composition": "stage996_learned_pair_comparator",
        "counterfactual_false_claim": "base",
        "exception": "stage976_model_owned_blend",
        "relation": "stage996_learned_pair_comparator",
    }
    ...
    summary = {
        "artifact_kind": "stage997_100m_learned_pair_comparator_hybrid",
        "status": "completed_loadable_learned_pair_comparator_hybrid_scorer",
        ...
        "decision": "Loadable scorer combining Stage976 100M model-owned exception residual with the Stage996 learned encoder-token pair equality/count comparator for composition and relation. Eval does not use qpair/dpair suffix set intersection, but still uses qpair/dpair marker tokens as the located comparison interface.",
    }
