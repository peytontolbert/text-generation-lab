...
STAGE = 9627
NAME = "stage9627_hard_phrase_tri_phase_tiny_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9626_hard_phrase_tri_phase_contract_preflight.json"
...
PHASE2_MANIFEST = ROOT / "runs/local/artifacts/stage9625_targeted_hard_phrase_continuation_manifest/targeted_hard_phrase_continuation_manifest.jsonl"
...
AUDIT = OUT_DIR / "tri_phase_reconnect_tiny_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "HARD_PHRASE_TRI_PHASE_TINY_PROBE_STAGE9627.md"
...
def command() -> list[str]:
    return [
        ...
        "--phase2-max-train-rows", "15", "--phase2-max-eval-rows", "4", "--phase2-max-strict-rows", "3",
        ...
        "--output-dir", str(RUN_DIR), "--run-id", "stage9627_hard_phrase_tri_phase_probe",
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9626_not_passed")
    ...
    next_step = "Audit Stage9627 samples and compare against Stage9623; if quality passes, design the next controlled residual-denoise expansion, otherwise patch the remaining hard-phrase failure bucket."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Ran tiny hard-phrase tri-phase reconnect probe under patched in-memory trainer. Quality gate is recorded separately from safety pass.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9627 Hard-Phrase Tri-Phase Tiny Probe",
        "",
