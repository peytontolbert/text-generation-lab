...
STAGE = 10777
NAME = "stage10777_bulk_support_candidate_ai_gold_drafts"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
SUMMARY_JSON = OUT_DIR / "bulk_support_candidate_ai_gold_drafts.json"
INDEX_JSONL = OUT_DIR / "bulk_support_candidate_ai_gold_index.jsonl"
SUMMARY_CARD = ROOT / "runs/summaries" / f"{NAME}.json"

ENRICHED_INDEX = ROOT / "runs/local/artifacts/stage10776_bulk_support_ready_packet_enrichment/enriched_packet_index.jsonl"
...
def is_test_path(value: str) -> bool:
    ...
    return (
        ...
        or name.endswith(".cpp") and "test" in name
    )
...
def snippet_plan(enriched: dict[str, Any]) -> dict[str, Any]:
    ...
    unique_paths = []
    seen = set()
    for value in [primary] + changed[:4] + tests[:4]:
...
def main() -> None:
    ...
    index_rows.sort(key=lambda r: (r["language_family"], -int(r["priority_score"]), int(r["queue_rank"]), r["root_id"]))
    payload = {
        "stage": STAGE,
        "stage_name": NAME,
        "created_at_utc": now_utc(),
        "passed": True,
        "decision": "bulk_support_candidate_ai_gold_drafts_attached",
        "claim_scope": [
            "Attach provisional AI gold drafts to the 18 enriched C/C++ and Python support candidates.",
        ...
        "headline_findings": [
            "All 18 enriched C/C++ and Python support candidates now carry provisional AI gold drafts across all eight perspectives.",
            ...
            "This is enough to package the candidates into the next reviewed multilingual support stage while keeping strict eval untouched.",
        ],
        "next_best_step": "Project these provisional-gold candidates into a reviewed support-only multilingual package, then use that package as the next training base while Rust/Web continue through richer competition and source repair.",
        "source_artifacts": {
            "enriched_index": display(ENRICHED_INDEX),
        },
        "outputs": {
            "summary_json": display(SUMMARY_JSON),
            "bulk_support_candidate_ai_gold_index": display(INDEX_JSONL),
        },
    }
