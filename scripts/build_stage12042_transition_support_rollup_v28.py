#!/usr/bin/env python3
"""Roll Stage12041 focused PASS_TO_PASS rows into support v28."""
...
ROOT = Path("runs/local/artifacts/stage12042_transition_support_rollup_v28")
BASE = Path("runs/local/artifacts/stage12040_transition_support_rollup_v27/transition_support_rows_v27.jsonl")
ADDED = Path("runs/local/artifacts/stage12041_focused_pass_to_pass_rows/focused_pass_to_pass_rows.jsonl")
ROWS_OUT = ROOT / "transition_support_rows_v28.jsonl"
SUMMARY = ROOT / "transition_support_rollup_v28.json"
SUMMARY_MIRROR = Path("runs/summaries/stage12042_transition_support_rollup_v28.json")
...
def main() -> None:
    ...
    duplicates = 0
    for source, path in [("stage12040", BASE), ("stage12041", ADDED)]:
        for row in read_jsonl(path):
            out = dict(row)
            out["stage12042_source"] = source
            out["split"] = "train"
            out["split_role"] = "stage12042_transition_support_v28_train_support_only"
    ...
    summary = {
        "stage": "stage12042_transition_support_rollup_v28",
        ...
        "focused_pass_to_pass_added": len(read_jsonl(ADDED)),
        "decision": "support_inventory_v28_not_train_ready" if not train_ready else "support_inventory_v28_floor_ready_requires_anti_cheat_audit",
        "reason": "Focused PASS_TO_PASS coverage improved, but PASS_TO_PASS plus four other status floors remain unmet." if not train_ready else "Floor counts are met; run anti-cheat and split audits before training.",
        "claim_boundary": "Support inventory only. No model promotion or Gemma comparison should be claimed from v28.",
        "next_stage_recommendation": {"stage": "stage12043_status_floor_closure", "action": "Continue PASS_TO_PASS coverage while also adding semantic FAIL_TO_PASS, PASS_CURRENT_BUILD_AND_RUN, INSUFFICIENT_EVIDENCE, and NOT_EXERCISED rows."},
    }
