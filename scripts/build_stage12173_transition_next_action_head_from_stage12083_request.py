...
NAME = "stage12173_transition_next_action_head_from_stage12083_request"
OUT = ART / NAME
...
INIT_RUNTIME = ART / "stage12083_next_action_repair_training_probe/runtime_model/runtime_model_bundle.json"
...
RUN_DIR = ART / "stage12174_transition_next_action_head_from_stage12083_probe"
...
def normalize_train_row(row: dict[str, Any], source: str) -> dict[str, Any]:
    out = dict(row)
    out["row_id"] = f"{row.get('row_id')}::stage12173_next_action_head"
    out["split"] = "train"
...
def main() -> None:
    ...
    request = {
        "stage": NAME,
        "created_at_utc": now(),
        "decision": "request_only_ready_for_guarded_stage12174_execution" if all(gates.values()) else "blocked_before_training",
        "execute_now": False,
        "training_allowed_by_request": False,
        "promotion_eligible": False,
        "claim_boundary": "Diagnostic request only. It tests whether initializing the frozen semantic base from Stage12083 and training only bounded_choice_transition_next_action_head can improve the product-route next_action component without shared-head interference.",
        "command": command,
