...
STAGE = 10014
NAME = "stage10014_quarantined_blended_weak_language_same_manifest_handoff_bundle"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
BUNDLE = OUT_DIR / "quarantined_blended_weak_language_same_manifest_handoff_bundle.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "QUARANTINED_BLENDED_WEAK_LANGUAGE_SAME_MANIFEST_HANDOFF_BUNDLE_STAGE10014.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
REQUEST_100M = ROOT / "runs/local/artifacts/stage10013_quarantined_blended_weak_language_target100m_execution_request/surface_requests/edit_localization.json"
...
CELL_KEY = "quarantined_blended_target100m::edit_localization::same_manifest_weak_language_gemma12b"
FUTURE_100M_STAGE = 10013
FUTURE_GEMMA_STAGE = 10015
FUTURE_GEMMA_STAGE_PREFIX = "stage10015_"
...
load_jsonl = _load_symbol("stage10014_runner_load_jsonl", RUNNER, "load_jsonl")
prompt_surface_hash = _load_symbol("stage10014_runner_prompt_surface_hash", RUNNER, "prompt_surface_hash")
...
def _future_output_stub() -> str:
    return f"runs/local/artifacts/{FUTURE_GEMMA_STAGE_PREFIX}quarantined_blended_weak_language_gemma_execution/same_prompt_surface_gemma12b_outputs.json"
...
def build_bundle() -> dict[str, Any]:
    ...
    gemma_command = [
        ...
        "--queue",
        "runs/local/artifacts/stage10014_quarantined_blended_weak_language_same_manifest_handoff_bundle/quarantined_blended_weak_language_gemma_queue.json",
        "--packets",
        "runs/local/artifacts/stage10014_quarantined_blended_weak_language_same_manifest_handoff_bundle/quarantined_blended_weak_language_gemma_packets.jsonl",
        "--model",
        "gemma3:12b",
        "--cell-key",
        CELL_KEY,
    ...
    queue_entry = {
        ...
        "skill_area": "edit_localization",
        "priority_rank": 1,
        "priority_reason": "same-manifest Gemma-12B comparison for the quarantined weak-language recovery execution candidate",
        ...
        "remaining_blockers": [
            ...
            "stage10013_100m_outputs_should_exist_before_final_same_surface_claim",
        ],
    ...
    handoff = {
        ...
        "why_this_bundle_exists": [
            "future_stage10013_100m_and_stage10015_gemma_must_stay_on_the_same_manifest",
            "the quarantined weak-language recovery slice is the current truthful path to close python and c_cpp while preserving web recovery",
        ...
        "comparison_rule": {
            ...
            "claim_permitted_only_after": [
                "stage10013_output_acceptance_ready",
                "stage10015_gemma_outputs_exist",
                "same_manifest_row_contract_still_matches_112_total_19_python_27_c_cpp_45_web",
    ...
    metrics = {
        ...
        "row_contract_ok": row_contract["hundred_m_rows"] == 112 and row_contract["gemma_rows"] == 112 and row_contract["hundred_m_python_rows"] == 19 and row_contract["hundred_m_c_cpp_rows"] == 27 and row_contract["hundred_m_web_rows"] == 45,
    ...
    if metrics["hundred_m_future_stage"] != FUTURE_100M_STAGE:
        failures.append("hundred_m_future_stage_not_10013")
    if metrics["gemma_future_stage"] != FUTURE_GEMMA_STAGE:
        failures.append("gemma_future_stage_not_10015")
    if metrics["row_contract_ok"] is not True:
        failures.append("row_contract_not_ok")
    if metrics["same_manifest_compare_rows"] != 72:
        failures.append("same_manifest_compare_rows_not_72")
...
def main() -> None:
    ...
    queue_path = OUT_DIR / "quarantined_blended_weak_language_gemma_queue.json"
    packets_path = OUT_DIR / "quarantined_blended_weak_language_gemma_packets.jsonl"
    ...
    next_step = "Use this bundle when execution is explicitly authorized: run the stage10013 100M command, run the matching Gemma queue command, then compare only those outputs on the exact same 112-row quarantined manifest."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Materialized a same-manifest handoff bundle that joins the quarantined weak-language 100M run, the matching Gemma run, and the filtered comparison rule into one comparison packet.",
