...
STAGE = 10455
NAME = "stage10455_reviewed_v27_narrow_residual_support_probe_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
REQUEST_JSON = OUT_DIR / "reviewed_v27_narrow_residual_support_probe_request.json"
MANIFEST_JSONL = OUT_DIR / "reviewed_v27_narrow_residual_support_probe_manifest.jsonl"
...
PYTHON_SUPPORT_JSONL = ROOT / "runs/local/artifacts/stage10453_python_verifier_only_support_package/python_verifier_only_support_rows.jsonl"
RUST_SUPPORT_JSONL = ROOT / "runs/local/artifacts/stage10454_rust_citation_only_support_package/rust_citation_only_support_rows.jsonl"
...
RUN_ID = "stage10456_reviewed_v27_narrow_residual_support_probe"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage10456_reviewed_v27_narrow_residual_support_probe/bounded_decoder_probe"
RUNTIME_MODEL_DIR = ROOT / "runs/local/artifacts/stage10456_reviewed_v27_narrow_residual_support_probe/runtime_model"
...
def main() -> None:
    python_rows = load_jsonl(PYTHON_SUPPORT_JSONL)
    rust_rows = load_jsonl(RUST_SUPPORT_JSONL)
    ...
    request = {
        ...
        "decision": "reviewed_v27_narrow_residual_support_probe_ready",
        "claim_scope": [
            "Diagnostic-only repaired-overlay probe using separate narrow Python verifier-only and Rust citation-only support packets.",
            "Avoid the prior mixed-support contamination that regressed Python evidence_citation.",
        ],
        "source_artifacts": {
            "python_support_rows": display(PYTHON_SUPPORT_JSONL),
            "rust_support_rows": display(RUST_SUPPORT_JSONL),
        ...
        "command": command,
        "success_criteria": [
            "At least one of the two targeted residual misses flips to correct.",
            "Python evidence_citation does not regress relative to the 22/24 repaired-overlay baseline.",
            "No more than one new strict regression elsewhere on the repaired overlay.",
        ],
        "required_honesty_gates": [
            "Python support remains verifier-only; no Python evidence_citation rows in train.",
            "Rust support remains evidence_citation-only and excludes tokenizers strict rows from train.",
            "Any result remains diagnostic-only until tested on fresh disjoint strict roots.",
        ],
        "outputs": {
