...
try:
    from scripts.build_stage9177_trainer_setup_emit_instance_opening_design import (
    ...
    from build_stage9177_trainer_setup_emit_instance_opening_design import (  # type: ignore
        EXPECTED_EMITTED_OUTPUTS,
        REQUIRED_OPENING_GUARDS,
        REQUIRED_OPENING_REFS,
        STILL_BLOCKED_OUTPUTS,
...
STAGE = 9178
NAME = "stage9178_trainer_setup_emit_instance_opening_audit"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
SOURCE_9177 = ROOT / "runs/summaries/stage9177_trainer_setup_emit_instance_opening_design.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "TRAINER_SETUP_EMIT_INSTANCE_OPENING_AUDIT_STAGE9178.md"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "trainer_setup_emit_instance_opening_audit.json"
...
def registry(latest: int = 9177) -> dict[str, Any]:
...
def build_audit(registry_card: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    source = load_json(SOURCE_9177)
    design = build_design(registry(9176))
    design_failures = validate_design(design)
    negatives = run_negative_cases()
    checks = {
        "source_stage9177_passed": source.get("passed") is True,
        "base_design_passes": design_failures == [],
        "negative_cases_rejected": all(item["rejected"] for item in negatives.values()),
        "required_opening_refs_complete": set(REQUIRED_OPENING_REFS).issubset(set(design["required_opening_refs"])),
        "required_opening_guards_complete": set(REQUIRED_OPENING_GUARDS).issubset(set(design["required_opening_guards"])),
        "expected_outputs_complete": set(EXPECTED_EMITTED_OUTPUTS).issubset(set(design["expected_emitted_outputs"])),
        "blocked_outputs_complete": set(STILL_BLOCKED_OUTPUTS).issubset(set(design["still_blocked_outputs"])),
        "opening_not_authorized_now": design["metrics"]["opening_authorized_now"] is False,
        ...
        "registry_frontier_stage9177": int((registry_card.get("metrics") or {}).get("latest_stage", -1)) == 9177,
    ...
    return {
        ...
        "decision": "Audited the emit-instance opening design and kept output emission and all execution paths closed.",
        "next_best_step": "Stop at the emit-instance opening boundary until a deliberate emit authorization instance is intentionally opened.",
    }
...
def main() -> None:
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage9178 Trainer Setup Emit-Instance Opening Audit",
                "",
                f"Passed: `{summary['passed']}`",
                "",
                "Audits the deliberate emit-instance opening design without emitting outputs.",
                "",
