...
STAGE = 9392
NAME = "stage9392_bounded_decoder_short_suffix_target_resolved_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9391_bounded_decoder_short_suffix_target_resolved_manifest.json"
...
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9393_bounded_decoder_short_suffix_target_resolved_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "bounded_decoder_short_suffix_target_resolved_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9393_bounded_decoder_short_suffix_target_resolved_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "BOUNDED_DECODER_SHORT_SUFFIX_TARGET_RESOLVED_PREEXECUTION_STAGE9392.md"
...
COMMAND = [
    ...
    "--enable-generation-audit", "--max-generation-rows", "23", "--max-generation-tokens", "24",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9393_bounded_decoder_short_suffix_target_resolved_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if any(bool(auth.get(key)) for key in AUTHORITY_CLOSED):
            unsafe.append(str(row.get("row_id")))
        if row.get("decoder_text") != target or nested.get("decoder_text") != target:
            inconsistent.append(str(row.get("row_id")))
    failures: list[str] = []
    if source.get("passed") is not True:
        failures.append("source_stage9391_not_passed")
    if len(rows) != 23:
    ...
    if inconsistent:
        failures.append("inconsistent_target_rows")
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared target-resolved short-suffix denoise probe. Decoder CE remains closed.",
        "next_best_step": "Execute Stage9393 target-resolved short-suffix probe under trellis and audit exact short target recovery.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9392 Bounded Decoder Short-Suffix Target-Resolved Preexecution", "", f"Passed: `{card['passed']}`", f"Rows: `{card['manifest_rows']}`", f"Splits: `{card['split_counts']}`", f"Routes: `{card['route_counts']}`", "", "Only the Stage9393 target-resolved short-suffix denoise probe is authorized. Decoder CE and external authority remain closed.", ""]), encoding="utf-8")
