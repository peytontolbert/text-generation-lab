#!/usr/bin/env python3
"""Materialize Digital-World-Model Python verifier support rows."""
...
STAGE = 11763
NAME = "stage11763_digital_world_model_python_support_rows"
OUT = ART / NAME
SUMMARY = OUT / "digital_world_model_python_support_rows.json"
ROWS = OUT / "digital_world_model_python_support_rows.jsonl"

FEASIBILITY = ART / "stage11762_digital_world_model_repaired_feasibility/digital_world_model_repaired_feasibility_results.jsonl"
...
def file_evidence(eid: str, kind: str, rel_path: str, summary: str, start: int = 1, end: int = 200) -> dict[str, Any]:
...
def base_row(root_id: str, suffix: str, task_type: str, options: list[dict[str, Any]], target_label: str) -> dict[str, Any]:
    ...
    return {
        ...
        "source_snapshot_id": "Digital-World-Model::local_git_snapshot",
...
def main() -> None:
    feasible = [
        ...
        if row.get("repo_family") == "Digital-World-Model"
        and row.get("passed") is True
        and row.get("admit_recommendation") == "candidate_for_row_materialization"
    ]
    if not feasible:
        raise SystemExit("Digital-World-Model feasibility did not pass")
    probe = feasible[0]
    root_id = "stage11763::digital_world_model::python::safe_edit_devtools_support"
    selected = probe["selected_result"]
    sibling = probe["sibling_result"]
    evidence = [
        file_evidence("E01", "implementation_source", "agi_dw/scripts/eval/ci_assert_safe_edits.py", "CI safety assertion script reads scheduler logs and exits non-zero when unsafe diff signals are present."),
        file_evidence("E02", "selected_test_anchor", "agi_dw/tests/test_ci_assert_safe_edits.py", "Focused test validates unsafe edit detection and non-zero exit behavior."),
        file_evidence("E03", "nearby_test_distractor", "agi_dw/tests/test_devtools_orchestrator.py", "Sibling devtools test validates whitespace diff generation."),
        file_evidence("E04", "nearby_source_distractor", "agi_dw/scripts/devtools/devtools_orchestrator.py", "Nearby devtools implementation surface is related but not the selected CI safety verifier."),
        log_evidence("V01", selected, f"Selected pytest passed: {probe['selected_test']}"),
        log_evidence("V02", sibling, f"Sibling pytest passed: {probe['sibling_test']}"),
    ]
    verifier_evidence = {
        "id": "V01",
        "kind": "actual_pytest_pass_log",
        "summary": f"Focused pytest passed for {probe['selected_test']}.",
    ...
    verifier_options = [
        ...
        {"label": "B", "role": "nearby_test_distractor", "value": probe["sibling_test"], "evidence_ids": ["E03", "V02"]},
        {"label": "C", "role": "implementation_only_no_verifier", "value": "agi_dw/scripts/eval/ci_assert_safe_edits.py", "evidence_ids": ["E01"]},
        {"label": "D", "role": "nearby_source_distractor", "value": "agi_dw/scripts/devtools/devtools_orchestrator.py", "evidence_ids": ["E04"]},
    ]
    evidence_options = [
        {"label": "A", "role": "candidate_change_surface", "value": "agi_dw/scripts/eval/ci_assert_safe_edits.py", "evidence_ids": ["E01"]},
        {"label": "B", "role": "verifier_and_test_constraint", "value": f"pytest {probe['selected_test']}", "evidence_ids": ["E02", "V01"]},
        {"label": "C", "role": "nearby_sibling_test", "value": f"pytest {probe['sibling_test']}", "evidence_ids": ["E03", "V02"]},
        {"label": "D", "role": "nearby_source_distractor", "value": "agi_dw/scripts/devtools/devtools_orchestrator.py", "evidence_ids": ["E04"]},
    ]
    rows = [
    ...
    artifact: dict[str, Any] = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "third_python_verifier_support_root_materialized",
        "passed": True,
        "row_count": len(rows),
        "root_count": 1,
        "repo_family": "Digital-World-Model",
        ...
        "claim_boundary": [
            ...
            "This adds one Python verifier support root but does not satisfy Stage11742 quota.",
        ],
    }
