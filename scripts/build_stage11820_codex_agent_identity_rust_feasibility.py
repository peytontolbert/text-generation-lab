#!/usr/bin/env python3
"""Capture codex-agent-identity Rust verifier feasibility."""
...
STAGE = 11820
NAME = "stage11820_codex_agent_identity_rust_feasibility"
OUT = ART / NAME
LOGS = OUT / "logs"
SUMMARY = OUT / "codex_agent_identity_rust_feasibility.json"
RESULTS = OUT / "codex_agent_identity_rust_feasibility_results.jsonl"
REPO = Path("/data/agentkernel/other_repos/codex/codex-rs")
SELECTED_TEST = "agent_identity_jwks_url_uses_backend_api_base_url"
SIBLING_TEST = "agent_identity_jwks_url_uses_codex_api_base_url"
...
def run_cargo(test_filter: str, name: str) -> dict[str, Any]:
    ...
    command = ["cargo", "test", "-p", "codex-agent-identity", test_filter, "--", "--nocapture"]
    env = os.environ.copy()
    env.update(
        {
...
def main() -> None:
    selected = run_cargo(SELECTED_TEST, "selected_backend_api_base_url")
    sibling = run_cargo(SIBLING_TEST, "sibling_codex_api_base_url")
    passed = bool(selected["passed"] and sibling["passed"])
    row = {
        "stage": STAGE,
        "stage_name": NAME,
        "created_at_utc": now(),
        "repo_family": "codex_agent_identity",
        "repo_path": str(REPO),
        ...
        "source_path": "agent-identity/src/lib.rs",
        "distractor_source_path": "rmcp-client/src/program_resolver.rs",
        "passed": passed,
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "codex_agent_identity_rust_feasibility_recorded",
        "passed": passed,
