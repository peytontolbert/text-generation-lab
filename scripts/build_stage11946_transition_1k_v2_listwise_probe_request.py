#!/usr/bin/env python3
"""Emit a Transition-1K-v2 semantic-head probe with contrast plus listwise losses."""
...
STAGE = 11946
NAME = "stage11946_transition_1k_v2_listwise_probe_request"
OUT = ART / NAME
SUMMARY = OUT / "transition_1k_v2_listwise_probe_request.json"
COMMAND_JSON = OUT / "transition_1k_v2_listwise_command.json"
MANIFEST = OUT / "transition_1k_v2_listwise_manifest.jsonl"

SUPPORT_ROWS = ART / "stage11945_transition_1k_v2_multisource_package/transition_projection_rows_v2.jsonl"
PROTECTED_ROWSETS = {
...
RUN_DIR = ART / "stage11947_transition_1k_v2_listwise_probe"
...
def train_copy(row: dict[str, Any]) -> dict[str, Any]:
    out = dict(row)
    out["row_id"] = f"{row.get('row_id')}::stage11946_transition_1k_v2_listwise"
    out["split"] = "train"
    out["package_split"] = "train"
    out["train_support_only"] = True
    out["strict_eval_eligible"] = False
    out["stage11946_replay_kind"] = "transition_1k_v2_listwise"
...
def main() -> None:
    ...
    train_rows = [train_copy(row) for row in support if str(row.get("split")) == "train"]
    eval_rows = [dict(row, split="eval", package_split="eval") for row in support if str(row.get("split")) == "validation"]
    strict_rows = [dict(row, split="strict_eval", package_split="strict_eval") for row in support if str(row.get("split")) == "strict_eval"]
    ...
    gates = {
        ...
        "support_rows_present": len(support) > 0,
        "train_rows_present": len(train_rows) > 0,
        "eval_rows_present": len(eval_rows) > 0,
        ...
        "no_protected_rows_in_train": all(str(row.get("stage11946_replay_kind")) == "transition_1k_v2_listwise" for row in train_rows),
    }
    artifact = {
        "stage": STAGE,
        "stage_name": NAME,
        ...
        "decision": "transition_1k_v2_listwise_probe_request_ready" if all(gates.values()) else "transition_1k_v2_listwise_probe_request_blocked",
        "command": command,
        ...
        "hypothesis": "Stage11924 remains behind Gemma because the original transition projection is too small and C/C++ transition rows are under-covered.",
        "intervention": "Continue from Stage11924, train only the semantic candidate head on Transition-1K-v2 train rows, retain transition contrast, and retain same-role plus transition verifier-value listwise losses.",
        "promotion_gate": {
            "routed_protected_gates_preserved": True,
            "transition_1k_v2_validation_and_strict_gain_required": "required",
            "same_manifest_gemma_delta_positive_required_for_later_stage": "required_for_promotion",
        },
        "source_artifacts": {
