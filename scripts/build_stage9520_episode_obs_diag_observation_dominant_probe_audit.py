...
STAGE = 9520
NAME = "stage9520_episode_obs_diag_observation_dominant_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9519_episode_obs_diag_observation_dominant_preflight_audit.json"
RUN_DIR = ROOT / "runs/local/artifacts/stage9520_episode_obs_diag_observation_dominant_target_100m_probe"
...
AUDIT = RUN_DIR / "stage9520_episode_obs_diag_observation_dominant_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "EPISODE_OBS_DIAG_OBSERVATION_DOMINANT_PROBE_AUDIT_STAGE9520.md"
...
def main() -> None:
    ...
    if source.get("passed") is not True or not source.get("metrics", {}).get("execution_authorized_for_next_stage"):
        safety_failures.append("stage9519_not_authorized")
    ...
    audit = {
        ...
        "quality_passed": not [f for f in failures if f not in safety_failures],
        "failures": failures,
        "safety_failures": safety_failures,
        "source_stage": "stage9519_episode_obs_diag_observation_dominant_preflight_audit",
        ...
        "diagnosis": "Safe quality failure. Observation-dominant neutralization removed high-confidence wrong predictions but hurt exactness by removing too much controlled context. Next manifest should use minimal controlled context plus primitive observation features.",
    ...
    summary = {
        ...
        "decision": "Observation-dominant probe failed quality safely; use a minimal-context plus observation-evidence design next.",
        "next_best_step": "Build Stage9521 minimal-context observation diagnosis manifest: keep only non-label context needed for routing while preserving primitive obs evidence and blocking surface memorization.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    DOC.write_text("\n".join([
        "# Stage9520 Episode Observation Diagnosis Observation-Dominant Probe Audit",
        "",
    ...
    print(json.dumps({"stage": STAGE, "passed": audit["passed"], "safety_passed": audit["safety_passed"], "final_eval": audit["final_eval_joint_proxy_exact"], "final_strict": audit["final_strict_joint_proxy_exact"], "wrong_rows": len(wrong_rows), "high_confidence_wrong_rows": len(high_conf_wrong), "failures": failures}, indent=2, sort_keys=True))
