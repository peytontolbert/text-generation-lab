#!/usr/bin/env python3
"""Materialize einops Python selected-verifier support rows."""
...
STAGE = 11857
NAME = "stage11857_einops_python_support_rows"
OUT = ART / NAME
SUMMARY = OUT / "einops_python_support_rows.json"
ROWS = OUT / "einops_python_support_rows.jsonl"
FEASIBILITY = ART / "stage11856_einops_python_feasibility/einops_python_feasibility_results.jsonl"
...
def base_row(root_id: str, suffix: str, task_type: str, options: list[dict[str, Any]], target_label: str) -> dict[str, Any]:
    ...
    return {
        ...
        "source_snapshot_id": "einops::local_git_snapshot",
...
def main() -> None:
    feasible = [row for row in read_jsonl(FEASIBILITY) if row.get("repo_family") == "einops" and row.get("passed") is True]
    if not feasible:
        raise SystemExit("einops feasibility did not pass")
    probe = feasible[0]
    root_id = "stage11857::einops::python::parsed_expression_support"
    evidence = [
        file_evidence("E01", "implementation_source", probe["source_path"], "Einops parsing source validates axis names, ellipsis, parentheses, anonymous axes, and ParsedExpression composition."),
        file_evidence("E02", "selected_test_anchor", probe["selected_test_path"], "Parsing tests cover elementary axis names and invalid expression handling as distinct selected/sibling verifier functions."),
        file_evidence("E03", "nearby_source_distractor", probe["distractor_source_path"], "Main einops transformation surface is related but not the selected parsing verifier target."),
        log_evidence("V01", probe["selected_result"], "Selected elementary axis-name pytest passed."),
        log_evidence("V02", probe["sibling_result"], "Sibling invalid-expression pytest passed."),
    ]
    verifier_evidence = {
        "id": "V01",
        "kind": "actual_pytest_pass_log",
        "summary": "Selected and sibling einops parsing pytest functions passed.",
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "ninth_python_verifier_support_root_materialized",
        "passed": True,
        "row_count": len(rows),
        "root_count": 1,
        "repo_family": "einops",
