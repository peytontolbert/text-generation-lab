...
STAGE = 10860
NAME = "stage10860_residual_family_rebalanced_probe_plus_python_materialized_audit"
OUT_DIR = ARTIFACTS / NAME
AUDIT_JSON = OUT_DIR / "residual_family_rebalanced_probe_plus_python_materialized_audit.json"
SUMMARY = ROOT / "runs" / "summaries" / f"{NAME}.json"

BASE_DIR = ARTIFACTS / "stage10820_queue_aligned_multilingual_support_probe" / "bounded_decoder_probe"
RUN_DIR = ARTIFACTS / "stage10859_residual_family_rebalanced_probe_plus_python_materialized" / "bounded_decoder_probe"
...
def main() -> None:
    ...
    eval_regressions = [card for card in run_eval if not card["constrained_choice_match"]]
    ...
    payload = {
        ...
        "passed": True,
        "decision": "residual_family_rebalanced_probe_plus_python_materialized_no_frontier_gain",
        "source_runs": {
            "baseline_10820": rel(BASE_DIR / "execution_result.json"),
            "diagnostic_10859": rel(RUN_DIR / "execution_result.json"),
        },
        "headline": {
        ...
        "interpretation": [
            "Adding one real materialized Python verifier root into the support package did not move the strict frontier.",
            "The Python strict verifier row remains wrong and actually got weaker under full-vocab rank, so one extra same-family support root is not enough to flip the B-vs-C decision boundary.",
            "The Rust strict evidence row is still wrong under constrained scoring and no longer even has the target at full-vocab rank 1, so scorer alignment plus data coverage both remain open.",
            "Validation is still 21/24, so this run remains diagnostic-only and does not replace the cleaner 10820 baseline.",
        ],
        "next_best_step": "Stop expecting a single additional train-support root to break the plateau. Build a second independent Python verifier-transition root, expand Rust E/F evidence-role coverage, and keep scorer-alignment audits active for citation rows.",
    }
