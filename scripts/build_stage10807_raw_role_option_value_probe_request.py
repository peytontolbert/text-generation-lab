...
STAGE = 10807
NAME = 'stage10807_raw_role_option_value_probe_request'
OUT_DIR = ARTIFACTS / NAME
REQUEST_JSON = OUT_DIR / 'raw_role_option_value_probe_request.json'
COMMAND_JSON = OUT_DIR / 'raw_role_option_value_probe_command.json'
MANIFEST_JSONL = OUT_DIR / 'raw_role_option_value_probe_manifest.jsonl'

PACKAGE_JSON = ARTIFACTS / 'stage10806_raw_role_option_value_support_package' / 'raw_role_option_value_support_package.json'
TRAIN_ROWS = ARTIFACTS / 'stage10806_raw_role_option_value_support_package' / 'agentkernel_lite_encdec_train.jsonl'
VALIDATION_ROWS = ARTIFACTS / 'stage10806_raw_role_option_value_support_package' / 'agentkernel_lite_encdec_validation.jsonl'
STRICT_ROWS = ARTIFACTS / 'stage10806_raw_role_option_value_support_package' / 'agentkernel_lite_encdec_strict_eval.jsonl'
STRESS_ROWS = ARTIFACTS / 'stage10806_raw_role_option_value_support_package' / 'agentkernel_lite_encdec_stress_eval.jsonl'

INIT_RUNTIME = ARTIFACTS / 'stage10802_python_plus_rust_competition_support_probe' / 'runtime_model' / 'runtime_model_bundle.json'
...
RUN_ID = 'stage10808_raw_role_option_value_probe'
OUTPUT_DIR = 'runs/local/artifacts/stage10808_raw_role_option_value_probe/bounded_decoder_probe'
RUNTIME_MODEL_DIR = 'runs/local/artifacts/stage10808_raw_role_option_value_probe/runtime_model'
...
def main() -> None:
    ...
    request = {
        ...
        'decision': 'raw_role_option_value_probe_ready',
        ...
        'claim_scope': [
            'Run a diagnostic probe from the raw-role option-value support package to test encoder_option_retrieval on the exact option interface of the Rust residual.',
            'Keep the honest 24-row strict frontier fixed while allowing one explicitly diagnostic same-surface Rust clone in train support.',
        ],
        'required_honesty_gates': [
            'strict and validation rows remain unchanged from the honest reviewed v2.7 frontier',
            'all same-surface rows are marked diagnostic-only and must not be used for promotion claims',
            'post-run promotion still requires better than 22/24 with zero strict regressions on the honest frontier',
        ],
        ...
        'next_best_step': 'run the diagnostic probe, then audit whether encoder_option_retrieval flips the Rust residual while keeping the frontier otherwise stable',
    }
