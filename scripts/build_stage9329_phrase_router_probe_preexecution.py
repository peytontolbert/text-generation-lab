...
STAGE = 9329
NAME = "stage9329_phrase_router_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9328_phrase_router_mixture_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9328_phrase_router_mixture_manifest/phrase_router_mixture_manifest.jsonl"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9330_phrase_router_mixture_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "phrase_router_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9330_phrase_router_mixture_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PHRASE_ROUTER_PROBE_PREEXECUTION_STAGE9329.md"
...
COMMAND = [
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9330_phrase_router_mixture_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9328_not_passed")
    if len(rows) != 24:
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared a 24-row phrase-router mixture target-100M denoise probe; decoder CE remains closed.",
        "next_best_step": "Execute Stage9330 phrase-router mixture probe under trellis and audit route/phrase quality.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9329 Phrase Router Probe Preexecution",
        "",
