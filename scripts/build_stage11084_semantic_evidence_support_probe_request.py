...
STAGE = 11084
NAME = "stage11084_semantic_evidence_support_probe_request"
OUT_DIR = ARTIFACTS / NAME
SUMMARY_JSON = OUT_DIR / "semantic_evidence_support_probe_request.json"
COMMAND_JSON = OUT_DIR / "semantic_evidence_support_probe_command.json"
MANIFEST_JSONL = OUT_DIR / "semantic_evidence_support_probe_manifest.jsonl"

PACKAGE_DIR = ARTIFACTS / "stage11082_ready_lane_semantic_evidence_support_package"
PACKAGE_JSON = PACKAGE_DIR / "ready_lane_semantic_evidence_support_package.json"
...
ADDED_ROWS_JSONL = PACKAGE_DIR / "added_semantic_evidence_rows.jsonl"
...
PROBE_OUT_DIR = ARTIFACTS / "stage11085_semantic_evidence_support_probe" / "bounded_decoder_probe"
RUNTIME_OUT_DIR = ARTIFACTS / "stage11085_semantic_evidence_support_probe" / "runtime_model"
...
def main() -> None:
    ...
    summary = {
        ...
        "passed": True,
        "decision": "semantic_evidence_support_probe_requested",
        "claim_scope": [
            "Run one evidence-focused bounded probe from the stage11082 semantic support package while preserving the unchanged cleaned validation/strict canary.",
            "Test whether direct semantic evidence-role supervision and pairwise contrast rows improve the reserved residual evidence bank without relying on a new inference-time scorer policy.",
        ],
        "source_artifacts": {
            "support_package": rel(PACKAGE_JSON),
            "runtime_bundle": rel(RUNTIME_BUNDLE),
            "reserved_candidates": rel(RESERVED_CANDIDATES),
            "added_semantic_rows": rel(ADDED_ROWS_JSONL),
        },
        "metrics": {
            ...
            "semantic_rows_added": len(added_rows),
            "semantic_role_full_rows": package.get("metrics", {}).get("semantic_role_full_rows"),
            "semantic_pairwise_rows": package.get("metrics", {}).get("semantic_pairwise_rows"),
        },
        "required_honesty_gates": [
            "Cleaned validation and strict remain the main scored canary surfaces and must stay unchanged.",
            "Semantic evidence rows are support-only and cannot be reported as heldout generalization.",
            "Reserved residual reporting must remain separate from both the cleaned canary and the new support rows.",
            "Any gain must come from the trained runtime, not from a new inference-side scorer override.",
        ],
        "next_best_step": "Execute this request, then audit cleaned validation/strict plus the reserved residual bank and compare directly against the stage11080/11081 baseline.",
        "outputs": {
