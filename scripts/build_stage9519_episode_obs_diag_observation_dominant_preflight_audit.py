...
STAGE = 9519
NAME = "stage9519_episode_obs_diag_observation_dominant_preflight_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9518_episode_obs_diag_observation_dominant_audit.json"
RUN_DIR = ROOT / "runs/local/artifacts/stage9519_episode_obs_diag_observation_dominant_target_100m_contract_preflight"
CONTRACT = RUN_DIR / "probe_contract_audit.json"
AUDIT = RUN_DIR / "stage9519_preflight_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "EPISODE_OBS_DIAG_OBSERVATION_DOMINANT_PREFLIGHT_STAGE9519.md"
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9518_not_passed")
    ...
    audit = {
        "passed": not failures,
        "failures": failures,
        "source_stage": "stage9518_episode_obs_diag_observation_dominant_audit",
    ...
    summary = {
        ...
        "decision": "Contract-only target-100M preflight passed for the observation-dominant diagnosis manifest.",
        "next_best_step": "Run Stage9520 target-100M observation-dominant diagnosis probe under trellis, then audit whether eval exactness improves.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    DOC.write_text("\n".join([
        "# Stage9519 Episode Observation Diagnosis Observation-Dominant Preflight",
        "",
