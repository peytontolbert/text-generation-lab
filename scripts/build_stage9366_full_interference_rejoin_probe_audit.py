...
STAGE = 9366
NAME = "stage9366_full_interference_rejoin_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9365_full_interference_rejoin_probe_preexecution.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9364_full_mixture_with_interference_repairs_manifest/full_mixture_with_interference_repairs_manifest.jsonl"
RUN_DIR = ROOT / "runs/local/artifacts/stage9366_full_interference_rejoin_probe"
AUDIT = RUN_DIR / "stage9366_full_interference_rejoin_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "FULL_INTERFERENCE_REJOIN_PROBE_AUDIT_STAGE9366.md"
...
def audit_run() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9365_not_passed")
    ...
    if contract.get("mode") != "denoise_repair_probe" or execution.get("mode") != "denoise_repair_probe":
        failures.append("wrong_probe_mode")
    ...
    for sample in samples:
        ...
        labels = ["dependency_patch_ins_duplicate"] if "patch ins the patch inside" in generated_text else ["other_exact_mismatch"]
        for label in labels:
    ...
    quality_gate_passed = bool(generated == len(rows) and exact == len(rows) and target_prefix == len(rows) and boundary == len(rows) and contentful == len(rows) and repetition_rows == 0 and short_rows == 0 and leak_rows == 0)
    return {
        ...
        "diagnosis": "The full rejoin is safe but not exact. Stage9361 fixed route_1 and route_file_path under mixture; the remaining failure is route_0 dependency-handle `patch ins the patch inside` duplication.",
        "next_patch_target": "route0_dependency_patch_inside_duplicate_repair",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "The full interference rejoin probe executed safely but did not pass exactness; decoder CE remains closed.",
        "next_best_step": "Build a route_0 dependency patch-ins duplicate residual manifest and isolate/rejoin it before bounded decoder CE.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9366 Full Interference Rejoin Probe Audit", "", f"Passed: `{audit['passed']}`", f"Safety gate passed: `{audit['safety_gate_passed']}`", f"Quality gate passed: `{audit['quality_gate_passed']}`", f"Exact match rows: `{audit['exact_match_rows']}` / `{audit['generated_rows']}`", f"Target-prefix rows: `{audit['target_prefix_match_rows']}` / `{audit['generated_rows']}`", f"Boundary next-token rows: `{audit['boundary_next_token_match_rows']}` / `{audit['generated_rows']}`", f"Contentful rows: `{audit['contentful_rows']}` / `{audit['generated_rows']}`", f"Short/junk rows: `{audit['short_or_junk_rows']}`", f"Degenerate repetition rows: `{audit['degenerate_repetition_rows']}`", f"Internal leak rows: `{audit['generated_internal_token_rows']}`", f"Error counts: `{audit['error_counts']}`", "", "Safety held. The remaining failure is route_0 dependency-handle `patch ins the patch inside` duplication; route_1, route_2, and route_file_path passed under this mixture.", ""]), encoding="utf-8")
