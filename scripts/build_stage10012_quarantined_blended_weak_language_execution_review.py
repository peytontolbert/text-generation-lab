...
STAGE = 10012
NAME = "stage10012_quarantined_blended_weak_language_execution_review"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage10011_quarantined_blended_weak_language_target100m_contract_preflight.json"
BLEND_AUDIT = ROOT / "runs/local/artifacts/stage10010_quarantined_blended_weak_language_successor_mix/quarantined_blended_weak_language_successor_mix_audit.json"
SOURCE_MANIFEST_DIR = ROOT / "runs/local/artifacts/stage10011_quarantined_blended_weak_language_target100m_contract_preflight/manifests"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
REVIEW_DIR = OUT_DIR / "review_manifests"
TICKET = OUT_DIR / "quarantined_blended_weak_language_execution_review_inactive.json"
AUDIT = OUT_DIR / "quarantined_blended_weak_language_execution_review_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "QUARANTINED_BLENDED_WEAK_LANGUAGE_EXECUTION_REVIEW_STAGE10012.md"
...
REQUIRED_BEFORE_EXECUTION = [
    "fresh_stage10011_quarantined_contract_only_preflight_passed",
    "explicit_one_run_execution_authorization_after_stage10012",
    "disk_free_space_preflight_passed",
    "safe_cleanup_marker_and_dry_run_passed",
    "run_uses_stage10010_quarantined_structured_state_or_stage10011_manifests_only",
    ...
    "exclude_unresolved_gemma_advantage_rows_from_active_mix",
...
def future_command(card: dict[str, Any]) -> list[str]:
    ...
    out_dir = f"runs/local/artifacts/stage10013_{surface}_quarantined_blended_weak_language_target100m_probe/{surface}_probe"
    return [
        ...
        "--output-dir", out_dir,
        "--run-id", f"stage10013_{surface}_quarantined_blended_weak_language_target100m_probe",
...
def build_ticket(surface_cards: list[dict[str, Any]], blend_audit: dict[str, Any]) -> dict[str, Any]:
    ...
    ticket = {
        ...
        "requested_stage": 10013,
        "requested_stage_name": "stage10013_quarantined_blended_weak_language_target100m_probe_series",
        "requested_capability": "quarantined_blended_multilingual_target_100m_structured_heads_probe_with_web_and_weak_language_recovery_rows",
        "source_stage": 10011,
        "blend_source_stage": 10010,
        ...
        "blend_context": {
            ...
            "base_rows_removed_from_stale_blend": metrics.get("base_rows_removed_from_stale_blend"),
...
def audit_ticket(ticket: dict[str, Any], source: dict[str, Any], blend_audit: dict[str, Any], surface_cards: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if blend_audit.get("passed") is not True:
        failures.append("stage10010_not_passed")
    ...
    if blend_context.get("base_rows_removed_from_stale_blend") != 8:
        failures.append("base_rows_removed_from_stale_blend_not_8")
    if blend_context.get("python_edit_rows_after") != 19:
        failures.append("python_edit_rows_after_not_19")
    if blend_context.get("c_cpp_edit_rows_after") != 27:
    ...
    if blend_context.get("edit_localization_python_rows_in_review_manifest") != 19:
        failures.append("review_manifest_python_edit_rows_not_19")
    if blend_context.get("edit_localization_c_cpp_rows_in_review_manifest") != 27:
        failures.append("review_manifest_c_cpp_edit_rows_not_27")
...
def main() -> None:
    ...
    next_step = "Use this inactive review ticket to authorize one quarantined blended target-100M structured probe series that preserves the stage9944 web recovery rows and the stage9961 weak-language recovery roots without reintroducing unresolved Gemma-advantage rows."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Built an inactive execution-review ticket for the stage10010/stage10011 quarantined weak-language successor mix so the next target-100M run stays tied to the filtered multilingual recovery rows under the same anti-cheat contract.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage10012 Quarantined Blended Weak-Language Execution Review",
        "",
