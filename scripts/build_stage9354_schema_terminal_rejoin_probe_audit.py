...
STAGE = 9354
NAME = "stage9354_schema_terminal_rejoin_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9353_schema_terminal_rejoin_probe_preexecution.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9352_full_mixture_with_schema_and_terminal_repairs_manifest/full_mixture_with_schema_and_terminal_repairs_manifest.jsonl"
RUN_DIR = ROOT / "runs/local/artifacts/stage9354_schema_terminal_rejoin_probe"
AUDIT = RUN_DIR / "stage9354_schema_terminal_rejoin_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "SCHEMA_TERMINAL_REJOIN_PROBE_AUDIT_STAGE9354.md"
...
def bucket(samples: list[dict[str, Any]], rows_by_id: dict[str, dict[str, Any]], key: str) -> dict[str, dict[str, Any]]:
    ...
    for sample in samples:
        ...
        if key == "source":
            value = str(row.get("merged_curriculum_source") or "unknown")
        elif key == "combined":
            value = str(row.get("combined_curriculum_source") or "unknown")
        elif key == "task":
...
def classify(text: str) -> list[str]:
    labels: list[str] = []
    if "patch ins" in text:
        labels.append("dependency_patch_inside_bridge")
    if "small constant that the expected" in text:
        labels.append("numeric_preserves_omission")
    if "localized patch operatch operator" in text:
        labels.append("file_path_localized_edit_to_operator_bridge")
    if "operatch" in text:
        labels.append("operatch_fragment")
    return labels or ["other_exact_mismatch"]
...
def audit_run() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9353_not_passed")
    ...
    for sample in samples:
        ...
        row = rows_by_id.get(str(sample.get("row_id")), {})
        mi = row.get("model_input") if isinstance(row.get("model_input"), dict) else {}
        labels = classify(str(sample.get("generated_text") or ""))
        for label in labels:
            error_counts[label] += 1
        failed_samples.append({"row_id": sample.get("row_id"), "split": sample.get("split"), "labels": labels, "source": row.get("merged_curriculum_source"), "combined": row.get("combined_curriculum_source"), "task": row.get("repair_task_type"), "route": mi.get("opaque_phrase_route_id"), "surface": mi.get("semantic_surface_kind"), "prefix": sample.get("generation_prefix_text"), "target": sample.get("target_text"), "generated": sample.get("generated_text"), "exact_match": sample.get("exact_match"), "target_prefix_match": sample.get("target_prefix_match")})
    safety_gate_passed = not failures
    quality_gate_passed = bool(generated == 122 and exact == 122 and target_prefix == 122 and boundary == 122 and contentful == 122 and repetition_rows == 0 and short_rows == 0 and leak_rows == 0)
    return {
        ...
        "by_source": bucket(samples, rows_by_id, "source"),
        "by_combined": bucket(samples, rows_by_id, "combined"),
        ...
        "diagnosis": "route_1 operator terminal repair survived the full mixture, but route_0 dependency rows now bridge `inside` as `ins ... inside`, route_2 numeric rows omit `preserves`, and route_file_path rows drift to localized patch/operator text.",
        "next_patch_target": "stage9355_non_operator_residual_repair_manifest",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "The schema-terminal full-mixture rejoin probe executed safely but still failed non-operator exactness; decoder CE remains closed.",
        "next_best_step": "Build Stage9355 non-operator residual repair rows for dependency `patch inside`, numeric `preserves`, and file-path `localized edit`, then probe in isolation before another rejoin.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9354 Schema Terminal Rejoin Probe Audit", "", f"Passed: `{audit['passed']}`", f"Safety gate passed: `{audit['safety_gate_passed']}`", f"Quality gate passed: `{audit['quality_gate_passed']}`", f"Exact match rows: `{audit['exact_match_rows']}` / `{audit['generated_rows']}`", f"Target-prefix rows: `{audit['target_prefix_match_rows']}` / `{audit['generated_rows']}`", f"Boundary next-token rows: `{audit['boundary_next_token_match_rows']}` / `{audit['generated_rows']}`", f"Contentful rows: `{audit['contentful_rows']}` / `{audit['generated_rows']}`", f"Degenerate repetition rows: `{audit['degenerate_repetition_rows']}`", f"Error counts: `{audit['error_counts']}`", "", "Safety held. Route_1 operator-terminal repair survived, but route_0/route_2/route_file_path residuals remain. Decoder CE stays closed.", ""]), encoding="utf-8")
