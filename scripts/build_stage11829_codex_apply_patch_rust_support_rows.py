#!/usr/bin/env python3
"""Materialize codex-apply-patch Rust selected-verifier support rows."""
...
STAGE = 11829
NAME = "stage11829_codex_apply_patch_rust_support_rows"
OUT = ART / NAME
SUMMARY = OUT / "codex_apply_patch_rust_support_rows.json"
ROWS = OUT / "codex_apply_patch_rust_support_rows.jsonl"
FEASIBILITY = ART / "stage11828_codex_apply_patch_rust_feasibility/codex_apply_patch_rust_feasibility_results.jsonl"
...
def file_evidence(repo: Path, eid: str, kind: str, rel_path: str, summary: str, start: int = 1, end: int = 850) -> dict[str, Any]:
...
def main() -> None:
    ...
    if not probes:
        raise SystemExit("no materializable codex-apply-patch Rust probe found")
    probe = probes[0]
    ...
    root_id = "stage11829::codex_apply_patch::rust::apply_patch_hunks_support"
    evidence = [
        file_evidence(repo, "E01", "candidate_change_surface", source_path, "Apply-patch library source implements patch parsing and apply_hunks behavior exercised by add/update unit tests."),
        file_evidence(repo, "E02", "nearby_source_distractor", distractor_source_path, "Invocation parser source is adjacent apply-patch logic but not the selected hunk-application verifier target."),
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "sixth_rust_verifier_support_root_materialized",
        "passed": True,
        "row_count": len(rows),
        "root_count": 1,
        "repo_family": repo_family,
        ...
        "claim_boundary": ["Rows are train-support-only.", "This expands Rust verifier support with a non-tokenizers Codex apply-patch repo family."],
    }
