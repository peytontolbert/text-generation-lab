...
STAGE = 10143
NAME = 'stage10143_compact_bounded_bundle_projection'
OUT_DIR = ROOT / 'runs/local/artifacts' / NAME
OUT_PATH = OUT_DIR / 'compact_bounded_bundle_projection.json'
...
def compile_bounded_row(bundle: dict[str, Any], gold: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        ...
        'anti_cheat': {
            ...
            'compact_prompt_contract_probe': True,
...
def build_payload(source: Path = SOURCE) -> dict[str, Any]:
    ...
    for bundle in bundles:
        ...
        kept_rows += len(rows)
        runs.append({
            'cell_key': f"compact_projection::{bundle['language_family']}::{bundle['bundle_id']}",
            'task_pack': {
                'bundle_id': bundle['bundle_id'],
                ...
                'slice_tags': ['maintainer_bundle', bundle['language_family'], 'compact_bounded_projection', 'admitted'],
                'thresholds': {'must_compare_100m_and_gemma': True, 'projection_only': True},
                'blocked_training_reason': 'compact_bounded_projection_eval_only',
                'rows': rows,
...
def main() -> None:
    ...
    SUMMARY.write_text(json.dumps({'stage': STAGE, 'passed': payload['passed'], 'payload': display(OUT_PATH), 'metrics': payload['metrics']}, indent=2, sort_keys=True) + '\n', encoding='utf-8')
    print(json.dumps({'stage': STAGE, 'passed': payload['passed'], 'payload': display(OUT_PATH), 'metrics': payload['metrics']}, indent=2, sort_keys=True))
