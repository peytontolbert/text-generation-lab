...
OUT = ROOT / "runs/local/tmp/pocketpal_stage634_rank2_residual_bridge_seed461"
ARTIFACT = ROOT / "runs/local/artifacts/stage634_rank2_residual_bridge_dataset.json"
TARGET_OPS = {"direct_fact", "entity_context", "rule_case_intersection_member"}
MAX_RANK = 4
...
def main() -> None:
    ...
    for source_id in selected_ids:
        ...
        row["source_type"] = f"{row.get('source_type', 'retrieval')}_stage634_rank2_residual_bridge"
        row["stage634_rank2_residual_bridge"] = True
    ...
    manifest.update(
        {
            "artifact_kind": "agentkernel_lite_encdec_stage634_rank2_residual_bridge",
            ...
            "stage634_target_ops": sorted(TARGET_OPS),
            "stage634_max_rank": MAX_RANK,
            "stage634_replay_examples": len(replay_rows),
        }
    )
    ...
    artifact = {
        "artifact_kind": "stage634_rank2_residual_bridge_dataset",
