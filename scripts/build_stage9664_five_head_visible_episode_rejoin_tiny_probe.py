...
STAGE = 9664
NAME = "stage9664_five_head_visible_episode_rejoin_tiny_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9663_five_head_visible_episode_rejoin_manifest.json"
...
AUDIT = OUT_DIR / "five_head_visible_episode_rejoin_tiny_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "FIVE_HEAD_VISIBLE_EPISODE_REJOIN_TINY_PROBE_STAGE9664.md"
...
def command() -> list[str]:
    return ["env", f"TMPDIR={TMPDIR}", f"TEMP={TMPDIR}", f"TMP={TMPDIR}", "conda", "run", "-n", "trellis", "python", str(TRAINER),
        ...
        "--max-train-rows", "48", "--max-eval-rows", "9", "--max-strict-rows", "9", "--max-steps", "512", "--batch-size", "3", "--learning-rate", "3e-4",
        ...
        "--output-dir", str(RUN_DIR), "--run-id", "stage9664_five_head_visible_episode_rejoin_probe", "--execution-authorized-for-recovery-probe"]
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        safety_failures.append("stage9663_not_passed")
    if int(source_metrics.get("rows") or 0) != 66:
        safety_failures.append("stage9663_unexpected_row_count")
    ...
    quality_pass = bool(eval_joint >= 0.95 and strict_joint >= 0.95 and min(eval_exact.values() or [0]) >= 0.95 and min(strict_exact.values() or [0]) >= 0.95 and not high_conf_wrong)
    if not quality_pass:
