...
STAGE = 11169
NAME = "stage11169_external_fail_to_pass_support_probe_request"
OUT_DIR = ARTIFACTS / NAME
SUMMARY_JSON = OUT_DIR / "external_fail_to_pass_support_probe_request.json"
COMMAND_JSON = OUT_DIR / "external_fail_to_pass_support_probe_command.json"
MANIFEST_JSONL = OUT_DIR / "external_fail_to_pass_support_probe_manifest.jsonl"

PACKAGE_DIR = ARTIFACTS / "stage11168_cleaned_plus_external_fail_to_pass_support_package"
PACKAGE_JSON = PACKAGE_DIR / "cleaned_plus_external_fail_to_pass_support_package.json"
...
ADDED_ROWS_JSONL = PACKAGE_DIR / "added_external_commit_fail_to_pass_rows.jsonl"
...
PROBE_OUT_DIR = ARTIFACTS / "stage11170_external_fail_to_pass_support_probe" / "bounded_decoder_probe"
RUNTIME_OUT_DIR = ARTIFACTS / "stage11170_external_fail_to_pass_support_probe" / "runtime_model"
...
def main() -> None:
    ...
    summary = {
        ...
        "passed": True,
        "decision": "external_fail_to_pass_support_probe_requested",
        "claim_scope": [
            "Run one diagnostic bounded probe with 5 admitted external commit-backed FAIL_TO_PASS verifier-target support rows.",
            "This is not a promotion run; added rows are train-support-only historical COMMIT_PLUS_VERIFY rows.",
        ],
        "source_artifacts": {
        ...
        "metrics": {
            ...
            "external_fail_to_pass_rows_added": len(added_rows),
            "external_fail_to_pass_unique_roots": len({str(row.get("source_root_id") or "") for row in added_rows}),
            "external_fail_to_pass_repo_counts": package.get("metrics", {}).get("added_by_repo"),
        },
        "required_honesty_gates": [
            "Strict and validation are unchanged from stage11146 and remain canary surfaces.",
            "Added external commit-backed rows are train-support-only and historical COMMIT_PLUS_VERIFY support, not fresh runtime heldout improvement.",
