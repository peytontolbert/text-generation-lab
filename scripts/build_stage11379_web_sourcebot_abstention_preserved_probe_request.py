...
STAGE = 11379
NAME = "stage11379_web_sourcebot_abstention_preserved_probe_request"
OUT = ART / NAME
SUMMARY = OUT / "web_sourcebot_abstention_preserved_probe_request.json"
CMD = OUT / "web_sourcebot_abstention_preserved_probe_command.json"
MAN = OUT / "web_sourcebot_abstention_preserved_probe_manifest.jsonl"
TRAIN = ART / "stage11378_web_sourcebot_plus_abstention_preservation_package/web_sourcebot_plus_abstention_preservation_rows.jsonl"
...
RUN = ART / "stage11380_web_sourcebot_abstention_preserved_probe"
...
def main() -> None:
    ...
    overlaps = sorted({root_key(r) for r in train} & {root_key(r) for r in val + strict + heldout})
    if overlaps:
    ...
    cmd = [
        ...
        "--decoder-ce-weight", "0.06",
        "--bounded-choice-aux-weight", "2.2",
        ...
        "--bounded-choice-contrast-weight", "0.7",
    ...
    summary = {
        ...
        "passed": True,
        "decision": "web_sourcebot_abstention_preserved_probe_requested_diagnostic_only",
        "metrics": {
        ...
        "source_artifacts": {"train": rel(TRAIN), "baseline": rel(BASELINE), "canary_validation": rel(VAL), "canary_strict": rel(STRICT), "sealed_heldout": rel(HELDOUT), "init_runtime": rel(INIT)},
        "outputs": {"summary": rel(SUMMARY), "command": rel(CMD), "manifest": rel(MAN), "probe_dir": rel(PROBE), "runtime_model": rel(RUNTIME)},
        "promotion_gate": {
            "diagnostic_only": True,
            "sourcebot_combined_support_should_improve": "above 0/18 baseline",
            "sealed_llama_stack_heldout_should_improve": "above 0/6 baseline",
