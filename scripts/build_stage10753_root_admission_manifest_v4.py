...
STAGE = 10753
NAME = "stage10753_root_admission_manifest_v4"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
MANIFEST_JSONL = OUT_DIR / "root_admission_manifest_v4.jsonl"
SUMMARY_JSON = OUT_DIR / "root_admission_manifest_v4.json"
...
LATEST_REVIEWED_PACKAGE = ROOT / "runs/local/artifacts/stage10743_reviewed_v27_plus_cpp_bootstrap_train_support_package/reviewed_v27_plus_cpp_bootstrap_train_support_package.json"
CURRENT_FRONTIER_QUEUE = ROOT / "runs/local/artifacts/stage10752_multilingual_bulk_root_materialization_queue/multilingual_bulk_root_materialization_queue.json"
...
def main() -> None:
    ...
    current_queue = load_json(CURRENT_FRONTIER_QUEUE)
    ...
    payload = {
        ...
        "passed": True,
        "decision": "root_admission_manifest_v4_ready",
        "claim_scope": [
            "Refresh the root admission manifest against the newer stage10743 reviewed support package that already includes the bootstrap C/C++ train-support expansion.",
            "Keep bootstrap compiled roots and reviewed maintainer bundles in one admission view for the next scale-control pass.",
            "This is an inventory/control artifact, not a new model score claim.",
        ],
        "source_artifacts": {
            "reviewed_support_package": display(LATEST_REVIEWED_PACKAGE),
            "reviewed_root_manifest": display(REVIEWED_ROOTS),
            "compiled_roots": display(COMPILED_ROOTS),
            "bootstrap_rows": display(BOOTSTRAP_ROWS),
            "bulk_materialization_queue": display(CURRENT_FRONTIER_QUEUE),
        },
        "headline_findings": [
            "The admission manifest now tracks the expanded reviewed multilingual support inventory from stage10743 instead of the older stage10735 package.",
            "This increases the admitted reviewed C/C++ support lane while leaving strict eval and stress boundaries unchanged.",
            "The next scale package should now read from this v4 manifest so batch planning reflects the real current support supply.",
        ],
        "metrics": {
            "manifest_rows": len(manifest_rows),
            "reviewed_root_rows": len(reviewed_roots),
            "compiled_root_rows": len(compiled_roots),
            "admit_role_counts": dict(sorted(Counter(str(row.get("admit_role") or "") for row in manifest_rows).items())),
            "language_counts": dict(sorted(Counter(str(row.get("language_family") or "") for row in manifest_rows).items())),
            "reviewed_root_language_counts": dict(sorted(Counter(str(row.get("language_family") or "") for row in reviewed_roots).items())),
            "current_reviewed_train_rows": int(((latest_reviewed_package.get("metrics") or {}).get("train_rows")) or 0),
        },
        "frontier_context": {
            "queue_decision": current_queue.get("decision"),
            "queue_counts_by_language": current_queue.get("queue_counts_by_language") or {},
        },
        "outputs": {
