...
STAGE = 11571
NAME = "stage11571_semantic_web_hybrid_postrun_audit"
OUT = ART / NAME
SUMMARY = OUT / "semantic_web_hybrid_postrun_audit.json"

RUNTIME = ART / "stage11570_semantic_web_non_evidence_support_probe/runtime_model/runtime_model_bundle.json"
ROWSETS = {
...
def main() -> None:
    ...
    for rowset_name, payload in rowsets.items():
        base_card = _write_bounded_choice_eval_audit(
            ...
            split_name=f"{rowset_name}_base_semantic",
            bounded_choice_aux_source=SEMANTIC_SCORER,
            eval_batch_size=8,
        )
        evidence_rows = []
        semantic_rows = []
        for original, mapped in zip(payload["original"], payload["gated"]):
            if str(mapped.get("task_type") or "") == "evidence_candidate_judgment":
                evidence_rows.append(mapped)
            else:
                semantic_rows.append(original)
        evidence_card = _write_bounded_choice_eval_audit(
            OUT,
            model=model,
            rows=evidence_rows,
            tokenizer=tokenizer,
            max_encoder_tokens=768,
            max_decoder_tokens=16,
            split_name=f"{rowset_name}_hybrid_evidence",
            bounded_choice_aux_source=PRODUCT_SCORER,
            eval_batch_size=8,
        ) if evidence_rows else {"row_cards": [], "rows": 0, "constrained_choice_correct": 0, "constrained_choice_rows": 0, "constrained_choice_coverage": 1.0, "constrained_choice_top1_accuracy": None}
        semantic_card = _write_bounded_choice_eval_audit(
            OUT,
            model=model,
            rows=semantic_rows,
            tokenizer=tokenizer,
            max_encoder_tokens=768,
            max_decoder_tokens=16,
            split_name=f"{rowset_name}_hybrid_semantic",
            bounded_choice_aux_source=SEMANTIC_SCORER,
            eval_batch_size=8,
        ) if semantic_rows else {"row_cards": [], "rows": 0, "constrained_choice_correct": 0, "constrained_choice_rows": 0, "constrained_choice_coverage": 1.0, "constrained_choice_top1_accuracy": None}
        hybrid_cards = list(evidence_card.get("row_cards") or []) + list(semantic_card.get("row_cards") or [])
        hybrid_correct = sum(1 for row in hybrid_cards if row.get("constrained_choice_match") is True)
        hybrid_scored = sum(1 for row in hybrid_cards if isinstance(row.get("constrained_choice_match"), bool))
        hybrid_metric = {
            "rows": len(hybrid_cards),
            "correct": hybrid_correct,
            "scored_rows": hybrid_scored,
            "coverage": (hybrid_scored / len(payload["original"])) if payload["original"] else 1.0,
            "accuracy": (hybrid_correct / hybrid_scored) if hybrid_scored else None,
            "misses": [
                {
                    "row_id": row.get("row_id"),
                    "target": row.get("bounded_choice_target_label"),
        ...
        results["rowsets"][rowset_name] = {
            ...
            "hybrid_evidence_rows": len(evidence_rows),
            "hybrid_semantic_rows": len(semantic_rows),
            "base": base,
            "gated": hybrid_metric,
            "delta_correct": (hybrid_metric.get("correct") or 0) - (base.get("correct") or 0),
        }
    ...
    summary = {
        ...
        "created_at_utc": now(),
        "passed": True,
        "decision": "semantic_web_hybrid_postrun_promising" if all(gates.values()) else "text_rich_bridge_postrun_gate_not_promotable",
