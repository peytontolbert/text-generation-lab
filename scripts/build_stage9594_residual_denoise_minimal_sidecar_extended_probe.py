...
STAGE = 9594
NAME = "stage9594_residual_denoise_minimal_sidecar_extended_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9593_residual_denoise_minimal_sidecar_probe.json"
...
DOC = ROOT / "docs" / "RESIDUAL_DENOISE_MINIMAL_SIDECAR_EXTENDED_PROBE_STAGE9594.md"
...
def main() -> None:
    ...
    if not failures:
        contract = subprocess.run(command(output_dir=CONTRACT_DIR, run_id="stage9594_contract_preflight", contract_only=True), cwd=ROOT, text=True, capture_output=True, check=False)
    ...
    if not failures:
        run = subprocess.run(command(output_dir=RUN_DIR, run_id="stage9594_minimal_sidecar_extended_probe", contract_only=False), cwd=ROOT, text=True, capture_output=True, check=False)
        if run.returncode != 0:
    ...
    summary = {
        ...
        "next_best_step": "If the extended probe still fails, inspect structured-head optimization/model sensitivity before adding more data.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9594 Residual Denoise Minimal Sidecar Extended Probe", "", f"Passed: `{audit['passed']}`", f"Eval suffix choice exact: `{eval_exact}`", f"Strict suffix choice exact: `{strict_exact}`", f"Pass gate: `{pass_gate}`", "", "Decoder, denoise, runtime, export, harness, and promotion remain closed.", ""]), encoding="utf-8")
