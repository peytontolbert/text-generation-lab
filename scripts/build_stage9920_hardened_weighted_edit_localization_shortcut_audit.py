...
STAGE = 9920
NAME = "stage9920_hardened_weighted_edit_localization_shortcut_audit"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "hardened_weighted_edit_localization_shortcut_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "HARDENED_WEIGHTED_EDIT_LOCALIZATION_SHORTCUT_AUDIT_STAGE9920.md"
...
build_prompt = _load_symbol(
    "stage9920_stage9919_cmp",
    ROOT / "scripts/build_stage9919_hardened_weighted_edit_localization_gemma_comparison.py",
    "build_prompt",
)
ROW_TEXT = _load_symbol(
    "stage9920_training_data",
...
def build_audit() -> dict[str, Any]:
    ...
    if comparison.get("passed") is not True:
        failures.append("stage9919_comparison_not_passed")
    ...
    gate = {
        ...
        "reason": "the weighted integrated packet preserves the same no-label-list prompt surface and row-local permutation diversity while changing only train pressure and review schedule",
    }
    return {
        "passed": not failures,
        "failures": failures,
...
def main() -> None:
    ...
    next_step = "Use Stage9919 plus this audit as the truthful integrated weighted edit-localization benchmark entry point, then decide which multilingual cells are strong enough for updated expert-maintainer review."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Audited the weighted integrated opaque-choice packet and verified that the prompt still hides the global valid-label list while the row-local permutation maps remain diverse enough to block stable label-proxy shortcuts.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9920 Hardened Weighted Edit Localization Shortcut Audit",
        "",
