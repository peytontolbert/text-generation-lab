...
STAGE = 9610
NAME = "stage9610_suffix_ladder_two_phase_contract_preflight"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9609_suffix_continuation_ladder_manifest.json"
...
PHASE2_MANIFEST = ROOT / "runs/local/artifacts/stage9609_suffix_continuation_ladder_manifest/suffix_continuation_ladder_manifest.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
RUN_DIR = OUT_DIR / "two_phase_contract"
AUDIT = OUT_DIR / "suffix_ladder_two_phase_contract_preflight.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "SUFFIX_LADDER_TWO_PHASE_CONTRACT_PREFLIGHT_STAGE9610.md"
...
def command() -> list[str]:
    return [
        ...
        "--phase2-max-train-rows", "40", "--phase2-max-eval-rows", "6", "--phase2-max-strict-rows", "6",
        ...
        "--output-dir", str(RUN_DIR), "--run-id", "stage9610_suffix_ladder_two_phase_contract",
        "--contract-only",
    ]
...
def main() -> None:
    ...
    if len(phase2_rows) != 52:
        failures.append("phase2_rows_not_52")
    ...
    next_step = "If execution is explicitly authorized, run a tiny Stage9611 suffix-ladder two-phase probe with phase2 caps 40/6/6 and generation-prefix-field enabled."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Validated the two-phase trainer contract against the suffix-continuation ladder manifest. No model execution occurred.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9610 Suffix Ladder Two-Phase Contract Preflight",
        "",
