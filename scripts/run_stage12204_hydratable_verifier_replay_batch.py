...
DEFAULT_EPISODE_ID = "stage12201_episode_e547aba70f65a981"
STAGE = "stage12204_hydratable_verifier_replay_batch"
OUT_DIR = ROOT / "runs/local/artifacts" / STAGE
SUMMARY_PATH = ROOT / "runs/summaries" / f"{STAGE}.json"
TMP_ROOT = Path("/data/tmp/stage12204_hydratable_verifier_replay_batch")
...
def command_for_action(action: dict[str, Any]) -> str | None:
    ...
    if cmd == "run broad test suite":
        return None
    # Stage12204 is intentionally narrow: replay selected pytest verifier
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="episode_id=selected_test_path override; target must already be in the episode selected_tests list",
    )
    args = parser.parse_args()
    target_overrides = {}
    for item in args.target_override:
        if "=" not in item:
            raise SystemExit(f"invalid --target-override {item!r}; expected episode_id=target")
        episode_id, target = item.split("=", 1)
        target_overrides[episode_id] = target
    ...
    for episode_id in wanted:
        ...
        if not ep or not action_set:
            ...
            continue
        action_set = copy.deepcopy(action_set)
        chosen = next((a for a in action_set.get("candidate_actions", []) if a.get("action_id") == action_set.get("chosen_action_id")), None)
        if episode_id in target_overrides:
            override_target = target_overrides[episode_id]
            if override_target not in (ep.get("selected_tests") or []):
                blocked.append({
                    "episode_id": episode_id,
                    "blockers": ["target_override_not_in_selected_tests"],
                    "override_target": override_target,
                })
                continue
            if chosen:
                chosen["arguments"] = {"cmd": f"pytest {override_target} -q", "target": override_target}
                chosen["plausibility_source"] = "selected_tests_target_override"
                chosen["stage12204_target_override"] = True
        ...
        command_id = stable_id("stage12204_command", episode_id, command, result["returncode"], result["stdout_sha256"], result["stderr_sha256"])
        command_row = {
            **plan,
        ...
        observation_id = stable_id("stage12204_observation", command_id, status)
        observations.append(
            {
        ...
        verifier_transitions.append(
            {
                "verifier_transition_id": stable_id("stage12204_verifier_transition", observation_id, transition),
                "episode_id": episode_id,
                "command_result_id": command_id,
                "status": status,
        ...
        state_update = {
            "state_update_id": stable_id("stage12204_state_update", observation_id),
            "episode_id": episode_id,
            "observation_id": observation_id,
        ...
        stop_decision = {
            "stop_decision_id": stable_id("stage12204_stop_decision", observation_id),
            "episode_id": episode_id,
            "observation_id": observation_id,
        ...
        level3_records.append(
            {
                ...
                "train_support_usable_for_passfail": status in {"PASS_CURRENT_STATE", "FAIL_CURRENT_STATE"},
    ...
    passfail_level3_records = [
        row for row in level3_records if row.get("verifier_status") in {"PASS_CURRENT_STATE", "FAIL_CURRENT_STATE"}
    ]
    non_passfail_level3_records = [
        row for row in level3_records if row.get("verifier_status") not in {"PASS_CURRENT_STATE", "FAIL_CURRENT_STATE"}
    ]
    paths = {
        ...
        "passfail_level3_episode_records": OUT_DIR / "passfail_level3_episode_records.jsonl",
        "non_passfail_level3_episode_records": OUT_DIR / "non_passfail_level3_episode_records.jsonl",
    ...
    write_jsonl(paths["passfail_level3_episode_records"], passfail_level3_records)
    write_jsonl(paths["non_passfail_level3_episode_records"], non_passfail_level3_records)
    write_jsonl(paths["blocked_candidates"], blocked)

    status_counts = Counter(row.get("verifier_status") for row in observations)
    usable_passfail_count = status_counts.get("PASS_CURRENT_STATE", 0) + status_counts.get("FAIL_CURRENT_STATE", 0)
    env_blocked_count = status_counts.get("ENV_BLOCKED", 0)
    observed_count = len(observations)
    usable_passfail_rate = (usable_passfail_count / observed_count) if observed_count else 0.0
    env_blocked_rate = (env_blocked_count / observed_count) if observed_count else 0.0
    batch_decision = "level3_passfail_train_support_usable"
    if not level3_records:
        batch_decision = "level3_replay_blocked"
    elif env_blocked_rate >= 0.5 or usable_passfail_rate < 0.5:
        batch_decision = "level3_records_materialized_batch_quarantined_env_blocked_heavy"
    admission_audit = {
        "stage": STAGE,
        ...
        "usable_passfail_count": usable_passfail_count,
        "usable_passfail_rate": usable_passfail_rate,
        "passfail_artifact": str(paths["passfail_level3_episode_records"]),
        "non_passfail_artifact": str(paths["non_passfail_level3_episode_records"]),
        "env_blocked_count": env_blocked_count,
        "env_blocked_rate": env_blocked_rate,
    ...
    summary = {
        **admission_audit,
        ...
        "source_stage_inputs": [
            ...
            "stage12204_hydratable_verifier_replay_batch_request",
        ],
