...
STAGE = 10337
NAME = "stage10337_balanced_web_python_cpp_anchorboost_execution_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
MANIFEST_JSONL = OUT_DIR / "balanced_web_python_cpp_anchorboost_manifest.jsonl"
REQUEST_JSON = OUT_DIR / "balanced_web_python_cpp_anchorboost_execution_request.json"
COMMAND_JSON = OUT_DIR / "balanced_web_python_cpp_anchorboost_command.json"
...
RUN_ID = "stage10338_balanced_web_python_cpp_anchorboost_probe"
OUTPUT_DIR = "runs/local/artifacts/stage10338_balanced_web_python_cpp_anchorboost_probe/bounded_decoder_probe"
RUNTIME_MODEL_DIR = "runs/local/artifacts/stage10338_balanced_web_python_cpp_anchorboost_probe/runtime_model"
MAX_STEPS = 56
LEARNING_RATE = "8e-6"
BATCH_SIZE = 2
...
def main() -> None:
    ...
    cpp_anchor_rows: list[dict[str, Any]] = []
    for repeat in range(3):
        for idx, row in enumerate(cpp_anchor_source_rows):
            cpp_anchor_rows.append(
                clone_row(
                    row,
                    stage_prefix=f"stage10337_cpp_anchor_r{repeat}",
                    query_suffix=f"anchor_r{repeat}_{idx:02d}",
                )
            )

    frontloaded_rows = web_support_rows + python_candidate_rows + cpp_anchor_rows
    combined_rows = frontloaded_rows + base_train_rows + base_strict_rows
    ...
    request = {
        ...
        "passed": True,
        "decision": "balanced_web_python_cpp_anchorboost_successor_ready",
        "manifest": display(MANIFEST_JSONL),
        "source_manifests": {
            "base": display(BASE_MANIFEST),
            "web_successor": display(WEB_MANIFEST),
        ...
        "frontloaded_support_rows": {
            ...
            "cpp_anchor_rows": len(cpp_anchor_rows),
        },
        "max_steps": MAX_STEPS,
        ...
        "required_honesty_gates": [
            ...
            "stage10332 web gains must survive unchanged",
            "python support is limited to candidate-path tasks that actually improved under stage10334",
            "c_cpp evidence-citation anchor rows are repeated to explicitly preserve the stable candidate_change_surface evidence prior",
        ],
        "next_best_step": "run one anchor-boosted warm-start probe and promote it only if the stage10336 gains survive while c_cpp evidence citation returns to 6/6",
        "command": command,
