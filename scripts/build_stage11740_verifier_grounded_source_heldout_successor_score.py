#!/usr/bin/env python3
"""Score verifier-grounded source-heldout smoke successor rows with Stage11507."""
...
STAGE = 11740
NAME = "stage11740_verifier_grounded_source_heldout_successor_score"
OUT = ART / NAME
SUMMARY = OUT / "verifier_grounded_source_heldout_successor_score.json"
RENDERED_ROWS = OUT / "verifier_grounded_successor_rows.jsonl"
SCORED_ROWS = OUT / "verifier_grounded_successor_100m_score_rows.jsonl"

STAGE11722 = ROOT / "scripts/build_stage11722_sentencepiece_cpp_source_heldout_100m_score.py"
spec = importlib.util.spec_from_file_location("stage11722_for_11740", STAGE11722)
...
ROW_SOURCES = {
    "c_cpp": ART / "stage11720_sentencepiece_cpp_admitted_smoke_packet/sentencepiece_cpp_admitted_smoke_rows.jsonl",
    "python": ART / "stage11728_bigram_python_no_train_overlap_and_admission/bigram_python_admitted_smoke_rows.jsonl",
    "rust": ART / "stage11733_tokenizers_rust_no_train_overlap_and_admission/tokenizers_rust_admitted_smoke_rows.jsonl",
}
VERIFIER_EXECUTION = ART / "stage11737_source_heldout_verifier_execution/source_heldout_verifier_execution.json"
...
def verifier_text(language: str, verifier: dict[str, Any]) -> str:
    transition = (verifier.get("verifier_transitions") or {}).get(language) or {}
    run = (verifier.get("verifier_runs") or {}).get(language) or {}
    if language == "python":
        scope = "model-focused Python verifier suite"
    elif language == "rust":
        scope = "primary inline whitespace pre-tokenizer verifier"
    elif language == "c_cpp":
        scope = "BPE model verifier cases inside the aggregate C++ test binary"
    else:
        scope = "focused verifier"
    return (
        f"V01: observed_transition={transition.get('observed_transition')}; "
        f"scope={scope}; result={transition.get('result')}; "
        f"command_returncode={run.get('returncode')}; stdout_sha256={run.get('stdout_sha256')}"
    )
...
def task_instruction(task_type: str) -> str:
    return {
        ...
        "verifier_outcome": "Choose the verifier/test anchor that should be used first for this maintainer root.",
        "verifier_outcome_or_abstain": "Choose the verifier/test anchor that should be used first for this maintainer root, or abstain if evidence is insufficient.",
    }.get(task_type, "Choose the best maintainer option from the visible evidence.")


def render_prompt(row: dict[str, Any], verifier: dict[str, Any]) -> str:
    ...
    lines = [
        "TASK",
        f"language_family: {language}",
        f"repo_family: {row.get('repo_family')}",
        f"task_type: {row.get('task_type')}",
        task_instruction(str(row.get("task_type") or "")),
        "",
        "OBSERVED_STATE",
        "A source-heldout maintainer smoke root has concrete source evidence and an executed verifier observation.",
        "Candidate file paths and exact verifier identifiers are visible only inside the candidate options.",
        "",
        "SOURCE_EVIDENCE",
    ]
    for item in used:
    ...
    lines.extend(["VERIFIER_EVIDENCE", verifier_text(language, verifier), "", "CANDIDATES"])
    for opt in row.get("opaque_options") or []:
...
def render_rows(raw_rows: list[dict[str, Any]], verifier: dict[str, Any]) -> list[dict[str, Any]]:
    rows = []
    for row in raw_rows:
        target = str(row.get("target_label") or "").strip()
        rendered = dict(row)
        rendered["row_id"] = f"{row.get('row_id')}::verifier_grounded_v1"
        rendered["source_row_id"] = row.get("row_id")
        rendered["stage11740_verifier_grounded"] = True
        rendered["prompt_text"] = render_prompt(row, verifier)
...
def main() -> None:
    verifier = stage11722.load_json(VERIFIER_EXECUTION)
    if verifier.get("passed") is not True:
        raise SystemExit("Stage11737 verifier execution did not pass")
    raw_rows = []
    for path in ROW_SOURCES.values():
        raw_rows.extend(stage11722.load_jsonl(path))
    OUT.mkdir(parents=True, exist_ok=True)
    rows = render_rows(raw_rows, verifier)
    ...
    card = stage11722._write_bounded_choice_eval_audit(
        ...
        split_name="verifier_grounded_source_heldout_smoke",
    ...
    gates = {
        "stage11737_verifiers_attached": verifier.get("passed") is True,
        "full_coverage": metrics["scored_rows"] == metrics["rows"] == 12,
        ...
        "hundred_m_all_correct": metrics["correct"] == 12,
    }
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "verifier_grounded_source_heldout_successor_scored",
        "passed": gates["full_coverage"],
        "model_runtime": rel(stage11722.RUNTIME_BUNDLE),
        "scorer": SCORER,
        "metrics": metrics,
        "by_language": grouped(scored, "language_family"),
        ...
        "source_artifacts": {
            "verifier_execution": rel(VERIFIER_EXECUTION),
            "row_sources": {key: rel(path) for key, path in ROW_SOURCES.items()},
            "runtime_bundle": rel(stage11722.RUNTIME_BUNDLE),
        },
        "claim_boundary": [
            "This is a verifier-grounded compact source-heldout successor score for Python, C/C++ and Rust smoke rows.",
            "It uses current-state pass verifier evidence, not fail-to-pass repair evidence.",
            "Gemma same-manifest outputs and full-product patch/tool traces are still not attached.",
        ],
        "outputs": {
            "summary": rel(SUMMARY),
            "rendered_rows": rel(RENDERED_ROWS),
            "score_rows": rel(SCORED_ROWS),
            "bounded_choice_audit": rel(OUT / "bounded_choice_eval_audit_verifier_grounded_source_heldout_smoke.json"),
        },
    }
