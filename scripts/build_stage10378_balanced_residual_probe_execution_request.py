...
STAGE = 10378
NAME = "stage10378_balanced_residual_probe_execution_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
MANIFEST_JSONL = OUT_DIR / "balanced_residual_probe_manifest.jsonl"
REQUEST_JSON = OUT_DIR / "balanced_residual_probe_execution_request.json"
COMMAND_JSON = OUT_DIR / "balanced_residual_probe_command.json"
...
RUN_ID = "stage10379_balanced_residual_probe"
OUTPUT_DIR = "runs/local/artifacts/stage10379_balanced_residual_probe/bounded_decoder_probe"
RUNTIME_MODEL_DIR = "runs/local/artifacts/stage10379_balanced_residual_probe/runtime_model"
...
CPP_PARAMETERGOLF_BUNDLE = (
    "stage10119::localsess_parametergolf_sessseed_codex_sessions_rollout_2026_03_28t16_56_21_019d3560_1014_7761_9ea7_5638_"
    "parameter_golf_records_track_10min_16mb_2026_03_26_statespace_causalmachine_cuda_a018ca0b43_aug_1500000_8b46e7f662::c_cpp"
)
CPP_AGENTKERNEL_ABSTAIN_PREFIX = (
    "stage10119::localsess_agentkernel_sessseed_codex_sessions_rollout_2026_03_26t17_53_18_019d2b47_7d2d_7622_8e05_3513_"
    "agent_kernel_modeling_ssm_kernels_src_selective_scan_cpp_agent_kernel_modeling_w_5511e7fec6_aug_1500000_8b46e7f662::"
    "c_cpp::abstention_insufficient_evidence"
)
RUST_CANDLE_VERIFIER_PREFIX = "stage10126::candle::candle-core::rust::verifier_outcome"
RUST_CANDLE_ABSTAIN_PREFIX = "stage10126::candle::candle-core::rust::abstention_insufficient_evidence"
...
def normalize_train_row(row: dict[str, Any], *, support_family: str) -> dict[str, Any]:
    ...
    out["disable_losses"] = []
    out["split"] = "train"
    out["row_id"] = f"stage10378::{support_family}::{out.get('row_id')}"
    out["semantic_key"] = f"stage10378::{support_family}::{out.get('semantic_key') or out.get('row_id')}"
    anti = dict(out.get("anti_cheat") or {})
    anti.update(
        {
            "stage10378_support_only": True,
            "focused_balanced_residual_probe": True,
            "non_promotable_until_disjoint_rebuild": True,
        }
    )
    out["anti_cheat"] = anti
    source = dict(out.get("standalone_projection_source") or {})
    source["stage10378_support_family"] = support_family
...
def clone_train_from_strict(base_row: dict[str, Any], *, support_family: str, perm_idx: int) -> dict[str, Any]:
    ...
    clone["row_id"] = f"stage10378::{support_family}::{base_row['row_id']}::perm_{perm_idx:02d}"
    clone["semantic_key"] = f"stage10378::{support_family}::{base_row.get('row_id')}::perm_{perm_idx:02d}"
    clone["split"] = "train"
    ...
    clone["query_text"] = f"{base_row.get('query_text','balanced_residual')}::stage10378::{support_family}::{perm_idx:02d}"
    clone["opaque_options"] = rotated
    projection = dict(clone.get("standalone_projection_source") or {})
    projection["opaque_options"] = rotated
    projection["stage10378_support_family"] = support_family
    ...
    anti["stage10378_same_surface_guard"] = True
...
def select_python_rows() -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for row in load_jsonl(PYTHON_SUPPORT):
        ...
        if task_type == "evidence_citation":
            ...
            rows.append(normalize_train_row(row, support_family="python_universe_verifier_anchor"))
    strict_rows = strict_rows_from_payload(load_json(STRICT_PAYLOAD))
    by_id = {str(row.get("row_id") or ""): row for row in strict_rows}
    base = by_id[PYTHON_MIRRORMIND_VERIFIER_ROW]
    option_count = len(base.get("opaque_options") or [])
    for perm_idx in range(option_count):
        rows.append(clone_train_from_strict(base, support_family="python_mirrormind_verifier_guard", perm_idx=perm_idx))
    return rows


def select_cpp_rows() -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for row in load_jsonl(CPP_CITATION_SUPPORT):
        ...
        if str(row.get("task_type") or "") != "evidence_citation":
            continue
        if str(row.get("source_bundle_id") or "") != CPP_PARAMETERGOLF_BUNDLE:
            continue
        source = row.get("standalone_projection_source") or {}
        if str(source.get("gold_value") or "") != "verifier_and_test_constraint":
            continue
        rows.append(normalize_train_row(row, support_family="cpp_parametergolf_citation_constraint"))
    for row in load_jsonl(BASE_TRAIN):
        if str(row.get("split") or "") != "train":
            continue
        if str(row.get("language_family") or "") != "c_cpp":
            continue
        if str(row.get("task_type") or "") != "abstention_insufficient_evidence":
            continue
        if str(((row.get("standalone_projection_source") or {}).get("gold_value")) or "") != "ABSTAIN_INSUFFICIENT_EVIDENCE":
            continue
        if CPP_AGENTKERNEL_ABSTAIN_PREFIX not in str(row.get("row_id") or ""):
            continue
        rows.append(normalize_train_row(row, support_family="cpp_agentkernel_abstain_anchor"))
    return rows


def select_rust_rows() -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for row in load_jsonl(BASE_TRAIN):
        if str(row.get("split") or "") != "train":
            continue
        ...
        if task_type == "verifier_outcome" and gold_value == "ABSTAIN_INSUFFICIENT_EVIDENCE" and RUST_CANDLE_VERIFIER_PREFIX in row_id:
            rows.append(normalize_train_row(row, support_family="rust_candle_verifier_abstain_anchor"))
        elif task_type == "abstention_insufficient_evidence" and gold_value == "ABSTAIN_INSUFFICIENT_EVIDENCE" and RUST_CANDLE_ABSTAIN_PREFIX in row_id:
            rows.append(normalize_train_row(row, support_family="rust_candle_abstain_anchor"))
    return rows
...
def main() -> None:
    strict_rows = strict_rows_from_payload(load_json(STRICT_PAYLOAD))
    train_rows = select_python_rows() + select_cpp_rows() + select_rust_rows()
    ...
    for row in train_rows:
        family = str(((row.get("standalone_projection_source") or {}).get("stage10378_support_family")) or "unknown")
    ...
    request = {
        ...
        "passed": True,
        "decision": "balanced_residual_probe_ready",
        ...
        "claim_scope": "diagnostic train-support-only balanced probe against the repaired 47-row honest frontier; non-promotable until any movement is rebuilt from disjoint roots",
        "required_honesty_gates": [
            "strict eval remains the unchanged 47-row quarantined full-visible frontier",
            "same-surface mirrormind verifier guard is diagnostic only and cannot support promotion claims",
            "runtime model saving is enabled solely to permit direct rescoring of this diagnostic if it improves the frontier",
            "success means reducing the remaining four-row miss set without reintroducing the fixed python or c_cpp citation rows",
        ],
        "next_best_step": "run one balanced residual probe that actually reaches rust abstention training while guarding the new python verifier regression",
        "command": command,
