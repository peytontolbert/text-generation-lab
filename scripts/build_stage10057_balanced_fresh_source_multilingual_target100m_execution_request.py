...
STAGE = 10057
NAME = "stage10057_balanced_fresh_source_multilingual_target100m_execution_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
REQUEST = OUT_DIR / "balanced_fresh_source_multilingual_target100m_execution_request.json"
SURFACE_REQUEST = OUT_DIR / "surface_requests" / "edit_localization.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "BALANCED_FRESH_SOURCE_MULTILINGUAL_TARGET100M_EXECUTION_REQUEST_STAGE10057.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
MANIFEST = ROOT / "runs/local/artifacts/stage10056_balanced_fresh_source_multilingual_geometry_successor_packet/balanced_fresh_source_multilingual_geometry_successor_manifest.jsonl"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage10056_balanced_fresh_source_multilingual_geometry_successor_packet.json"
...
FUTURE_RUN_ID = "stage10057_balanced_fresh_source_multilingual_target100m_probe"
FUTURE_OUTPUT_DIR = "runs/local/artifacts/stage10057_balanced_fresh_source_multilingual_target100m_probe/edit_localization_probe"
...
def update_registry(summary: dict[str, Any]) -> None:
    registry = load_json(ROOT / "runs/local/artifacts/reconstructed_stage_registry.json") or {"rows": [], "metrics": {}}
    rows = [row for row in registry.get("rows", []) if row.get("stage") != STAGE and row.get("stage_name") != NAME]
    rows.append({"stage": STAGE, "stage_name": NAME, "passed": summary["passed"], "path": str(SUMMARY), "authority": dict(AUTHORITY_CLOSED), "next_best_step": summary["next_best_step"]})
    registry["rows"] = sorted(rows, key=lambda row: (int(row.get("stage", -1)), row.get("stage_name", "")))
    registry["passed"] = bool(registry["rows"])
    registry["metrics"] = {**(registry.get("metrics") or {}), "latest_stage": STAGE, "latest_stage_name": NAME, "latest_stage_next_best_step": summary["next_best_step"], "max_stage": STAGE, "registry_rows": len(registry["rows"])}
    (ROOT / "runs/local/artifacts/reconstructed_stage_registry.json").write_text(json.dumps(registry, indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
def build_request() -> dict[str, Any]:
    ...
    stage10056_train_rows = sum(1 for row in rows if str(row.get("stage10056_role") or "") == "train")
    ...
    metrics = {
        ...
        "stage10056_train_rows": stage10056_train_rows,
    }
    if metrics["rows"] != 127: failures.append("rows_not_127")
    if split_counts.get("train") != 72: failures.append("train_rows_not_72")
    if split_counts.get("eval") != 38: failures.append("eval_rows_not_38")
    if split_counts.get("strict_eval") != 17: failures.append("strict_rows_not_17")
    if language_counts.get("python") != 27: failures.append("python_rows_not_27")
    if language_counts.get("c_cpp") != 38: failures.append("c_cpp_rows_not_38")
    if language_counts.get("rust") != 17: failures.append("rust_rows_not_17")
    if language_counts.get("web_js_ts_html") != 45: failures.append("web_rows_not_45")
    if stage10047_train_rows != 8: failures.append("stage10047_train_rows_not_8")
    if stage10056_train_rows != 24: failures.append("stage10056_train_rows_not_24")
    if heldout_rows != 55: failures.append("heldout_compare_rows_not_55")
    if source_summary.get("passed") is not True: failures.append("stage10056_not_passed")
    if baseline.get("runtime_executed") is not True: failures.append("stage10040_runtime_not_executed")
    ...
    req = {
        ...
        "required_contract_invariants": {
            "source_stage10056_passed": bool(source_summary.get("passed") is True),
    ...
    return {
        ...
        "metrics": metrics,
        "source_artifacts": {
            "stage10056_packet_summary": display(SOURCE_SUMMARY),
        ...
        "execution_constraints": {
            ...
            "balanced_geometry_required": True,
        },
...
def main() -> None:
    ...
    next_step = "Use this request to run one target-100m edit-localization probe on the geometry-balanced fresh-source successor manifest, then compare it against stage10040 and Gemma on the same 55 heldout rows."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Prepared the execution request for the geometry-balanced fresh-source multilingual successor so the next probe can test whether broader non-python label coverage prevents the rust/web drift seen in stage10054.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage10057 Balanced Fresh Source Multilingual Target100M Execution Request",
        "",
