...
STAGE = 11673
NAME = "stage11673_web_canonical_verifier_value_listwise_postrun_audit"
OUT = ART / NAME
SUMMARY = OUT / "web_canonical_verifier_value_listwise_postrun_audit.json"

RUNTIME = ART / "stage11672_web_canonical_verifier_value_listwise_probe/runtime_model/runtime_model_bundle.json"
TRAIN_LOG = ART / "stage11672_web_canonical_verifier_value_listwise_probe/bounded_decoder_probe/loss_by_step.jsonl"
ROWSETS = {
...
def training_contrast_summary() -> dict[str, Any]:
    ...
    contrast_skipped = 0
    contrast_steps = 0
    listwise_applicable = 0
    listwise_skipped = 0
    listwise_steps = 0
    listwise_target_rank1 = 0
    target_values: dict[str, int] = {}
    for row in rows:
        contrast_card = row.get("bounded_choice_contrast_card") or {}
        app = int(contrast_card.get("applicable_rows") or 0)
        skip = int(contrast_card.get("skipped_rows") or 0)
        contrast_applicable += app
        contrast_skipped += skip
        if app:
            contrast_steps += 1
        listwise_card = row.get("bounded_choice_verifier_value_listwise_card") or {}
        list_app = int(listwise_card.get("applicable_rows") or 0)
        list_skip = int(listwise_card.get("skipped_rows") or 0)
        listwise_applicable += list_app
        listwise_skipped += list_skip
        listwise_target_rank1 += int(listwise_card.get("target_rank1_rows") or 0)
        if list_app:
            listwise_steps += 1
        for key, value in (listwise_card.get("target_values") or {}).items():
            target_values[key] = target_values.get(key, 0) + int(value)
    return {
        "train_log_exists": True,
        "steps": len(rows),
        "contrast_applicable_rows_total": contrast_applicable,
        "contrast_skipped_rows_total": contrast_skipped,
        "steps_with_contrast_applicable": contrast_steps,
        "contrast_inactive": contrast_applicable == 0 and contrast_skipped > 0,
        "listwise_applicable_rows_total": listwise_applicable,
        "listwise_skipped_rows_total": listwise_skipped,
        "steps_with_listwise_applicable": listwise_steps,
        "listwise_target_rank1_rows_total": listwise_target_rank1,
        "listwise_target_rank1_rate": (listwise_target_rank1 / listwise_applicable) if listwise_applicable else None,
        "listwise_target_values": target_values,
...
def main() -> None:
    ...
    if not preservation_ok:
        decision = "web_canonical_verifier_value_listwise_rejected_protected_gate_regression"
    elif gates["original_web_heldout_beats_gemma_52_of_66"]:
        decision = "web_canonical_verifier_value_listwise_beats_original_web_gemma_but_requires_external_validation"
    elif gates["original_web_heldout_beats_routed_38_of_66"]:
        decision = "web_canonical_verifier_value_listwise_improves_original_web_heldout"
    elif gates["canonical_heldout_beats_routed_38_of_66"]:
        decision = "web_canonical_verifier_value_listwise_improves_canonical_heldout_only"
    elif gates["canonical_train_support_at_least_300_of_334"]:
        decision = "web_canonical_verifier_value_listwise_support_fit_without_frontier_gain"
    else:
        decision = "web_canonical_verifier_value_listwise_no_support_or_heldout_gain"
    ...
    summary = {
        ...
        "baseline_reference": {
            ...
            "stage11667_active_contrast_canonical_heldout": "50/66",
            "stage11667_active_contrast_original_web": "27/66",
            "stage11670_verifier_value_contrast_canonical_heldout": "50/66",
            "stage11670_verifier_value_contrast_original_web": "27/66",
        ...
        "claim_boundary": [
            "Stage11672 is a head-only diagnostic on canonical-rendered Web rows with same-role verifier-transition listwise loss.",
