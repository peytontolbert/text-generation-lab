...
STAGE = 10840
NAME = "stage10840_current_residual_support_probe_request"
OUT_DIR = ARTIFACTS / NAME
REQUEST_JSON = OUT_DIR / "current_residual_support_probe_request.json"
COMMAND_JSON = OUT_DIR / "current_residual_support_probe_command.json"
MANIFEST_JSONL = OUT_DIR / "current_residual_support_probe_manifest.jsonl"
...
STRESS_ROWS = ARTIFACTS / "stage10839_current_residual_support_package" / "agentkernel_lite_encdec_stress_eval.jsonl"
...
RUN_ID = "stage10841_current_residual_support_probe"
OUTPUT_DIR = "runs/local/artifacts/stage10841_current_residual_support_probe/bounded_decoder_probe"
RUNTIME_MODEL_DIR = "runs/local/artifacts/stage10841_current_residual_support_probe/runtime_model"
...
def main() -> None:
    ...
    request = {
        ...
        "decision": "current_residual_support_probe_ready",
        ...
        "claim_scope": [
            "Run the next standalone multilingual residual-support probe from the current package with the repaired hf_local multitest packet added on top of the queue-aligned support base.",
            "Keep the 24-row validation and 24-row strict frontier fixed while testing whether the added Python verifier geometry improves the saved runtime without altering the honest eval surface.",
        ],
        "required_honesty_gates": [
            "strict and validation rows remain unchanged from stage10839",
            "hf_local repaired rows remain train-support only and cannot be reinterpreted as a heldout Python win",
            "linux Rust support remains train-support only and is not upgraded into a headline Rust comparison claim",
            "any post-run improvement must still report Python verifier_outcome and Rust evidence_citation family deltas explicitly",
        ],
        "lane_state_notes": package.get("lane_state_notes"),
        ...
        "next_best_step": "run the probe, then compare the saved runtime on the unchanged 24-row strict frontier and inspect whether the Python verifier family moved without regressions.",
    }
