#!/usr/bin/env python3
"""Refresh support supply after codex-rmcp-client Rust admission."""
...
STAGE = 11835
NAME = "stage11835_source_heldout_support_supply_status_v22"
OUT = ART / NAME
SUMMARY = OUT / "source_heldout_support_supply_status_v22.json"
...
RUST = ART / "stage11834_rust_verifier_support_combined_audit_v6/rust_verifier_support_combined_audit_v6.json"
...
def main() -> None:
    ...
    artifact = {
        ...
        if not training_probe_ready
        else "support_supply_ready_for_training_probe_request",
        "passed": True,
        "training_probe_ready": training_probe_ready,
        "support_status": support_status,
        "delta_since_stage11831": {
            "rust_verifier_ready_roots_added": max(0, rust_roots - 6),
            "rust_verifier_ready_rows_added": max(0, int(rust.get("admitted_rows", 0)) - 24),
        },
        "next_actions": [
            "Continue Python verifier expansion beyond 8/12 roots.",
            "Continue C/C++ abstain-attractor expansion beyond 7/16 roots.",
            f"Continue Rust verifier expansion beyond {rust_roots}/12 roots.",
        ],
        "claim_boundary": [
