#!/usr/bin/env python3
"""Materialize bitsandbytes Python verifier support rows."""
...
STAGE = 11771
NAME = "stage11771_bitsandbytes_python_support_rows"
OUT = ART / NAME
SUMMARY = OUT / "bitsandbytes_python_support_rows.json"
ROWS = OUT / "bitsandbytes_python_support_rows.jsonl"

FEASIBILITY = ART / "stage11770_bitsandbytes_python_repaired_feasibility/bitsandbytes_python_repaired_feasibility_results.jsonl"
...
def main() -> None:
    feasible = [row for row in read_jsonl(FEASIBILITY) if row.get("repo_family") == "bitsandbytes" and row.get("passed") is True]
    if not feasible:
        raise SystemExit("bitsandbytes Python feasibility did not pass")
    probe = feasible[0]
    root_id = "stage11771::bitsandbytes::python::cuda_library_path_support"
    selected = probe["selected_result"]
    sibling = probe["sibling_result"]
    evidence = [
        file_evidence("E01", "implementation_source", "bitsandbytes/cextension.py", "Native library path selection honors CUDA specs and BNB_CUDA_VERSION overrides."),
        file_evidence("E02", "selected_test_anchor", "tests/test_cuda_setup_evaluator.py", "Function-level tests cover CUDA binary path selection and override behavior."),
        file_evidence("E03", "nearby_source_distractor", "bitsandbytes/cuda_specs.py", "CUDA specification discovery is related but not the selected library path function itself."),
        log_evidence("V01", selected, f"Pytest file passed; selected function anchor: {probe['selected_test']}"),
    ]
    verifier_evidence = {"id": "V01", "kind": "actual_pytest_pass_log", "summary": "Focused pytest file passed with two function-level verifier anchors.", "selected_test": probe["selected_test"], "sibling_test": probe["sibling_test"], "selected_result": selected, "sibling_result": sibling, "result": "PASS_CURRENT_STATE"}
    verifier_options = [
        ...
        {"label": "C", "role": "implementation_only_no_verifier", "value": "bitsandbytes/cextension.py", "evidence_ids": ["E01"]},
        {"label": "D", "role": "nearby_source_distractor", "value": "bitsandbytes/cuda_specs.py", "evidence_ids": ["E03"]},
    ]
    evidence_options = [
        {"label": "A", "role": "candidate_change_surface", "value": "bitsandbytes/cextension.py", "evidence_ids": ["E01"]},
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "fourth_python_verifier_support_root_materialized",
        "passed": True,
        "row_count": len(rows),
        "root_count": 1,
        "repo_family": "bitsandbytes",
