...
BASE = ROOT / "runs/local/artifacts/knowledge_compression_moe_residual_10k_stage601_stage599_entity_field_context_lr5e6_steps300"
CAND = ROOT / "runs/local/artifacts/knowledge_compression_moe_residual_10k_stage602_stage601_entity_field_balanced_replay_lr3e6_steps200"
OUT = ROOT / "runs/local/artifacts/stage602_entity_field_balanced_replay_summary.json"
DOC = ROOT / "docs/stage602_entity_field_balanced_replay.md"
...
def compact(result: dict[str, Any], hard: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "multi_candidate_queries": stats["queries_with_multiple_exact_key_candidates"],
    }
...
def build_summary() -> dict[str, Any]:
    ...
    dataset = load(ROOT / "runs/local/artifacts/stage602_entity_field_balanced_replay_dataset.json")
    return {
        "artifact_kind": "stage602_entity_field_balanced_replay_summary",
        "candidate": "stage602_stage601_entity_field_balanced_replay_lr3e6_steps200",
        "bundle_dir": str(CAND.relative_to(ROOT)),
        "checkpoint": str((CAND / "checkpoints/step_00000200.pt").relative_to(ROOT)),
        "train_dataset_manifest": "runs/local/tmp/pocketpal_stage602_entity_field_balanced_replay_seed461/agentkernel_lite_encdec_dataset_manifest.json",
        ...
        "dataset": {
            "extra_copies": dataset["extra_copies"],
        ...
        "baseline_stage601": compact(base_no, base_hard),
        "candidate_stage602": compact(cand_no, cand_hard),
        "deltas_vs_stage601": {
        ...
        "decision": "accepted_as_stage601_entity_field_surface_best",
        "finding": (
            "Balanced replay repairs the Stage601 non-target regressions while keeping and extending the field-level entity gain. "
            "Direct_fact and set_intersection_member recover above their Stage599-on-Stage601 baselines, entity_context crosses "
            "0.50 exact on the field surface, and hard-filter corrections fall further. This confirms the route: field-level "
            "entity context plus balanced replay is a better KBPP schema than whole-card entity replay."
        ),
    }


def write_doc(summary: dict[str, Any]) -> None:
    base = summary["baseline_stage601"]
    cand = summary["candidate_stage602"]
    ...
    doc = f"""# Stage602 Entity Field Balanced Replay
...
Hard-filter corrections moved from `{base['hard_filter_corrections']}` to `{cand['hard_filter_corrections']}` with `{cand['hard_filter_damage']}` damage.
