#!/usr/bin/env python3
"""Audit Stage11947 Transition-1K-v2 routed runtime."""
...
STAGE = 11948
NAME = "stage11948_transition_1k_v2_routed_postrun_audit"
OUT = ART / NAME
SUMMARY = OUT / "transition_1k_v2_routed_postrun_audit.json"
...
RUNTIMES = {
    ...
    "stage11947_transition_1k_v2_listwise": ART / "stage11947_transition_1k_v2_listwise_probe/runtime_model/runtime_model_bundle.json",
}
PROTECTED_ROWSETS = {
...
def main() -> None:
    ...
    previous = results["stage11919_transition_contrast_head_only"]
    postrun = results["stage11947_transition_1k_v2_listwise"]
    gates = {
        ...
        "transition_projection_improved_vs_stage11919": (postrun["transition_projection_routed"].get("correct") or 0) > (previous["transition_projection_routed"].get("correct") or 0),
        "transition_projection_at_least_355_of_640": (postrun["transition_projection_routed"].get("correct") or 0) >= 355,
    ...
    if all(gates.values()):
        decision = "transition_1k_v2_routed_candidate_ready_for_gemma_comparison"
    elif gates["transition_projection_improved_vs_stage11919"] and gates["filtered_strict_preserved"] and gates["old_canary_strict_preserved"]:
        decision = "transition_1k_v2_routed_partial_candidate"
    else:
        decision = "transition_1k_v2_routed_rejected"
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "created_at_utc": now(),
    ...
    print(json.dumps({"decision": decision, "gates": gates, "previous_transition": previous["transition_projection_routed"], "postrun_transition": postrun["transition_projection_routed"]}, indent=2, sort_keys=True))
