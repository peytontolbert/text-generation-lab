...
STAGE = 9320
NAME = "stage9320_phrase_completion_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9319_phrase_completion_balance_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9319_phrase_completion_balance_manifest/phrase_completion_balance_manifest.jsonl"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9321_phrase_completion_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "phrase_completion_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9321_phrase_completion_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PHRASE_COMPLETION_PROBE_PREEXECUTION_STAGE9320.md"
...
COMMAND = [
    ...
    "--enable-generation-audit", "--max-generation-rows", "24", "--max-generation-tokens", "64",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9321_phrase_completion_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9319_not_passed")
    if len(rows) != 24:
        failures.append("manifest_row_count_not_24")
    ...
    required_pairs = {
        ...
        "--max-eval-rows": "8", "--max-strict-rows": "8", "--max-steps": "64",
        "--max-generation-rows": "24", "--decoder-ce-weight": "0.0", "--structured-aux-weight": "0.0",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared a 24-row phrase-completion target-100M denoise probe; decoder CE remains closed.",
        "next_best_step": "Execute Stage9321 phrase-completion probe under trellis and audit phrase-family quality.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9320 Phrase Completion Probe Preexecution",
        "",
