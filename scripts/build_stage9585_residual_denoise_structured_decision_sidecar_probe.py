...
STAGE = 9585
NAME = "stage9585_residual_denoise_structured_decision_sidecar_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9584_residual_denoise_structured_decision_sidecar_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9584_residual_denoise_structured_decision_sidecar_manifest/residual_denoise_structured_decision_sidecar_manifest.jsonl"
RUN_ROOT = ROOT / "runs/local/artifacts" / NAME
CONTRACT_DIR = RUN_ROOT / "contract_preflight"
RUN_DIR = RUN_ROOT / "structured_sidecar_probe"
AUDIT = RUN_ROOT / "residual_denoise_structured_decision_sidecar_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "RESIDUAL_DENOISE_STRUCTURED_DECISION_SIDECAR_PROBE_STAGE9585.md"
...
def main() -> None:
    ...
    if not failures:
        contract = subprocess.run(command(output_dir=CONTRACT_DIR, run_id="stage9585_contract_preflight", contract_only=True), cwd=ROOT, text=True, capture_output=True, check=False)
    ...
    if not failures:
        run = subprocess.run(command(output_dir=RUN_DIR, run_id="stage9585_structured_sidecar_probe", contract_only=False), cwd=ROOT, text=True, capture_output=True, check=False)
        if run.returncode != 0:
    ...
    summary = {
        ...
        "next_best_step": "If the sidecar passes, compile sidecar-gated denoise rendering rows; otherwise add counterbalanced non-shortcut sidecar evidence before returning to denoise CE.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9585 Residual Denoise Structured Decision Sidecar Probe", "", f"Passed: `{audit['passed']}`", f"Eval suffix choice exact: `{eval_exact}`", f"Strict suffix choice exact: `{strict_exact}`", f"Pass gate: `{pass_gate}`", "", "Decoder, denoise, runtime, export, harness, and promotion remain closed.", ""]), encoding="utf-8")
