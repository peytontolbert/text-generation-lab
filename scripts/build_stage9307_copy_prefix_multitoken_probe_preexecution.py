...
STAGE = 9307
NAME = "stage9307_copy_prefix_multitoken_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9306_supported_suffix_boundary_probe_audit.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9304_operator_suffix_support_manifest/operator_suffix_support_manifest.jsonl"
MANIFEST_AUDIT = ROOT / "runs/local/artifacts/stage9304_operator_suffix_support_manifest/operator_suffix_support_manifest_audit.json"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9308_copy_prefix_multitoken_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "copy_prefix_multitoken_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9308_copy_prefix_multitoken_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "COPY_PREFIX_MULTITOKEN_PROBE_PREEXECUTION_STAGE9307.md"
...
COMMAND = [
    ...
    "--max-train-rows", "5", "--max-eval-rows", "1", "--max-strict-rows", "1",
    "--max-steps", "32", "--batch-size", "2", "--learning-rate", "1e-5",
    ...
    "--enable-generation-audit", "--max-generation-rows", "7", "--max-generation-tokens", "64",
    "--generation-prefix-field", "model_input.copy_prefix_span", "--generation-audit-splits", "train,eval,strict_eval",
    "--require-loss-mask-enforcement-audit", "--no-final-checkpoint-export", "--cleanup-checkpoints-after-probe", "--skip-final-model-save", "1",
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9308_copy_prefix_multitoken_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    failures: list[str] = []
    if source.get("passed") is not True or source_metrics.get("quality_gate_passed") is not True:
        failures.append("source_stage9306_quality_not_passed")
    if manifest_audit.get("passed") is not True:
        failures.append("manifest_audit_not_passed")
    if len(rows) != 7:
        failures.append("manifest_row_count_not_7")
    ...
    required_pairs = {
        ...
        "--max-eval-rows": "1", "--max-strict-rows": "1", "--max-steps": "32",
        "--max-generation-tokens": "64", "--max-decoder-tokens": "96", "--decoder-ce-weight": "0.0",
        "--denoise-weight": "1.0", "--generation-prefix-field": "model_input.copy_prefix_span",
    ...
    return {
        ...
        "authorized_next_stage": 9308,
        "tmpdir": str(TMPDIR),
        "required_postrun_artifacts": ["sample_generation_audit.json", "boundary_next_token_logits.jsonl", "row_token_loss.jsonl", "cleanup_proof.json"],
...
def main() -> None:
    ...
    summary = {
        ...
        "metrics": {**{key: bool(card["authority"].get(key, False)) for key in AUTHORITY_CLOSED}, "command_ready": card["command_ready"], "will_execute_now": False, "authorized_next_stage": 9308, "manifest_rows": card["manifest_rows"], "tmpdir": card["tmpdir"], "required_postrun_artifacts": card["required_postrun_artifacts"], "failures": len(card["failures"])},
        "artifacts": {"audit": str(AUDIT.relative_to(ROOT)), "command": str(COMMAND_JSON.relative_to(ROOT)), "doc": str(DOC.relative_to(ROOT))},
        "decision": "Prepared copy-prefix multi-token target-100M denoise probe command; this measures longer supported suffix generation without opening decoder CE.",
        "next_best_step": "Execute Stage9308 copy-prefix multi-token probe under trellis, then audit exact generation and authority closure.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9307 Copy-Prefix Multi-Token Probe Preexecution", "", f"Passed: `{card['passed']}`", f"TMPDIR: `{card['tmpdir']}`", "Command artifact prepares Stage9308 but does not execute it.", "Decoder CE, runtime, Gemma, harness, source/body emission, scoring, and promotion remain closed.", ""]) , encoding="utf-8")
