...
STAGE = 9350
NAME = "stage9350_operator_terminal_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9349_operator_terminal_anti_repetition_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9349_operator_terminal_anti_repetition_manifest/operator_terminal_anti_repetition_manifest.jsonl"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9351_operator_terminal_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "operator_terminal_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9351_operator_terminal_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "OPERATOR_TERMINAL_PROBE_PREEXECUTION_STAGE9350.md"
...
COMMAND = [
    ...
    "--max-train-rows", "8", "--max-eval-rows", "7", "--max-strict-rows", "2",
    ...
    "--enable-generation-audit", "--max-generation-rows", "17", "--max-generation-tokens", "64",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9351_operator_terminal_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9349_not_passed")
    if len(rows) != 17:
        failures.append("unexpected_manifest_row_count")
    if dict(split_counts) != {"train": 8, "eval": 7, "strict_eval": 2}:
    ...
    required_pairs = {
        ...
        "--max-eval-rows": "7", "--max-strict-rows": "2", "--max-steps": "80", "--max-generation-rows": "17",
    ...
    return {"passed": not failures, "failures": failures, "manifest_rows": len(rows), "split_counts": dict(sorted(split_counts.items())), "unsafe_rows": sorted(set(unsafe_rows)), "command": COMMAND, "command_ready": not failures, "authority": AUTHORITY_RUN if not failures else dict(AUTHORITY_CLOSED)}
...
def main() -> None:
    ...
    summary = {
        ...
        "metrics": {**{key: bool(card["authority"].get(key, False)) for key in AUTHORITY_CLOSED}, "command_ready": card["command_ready"], "manifest_rows": card["manifest_rows"], "split_counts": card["split_counts"], "failures": len(card["failures"])},
        "artifacts": {"audit": str(AUDIT.relative_to(ROOT)), "command": str(COMMAND_JSON.relative_to(ROOT)), "doc": str(DOC.relative_to(ROOT))},
        "decision": "Prepared a closed-boundary 17-row route_1 operator-terminal denoise probe; decoder CE remains closed.",
        "next_best_step": "Execute Stage9351 operator-terminal probe under trellis and audit whether the `operatch` repetition is removed.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9350 Operator Terminal Probe Preexecution", "", f"Passed: `{card['passed']}`", f"Rows: `{card['manifest_rows']}`", f"Splits: `{card['split_counts']}`", "", "Only the Stage9351 17-row target-100M denoise probe is authorized. Decoder CE and external authority remain closed.", ""]), encoding="utf-8")
