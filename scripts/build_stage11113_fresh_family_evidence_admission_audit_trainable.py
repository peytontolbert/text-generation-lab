...
STAGE = 11113
NAME = "stage11113_fresh_family_evidence_admission_audit_trainable"
OUT_DIR = ARTIFACTS / NAME
SUMMARY_JSON = OUT_DIR / "fresh_family_evidence_admission_audit_trainable.json"
ADMITTED_ROWS_JSONL = OUT_DIR / "admitted_evidence_rows_trainable.jsonl"
BLOCKED_ROWS_JSONL = OUT_DIR / "blocked_evidence_rows_trainable.jsonl"
...
def main() -> None:
    ...
    for row in rows:
        ...
        admitted_anti_cheat["trainable_loss_contract_added"] = True
        ...
        admitted["loss_mask"] = {"decoder_ce": True}
        admitted["expected_enabled_loss"] = "decoder_ce"
        admitted["disable_losses"] = []
    ...
    summary = {
        ...
        "claim_scope": [
            "Audit the deshortcutted fresh-family evidence rows for train-support admission only and add the missing explicit decoder loss contract.",
        ...
        "headline_findings": [
            ...
            "Admission is gated on zero role-name leakage before options and now also adds the explicit decoder loss contract required by the trainer.",
        ],
        "limits": [
        ...
        "next_best_step": "Merge the admitted trainable subset into the latest clean support package and rerun the probe with the corrected loss contract.",
        "outputs": {
