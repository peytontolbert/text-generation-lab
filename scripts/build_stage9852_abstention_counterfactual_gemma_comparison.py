...
STAGE = 9852
NAME = "stage9852_abstention_counterfactual_gemma_comparison"
MANIFEST = ROOT / "runs/local/artifacts/stage9850_abstention_counterfactual_curriculum_manifest/abstention_counterfactual_curriculum_manifest.jsonl"
ROWS_100M = ROOT / "runs/local/artifacts/stage9851_direct_abstention_counterfactual_exec/row_field_logits.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
ROWS_OUT = OUT_DIR / "abstention_counterfactual_gemma_rows.jsonl"
AUDIT = OUT_DIR / "abstention_counterfactual_gemma_comparison.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "ABSTENTION_COUNTERFACTUAL_GEMMA_COMPARISON_STAGE9852.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
MODEL_ID = "gemma3:12b"
LANGS = ["python", "rust", "c_cpp", "web_js_ts_html"]
SPLITS = ["eval", "strict_eval"]
LABELS = ["A", "B", "C", "D", "E", "ABSTAIN_INSUFFICIENT_EVIDENCE"]
...
_row_text = _load_symbol("stage9852_training_data", ROOT / "legacy_src/agentkernel_lite/training_data.py", "_row_text")
...
def main() -> None:
    ...
    next_step = "If abstention improves the harder multilingual comparison, fold it into the anti-cheat review packet and scale the objective into the main v2.7 training path."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Ran local Gemma on the abstention-target heldout harder packet and compared it against the abstention-trained 100M row-level outputs on the same multilingual surface.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9852 Abstention Counterfactual Gemma Comparison",
        "",
        ...
        "",
        "This stage measures whether explicit abstention on missing-evidence rows improves the harder multilingual comparison against Gemma.",
        "",
        f"Next: {next_step}",
        "",
    ]), encoding="utf-8")
