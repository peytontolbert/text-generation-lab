...
STAGE = 11553
NAME = "stage11553_text_rich_evidence_judgment_gate_audit"
OUT = ART / NAME
SUMMARY = OUT / "text_rich_evidence_judgment_gate_audit.json"
...
def text_rich_reroute(row: dict[str, Any]) -> tuple[dict[str, Any], bool, str]:
    ...
    for option in options:
        ...
        mapped.append(item)
    if mapped_count < 2:
        return out, False, "insufficient_supported_roles"
    source = dict(out.get("standalone_projection_source") or {})
    source["opaque_options"] = mapped
    source["stage11553_reroute_gate"] = "text_rich_supported_evidence_roles"
...
def main() -> None:
    ...
    for name, path in ROWSETS.items():
        ...
        rerouted_count = 0
        for row in original:
            mapped, did, reason = text_rich_reroute(row)
    ...
    summary = {
        ...
        "created_at_utc": now(),
        "passed": True,
        "decision": "text_rich_evidence_judgment_gate_promising" if all(gates.values()) else "text_rich_evidence_judgment_gate_not_promotable",
        "product_scorer": PRODUCT_SCORER,
        "results": results,
        "gates": gates,
        "claim_boundary": [
            "Inference-side routing audit only; product scorer contract is unchanged.",
            "Gate requires evidence_citation rows with non-empty option text and at least two supported evidence-role values.",
