...
STAGE = 9365
NAME = "stage9365_full_interference_rejoin_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9364_full_mixture_with_interference_repairs_manifest.json"
...
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9366_full_interference_rejoin_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "full_interference_rejoin_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9366_full_interference_rejoin_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "FULL_INTERFERENCE_REJOIN_PROBE_PREEXECUTION_STAGE9365.md"
...
COMMAND = [
    ...
    "--max-train-rows", "109", "--max-eval-rows", "61", "--max-strict-rows", "39",
    ...
    "--enable-generation-audit", "--max-generation-rows", "209", "--max-generation-tokens", "64",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9366_full_interference_rejoin_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9364_not_passed")
    if len(rows) != 209:
    ...
    if dict(route_counts) != {"route_0": 52, "route_1": 117, "route_2": 18, "route_file_path": 22}:
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared a closed-boundary 209-row full-mixture denoise rejoin probe; decoder CE remains closed.",
        "next_best_step": "Execute Stage9366 full interference rejoin probe under trellis and audit whether denoise repair is clean enough to return to bounded decoder CE readiness.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9365 Full Interference Rejoin Probe Preexecution", "", f"Passed: `{card['passed']}`", f"Rows: `{card['manifest_rows']}`", f"Splits: `{card['split_counts']}`", f"Routes: `{card['route_counts']}`", "", "Only the Stage9366 209-row target-100M denoise probe is authorized. Decoder CE and external authority remain closed.", ""]), encoding="utf-8")
