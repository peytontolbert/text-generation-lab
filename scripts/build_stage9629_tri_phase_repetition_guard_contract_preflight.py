...
STAGE = 9629
NAME = "stage9629_tri_phase_repetition_guard_contract_preflight"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9628_hard_phrase_tri_phase_regression_audit.json"
...
RUN_DIR = OUT_DIR / "tri_phase_contract"
AUDIT = OUT_DIR / "tri_phase_repetition_guard_contract_preflight.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "TRI_PHASE_REPETITION_GUARD_CONTRACT_PREFLIGHT_STAGE9629.md"
...
def command() -> list[str]:
    return [
        ...
        "--phase3-max-steps", "0", "--phase3-max-decoder-tokens", "96",
        ...
        "--output-dir", str(RUN_DIR), "--run-id", "stage9629_tri_phase_repetition_guard_contract",
        "--contract-only",
    ]
...
def main() -> None:
    ...
    phase3_rows = load_jsonl(PHASE3_MANIFEST)
    failures: list[str] = []
    if source.get("passed") is not True:
        failures.append("stage9628_not_passed")
    if len(phase1_rows) != 96:
        failures.append("phase1_rows_not_96")
    if len(phase2_rows) != 28:
        failures.append("phase2_rows_not_28")
    if len(phase3_rows) != 52:
        failures.append("phase3_rows_not_52")
    ...
    if run is not None and run.returncode != 0:
        failures.append("tri_phase_contract_command_failed")
    ...
    if card.get("mode") != "tri_phase_suffix_phrase_residual_reconnect_probe":
        failures.append("wrong_contract_mode")
    if card.get("phase2_rows") != 28:
        failures.append("contract_phase2_rows_not_28")
    if card.get("phase3_rows") != 52:
        failures.append("contract_phase3_rows_not_52")
    ...
    if card.get("generation_repetition_guard") is not True:
        failures.append("generation_repetition_guard_not_recorded")
    if card.get("generation_repetition_guard_top_k") != 32:
        failures.append("generation_repetition_guard_top_k_not_32")
    ...
    audit = {
        ...
        "phase3_manifest": str(PHASE3_MANIFEST.relative_to(ROOT)),
        ...
        "generation_repetition_guard": card.get("generation_repetition_guard"),
        "generation_repetition_guard_top_k": card.get("generation_repetition_guard_top_k"),
        "run_dir": str(RUN_DIR.relative_to(ROOT)),
        "nonzero_execution_authorized": False,
        "authority": dict(AUTHORITY_CLOSED),
    }
    AUDIT.write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    next_step = "Run Stage9630 tiny tri-phase baseline with the audited generation repetition guard and compare against Stage9623/9627 quality."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Tri-phase repetition-guard command surface is contract-clean. No model execution occurred.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9629 Tri-Phase Repetition Guard Contract Preflight",
        "",
        f"Passed: `{audit['passed']}`",
        f"Phase rows: `{audit['phase1_rows']}` / `{audit['phase2_rows']}` / `{audit['phase3_rows']}`",
        f"Contract passed: `{audit['contract_passed']}`",
        f"Generation prefix field: `{audit['generation_prefix_field']}`",
        "",
        "This is a non-executing preflight for the Stage9623 baseline curriculum with an opt-in generation repetition guard. Decoder CE, runtime, Gemma, harness, checkpoint export, and promotion remain closed.",
        "",
        f"Next: {next_step}",
        "",
    ]), encoding="utf-8")
