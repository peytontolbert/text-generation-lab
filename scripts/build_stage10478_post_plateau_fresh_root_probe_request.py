...
STAGE = 10478
NAME = "stage10478_post_plateau_fresh_root_probe_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
REQUEST_JSON = OUT_DIR / "post_plateau_fresh_root_probe_request.json"
MANIFEST_JSONL = OUT_DIR / "post_plateau_fresh_root_probe_manifest.jsonl"
...
RUN_ID = "stage10478_post_plateau_fresh_root_probe"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage10478_post_plateau_fresh_root_probe/bounded_decoder_probe"
RUNTIME_MODEL_DIR = ROOT / "runs/local/artifacts/stage10478_post_plateau_fresh_root_probe/runtime_model"
...
def main() -> None:
    ...
    request = {
        ...
        "decision": "post_plateau_fresh_root_probe_ready",
        "claim_scope": [
            "Diagnostic preserved-runtime probe against the repaired v2.7 strict overlay using the materialized post-plateau fresh-root support package.",
            "Python movement is the main promotable signal in this run because its support roots are executable and disjoint.",
            "Any Rust movement remains diagnostic until a non-tokenizers fresh-root artifact reproduces the same contrast on unseen roots.",
        ],
        "source_artifacts": {
            "support_package": display(SUPPORT_PACKAGE),
        ...
        "diagnostic_boundaries": {
            ...
            "reason_rust_not_promotable_yet": "Non-tokenizers fresh-root contrast rows are still pending materialization or adjudication.",
        },
        "success_criteria": [
            "Strict constrained accuracy must exceed 22/24 with zero new regressions to remain promotable-interesting.",
            "Python verifier residual should flip or show materially improved margin.",
            "Any Rust improvement must still be backed later by a fresh non-tokenizers residual-root artifact.",
        ],
        "promotion_gate_requirements": gate["required_for_future_promotion"],
        "required_honesty_gates": execution_path["forbidden_shortcuts"],
        "outputs": {
