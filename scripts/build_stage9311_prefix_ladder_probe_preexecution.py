...
STAGE = 9311
NAME = "stage9311_prefix_ladder_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9310_prefix_ladder_supported_suffix_manifest.json"
...
MANIFEST_AUDIT = ROOT / "runs/local/artifacts/stage9310_prefix_ladder_supported_suffix_manifest/prefix_ladder_supported_suffix_manifest_audit.json"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9312_prefix_ladder_supported_suffix_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "prefix_ladder_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9312_prefix_ladder_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PREFIX_LADDER_PROBE_PREEXECUTION_STAGE9311.md"
...
COMMAND = [
    ...
    "--max-train-rows", "23", "--max-eval-rows", "5", "--max-strict-rows", "4",
    "--max-steps", "64", "--batch-size", "2", "--learning-rate", "1e-5",
    ...
    "--enable-generation-audit", "--max-generation-rows", "32", "--max-generation-tokens", "64",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9312_prefix_ladder_supported_suffix_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    manifest_audit = load_json(MANIFEST_AUDIT)
    ...
    for row in rows:
        ...
        prefix_words = str(model_input.get("active_generation_prefix_words"))
        prefix_counts[prefix_words] = prefix_counts.get(prefix_words, 0) + 1
        ...
        if any(bool(authority.get(key)) for key in authority):
            unsafe_rows.append(str(row.get("row_id")))

    command_text = " ".join(COMMAND)
    failures: list[str] = []
    if source.get("passed") is not True:
        failures.append("source_stage9310_not_passed")
    if manifest_audit.get("passed") is not True:
        failures.append("manifest_audit_not_passed")
    if len(rows) != 32:
        failures.append("manifest_row_count_not_32")
    if split_counts != {"train": 23, "eval": 5, "strict_eval": 4}:
    ...
    if COMMAND[:4] != ["env", f"TMPDIR={TMPDIR}", f"TEMP={TMPDIR}", f"TMP={TMPDIR}"]:
        failures.append("tmpdir_not_redirected_to_data_tmp")
    required_pairs = {
        "--mode": "denoise_repair_probe", "--probe-scale": "target_100m", "--max-train-rows": "23",
        "--max-eval-rows": "5", "--max-strict-rows": "4", "--max-steps": "64",
        "--max-generation-rows": "32", "--max-generation-tokens": "64", "--max-decoder-tokens": "96",
        "--decoder-ce-weight": "0.0", "--structured-aux-weight": "0.0", "--denoise-weight": "1.0",
    ...
    return {
        ...
        "manifest_audit_sha256": sha256(MANIFEST_AUDIT),
        "command": COMMAND,
        "command_ready": not failures,
        "will_execute_now": False,
        "authorized_next_stage": 9312,
        "tmpdir": str(TMPDIR),
        "required_postrun_artifacts": ["sample_generation_audit.json", "boundary_next_token_logits.jsonl", "row_token_loss.jsonl", "module_delta_norms.json", "cleanup_proof.json"],
...
def main() -> None:
    ...
    summary = {
        ...
        "metrics": {**{key: bool(card["authority"].get(key, False)) for key in AUTHORITY_CLOSED}, "command_ready": card["command_ready"], "will_execute_now": False, "authorized_next_stage": 9312, "manifest_rows": card["manifest_rows"], "split_counts": card["split_counts"], "prefix_counts": card["prefix_counts"], "tmpdir": card["tmpdir"], "required_postrun_artifacts": card["required_postrun_artifacts"], "failures": len(card["failures"])},
        "artifacts": {"audit": str(AUDIT.relative_to(ROOT)), "command": str(COMMAND_JSON.relative_to(ROOT)), "doc": str(DOC.relative_to(ROOT))},
        "decision": "Prepared a target-100M denoise probe over the Stage9310 prefix-ladder suffix curriculum; decoder CE and all external authority remain closed.",
        "next_best_step": "Execute Stage9312 prefix-ladder supported-suffix probe under trellis, then audit generation by split, prefix length, and variant type.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9311 Prefix-Ladder Probe Preexecution",
        "",
        f"Passed: `{card['passed']}`",
        f"Rows: `{card['manifest_rows']}`",
        f"Splits: `{card['split_counts']}`",
        f"Prefix counts: `{card['prefix_counts']}`",
        "This authorizes only the next tiny target-100M denoise probe. Decoder CE, runtime, Gemma, harness, source/body emission, scoring, and promotion remain closed.",
        "",
    ]), encoding="utf-8")
