...
STAGE = 9338
NAME = "stage9338_merged_repair_interference_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9337_routed_ladder_with_targeted_repair_manifest.json"
...
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9339_merged_repair_interference_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "targeted_repair_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9339_merged_repair_interference_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "MERGED_REPAIR_INTERFERENCE_PROBE_PREEXECUTION_STAGE9338.md"
...
COMMAND = [
    ...
    "--max-train-rows", "39", "--max-eval-rows", "22", "--max-strict-rows", "16",
    ...
    "--enable-generation-audit", "--max-generation-rows", "77", "--max-generation-tokens", "64",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9339_merged_repair_interference_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9337_not_passed")
    if len(rows) != 77:
    ...
    required_pairs = {
        "--mode": "denoise_repair_probe", "--probe-scale": "target_100m", "--max-train-rows": "39",
        "--max-eval-rows": "22", "--max-strict-rows": "16", "--max-steps": "80",
        "--max-generation-rows": "77", "--decoder-ce-weight": "0.0", "--structured-aux-weight": "0.0",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared the 77-row merged routed-ladder plus targeted-repair target-100M denoise interference probe; decoder CE remains closed.",
        "next_best_step": "Execute Stage9339 merged repair interference probe under trellis and audit whether targeted repair rows stabilize the full routed ladder.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9338 Merged Repair Interference Probe Preexecution",
        "",
        f"Passed: `{card['passed']}`",
        f"Rows: `{card['manifest_rows']}`",
        f"Splits: `{card['split_counts']}`",
        f"Tasks: `{card['task_counts']}`",
        "Only the next 77-row target-100M denoise interference probe is authorized. Decoder CE and external authority remain closed.",
        "",
    ]), encoding="utf-8")
