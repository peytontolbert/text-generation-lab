#!/usr/bin/env python3
"""Roll Stage12031 build+run rows into transition support v23."""
...
ROOT = Path("runs/local/artifacts/stage12032_transition_support_rollup_v23")
BASE = Path("runs/local/artifacts/stage12030_transition_support_rollup_v22/transition_support_rows_v22.jsonl")
ADDED = Path("runs/local/artifacts/stage12031_buildrun_pair_expansion_rows/buildrun_pair_expansion_rows.jsonl")
ROWS_OUT = ROOT / "transition_support_rows_v23.jsonl"
SUMMARY = ROOT / "transition_support_rollup_v23.json"
SUMMARY_MIRROR = Path("runs/summaries/stage12032_transition_support_rollup_v23.json")
...
def main() -> None:
    ...
    duplicates = 0
    for source, path in [("stage12030", BASE), ("stage12031", ADDED)]:
        for row in read_jsonl(path):
            out = dict(row)
            out["stage12032_source"] = source
            out["split"] = "train"
            out["split_role"] = "stage12032_transition_support_v23_train_support_only"
    ...
    summary = {
        "stage": "stage12032_transition_support_rollup_v23",
        ...
        "decision": "support_inventory_v23_not_train_ready" if not train_ready else "support_inventory_v23_floor_ready_requires_anti_cheat_audit",
        "reason": "Build+run coverage improved, but Transition-Root-250 floors remain unmet." if not train_ready else "Floor counts are met; run anti-cheat and split audits before training.",
        "claim_boundary": "Support inventory only. No model promotion or Gemma comparison should be claimed from v23.",
        "next_stage_recommendation": {"stage": "stage12033_transition_root_250_floor_closure_plan", "action": "Prioritize PASS_TO_PASS and FAIL_TO_PASS floor closure, then run anti-cheat/readiness audit before a guarded training request."},
    }
