...
STAGE = 9356
NAME = "stage9356_non_operator_residual_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9355_non_operator_residual_repair_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9355_non_operator_residual_repair_manifest/non_operator_residual_repair_manifest.jsonl"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9357_non_operator_residual_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "non_operator_residual_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9357_non_operator_residual_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "NON_OPERATOR_RESIDUAL_PROBE_PREEXECUTION_STAGE9356.md"
...
COMMAND = [
    ...
    "--max-train-rows", "17", "--max-eval-rows", "8", "--max-strict-rows", "7",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9357_non_operator_residual_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9355_not_passed")
    if len(rows) != 32:
    ...
    if set(task_counts) != {"dependency_inside_terminal_repair", "numeric_preserves_omission_repair", "file_path_localized_edit_repair"}:
        failures.append("unexpected_task_set")
    if unsafe:
    ...
    required_pairs = {
        "--mode": "denoise_repair_probe", "--probe-scale": "target_100m", "--max-train-rows": "17",
        "--max-eval-rows": "8", "--max-strict-rows": "7", "--max-steps": "80", "--max-generation-rows": "32",
    ...
    return {"passed": not failures, "failures": failures, "manifest_rows": len(rows), "split_counts": dict(sorted(split_counts.items())), "task_counts": dict(sorted(task_counts.items())), "unsafe_rows": sorted(set(unsafe)), "command": COMMAND, "command_ready": not failures, "authority": AUTHORITY_RUN if not failures else dict(AUTHORITY_CLOSED)}
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared a closed-boundary 32-row non-operator residual denoise probe; decoder CE remains closed.",
        "next_best_step": "Execute Stage9357 non-operator residual probe under trellis and audit exactness before another full rejoin.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9356 Non-Operator Residual Probe Preexecution", "", f"Passed: `{card['passed']}`", f"Rows: `{card['manifest_rows']}`", f"Splits: `{card['split_counts']}`", f"Tasks: `{card['task_counts']}`", "", "Only the Stage9357 32-row target-100M denoise probe is authorized. Decoder CE and external authority remain closed.", ""]), encoding="utf-8")
