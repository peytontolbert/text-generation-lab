...
try:
    ...
    from scripts.build_stage9096_trainer_runtime_assertion_inventory import (
        ASSERTION_FAMILIES,
    ...
    from build_stage9096_trainer_runtime_assertion_inventory import (  # type: ignore
        ASSERTION_FAMILIES,
        REQUIRED_TELEMETRY_ARTIFACTS,
        build_inventory,
        validate_inventory,
...
STAGE = 9097
NAME = "stage9097_trainer_runtime_assertion_inventory_audit"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
SOURCE_9096 = ROOT / "runs/summaries/stage9096_trainer_runtime_assertion_inventory.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "TRAINER_RUNTIME_ASSERTION_INVENTORY_AUDIT_STAGE9097.md"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "trainer_runtime_assertion_inventory_audit.json"
...
def registry(latest: int = 9095) -> dict[str, Any]:
    return {"metrics": {"latest_stage": latest, "authority_counts": {key: 0 for key in AUTHORITY_CLOSED}}}


def validate_audit_candidate(candidate: dict[str, Any], registry_json: dict[str, Any]) -> list[str]:
    failures = validate_inventory(candidate, registry_json)
    for family, terms in ASSERTION_FAMILIES.items():
        if family not in candidate.get("assertion_families", {}):
            failures.append(f"missing_assertion_family:{family}")
            continue
        for term in terms:
            if term not in candidate["assertion_families"].get(family, []):
                failures.append(f"missing_assertion_term:{family}:{term}")
    for artifact in REQUIRED_TELEMETRY_ARTIFACTS:
        if artifact not in candidate.get("required_telemetry_artifacts", []):
            failures.append(f"missing_telemetry_artifact:{artifact}")
    return failures


def run_negative_cases() -> dict[str, Any]:
    base = build_inventory(registry())
    cases: dict[str, dict[str, Any]] = {}

    missing_family = copy.deepcopy(base)
    missing_family["assertion_families"].pop("loss_mask_enforcement")
    cases["missing_loss_mask_family"] = missing_family

    missing_term = copy.deepcopy(base)
    missing_term["assertion_families"]["execution_gates"].remove("cannot execute recovery probe because contract did not pass")
    cases["missing_execution_gate_term"] = missing_term

    missing_telemetry = copy.deepcopy(base)
    missing_telemetry["required_telemetry_artifacts"].remove("row_gradient_norms.jsonl")
    cases["missing_row_gradient_norms_artifact"] = missing_telemetry

    runtime_executed = copy.deepcopy(base)
    runtime_executed["metrics"]["runtime_assertions_executed_now"] = True
    cases["runtime_assertions_executed_now"] = runtime_executed
...
def build_audit() -> dict[str, Any]:
    source = load_json(SOURCE_9096)
    base = build_inventory(registry())
    base_failures = validate_audit_candidate(base, registry())
    negatives = run_negative_cases()
    checks = {
        "source_stage9096_present": SOURCE_9096.exists(),
        "source_stage9096_passed": source.get("passed") is True,
        "base_inventory_passes": base_failures == [],
        "assertion_families_present": set(ASSERTION_FAMILIES).issubset(set(base["assertion_families"])),
        "telemetry_artifacts_present": set(REQUIRED_TELEMETRY_ARTIFACTS).issubset(set(base["required_telemetry_artifacts"])),
        "negative_cases_rejected": all(item["rejected"] for item in negatives.values()),
        "runtime_assertions_not_executed": base["metrics"]["runtime_assertions_executed_now"] is False,
    ...
    return {
        ...
        "decision": "Trainer runtime assertion inventory rejects missing assertion families, missing assertion terms, missing telemetry artifact declarations, runtime assertion execution, trainer invocation, model rows, decoder CE, authority, and training.",
    }
...
def main() -> None:
    ...
    audit = build_audit()
    if int((registry_json.get("metrics") or {}).get("latest_stage", -1)) not in {9096, STAGE}:
    ...
    summary = {
        ...
        "decision": audit["decision"] if audit["passed"] else "Trainer runtime assertion inventory audit failed.",
        "next_best_step": "Attach trainer runtime assertion controls to the central graph; do not execute trainer or load rows.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9097 Trainer Runtime Assertion Inventory Audit",
        "",
        f"Passed: `{summary['passed']}`",
        "",
        "Audits the Stage9096 runtime assertion inventory with negative cases. The trainer is not invoked.",
        "",
