...
STAGE = 9175
NAME = "stage9175_trainer_setup_emit_instance_design"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
SOURCE_9174 = ROOT / "runs/summaries/stage9174_trainer_setup_emit_only_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "TRAINER_SETUP_EMIT_INSTANCE_DESIGN_STAGE9175.md"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
DESIGN = OUT_DIR / "trainer_setup_emit_instance_design.json"

REQUIRED_INSTANCE_REFS = [
    "source_stage9174_audit_ref",
    "emit_only_design_ref",
    "objective_rows_ref",
    "judge_rows_ref",
    "ranker_rows_ref",
...
REQUIRED_INSTANCE_GUARDS = [
    "emit_instance_requires_stage9174_passed",
    "emit_instance_requires_repo_local_output_dir",
    "emit_instance_requires_exact_output_cardinality",
    "emit_instance_requires_provenance_join_audit",
    "emit_instance_requires_post_emit_audit_card",
    "emit_instance_forbids_trainer_invocation",
    "emit_instance_forbids_model_forward",
    "emit_instance_forbids_optimizer_or_backward",
    "emit_instance_forbids_arxiv_reads",
    "emit_instance_forbids_source_body_reads",
]
...
def registry(latest: int = 9174) -> dict[str, Any]:
...
def build_design(registry_card: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    source = load_json(SOURCE_9174)
    return {
        "stage": STAGE,
        "stage_name": NAME,
        "design_type": "trainer_setup_emit_instance_v1",
        "source_stage9174_passed": source.get("passed") is True,
        "registry_frontier_stage9174": int((registry_card.get("metrics") or {}).get("latest_stage", -1)) == 9174,
        ...
        "decision": (
            "Designed the emit-instance stage. The instance may define the exact emitted trainer-setup "
            "artifact set and post-emit audit obligations, but it may not emit outputs or invoke trainer "
            "execution in this stage."
        ),
        "next_best_step": "Audit the emit-instance design; still do not emit outputs.",
    }
...
def validate_design(design: dict[str, Any]) -> list[str]:
    ...
    if design.get("source_stage9174_passed") is not True or metrics.get("source_stage9174_passed") is not True:
        failures.append("source_stage9174_not_passed")
    if design.get("registry_frontier_stage9174") is not True or metrics.get("registry_frontier_stage9174") is not True:
        failures.append("registry_frontier_stage9174")
...
def run_negative_cases() -> dict[str, Any]:
    ...
    for case in NEGATIVE_CASES:
        ...
        if case == "source_stage_missing":
            design["source_stage9174_passed"] = False
            design["metrics"]["source_stage9174_passed"] = False
        elif case == "registry_frontier_bad":
            design["registry_frontier_stage9174"] = False
            design["metrics"]["registry_frontier_stage9174"] = False
            ...
            design["required_instance_guards"].remove("emit_instance_forbids_trainer_invocation")
        ...
        elif case == "emit_instance_open_now":
            design["metrics"]["emit_instance_open_now"] = True
...
def main() -> None:
    ...
    artifact = {
        ...
        "decision": design["decision"] if passed else "Emit-instance design failed validation.",
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage9175 Trainer Setup Emit-Instance Design",
                "",
                f"Passed: `{summary['passed']}`",
                "",
                "Designs the exact emit-instance without emitting outputs.",
                "",
