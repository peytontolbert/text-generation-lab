...
STAGE = 10195
NAME = "stage10195_compact_bounded_harness_payload_from_saved_stage10189_runtime"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
PAYLOAD = OUT_DIR / "compact_bounded_harness_payload_from_saved_stage10189_runtime.json"
...
RUNTIME_DIR = ROOT / "runs/local/artifacts/stage10189_saved_choice_aux_encoder_option_retrieval_target100m_probe/runtime_model"
...
def build_payload() -> dict[str, Any]:
    ...
    runtime_bundle = load_json(RUNTIME_DIR / "runtime_model_bundle.json")
    weights_path = Path(str(runtime_bundle.get("weights_path") or ""))
        ...
        runs.append(
            {
                "cell_key": cell_key,
                "task_pack": {
                    "bundle_id": f"stage10189_saved_runtime_compact_bounded::{language}",
                    "task_pack_id": f"stage10189_saved_runtime_compact_bounded::{language}",
                    "source_id": f"stage10189_saved_runtime_compact_bounded::{language}",
                    "lineage_hash": f"stage10189_saved_runtime_compact_bounded::{language}",
                    ...
                    "slice_tags": ["maintainer_bundle", language, "compact_bounded", "saved_100m_runtime", "diagnostic_harness"],
                    "thresholds": {"must_compare_100m_and_gemma": True, "projection_only": False, "live_saved_runtime": True},
                    "blocked_training_reason": "saved_runtime_harness_diagnostic_only",
                    "rows": rows,
                ...
                "hundred_m_backend": {
                    ...
                    "model_bundle_manifest": str(MODEL_CONFIG),
    ...
    return {
        ...
        "passed": bool(runs) and weights_path.exists() and MODEL_CONFIG.exists(),
        "source_package": display(PACKAGE),
        "saved_runtime_bundle": display(RUNTIME_DIR / "runtime_model_bundle.json"),
        "claim_scope": "diagnostic compact bounded maintainer comparison using saved live stage10189 100M runtime bundle versus live gemma3:12b same-row outputs",
        "eval_hardening": {
            "primary_maintainer_leaderboard_allowed": False,
            "diagnostic_harness": True,
            "same_task_pack_as_gemma": True,
            "saved_live_hundred_m_backend": True,
        },
        "metrics": {
            "runs": len(runs),
        ...
        "runtime_requirements": {
            ...
            "model_config": display(MODEL_CONFIG),
            "tokenizer_json": display(TOKENIZER_JSON),
            "tokenizer_config": display(TOKENIZER_CONFIG),
        },
        "runs": runs,
