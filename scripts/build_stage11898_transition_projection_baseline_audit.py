...
STAGE = 11898
NAME = "stage11898_transition_projection_baseline_audit"
OUT = ART / NAME
SUMMARY = OUT / "transition_projection_baseline_audit.json"
...
RUNTIMES = {
    ...
    "stage11893_nonpromotable_protected_replay": ART / "stage11893_rendered_support_protected_replay_probe/runtime_model/runtime_model_bundle.json",
}
...
def main() -> None:
    ...
    replay_all = results["stage11893_nonpromotable_protected_replay"]["all_transition_projection_rows"]
    decision = (
        "transition_projection_baseline_replay_improves_fit"
        if (replay_all.get("correct") or 0) > (selected_all.get("correct") or 0)
        else "transition_projection_baseline_no_replay_fit_gain"
    )
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        ...
        "claim_boundary": [
            ...
            "Stage11893 is non-promotable because it trained on protected replay; it is included only to test whether projection rows reflect the replay mechanism.",
            "Promotion still requires disjoint analogue replay or heldout-safe transition supervision.",
        ],
    }
        ...
        json.dumps(
            {
                ...
                "replay_all": {"correct": replay_all.get("correct"), "rows": replay_all.get("rows")},
            },
            indent=2,
            sort_keys=True,
        )
