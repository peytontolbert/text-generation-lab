...
STAGE = 9173
NAME = "stage9173_trainer_setup_emit_only_design"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
SOURCE_9172 = ROOT / "runs/summaries/stage9172_trainer_setup_materialization_instance_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "TRAINER_SETUP_EMIT_ONLY_DESIGN_STAGE9173.md"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
DESIGN = OUT_DIR / "trainer_setup_emit_only_design.json"

REQUIRED_EMIT_INPUTS = [
    "source_stage9172_audit_ref",
    "materialization_instance_ref",
    "objective_rows_ref",
    "judge_rows_ref",
    "ranker_rows_ref",
...
REQUIRED_EMIT_GUARDS = [
    "emit_only_requires_stage9172_passed",
    "emit_only_requires_repo_local_output_dir",
    "emit_only_requires_output_cardinality_audit",
    "emit_only_requires_output_provenance_join_audit",
    "emit_only_forbids_trainer_invocation",
    "emit_only_forbids_model_forward",
    "emit_only_forbids_optimizer_or_backward",
    "emit_only_forbids_arxiv_reads",
    "emit_only_forbids_source_body_reads",
    "emit_only_forbids_execution_authority",
]
...
def registry(latest: int = 9172) -> dict[str, Any]:
...
def build_design(registry_card: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    source = load_json(SOURCE_9172)
    return {
        "stage": STAGE,
        "stage_name": NAME,
        "design_type": "trainer_setup_emit_only_v1",
        "source_stage9172_passed": source.get("passed") is True,
        "registry_frontier_stage9172": int((registry_card.get("metrics") or {}).get("latest_stage", -1)) == 9172,
        "required_emit_inputs": list(REQUIRED_EMIT_INPUTS),
        "required_emit_guards": list(REQUIRED_EMIT_GUARDS),
        "emittable_outputs": list(EMITTABLE_OUTPUTS),
        ...
        "metrics": {
            ...
            "source_stage9172_passed": source.get("passed") is True,
            "registry_frontier_stage9172": int((registry_card.get("metrics") or {}).get("latest_stage", -1)) == 9172,
            "required_emit_inputs": len(REQUIRED_EMIT_INPUTS),
            "required_emit_guards": len(REQUIRED_EMIT_GUARDS),
            "emittable_outputs": len(EMITTABLE_OUTPUTS),
            "still_blocked_outputs": len(STILL_BLOCKED_OUTPUTS),
            "emit_authorized_now": False,
        ...
        "decision": (
            "Designed the emit-only trainer-setup stage. If later instantiated and audited, it may "
            "emit route/loss/trainer-input artifacts only, while trainer invocation and all model "
            "execution remain closed."
        ),
        "next_best_step": "Audit the emit-only design; still do not emit outputs.",
    }
...
def validate_design(design: dict[str, Any]) -> list[str]:
    ...
    if design.get("source_stage9172_passed") is not True or metrics.get("source_stage9172_passed") is not True:
        failures.append("source_stage9172_not_passed")
    if design.get("registry_frontier_stage9172") is not True or metrics.get("registry_frontier_stage9172") is not True:
        failures.append("registry_frontier_stage9172")
    for item in REQUIRED_EMIT_INPUTS:
        if item not in design.get("required_emit_inputs", []):
            failures.append(f"missing_emit_input:{item}")
    for item in REQUIRED_EMIT_GUARDS:
        if item not in design.get("required_emit_guards", []):
            failures.append(f"missing_emit_guard:{item}")
    for item in EMITTABLE_OUTPUTS:
        if item not in design.get("emittable_outputs", []):
...
def run_negative_cases() -> dict[str, Any]:
    ...
    for case in NEGATIVE_CASES:
        ...
        if case == "source_stage_missing":
            design["source_stage9172_passed"] = False
            design["metrics"]["source_stage9172_passed"] = False
        elif case == "registry_frontier_bad":
            design["registry_frontier_stage9172"] = False
            design["metrics"]["registry_frontier_stage9172"] = False
        elif case == "missing_emit_input":
            design["required_emit_inputs"].remove("repo_local_output_dir_ref")
        elif case == "missing_emit_guard":
            design["required_emit_guards"].remove("emit_only_forbids_trainer_invocation")
        elif case == "missing_emittable_output":
            design["emittable_outputs"].remove("trainer_rows.jsonl")
        elif case == "missing_blocked_output":
            design["still_blocked_outputs"].remove("model_input_rows.jsonl")
        elif case == "emit_authorized_now":
            design["metrics"]["emit_authorized_now"] = True
...
def main() -> None:
    ...
    artifact = {
        ...
        "decision": design["decision"] if passed else "Emit-only design failed validation.",
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage9173 Trainer Setup Emit-Only Design",
                "",
                f"Passed: `{summary['passed']}`",
                "",
                "Designs an emit-only stage without trainer execution.",
                "",
                f"Emittable outputs: `{artifact['metrics']['emittable_outputs']}`",
