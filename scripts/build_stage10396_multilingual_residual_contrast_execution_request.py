...
STAGE = 10396
NAME = "stage10396_multilingual_residual_contrast_execution_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
MANIFEST_JSONL = OUT_DIR / "multilingual_residual_contrast_manifest.jsonl"
REQUEST_JSON = OUT_DIR / "multilingual_residual_contrast_execution_request.json"
COMMAND_JSON = OUT_DIR / "multilingual_residual_contrast_command.json"
...
SUPPORT_JSONL = ROOT / "runs/local/artifacts/stage10247_weakness_counterbalance_package/agentkernel_lite_encdec_train.jsonl"
...
RUN_ID = "stage10397_multilingual_residual_contrast_probe"
OUTPUT_DIR = "runs/local/artifacts/stage10397_multilingual_residual_contrast_probe/bounded_decoder_probe"
RUNTIME_MODEL_DIR = "runs/local/artifacts/stage10397_multilingual_residual_contrast_probe/runtime_model"
...
SCRIPTS_UNIVERSE_VERIFIER_SUPPORT = (
    "stage10119::localsess_repository_library_sessseed_codex_sessions_rollout_2025_11_26t00_55_44_019abda8_c0d8_7803_8a77_1e0f_"
    "scripts_universe_build_py_64d2cf209b_aug_1500000_8b46e7f662::python::verifier_outcome::compact_bounded"
)
...
def normalize_train_row(row: dict[str, Any], *, support_family: str, tags: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    out["disable_losses"] = []
    out["split"] = "train"
    out["row_id"] = f"stage10396::{support_family}::{out.get('row_id')}"
    out["semantic_key"] = f"stage10396::{support_family}::{out.get('semantic_key') or out.get('row_id')}"
    anti = dict(out.get("anti_cheat") or {})
    anti.update(
        {
            "stage10396_support_only": True,
            "multilingual_residual_contrast_probe": True,
    ...
    source["stage10396_support_family"] = support_family
    if tags:
        source["stage10396_tags"] = dict(tags)
...
def clone_train_from_strict(
    ...
    *,
    ...
    clone["row_id"] = f"stage10396::{support_family}::{base_row['row_id']}::perm_{perm_idx:02d}"
    clone["semantic_key"] = f"stage10396::{support_family}::{base_row.get('row_id')}::perm_{perm_idx:02d}"
    clone["split"] = "train"
    ...
    clone["query_text"] = f"{base_row.get('query_text','stage10396')}::stage10396::{support_family}::{perm_idx:02d}"
    clone["opaque_options"] = rotated
    projection = dict(clone.get("standalone_projection_source") or {})
    projection["opaque_options"] = rotated
    projection["stage10396_support_family"] = support_family
...
def support_rows() -> list[dict[str, Any]]:
    ...
    for row in load_jsonl(SUPPORT_JSONL):
        ...
        if row_id.startswith(PYTHON_EVIDENCE_PREFIX.replace("::full_visible_compact_bounded", "::compact_bounded")):
            rows.append(
                normalize_train_row(
                    row,
                    support_family="python_evidence_compact_support",
                    tags={"recovery_target": "python_verifier_and_test_constraint"},
                )
            )
        elif row_id == SCRIPTS_UNIVERSE_VERIFIER_SUPPORT:
            rows.append(
                normalize_train_row(
                    row,
                    support_family="python_verifier_compact_support",
                    tags={"verifier_anchor": True},
                )
            )
    ...
    clone_specs = [
        (
            PYTHON_EVIDENCE_PREFIX,
            "python_evidence_full_visible_recovery",
            {"recovery_target": "python_verifier_and_test_constraint"},
        ),
        (
            CPP_EVIDENCE_ROW,
            "c_cpp_evidence_full_visible_recovery",
            {"recovery_target": "cpp_verifier_and_test_constraint"},
        ),
        (
            PYTHON_MIRRORMIND_EVIDENCE_ROW,
        ...
        (
            PYTHON_MIRRORMIND_VERIFIER_ROW,
            "python_mirrormind_verifier_recovery",
...
def main() -> None:
    ...
    for row in train_rows:
        family = str(((row.get("standalone_projection_source") or {}).get("stage10396_support_family")) or "unknown")
    ...
    request = {
        ...
        "passed": True,
        "decision": "multilingual_residual_contrast_probe_ready",
        ...
        "claim_scope": "non-promotable multilingual residual contrast probe; success is measured only by rescoring the saved runtime against the unchanged 47-row frontier with the stable language-conditioned scorer",
        "required_honesty_gates": [
            "strict eval remains the unchanged 47-row honest frontier",
            "same-surface recovery and preservation clones are diagnostic only and cannot support promotion claims",
            "preservation KL stays tied to the stage10379 runtime to reduce cross-language citation drift",
            "promotion is disallowed until any recovered movement is rebuilt from disjoint roots or scorer-side proof",
        ],
        "target_hypothesis": "explicit preservation anchors for candidate_change_surface rows can keep web and Python citation geometry stable while targeted recovery rows move the two verifier_and_test_constraint misses and the Python verifier file-choice miss",
        "next_best_step": "run one preservation-heavy multilingual contrast probe, then rescore the saved runtime with the stable stage10385 language-conditioned evaluator",
        "command": command,
