...
STAGE = 10598
NAME = "stage10598_mixed_contract_non_a_diagnostic_probe_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
REQUEST_JSON = OUT_DIR / "mixed_contract_non_a_diagnostic_probe_request.json"
COMMAND_JSON = OUT_DIR / "mixed_contract_non_a_diagnostic_probe_command.json"
MANIFEST_JSONL = OUT_DIR / "mixed_contract_non_a_diagnostic_probe_manifest.jsonl"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"

SUPPORT_PACKAGE = ROOT / "runs/local/artifacts/stage10597_mixed_contract_non_a_diagnostic_support_package/mixed_contract_non_a_diagnostic_support_package.json"
SUPPORT_ROWS = ROOT / "runs/local/artifacts/stage10597_mixed_contract_non_a_diagnostic_support_package/train_rows.jsonl"
STRICT_ROWS = ROOT / "runs/local/artifacts/stage10597_mixed_contract_non_a_diagnostic_support_package/strict_eval_rows.jsonl"
...
RUN_ID = "stage10599_mixed_contract_non_a_diagnostic_probe"
OUTPUT_DIR = "runs/local/artifacts/stage10599_mixed_contract_non_a_diagnostic_probe/bounded_decoder_probe"
RUNTIME_MODEL_DIR = "runs/local/artifacts/stage10599_mixed_contract_non_a_diagnostic_probe/runtime_model"
...
def main() -> None:
    ...
    request = {
        ...
        "decision": "mixed_contract_non_a_diagnostic_probe_ready",
        ...
        "claim_scope": [
            "Diagnostic target-100M probe on the stage10597 mixed-contract non-A support package.",
            "Starts from the current stage10586 fresh-transfer runtime so any gain measures whether missing B/D support caused the stage10594 mixed-contract collapse.",
            "This run is intentionally non-promotable because it trains on same-surface strict roots; its only purpose is to test learnability of the mixed B/D support patterns.",
        ],
        "motivation": {
        ...
        "training_deltas": {
            "train_support_manifest": "stage10597_mixed_contract_non_a_diagnostic_support_package",
        ...
        "required_honesty_gates": [
            "stage10591 support rows remain train-only and repo-family-disjoint from the stage10591 strict roots",
            ...
            "this run must be reported as diagnostic-only and cannot be compared to Gemma as a promotion candidate",
            "no claim of Rust/Web uplift can be made from this package because stage10597 only covers Python and C/C++",
        ],
        "post_run_required_artifacts": [
            "bounded_decoder_probe/execution_result.json",
            "post-run repaired v2.7 canary audit against stage10599 runtime",
            "post-run rebuilt 54-row same-manifest comparison against stage10599 runtime",
            "post-run fresh strict same-manifest comparison versus Gemma on stage10591 strict rows",
        ],
        "known_limits": [
            ...
            "This package adds same-surface B/D support and is meant only to test whether the stage10594 collapse was caused by missing non-A supervision.",
        ],
        "next_best_step": "Launch the stage10599 diagnostic probe from this request, then compare it to stage10594 on the same strict package and keep the canary as a regression gate.",
        "command": command,
    }
