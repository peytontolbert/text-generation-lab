...
STAGE = 9325
NAME = "stage9325_phrase_b_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9322_phrase_family_split_manifests.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9322_phrase_family_split_manifests/phrase_b_patch_operator_manifest.jsonl"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9326_phrase_b_patch_operator_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "phrase_b_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9326_phrase_b_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PHRASE_B_PROBE_PREEXECUTION_STAGE9325.md"
...
COMMAND = [
    ...
    "--max-train-rows", "3", "--max-eval-rows", "2", "--max-strict-rows", "3",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9326_phrase_b_patch_operator_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if target.get("phrase_id") != "phrase_b" or loss_mask.get("decoder_ce") or loss_mask.get("structured_aux") or not loss_mask.get("denoise_ce"):
            unsafe_rows.append(str(row.get("row_id")))
        if any(bool(authority.get(key)) for key in authority):
            unsafe_rows.append(str(row.get("row_id")))
    failures: list[str] = []
    if source.get("passed") is not True:
        failures.append("source_stage9322_not_passed")
    if len(rows) != 8:
        failures.append("manifest_row_count_not_8")
    if split_counts != {"train": 3, "eval": 2, "strict_eval": 3}:
    ...
    required_pairs = {
        "--mode": "denoise_repair_probe", "--probe-scale": "target_100m", "--max-train-rows": "3",
        "--max-eval-rows": "2", "--max-strict-rows": "3", "--max-steps": "80",
        "--max-generation-rows": "8", "--decoder-ce-weight": "0.0", "--structured-aux-weight": "0.0",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared an isolated phrase-B target-100M denoise probe for the patch-operator failure family.",
        "next_best_step": "Execute Stage9326 phrase-B probe under trellis and audit whether patch-operator is family-local learnable.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9325 Phrase B Probe Preexecution",
        "",
