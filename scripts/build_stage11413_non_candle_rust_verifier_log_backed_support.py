...
STAGE = 11413
NAME = "stage11413_non_candle_rust_verifier_log_backed_support"
OUT = ART / NAME
SUMMARY = OUT / "non_candle_rust_verifier_log_backed_support.json"
ROWS = OUT / "non_candle_rust_verifier_log_backed_support_rows.jsonl"
AUDIT = OUT / "non_candle_rust_verifier_log_backed_support_audit.json"

CAPTURE = ART / "stage11412_non_candle_rust_verifier_log_capture/non_candle_rust_verifier_log_capture.json"
OPTIONS = [
...
def row_for(result: dict[str, Any], kind: str, target: str, path: str, excerpt: str) -> dict[str, Any]:
    ...
    item = (
        ...
        f"Source path: {path}\n"
        "Evidence note: materialized non-Candle Rust verifier-log-backed evidence excerpt.\n"
        f"Excerpt:\n{excerpt.strip()}"
    )
    prompt = (
        "Language: rust\n"
    ...
    return {
        "row_id": f"stage11413::{root_id}::{kind}",
        "root_id": root_id,
        ...
        "surface": "maintainer_non_candle_rust_verifier_log_backed_evidence_candidate_judgment_bounded_choice",
        "split": "train",
        "package_split": "train",
...
def main() -> None:
    ...
    for result in capture.get("results", []):
        ...
        log_excerpt = safe_read(str(ROOT / result["log_path"]), limit=1600)
    ...
    summary = {
        ...
        "created_at_utc": now(),
        "passed": True,
        "decision": "non_candle_rust_verifier_log_backed_support_materialized_not_probe_ready",
        "counts": {
        ...
        "recommended_next_action": "Combine with Candle partial support for inventory only; acquire a third Rust repo family and at least four more roots before probing.",
        "source_artifacts": {"stage11412_capture": rel(CAPTURE)},
