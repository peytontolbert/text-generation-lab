...
STAGE = 9807
NAME = "stage9807_web_disambiguated_opaque_choice_surface"
...
MANIFEST = OUT_DIR / "web_disambiguated_opaque_choice_surface.jsonl"
AUDIT = OUT_DIR / "web_disambiguated_opaque_choice_surface_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "WEB_DISAMBIGUATED_OPAQUE_CHOICE_SURFACE_STAGE9807.md"
...
def _load_row_text():
    ...
    spec = importlib.util.spec_from_file_location("stage9807_training_data", path)
    module = importlib.util.module_from_spec(spec)
    assert spec and spec.loader
    sys.modules["stage9807_training_data"] = module
...
def _inject_web_disambiguator(row: dict[str, Any]) -> None:
    ...
    if detail:
        state["visible_locality_evidence"] = f"{state.get('visible_locality_evidence') or ''} {detail}".strip()
        state["web_surface_disambiguator"] = detail
    bits = corrupted.get("neutral_context_bits") if isinstance(corrupted.get("neutral_context_bits"), dict) else {}
    state["context_config_visible"] = bool(bits.get("config_visible"))
    state["context_entrypoint_visible"] = bool(bits.get("entrypoint_visible"))
    state["context_symbol_names_visible"] = bool(bits.get("symbol_names_visible"))
    state["context_tests_visible"] = bool(bits.get("tests_visible"))
...
def transform_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    new_row["surface"] = "edit_localization_opaque_choice_surface_v2_web_disambiguated"
...
def source_rows() -> list[dict[str, Any]]:
    ...
    if rows and isinstance(rows[0], dict) and rows[0].get("row_id"):
        return rows
    lift_rows = _load_symbol("stage9807_stage9771_builder", ROOT / "scripts/build_stage9771_edit_localization_visible_evidence_lift_package.py", "lift_rows")
    source_loader = _load_symbol("stage9807_stage9771_source_loader", ROOT / "scripts/build_stage9771_edit_localization_visible_evidence_lift_package.py", "load_jsonl")
    source_path = _load_symbol("stage9807_stage9771_source_path", ROOT / "scripts/build_stage9771_edit_localization_visible_evidence_lift_package.py", "SOURCE")
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Materialized a successor opaque-choice surface that adds web-specific visible disambiguators on top of the corrected Stage9790 package while preserving the same A-E output protocol.",
    ...
    DOC.write_text(
        "\n".join(
            [
                ...
                "",
                "This successor surface keeps the corrected opaque-choice protocol but adds web-specific visible disambiguators derived from locality-signal semantics, because Stage9806 showed that web is the only remaining standalone gap and that it fails broadly across non-symbol classes.",
                "",
                f"Next: {next_step}",
                "",
            ]
