...
STAGE = 10058
NAME = "stage10058_balanced_fresh_source_multilingual_same_manifest_comparison_audit"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "balanced_fresh_source_multilingual_same_manifest_comparison_audit.json"
ROWS = OUT_DIR / "balanced_fresh_source_multilingual_same_manifest_comparison_rows.jsonl"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "BALANCED_FRESH_SOURCE_MULTILINGUAL_SAME_MANIFEST_COMPARISON_AUDIT_STAGE10058.md"
...
SUCCESSOR_HUNDRED_M = ROOT / "runs/local/artifacts/stage10057_balanced_fresh_source_multilingual_target100m_probe/edit_localization_probe/row_field_logits.jsonl"
...
def build_audit() -> dict[str, Any]:
    ...
    for language in sorted(successor_by_language):
        ...
        if succ_exact > gemma_exact:
            verdict = "100m_better"; wins_100m += 1
        elif gemma_exact > succ_exact:
            verdict = "gemma_better"; wins_gemma += 1
        else:
            verdict = "tie"; ties += 1
    ...
    if len(shared_ids) != len(successor): failures.append("shared_ids_not_equal_to_successor_rows")
    if len(shared_ids) != len(gemma): failures.append("shared_ids_not_equal_to_gemma_rows")
    if len(shared_ids) != 55: failures.append("shared_ids_not_equal_to_55")
    if len(per_language) != 4: failures.append("per_language_not_4")
    return {"passed": not failures, "failures": failures, "metrics": {"rows_successor_present": len(successor), "rows_gemma_present": len(gemma), "shared_rows": len(shared_ids), "comparison_rows": len(comparisons), "macro_exact_successor_100m": macro_successor, "macro_exact_baseline_100m": macro_baseline, "macro_exact_gemma": macro_gemma, "macro_delta_successor_minus_baseline": macro_successor - macro_baseline, "macro_delta_successor_minus_gemma": macro_successor - macro_gemma, "wins_100m": wins_100m, "wins_gemma": wins_gemma, "ties": ties, "per_language": per_language}, "rows": comparisons}
...
def main() -> None:
    ...
    next_step = "Use this audit to decide whether geometry-balanced fresh-source coverage fixes the rust/web drift while retaining the stage10047 Python gain against Gemma."
    summary = {"stage": STAGE, "stage_name": NAME, "name": NAME, "passed": built["passed"], "metrics": {**built["metrics"], "failures": built["failures"]}, "artifacts": {"audit": display(AUDIT), "rows": display(ROWS), "doc": display(DOC)}, "decision": "Materialized the same-manifest comparison audit for the geometry-balanced fresh-source multilingual successor run so the resulting 100M outputs can be judged directly against stage10040 and the fixed Gemma comparator.", "next_best_step": next_step, "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage10058 Balanced Fresh Source Multilingual Same Manifest Comparison Audit", "", f"Passed: `{summary['passed']}`", "", summary["decision"], "", f"Next: {next_step}", ""]), encoding="utf-8")
