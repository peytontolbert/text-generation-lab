...
STAGE = 9772
NAME = "stage9772_edit_localization_visible_evidence_target100m_contract_preflight"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9771_edit_localization_visible_evidence_lift_package.json"
...
AUDIT = OUT_DIR / "edit_localization_visible_evidence_target100m_contract_preflight_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "EDIT_LOCALIZATION_VISIBLE_EVIDENCE_TARGET100M_CONTRACT_PREFLIGHT_STAGE9772.md"
...
def command() -> list[str]:
    return [
        sys.executable, str(TRAINER), '--repo-root', str(ROOT), '--manifest', str(MANIFEST.relative_to(ROOT)), '--mode', TARGET_MODE,
        ...
        '--tokenizer-hashlock', str(TOKENIZER_HASHLOCK.relative_to(ROOT)), '--max-train-rows', '20', '--max-eval-rows', '20',
        '--max-strict-rows', '20', '--max-steps', '0', '--batch-size', '2', '--learning-rate', '5e-5', '--max-encoder-tokens', '512',
...
def main() -> None:
    ...
    if source.get('passed') is not True:
        failures.append('stage9771_not_passed')
    if run.returncode != 0:
    ...
    next_step = 'If execution continues cleanly, run one target-100M multilingual edit-localization structured probe from the Stage9771 visible-evidence package and compare it against the Stage9744 target-only baseline.'
    summary = {
        'stage': STAGE,
        'stage_name': NAME,
        'name': NAME,
        ...
        'decision': 'Validated the Stage9771 visible-evidence edit-localization package under target-100M contract-only preflight without executing training.',
        'next_best_step': next_step,
        'created_at_utc': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + '\n', encoding='utf-8')
    DOC.write_text('\n'.join([
        '# Stage9772 Edit Localization Visible Evidence Target-100M Contract Preflight',
        '',
        f"Passed: `{summary['passed']}`",
        f"Rows: `{len(rows)}`",
        f"Loss counts: `{losses}`",
        '',
        'This is contract-only and validates the Stage9771 visible-evidence edit-localization package under target-100M settings without opening model execution claims.',
        '',
        f'Next: {next_step}',
        '',
    ]), encoding='utf-8')
