...
STAGE = 9515
NAME = "stage9515_episode_obs_diag_counterbalance_repair_preflight_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9514_episode_obs_diag_counterbalance_repair_audit.json"
RUN_DIR = ROOT / "runs/local/artifacts/stage9515_episode_obs_diag_counterbalance_repair_target_100m_contract_preflight"
CONTRACT = RUN_DIR / "probe_contract_audit.json"
AUDIT = RUN_DIR / "stage9515_preflight_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "EPISODE_OBS_DIAG_COUNTERBALANCE_REPAIR_PREFLIGHT_STAGE9515.md"
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9514_not_passed")
    ...
    audit = {
        "passed": not failures,
        "failures": failures,
        "source_stage": "stage9514_episode_obs_diag_counterbalance_repair_audit",
    ...
    summary = {
        ...
        "decision": "Contract-only target-100M preflight passed for the observation-diagnosis counterbalance repair manifest.",
        "next_best_step": "Run Stage9516 target-100M counterbalance repair probe under trellis, then audit whether eval high-confidence failures are fixed.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    DOC.write_text("\n".join([
        "# Stage9515 Episode Observation Diagnosis Counterbalance Repair Preflight",
        "",
