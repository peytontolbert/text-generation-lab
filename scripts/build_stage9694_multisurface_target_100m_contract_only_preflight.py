...
STAGE = 9694
NAME = "stage9694_multisurface_target_100m_contract_only_preflight"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9693_locked_guarded_source_backed_multisurface_compiler_refresh.json"
STRUCTURED = ROOT / "runs/local/artifacts/stage9693_locked_guarded_source_backed_multisurface_compiler_refresh/compiled/structured_state.jsonl"
BOUNDED = ROOT / "runs/local/artifacts/stage9693_locked_guarded_source_backed_multisurface_compiler_refresh/compiled/bounded_decoder_ce.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
MANIFEST_DIR = OUT_DIR / "manifests"
RUNS_DIR = OUT_DIR / "contract_runs"
AUDIT = OUT_DIR / "multisurface_target_100m_contract_only_preflight_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "MULTISURFACE_TARGET_100M_CONTRACT_ONLY_PREFLIGHT_STAGE9694.md"
...
def trainer_command(surface: str, manifest: Path, rows: list[dict[str, Any]]) -> list[str]:
    ...
    cmd = [
        ...
        "--max-decoder-tokens", "768" if decoder else "8",
        "--decoder-ce-weight", "1.0" if decoder else "0.0",
        "--structured-aux-weight", "0.0" if decoder else "1.0",
        "--denoise-weight", "0.0",
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9693_not_passed")
    ...
    next_step = "Build Stage9695 execution-authorization review for a tiny multisurface target-100M structured probe, or first patch any contract-only telemetry gaps if Stage9694 fails."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Ran target-100M contract-only trainer preflights for each Stage9693 source-backed surface; no model execution or training was authorized.",
