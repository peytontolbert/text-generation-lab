#!/usr/bin/env python3
"""Roll Stage12011 PASS_TO_PASS expansion rows into transition support v13."""
...
ROOT = Path("runs/local/artifacts/stage12012_transition_support_rollup_v13")
BASE = Path("runs/local/artifacts/stage12010_transition_support_rollup_v12/transition_support_rows_v12.jsonl")
ADDED = Path("runs/local/artifacts/stage12011_pass_to_pass_expansion_rows/pass_to_pass_expansion_rows.jsonl")
ROWS_OUT = ROOT / "transition_support_rows_v13.jsonl"
SUMMARY = ROOT / "transition_support_rollup_v13.json"
SUMMARY_MIRROR = Path("runs/summaries/stage12012_transition_support_rollup_v13.json")
...
def main() -> None:
    ...
    duplicates = 0
    for source, path in [("stage12010", BASE), ("stage12011", ADDED)]:
        for row in read_jsonl(path):
            out = dict(row)
            out["stage12012_source"] = source
            out["split"] = "train"
            out["split_role"] = "stage12012_transition_support_v13_train_support_only"
    ...
    summary = {
        "stage": "stage12012_transition_support_rollup_v13",
        ...
        "decision": "support_inventory_v13_not_train_ready",
        "reason": "PASS_TO_PASS supply improved slightly, but Transition-Root-250 remains far short and still should not trigger a model-training run.",
        "claim_boundary": "Support inventory only. No model promotion or Gemma comparison should be claimed from v13.",
        "next_stage_recommendation": {"stage": "stage12013_transition_root_250_positive_supply_expansion", "action": "Continue mining independent executable PASS_TO_PASS and NOT_EXERCISED roots, with Rust and C/C++ prioritized next."},
    }
