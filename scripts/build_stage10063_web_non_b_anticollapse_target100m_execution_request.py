...
STAGE = 10063
NAME = "stage10063_web_non_b_anticollapse_target100m_execution_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
REQUEST = OUT_DIR / "web_non_b_anticollapse_target100m_execution_request.json"
SURFACE_REQUEST = OUT_DIR / "surface_requests" / "edit_localization.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "WEB_NON_B_ANTICOLLAPSE_TARGET100M_EXECUTION_REQUEST_STAGE10063.md"
...
SOURCE_SUMMARY = ROOT / "runs/summaries/stage10062_web_non_b_anticollapse_successor_packet.json"
...
FUTURE_RUN_ID = "stage10063_web_non_b_anticollapse_target100m_probe"
FUTURE_OUTPUT_DIR = "runs/local/artifacts/stage10063_web_non_b_anticollapse_target100m_probe/edit_localization_probe"
...
def build_request() -> dict[str, Any]:
    ...
    stage10062_train_rows = sum(1 for row in rows if str(row.get("source_row_id") or "").startswith("stage8765_row_") and str(row.get("split") or "") == "train") - (38 + 17 + 27)
    ...
    metrics = {
        ...
        "stage10062_train_rows": stage10062_train_rows,
    }
    if metrics["rows"] != 142:
    ...
    if source_summary.get("passed") is not True:
        failures.append("stage10062_not_passed")
    ...
    req = {
        ...
        "required_contract_invariants": {
            "source_stage10062_passed": bool(source_summary.get("passed") is True),
    ...
    return {
        ...
        "metrics": metrics,
        "source_artifacts": {
            "stage10062_packet_summary": display(SOURCE_SUMMARY),
        ...
        "execution_constraints": {
            ...
            "web_non_b_anticollapse_packet_required": True,
        },
...
def main() -> None:
    ...
    next_step = "Use this request to run one target-100m edit-localization probe on the web non-B anti-collapse successor manifest, then compare it against stage10040 and Gemma on the same 55 heldout rows."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Prepared the execution request for the web non-B anti-collapse successor so the next probe can test whether adding fresh web C/D/E rows fixes the stage10058 all-B web failure without disturbing the stronger multilingual base.",
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage10063 Web Non-B Anticollapse Target100M Execution Request",
                "",
