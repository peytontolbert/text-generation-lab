...
STAGE = 9653
NAME = "stage9653_comparator_feature_tiny_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9652_comparator_feature_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9652_comparator_feature_manifest/comparator_feature_manifest.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
RUN_DIR = OUT_DIR / "episode_step_probe"
AUDIT = OUT_DIR / "comparator_feature_tiny_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "COMPARATOR_FEATURE_TINY_PROBE_STAGE9653.md"
...
def command() -> list[str]:
    return ["env", f"TMPDIR={TMPDIR}", f"TEMP={TMPDIR}", f"TMP={TMPDIR}", "conda", "run", "-n", "trellis", "python", str(TRAINER),
        ...
        "--output-dir", str(RUN_DIR), "--run-id", "stage9653_comparator_feature_probe", "--execution-authorized-for-recovery-probe"]
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        safety_failures.append("stage9652_not_passed")
    comparator = source_metrics.get("comparator_feature_baselines") if isinstance(source_metrics.get("comparator_feature_baselines"), dict) else {}
    if comparator and min(float(value) for value in comparator.values()) != 1.0:
        safety_failures.append("stage9652_comparator_not_predictive")
