...
OUT = ROOT / "runs/local/tmp/pocketpal_stage611_compact_entity_selector_seed461"
ARTIFACT = ROOT / "runs/local/artifacts/stage611_compact_entity_selector_dataset.json"
DOC = ROOT / "docs/stage611_compact_entity_selector_dataset.md"
...
def selector_marker(entity: str, field: str) -> str:
    return f"ENTITY_SELECTOR selector_pair={entity}|{field}"


def inject_after_operation(text: str, marker: str) -> str:
    if not text or "ENTITY_SELECTOR" in text:
        return text
...
def rewrite_row(row: dict[str, Any]) -> tuple[dict[str, Any], bool]:
    ...
    if not (entity and field):
        return dict(row), False
    marker = selector_marker(entity, field)
    ...
    for key, value in row.items():
        if isinstance(value, str) and key in TEXT_FIELDS:
            ...
            rewritten[key] = value
    rewritten["entity_context_compact_selector"] = True
    rewritten["entity_context_selector_entity"] = entity
    rewritten["entity_context_selector_field"] = field
    rewritten["source_type"] = f"{row.get('source_type', 'retrieval')}_compact_entity_selector"
...
def selector_counts(rows: list[dict[str, Any]]) -> dict[str, int]:
    return dict(Counter(str(row.get("entity_context_selector_field", "") or "") for row in rows if row.get("entity_context_compact_selector")))
...
def main() -> None:
    ...
    for split in ("train", "eval"):
        ...
        field_counts[split] = selector_counts(converted)

    manifest = dict(source_manifest)
    manifest.update(
        {
            "artifact_kind": "agentkernel_lite_encdec_stage611_compact_entity_selector",
            ...
            "entity_context_compact_selector": True,
            "stage611_selector_changed_train_examples": changed_counts["train"],
            "stage611_selector_changed_eval_examples": changed_counts["eval"],
            "collision_goal": "Compress Stage610 selector markers while preserving entity_context selector gains.",
        }
    )
    ...
    summary = {
        "artifact_kind": "stage611_compact_entity_selector_dataset",
        ...
        "decision": "compact_entity_selector_dataset_ready",
        "finding": (
            "Stage611 compresses Stage610's selector to a single entity-field pair marker. It tests whether selector weighting, "
            "rather than repeated domain/entity/field spelling, carries the Stage610 KBPP gain."
        ),
    }
    ARTIFACT.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text(
        f"""# Stage611 Compact Entity Selector Dataset
