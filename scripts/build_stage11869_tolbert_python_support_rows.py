#!/usr/bin/env python3
"""Materialize tolbert Python selected-verifier support rows."""
...
STAGE = 11869
NAME = "stage11869_tolbert_python_support_rows"
OUT = ART / NAME
SUMMARY = OUT / "tolbert_python_support_rows.json"
ROWS = OUT / "tolbert_python_support_rows.jsonl"
FEASIBILITY = ART / "stage11868_tolbert_python_feasibility/tolbert_python_feasibility_results.jsonl"
REPO = Path("/data/tolbert")
...
def file_evidence(eid: str, kind: str, rel_path: str, summary: str, start: int = 1, end: int = 240) -> dict[str, Any]:
...
def base_row(root_id: str, suffix: str, task_type: str, options: list[dict[str, Any]], target_label: str) -> dict[str, Any]:
    ...
    return {
        ...
        "source_snapshot_id": "tolbert::local_git_snapshot",
...
def main() -> None:
    feasible = [row for row in read_jsonl(FEASIBILITY) if row.get("repo_family") == "tolbert" and row.get("passed") is True]
    if not feasible:
        raise SystemExit("tolbert feasibility did not pass")
    probe = feasible[0]
    root_id = "stage11869::tolbert::python::hierarchical_decode_support"
    evidence = [
        file_evidence("E01", "implementation_source", probe["source_path"], "Hierarchical decoder source constrains child predictions by parent-child mapping after the first-level argmax."),
        file_evidence("E02", "selected_test_anchor", probe["selected_test_path"], "Losses and decoding tests include the selected parent-child constrained decoding verifier and contrastive-loss sibling verifier."),
        file_evidence("E03", "nearby_source_distractor", probe["distractor_source_path"], "Tree contrastive loss source is related hierarchy logic but is not the selected decoding verifier surface."),
        file_evidence("E04", "nearby_source_distractor", probe["second_distractor_source_path"], "TOLBERT modeling source is a sibling verifier surface using a dummy encoder."),
        log_evidence("V01", probe["selected_result"], "Selected hierarchical decoding pytest passed."),
        log_evidence("V02", probe["sibling_result"], "Sibling tree contrastive-loss pytest passed."),
        log_evidence("V03", probe["second_sibling_result"], "Sibling TOLBERT dummy-encoder modeling pytest passed."),
    ]
    verifier_evidence = {
        "id": "V01",
        "kind": "actual_pytest_pass_log",
        "summary": "Selected and sibling tolbert pytest functions passed.",
    ...
    verifier_options = [
        ...
        {"label": "E", "role": "modeling_source_distractor", "value": probe["second_distractor_source_path"], "evidence_ids": ["E04"]},
    ]
    evidence_options = [
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "twelfth_python_verifier_support_root_materialized",
        "passed": True,
        "row_count": len(rows),
        "root_count": 1,
        "repo_family": "tolbert",
