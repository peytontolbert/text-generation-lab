...
STAGE = 9773
NAME = "stage9773_edit_localization_visible_evidence_execution_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9772_edit_localization_visible_evidence_target100m_contract_preflight.json"
BASELINE = ROOT / "runs/local/artifacts/stage9744_multilingual_edit_localization_target_only_exec/execution_result.json"
RUN_DIR = ROOT / "runs/local/artifacts/stage9773_edit_localization_visible_evidence_exec"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "edit_localization_visible_evidence_execution_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "EDIT_LOCALIZATION_VISIBLE_EVIDENCE_EXECUTION_AUDIT_STAGE9773.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
REQUIRED = [
    'probe_contract_audit.json', 'execution_result.json', 'loss_by_step.jsonl', 'eval_loss_by_checkpoint.jsonl', 'row_field_logits.jsonl',
    'row_field_losses.jsonl', 'row_gradient_norms.jsonl', 'activation_summary.jsonl', 'feature_ablation_attribution.jsonl',
    'activation_patch_recovery.jsonl', 'row_dynamics_history.jsonl', 'field_exact_by_cell.json', 'field_label_vocabs.json',
    'structured_confusion_matrix.json', 'module_delta_norms.json', 'failure_bucket_card.json', 'cleanup_proof.json'
]
...
def main() -> None:
    ...
    if source.get('passed') is not True:
        failures.append('stage9772_not_passed')
    if not result:
    ...
    audit = {
        ...
        'improved_over_stage9744': improved,
        'missing_required_artifacts': missing,
        'authority': dict(AUTHORITY_CLOSED),
    }
    AUDIT.write_text(json.dumps(audit, indent=2, sort_keys=True) + '\n', encoding='utf-8')
    next_step = 'If the visible-evidence package beats Stage9744, rerun the deterministic four-language Gemma comparison on this surface; otherwise the remaining bottleneck is model capacity/objective, not hidden evidence.'
    summary = {
        'stage': STAGE,
        'stage_name': NAME,
        'name': NAME,
        ...
        'decision': 'Audited the executed Stage9771 visible-evidence multilingual edit-localization run against the current Stage9744 target-only baseline.',
    ...
    DOC.write_text('\n'.join([
        ...
        f"Improved over Stage9744: `{improved}`",
        '',
