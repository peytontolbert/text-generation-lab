...
STAGE = 9323
NAME = "stage9323_phrase_a_probe_preexecution"
...
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9324_phrase_a_patch_inside_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "phrase_a_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9324_phrase_a_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PHRASE_A_PROBE_PREEXECUTION_STAGE9323.md"
...
COMMAND = [
    ...
    "--max-train-rows", "3", "--max-eval-rows", "3", "--max-strict-rows", "2",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9324_phrase_a_patch_inside_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if target.get("phrase_id") != "phrase_a" or loss_mask.get("decoder_ce") or loss_mask.get("structured_aux") or not loss_mask.get("denoise_ce"):
    ...
    if split_counts != {"train": 3, "eval": 3, "strict_eval": 2}:
    ...
    required_pairs = {
        ...
        "--max-eval-rows": "3", "--max-strict-rows": "2", "--max-steps": "80",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared an isolated phrase-A target-100M denoise probe for the patch-inside failure family.",
        "next_best_step": "Execute Stage9324 phrase-A probe under trellis and audit whether patch-inside is family-local learnable.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9323 Phrase A Probe Preexecution",
        "",
