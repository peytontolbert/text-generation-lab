...
STAGE = 10056
NAME = "stage10056_balanced_fresh_source_multilingual_geometry_successor_packet"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
PACKET = OUT_DIR / "balanced_fresh_source_multilingual_geometry_successor_packet.json"
ROWS = OUT_DIR / "balanced_fresh_source_multilingual_geometry_successor_rows.jsonl"
MANIFEST = OUT_DIR / "balanced_fresh_source_multilingual_geometry_successor_manifest.jsonl"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "BALANCED_FRESH_SOURCE_MULTILINGUAL_GEOMETRY_SUCCESSOR_PACKET_STAGE10056.md"
...
def _convert_source_row(row: dict[str, Any], *, family: str, label: str, hidden_target: str, rank: int, role: str) -> dict[str, Any]:
    ...
    return {
        "row_id": f"stage10056_{row.get('row_id')}::{role}_{rank}",
        ...
        "semantic_key": f"stage10056:train:{family}:{row.get('row_id')}",
        ...
        "counterfactual_root_row_id": f"{row.get('row_id')}::stage10056_train_root",
        "counterfactual_role": "positive_original",
        "counterfactual_required_obligations": [
            "POSITIVE_ORIGINAL",
            "SOURCE_BACKED",
            "BALANCED_MULTILINGUAL_GEOMETRY_SUCCESSOR",
        ...
        "counterfactual_group_id": f"stage10056:{family}:{label}:{row.get('row_id')}",
        ...
        "stage10056_role": "train",
        "stage10056_selection_role": role,
        "stage10056_source_language": family,
        "stage10056_hidden_target": hidden_target,
        "stage10056_expected_label": label,
    }
...
def build_packet() -> dict[str, Any]:
    ...
    topup_counts: Counter[str] = Counter()

    for family in ["c_cpp", "rust", "web_js_ts_html"]:
        for label in LABELS:
            candidates = grouped_candidates.get((family, label), [])
            if not candidates:
                failures.append(f"missing_geometry_candidate:{family}:{label}")
                continue
            row = candidates.pop(0)
            hidden_target = str((row.get("clean_state") or {}).get("edit_localization_target") or "")
            converted = _convert_source_row(row, family=family, label=label, hidden_target=hidden_target, rank=geometry_counts[family] + 1, role="geometry")
            selected_rows.append(converted)
            selected_counts[(family, label)] += 1
            geometry_counts[family] += 1

    for family in ["c_cpp", "rust", "web_js_ts_html"]:
        for label in LABELS:
            need = regression_counts.get((family, label), 0)
            extra = max(0, need - 1)
            for _ in range(extra):
                candidates = grouped_candidates.get((family, label), [])
                if not candidates:
                    failures.append(f"missing_topup_candidate:{family}:{label}")
                    continue
                row = candidates.pop(0)
                hidden_target = str((row.get("clean_state") or {}).get("edit_localization_target") or "")
                converted = _convert_source_row(row, family=family, label=label, hidden_target=hidden_target, rank=topup_counts[f"{family}:{label}"] + 1, role="topup")
                selected_rows.append(converted)
                selected_counts[(family, label)] += 1
                topup_counts[f"{family}:{label}"] += 1
    ...
    if not readiness or readiness.get("passed") is not True:
        failures.append("geometry_successor_readiness_failed")
    ...
    metrics = {
        ...
        "topup_counts": dict(sorted(topup_counts.items())),
    ...
    if metrics["fresh_source_rows"] != 24:
        failures.append("fresh_source_rows_not_24")
    if metrics["successor_rows"] != 127:
        failures.append("successor_rows_not_127")
    if split_counts.get("train") != 72:
        failures.append("train_rows_not_72")
    ...
    if metrics["geometry_counts"] != {"c_cpp": 5, "rust": 5, "web_js_ts_html": 5}:
        failures.append("geometry_counts_mismatch")
    ...
    return {
        ...
        "policy": {
            ...
            "balanced_non_python_label_geometry": True,
            "topup_only_audited_stage10049_deficit_labels": True,
        },
        "inputs": {
...
def main() -> None:
    ...
    next_step = "Run one capped target-100m probe on this geometry-balanced fresh-source successor manifest to test whether non-python label coverage stabilizes rust/web while preserving the stage10047 Python gain."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Materialized a geometry-balanced multilingual successor packet that keeps the stage10047 Python fix and adds fresh-source non-python anchors across every label before topping up the audited deficit labels.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage10056 Balanced Fresh Source Multilingual Geometry Successor Packet",
        "",
