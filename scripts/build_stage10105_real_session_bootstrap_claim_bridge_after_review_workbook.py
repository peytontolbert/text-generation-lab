...
STAGE = 10105
NAME = "stage10105_real_session_bootstrap_claim_bridge_after_review_workbook"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
ARTIFACT = OUT_DIR / "real_session_bootstrap_claim_bridge_after_review_workbook.json"
ADJUDICATED_TARGET = OUT_DIR / "real_session_bootstrap_adjudicated_manifest.jsonl"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "REAL_SESSION_BOOTSTRAP_CLAIM_BRIDGE_AFTER_REVIEW_WORKBOOK_STAGE10105.md"
...
WORKBOOK = ROOT / "runs/local/artifacts/stage10104_real_session_bootstrap_review_packets/real_session_bootstrap_signoff_workbook.json"
...
def build_refresh() -> dict[str, Any]:
    ...
    if int(workbook.get("row_count") or 0) != 66:
        failures.append("stage10104_workbook_rows_not_66")
    ...
    for row in packet_rows:
        ...
        if len(workbook_rows_for_row) != 2:
            ...
            updated["attached_evidence"] = [
                {
                    "kind": "real_session_bootstrap_review_workbook_support",
                    "stage": 10104,
    ...
    if metrics["bridge_records"] != 33:
        failures.append("bridge_records_not_33")
    if metrics["rows_with_workbook_support"] != 33:
        failures.append("rows_with_workbook_support_not_33")
    if metrics["rows_machine_complete_but_review_blocked"] != 33:
        failures.append("rows_machine_complete_but_review_blocked_not_33")
...
def main() -> None:
    ...
    built = build_refresh()
    next_step = "Complete the stage10104 rubric and anti-cheat tasks, write adjudicated gold labels into the designated manifest target, then rerun a shortcut baseline on that adjudicated manifest before any training or Gemma comparison."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Refreshed the real-session bootstrap claim bridge so the 33 provisional packet rows are classified as machine-complete but blocked on human rubric, anti-cheat confirmation, and gold-label adjudication.",
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage10105 Real Session Bootstrap Claim Bridge After Review Workbook",
                "",
