...
STAGE = 10779
NAME = "stage10779_bulk_reviewed_support_candidate_probe_request"
OUT_DIR = ARTIFACTS / NAME
REQUEST_JSON = OUT_DIR / "bulk_reviewed_support_candidate_probe_request.json"
COMMAND_JSON = OUT_DIR / "bulk_reviewed_support_candidate_probe_command.json"
MANIFEST_JSONL = OUT_DIR / "bulk_reviewed_support_candidate_probe_manifest.jsonl"

PACKAGE_JSON = ARTIFACTS / "stage10778_bulk_reviewed_support_candidate_training_package" / "bulk_reviewed_support_candidate_training_package.json"
TRAIN_ROWS = ARTIFACTS / "stage10778_bulk_reviewed_support_candidate_training_package" / "agentkernel_lite_encdec_train.jsonl"
VALIDATION_ROWS = ARTIFACTS / "stage10778_bulk_reviewed_support_candidate_training_package" / "agentkernel_lite_encdec_validation.jsonl"
STRICT_ROWS = ARTIFACTS / "stage10778_bulk_reviewed_support_candidate_training_package" / "agentkernel_lite_encdec_strict_eval.jsonl"
STRESS_ROWS = ARTIFACTS / "stage10778_bulk_reviewed_support_candidate_training_package" / "agentkernel_lite_encdec_stress_eval.jsonl"
...
RUN_ID = "stage10780_bulk_reviewed_support_candidate_probe"
OUTPUT_DIR = "runs/local/artifacts/stage10780_bulk_reviewed_support_candidate_probe/bounded_decoder_probe"
RUNTIME_MODEL_DIR = "runs/local/artifacts/stage10780_bulk_reviewed_support_candidate_probe/runtime_model"
...
def main() -> None:
    ...
    support_rows = [row for row in train_rows if str(row.get("support_package_stage") or "") == str(STAGE - 1)]
    ...
    request = {
        ...
        "decision": "bulk_reviewed_support_candidate_probe_ready",
        ...
        "support_snapshot": {
            "support_rows_added_from_stage10778": len(support_rows),
            "support_languages": dict(sorted(Counter(str(row.get("language_family") or "unknown") for row in support_rows).items())),
            "support_perspectives": dict(sorted(Counter(str(row.get("perspective") or "unknown") for row in support_rows).items())),
        },
        "stress_rows_excluded_from_manifest": len(stress_rows),
        "claim_scope": [
            "Run a standalone support-only probe that keeps the reviewed v2.7 strict frontier fixed while adding the new multilingual C/C++ and Python reviewed-support rows.",
            "Require any gain to beat the live 22/24 baseline with zero strict regressions before promotion.",
        ],
        "required_honesty_gates": [
            "strict and validation rows remain unchanged from the current reviewed v2.7 frontier",
            "all stage10778 rows remain train_support_only and strict_eval_eligible=false",
            "all stage10778 rows use provisional AI gold only for support training, not for promotable eval",
        ...
        "next_best_step": "run the probe, then audit whether the multilingual reviewed-support rows improve strict performance without regressions before widening to Rust",
    }
