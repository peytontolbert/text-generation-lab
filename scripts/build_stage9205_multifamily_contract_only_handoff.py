...
STAGE = 9205
NAME = "stage9205_multifamily_contract_only_handoff"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
SOURCE_9204 = ROOT / "runs/summaries/stage9204_repo_local_multifamily_input_selector.json"
SELECTED_BUNDLES = ROOT / "runs/local/artifacts/stage9204_repo_local_multifamily_input_selector/selected_repo_local_family_bundles.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "MULTIFAMILY_CONTRACT_ONLY_HANDOFF_STAGE9205.md"
...
def main() -> None:
    ...
    checks = {
        "source_stage9204_passed": bool(source.get("passed") is True),
        "selected_bundles_present": isinstance(bundles, list) and len(bundles) >= 2,
    ...
    audit = {
        ...
        "decision": (
            "Consumed the selected repo-local family bundles and ran contract-only handoff smokes for each recovered "
            "probe family without reopening model execution or runtime."
        ),
        "next_best_step": (
            "If desired, add a real denoise-eligible bundle under the same selector contract. Otherwise this path is ready for future explicit execution review because both structured and bounded decoder families now hand off cleanly in contract-only mode."
        ),
    }
    audit_path = OUT_DIR / "multifamily_contract_only_handoff.json"
