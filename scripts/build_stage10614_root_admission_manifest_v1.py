...
STAGE = 10614
NAME = "stage10614_root_admission_manifest_v1"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
MANIFEST_JSONL = OUT_DIR / "root_admission_manifest_v1.jsonl"
SUMMARY_JSON = OUT_DIR / "root_admission_manifest_v1.json"
RUN_SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"

REVIEWED_ROOTS = ROOT / "runs/local/artifacts/stage10420_reviewed_multilingual_v27_manifest_package/reviewed_multilingual_v27_root_manifest.jsonl"
...
def main() -> None:
    ...
    summary = {
        ...
        "passed": True,
        "decision": "root_admission_manifest_materialized",
        "claim_scope": [
            "Materialize one root-level admission table that combines reviewed v2.7 bundle roots and compiled bootstrap roots.",
            "Expose admit_role, quality score, and anti-cheat leakage status at root granularity before the next multilingual training package is built.",
            "Prevent heldout/bootstrap roots with current target-interface leakage from silently becoming promotable strict roots.",
        ],
        "source_artifacts": {
        ...
        "metrics": {
            "roots_total": len(manifest_rows),
            "reviewed_bundle_roots": sum(1 for row in manifest_rows if row["source_kind"] == "reviewed_bundle_root"),
            "compiled_root_state_roots": sum(1 for row in manifest_rows if row["source_kind"] == "compiled_root_state"),
            "admit_role_counts": dict(sorted(admit_role_counts.items())),
            "language_role_counts": language_role_counts,
            "roots_with_prompt_target_leak_rows": sum(1 for row in manifest_rows if row["prompt_target_leak_rows"] > 0),
        },
        "quality_policy": {
            "strict_eval_is_reserved_for": [
                "reviewed maintainer-grade roots already admitted on the standalone compact v2.7 path",
            ],
            "diagnostic_is_used_for": [
                "heldout long-context roots whose current row interfaces still carry target-handle leakage or unresolved answerability limitations",
                "stress or reference roots that should inform training or audits without upgrading the frontier claim",
            ],
            "quarantine_is_used_for": [
                "train-side bootstrap roots with prompt-target leakage that should not enter the next promotable training package until rewritten",
            ],
        },
        "next_best_step": "Use this manifest to build a multilingual root-supply balance audit and then a training package that includes only train-admitted roots plus standalone canary replay.",
        ...
        "outputs": {
            "root_admission_manifest": display(MANIFEST_JSONL),
        },
    }
