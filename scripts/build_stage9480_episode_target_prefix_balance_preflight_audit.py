...
STAGE = 9480
NAME = "stage9480_episode_target_prefix_balance_preflight_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9479_episode_target_prefix_balance_manifest.json"
PREFLIGHT_DIR = ROOT / "runs/local/artifacts/stage9480_episode_target_prefix_balance_target_100m_contract_preflight"
CONTRACT = PREFLIGHT_DIR / "probe_contract_audit.json"
AUDIT = PREFLIGHT_DIR / "stage9480_preflight_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "EPISODE_TARGET_PREFIX_BALANCE_PREFLIGHT_AUDIT_STAGE9480.md"
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9479_not_passed")
    ...
    if contract.get("rows") != 50 or contract.get("split_counts") != {"eval": 2, "other": 0, "strict_eval": 2, "train": 46}:
    ...
    audit = {
        "passed": not failures,
        "failures": failures,
        "source_stage": "stage9479_episode_target_prefix_balance_manifest",
    ...
    summary = {
        ...
        "decision": "Authorized only Stage9481 target-prefix focused target-100M structured execution if this preflight passes.",
        "next_best_step": "Run Stage9481 target-prefix focused best-state-restored probe under trellis, then audit balanced eval/strict exactness.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    DOC.write_text("\n".join([
        "# Stage9480 Episode Target-Prefix Balance Preflight Audit",
        "",
        ...
        "",
        "Only the target-prefix structured head is open. Decoder CE, denoise CE, runtime, source/body emission, Gemma, harness, scoring, checkpoint export, controller merge, and promotion remain closed.",
        "",
    ]))
