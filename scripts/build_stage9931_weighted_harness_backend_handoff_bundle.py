...
STAGE = 9931
NAME = "stage9931_weighted_harness_backend_handoff_bundle"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
BUNDLE = OUT_DIR / "weighted_harness_backend_handoff_bundle.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "WEIGHTED_HARNESS_BACKEND_HANDOFF_BUNDLE_STAGE9931.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"

REQUESTS = ROOT / "runs/local/artifacts/stage9930_weighted_harness_execution_requests/weighted_harness_execution_requests.json"
...
def build_bundle() -> dict[str, Any]:
    requests = load_json(REQUESTS).get("requests") or []
    failures: list[str] = []
    ...
    for row in requests:
        ...
        payload = {
            ...
            "why_external_backend_is_required": [
                "locked_task_pack_contains_only metadata_and_threshold_contracts",
    ...
    metrics = {
        "weighted_handoff_cells": len(rows),
    ...
    if metrics["weighted_handoff_cells"] != 4:
        failures.append("weighted_handoff_cells_not_4")
...
def main() -> None:
    ...
    next_step = (
        "Hand the four harness_backend_handoff_bundle.json files to the external full-product runtime so it can execute the same task pack for 100M and Gemma12b and write back only to the reserved artifact paths."
    )
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        ...
        "decision": "Materialized backend handoff bundles proving that the remaining full-product gap is external runtime execution, because the locked task packs in this repo contain only metadata and threshold contracts, not executable payload.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9931 Weighted Harness Backend Handoff Bundle",
        "",
        f"Passed: `{summary['passed']}`",
        f"Weighted handoff cells: `{summary['metrics']['weighted_handoff_cells']}`",
