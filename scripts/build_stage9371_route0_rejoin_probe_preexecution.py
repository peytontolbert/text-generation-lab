...
STAGE = 9371
NAME = "stage9371_route0_rejoin_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9370_full_mixture_with_route0_repairs_manifest.json"
...
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9372_route0_rejoin_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "route0_rejoin_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9372_route0_rejoin_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "ROUTE0_REJOIN_PROBE_PREEXECUTION_STAGE9371.md"
...
COMMAND = [
    ...
    "--max-train-rows", "133", "--max-eval-rows", "72", "--max-strict-rows", "45",
    "--max-steps", "160", "--batch-size", "2", "--learning-rate", "1e-5",
    ...
    "--enable-generation-audit", "--max-generation-rows", "250", "--max-generation-tokens", "64",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9372_route0_rejoin_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9370_not_passed")
    if len(rows) != 250:
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared a closed-boundary 250-row full-mixture denoise rejoin probe; decoder CE remains closed.",
        "next_best_step": "Execute Stage9372 route0 rejoin probe under trellis and audit whether denoise repair is clean enough to return to bounded decoder CE readiness.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9371 Route0 Rejoin Probe Preexecution", "", f"Passed: `{card['passed']}`", f"Rows: `{card['manifest_rows']}`", f"Splits: `{card['split_counts']}`", f"Routes: `{card['route_counts']}`", "", "Only the Stage9372 250-row target-100M denoise probe is authorized. Decoder CE and external authority remain closed.", ""]), encoding="utf-8")
