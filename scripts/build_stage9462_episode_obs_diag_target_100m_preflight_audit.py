...
STAGE = 9462
NAME = "stage9462_episode_obs_diag_target_100m_preflight_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9461_episode_step_observation_diagnosis_manifest.json"
PREFLIGHT_DIR = ROOT / "runs/local/artifacts/stage9462_episode_obs_diag_target_100m_contract_preflight"
CONTRACT = PREFLIGHT_DIR / "probe_contract_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "EPISODE_OBS_DIAG_TARGET_100M_PREFLIGHT_AUDIT_STAGE9462.md"
AUDIT = PREFLIGHT_DIR / "stage9462_preflight_audit.json"
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9461_not_passed")
    ...
    audit = {
        "passed": not failures,
        "failures": failures,
        "source_stage": "stage9461_episode_step_observation_diagnosis_manifest",
    ...
    summary = {
        ...
        "decision": "Authorized only the next tiny target-100M observation-diagnosis structured probe if this preflight passes.",
        "next_best_step": "Run Stage9463 tiny target-100M observation-diagnosis probe under trellis, then audit strict diagnosis quality.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    DOC.write_text("\n".join([
        "# Stage9462 Episode Observation Diagnosis Target-100M Preflight Audit",
        "",
        ...
        "",
        "Only Stage9463 observation-diagnosis structured execution is authorized. Decoder CE, denoise CE, runtime, source/body emission, Gemma, harness, scoring, controller merge, checkpoint export, and promotion remain closed.",
        "",
    ]))
