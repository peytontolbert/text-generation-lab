...
STAGE = 9918
NAME = "stage9918_hardened_weighted_v27_execution_delta_audit"
CURRENT = ROOT / "runs/summaries/stage9917_hardened_weighted_structured_execution_review.json"
BASELINE = ROOT / "runs/summaries/stage9914_hardened_structured_tiny_execution_review.json"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "hardened_weighted_v27_execution_delta_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "HARDENED_WEIGHTED_V27_EXECUTION_DELTA_AUDIT_STAGE9918.md"
...
def build_audit() -> dict[str, Any]:
    ...
    if current.get("passed") is not True:
        failures.append("stage9917_not_passed")
    ...
    for surface in sorted(set(current_surfaces) | set(baseline_surfaces)):
        ...
        delta[surface] = {
            ...
            "schedule_current": cur.get("schedule"),
            "schedule_baseline": base.get("schedule"),
        }
    metrics = {
        "surface_delta": delta,
    ...
    decision = (
        ...
        if not failures
        else "The hardened weighted v2.7 execution delta audit is incomplete."
    )
    caveats = [
        "This remains a tiny capped structured review, not a final standalone or harness benchmark.",
        "Any integrated gain should still be validated later against the multilingual Gemma comparison and counterfactual anti-cheat surfaces.",
        "The critical question is whether edit localization improves without creating new regressions or shortcut dependence elsewhere in the mix.",
    ]
...
def main() -> None:
    ...
    next_step = "If edit localization improves here without collateral regression, promote this weighted hardened package into the next fuller target-100M review and then rerun the multilingual Gemma comparison on the same hardened surface."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
