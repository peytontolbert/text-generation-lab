...
STAGE = 9342
NAME = "stage9342_operator_route_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9341_operator_route_probe_preexecution.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9340_operator_route_separation_manifest/operator_route_separation_manifest.jsonl"
RUN_DIR = ROOT / "runs/local/artifacts/stage9342_operator_route_separation_probe"
AUDIT = RUN_DIR / "stage9342_operator_route_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "OPERATOR_ROUTE_PROBE_AUDIT_STAGE9342.md"
...
def audit_run() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9341_not_passed")
    ...
    quality_gate_passed = bool(generated == 18 and exact == 18 and target_prefix == 18 and boundary == 18 and contentful == 18 and repetition_rows == 0 and short_rows == 0 and leak_rows == 0)
    return {
        ...
        "by_task": bucket(samples, manifest_by_id, "task"),
        "by_split": bucket(samples, manifest_by_id, "split"),
        "diagnosis": "operator_route_separation_rows_solved_stage9339_route_interference_residuals_in_isolation",
        "next_patch_target": "stage9343_merge_operator_route_repairs_with_full_mixture_and_reprobe_interference",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "The operator-route separation repair probe passed exactly in isolation; decoder CE remains closed.",
        "next_best_step": "Merge Stage9340 operator-route separation rows back with the Stage9337 mixture and rerun an interference probe before any bounded decoder CE step.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9342 Operator Route Probe Audit",
        "",
        ...
        "",
        "The repair objective corrected the Stage9339 operator/dependency route-interference and file-path `associated lis` failures in isolation.",
        "Authority is closed after audit. Decoder CE remains closed.",
        "",
    ]), encoding="utf-8")
