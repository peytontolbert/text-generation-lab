#!/usr/bin/env python3
"""Roll Stage12025 controlled FAIL_TO_PASS rows into transition support v20."""
...
ROOT = Path("runs/local/artifacts/stage12026_transition_support_rollup_v20")
BASE = Path("runs/local/artifacts/stage12024_transition_support_rollup_v19/transition_support_rows_v19.jsonl")
ADDED = Path("runs/local/artifacts/stage12025_controlled_fail_to_pass_expansion_rows/controlled_fail_to_pass_expansion_rows.jsonl")
ROWS_OUT = ROOT / "transition_support_rows_v20.jsonl"
SUMMARY = ROOT / "transition_support_rollup_v20.json"
SUMMARY_MIRROR = Path("runs/summaries/stage12026_transition_support_rollup_v20.json")
...
def main() -> None:
    ...
    duplicates = 0
    for source, path in [("stage12024", BASE), ("stage12025", ADDED)]:
        for row in read_jsonl(path):
            out = dict(row)
            out["stage12026_source"] = source
            out["split"] = "train"
            out["split_role"] = "stage12026_transition_support_v20_train_support_only"
    ...
    summary = {
        ...
        "decision": "support_inventory_v20_not_train_ready",
        "reason": "FAIL_TO_PASS coverage improved with controlled fixtures, but Transition-Root-250 still lacks enough real independent roots and several status floors remain unmet.",
        "claim_boundary": "Support inventory only. Controlled fixtures are not strict/source-heldout evidence and no model promotion should be claimed from v20.",
        "next_stage_recommendation": {
            "stage": "stage12027_transition_root_250_not_exercised_and_real_fail_expansion",
            "action": "Prioritize non-synthetic NOT_EXERCISED rows and real-repo FAIL_TO_PASS mutations to reduce dependence on controlled fixtures.",
        },
    }
