...
STAGE = 9465
NAME = "stage9465_episode_obs_diag_micro_target_100m_preflight_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9464_episode_obs_diag_micro_overfit_manifest.json"
PREFLIGHT_DIR = ROOT / "runs/local/artifacts/stage9465_episode_obs_diag_micro_target_100m_contract_preflight"
CONTRACT = PREFLIGHT_DIR / "probe_contract_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "EPISODE_OBS_DIAG_MICRO_TARGET_100M_PREFLIGHT_AUDIT_STAGE9465.md"
AUDIT = PREFLIGHT_DIR / "stage9465_preflight_audit.json"
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9464_not_passed")
    ...
    if contract.get("rows") != 18 or contract.get("split_counts") != {"eval": 1, "other": 0, "strict_eval": 1, "train": 16}:
        failures.append("unexpected_micro_manifest_shape")
    if contract.get("caps", {}).get("max_steps") != 64:
    ...
    audit = {
        "passed": not failures,
        "failures": failures,
        "source_stage": "stage9464_episode_obs_diag_micro_overfit_manifest",
    ...
    summary = {
        ...
        "decision": "Authorized only the next 64-step target-100M observation-diagnosis micro-overfit probe if this preflight passes.",
        "next_best_step": "Run Stage9466 micro-overfit probe under trellis, then audit whether strict residual cell reaches exact 1.0.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    DOC.write_text("\n".join([
        "# Stage9465 Episode Observation Diagnosis Micro Target-100M Preflight Audit",
        "",
        ...
        f"Caps: `{audit['caps']}`",
        "",
        "Only Stage9466 micro-overfit structured execution is authorized. Decoder CE, denoise CE, runtime, source/body emission, Gemma, harness, scoring, controller merge, checkpoint export, and promotion remain closed.",
        "",
    ]))
