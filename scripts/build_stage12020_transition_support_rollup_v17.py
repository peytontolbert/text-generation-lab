#!/usr/bin/env python3
"""Roll Stage12019 positive expansion rows into transition support v17."""
...
ROOT = Path("runs/local/artifacts/stage12020_transition_support_rollup_v17")
BASE = Path("runs/local/artifacts/stage12018_transition_support_rollup_v16/transition_support_rows_v16.jsonl")
ADDED = Path("runs/local/artifacts/stage12019_positive_expansion_rows/positive_expansion_rows.jsonl")
ROWS_OUT = ROOT / "transition_support_rows_v17.jsonl"
SUMMARY = ROOT / "transition_support_rollup_v17.json"
SUMMARY_MIRROR = Path("runs/summaries/stage12020_transition_support_rollup_v17.json")
...
def main() -> None:
    ...
    duplicates = 0
    for source, path in [("stage12018", BASE), ("stage12019", ADDED)]:
        for row in read_jsonl(path):
            out = dict(row)
            out["stage12020_source"] = source
            out["split"] = "train"
            out["split_role"] = "stage12020_transition_support_v17_train_support_only"
    ...
    summary = {
        "stage": "stage12020_transition_support_rollup_v17",
        ...
        "decision": "support_inventory_v17_not_train_ready",
        "reason": "PASS_TO_PASS improved, but status and language floors remain too far from Transition-Root-250 targets for a serious train run.",
        "claim_boundary": "Support inventory only. No model promotion or Gemma comparison should be claimed from v17.",
        "next_stage_recommendation": {"stage": "stage12021_transition_root_250_cpp_rust_positive_expansion", "action": "Prioritize C++/Rust executable positives and continue reducing PASS_TO_PASS gap."},
    }
