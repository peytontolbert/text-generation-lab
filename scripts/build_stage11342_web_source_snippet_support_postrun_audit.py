...
STAGE = 11342
NAME = 'stage11342_web_source_snippet_support_postrun_audit'
OUT = ART / NAME
SUMMARY = OUT / 'web_source_snippet_support_postrun_audit.json'
RUNTIME = ART / 'stage11341_web_source_snippet_support_probe/runtime_model/runtime_model_bundle.json'
SPLITS = {
    ...
    'web_alias_free_support': ART / 'stage11332_web_alias_free_evidence_materialization/web_alias_free_evidence_rows.jsonl',
    'rust_admitted_replacements': ART / 'stage11331_rust_web_gap_recovery_manifest/admitted_rust_strict_replacement_rows.jsonl',
    'alias_free_residual_diagnostic': ART / 'stage11326_alias_free_evidence_item_selection_scale_package/alias_free_evidence_item_selection_scale_diagnostic_rows.jsonl',
...
SCORERS = ['encoder_option_retrieval_evidence_judgment_head','encoder_option_retrieval_evidence_ledger_head','encoder_option_retrieval_verifier_conditioned','encoder_option_retrieval','decoder_first_step']
...
def main() -> None:
    ...
    summary = {
        ...
        'passed': True,
        'decision': 'web_source_snippet_support_postrun_audit_complete',
        'runtime_initialization': init,
        'product_metrics': {k: v['metric'] for k, v in product.items()},
        'scored': scored,
        'source_artifacts': {'runtime': rel(RUNTIME), **{k: rel(v) for k, v in SPLITS.items()}},
        'outputs': {'summary': rel(SUMMARY)},
        'recommended_next_action': 'Only train on Web support if current runtime is weak there and package includes canary preservation; keep Rust replacements as strict diagnostic unless fresh disjoint Rust train roots are mined.',
    }
