...
STAGE = 9384
NAME = "stage9384_bounded_decoder_failure_denoise_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9383_bounded_decoder_failure_denoise_preexecution.json"
RUN_DIR = ROOT / "runs/local/artifacts/stage9384_bounded_decoder_failure_denoise_probe"
AUDIT = RUN_DIR / "stage9384_bounded_decoder_failure_denoise_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "BOUNDED_DECODER_FAILURE_DENOISE_PROBE_AUDIT_STAGE9384.md"
...
def audit_run() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9383_not_passed")
    ...
    safety_gate_passed = not failures and generated == 23 and leak_rows == 0 and short_rows == 0 and unterminated_rows == 0
    quality_gate_passed = bool(safety_gate_passed and exact == 23 and prefix == 23 and contentful == 23 and repetition_rows == 0)
    return {
        ...
        "diagnosis": "Denoise repair removed the Stage9381 unterminated failure and mostly suppressed repetition, but unprimed generation did not recover the bounded targets.",
        "next_patch_target": "prefix_primed_bounded_decoder_failure_denoise_manifest",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "The bounded decoder failure denoise probe is safety-clean but not quality-passing; do not rejoin or rerun decoder CE yet.",
        "next_best_step": "Build a prefix-primed bounded decoder failure denoise manifest, using the active-generation-prefix pattern that passed Stage9369-9378.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9384 Bounded Decoder Failure Denoise Probe Audit", "", f"Passed: `{audit['passed']}`", f"Safety gate passed: `{audit['safety_gate_passed']}`", f"Quality gate passed: `{audit['quality_gate_passed']}`", f"Exact rows: `{audit['exact_match_rows']}` / `{audit['generated_rows']}`", f"Target-prefix rows: `{audit['target_prefix_match_rows']}` / `{audit['generated_rows']}`", f"Contentful rows: `{audit['contentful_rows']}` / `{audit['generated_rows']}`", f"Repetition rows: `{audit['degenerate_repetition_rows']}`", f"Unterminated rows: `{audit['unterminated_rows']}`", f"Short/junk rows: `{audit['short_or_junk_rows']}`", f"Leak rows: `{audit['generated_internal_token_rows']}`", "", "The repair removed unterminated generation and mostly suppressed repetition, but the target recovery signal is still weak without a generation prefix field.", ""]), encoding="utf-8")
    update_registry(summary)
    print(json.dumps({"stage": STAGE, "passed": summary["passed"], "metrics": {k: audit[k] for k in ["safety_gate_passed", "quality_gate_passed", "target_prefix_match_rate", "contentful_rate", "degenerate_repetition_rate", "unterminated_rate"]}}, indent=2, sort_keys=True))
