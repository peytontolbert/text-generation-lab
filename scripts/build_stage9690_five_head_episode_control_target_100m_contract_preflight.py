...
STAGE = 9690
NAME = "stage9690_five_head_episode_control_target_100m_contract_preflight"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9665_observe_repair_control_promotion_contract.json"
GUARD_SUMMARY = ROOT / "runs/summaries/stage9688_locked_eval_guard_graph_attachment.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9663_five_head_visible_episode_rejoin_manifest/five_head_visible_episode_rejoin_manifest.jsonl"
...
AUDIT = OUT_DIR / "five_head_episode_control_target_100m_contract_preflight_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "FIVE_HEAD_EPISODE_CONTROL_TARGET_100M_CONTRACT_PREFLIGHT_STAGE9690.md"
...
def main() -> None:
    ...
    guard_source = load_json(GUARD_SUMMARY)
    if source.get("passed") is not True:
        safety_failures.append("stage9665_not_passed")
    if guard_source.get("passed") is not True:
    ...
    expected_episode_losses = ["episode_boundary_match_ce", "episode_failure_type_ce", "episode_repair_outcome_ce", "episode_step_value_mse", "episode_target_prefix_match_ce"]
    for loss_name in expected_episode_losses:
        if loss_counts.get(loss_name) != 66:
            safety_failures.append(f"{loss_name}_count_not_66")
    ...
    audit = {
        ...
        "guard_summary": str(GUARD_SUMMARY.relative_to(ROOT)),
        "manifest": str(MANIFEST.relative_to(ROOT)),
    ...
    next_step = "Build Stage9691 execution-authorization review for a guarded five-head episode-control refresh probe, or attach this preflight as sufficient evidence and reconnect the observe/repair gate to residual denoise routing."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Validated the promoted five-head visible episode observe/repair control manifest under locked-source guard and target-100M contract-only trainer preflight.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9690 Five-Head Episode Control Target-100M Contract Preflight",
        "",
        ...
        "",
        "This is contract-only. No model forward/backward, decoder CE, denoise CE, runtime, Gemma, harness, scoring, source/body emission, checkpoint export, or promotion is authorized.",
        "",
        f"Next: {next_step}",
        "",
    ]), encoding="utf-8")
