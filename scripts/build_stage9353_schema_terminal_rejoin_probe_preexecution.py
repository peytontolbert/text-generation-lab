...
STAGE = 9353
NAME = "stage9353_schema_terminal_rejoin_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9352_full_mixture_with_schema_and_terminal_repairs_manifest.json"
...
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9354_schema_terminal_rejoin_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "schema_terminal_rejoin_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9354_schema_terminal_rejoin_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "SCHEMA_TERMINAL_REJOIN_PROBE_PREEXECUTION_STAGE9353.md"
...
COMMAND = [
    ...
    "--max-train-rows", "65", "--max-eval-rows", "36", "--max-strict-rows", "21",
    "--max-steps", "100", "--batch-size", "2", "--learning-rate", "1e-5",
    ...
    "--enable-generation-audit", "--max-generation-rows", "122", "--max-generation-tokens", "64",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9354_schema_terminal_rejoin_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9352_not_passed")
    if len(rows) != 122:
    ...
    if dict(source_counts) != {"base_schema_normalized": 74, "operator_terminal_repair": 17, "schema_bridge_repair": 31}:
...
def main() -> None:
    ...
    summary = {
        ...
        "metrics": {**{key: bool(card["authority"].get(key, False)) for key in AUTHORITY_CLOSED}, "command_ready": card["command_ready"], "manifest_rows": card["manifest_rows"], "split_counts": card["split_counts"], "source_counts": card["source_counts"], "failures": len(card["failures"])},
        "artifacts": {"audit": str(AUDIT.relative_to(ROOT)), "command": str(COMMAND_JSON.relative_to(ROOT)), "doc": str(DOC.relative_to(ROOT))},
        "decision": "Prepared a closed-boundary 122-row schema-normalized full-mixture rejoin denoise probe; decoder CE remains closed.",
        "next_best_step": "Execute Stage9354 schema-terminal rejoin probe under trellis and audit whether the full mixture is clean enough to approach bounded decoder CE again.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9353 Schema Terminal Rejoin Probe Preexecution", "", f"Passed: `{card['passed']}`", f"Rows: `{card['manifest_rows']}`", f"Splits: `{card['split_counts']}`", f"Sources: `{card['source_counts']}`", "", "Only the Stage9354 122-row target-100M denoise probe is authorized. Decoder CE and external authority remain closed.", ""]), encoding="utf-8")
