...
STAGE = 11029
NAME = "stage11029_reviewed_multilingual_next_root_bundle_resolved"
OUT_DIR = ARTIFACTS / NAME
SUMMARY_JSON = OUT_DIR / "reviewed_multilingual_next_root_bundle_resolved.json"
...
def load_specs() -> list[dict[str, Any]]:
    ...
    for row in requests:
        ...
        if atlas_row is None or not atlas_row.get("packet_dir"):
            continue
        specs.append(
            {
...
def enrich_row(spec: dict[str, Any], row: dict[str, Any], *, candidate: bool) -> dict[str, Any]:
    updated = dict(row)
    updated["bundle_id"] = f"stage11029::{spec['queue_id']}::reviewed_multilingual_next_root"
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "reviewed_multilingual_next_root_bundle_materialized_where_resolvable",
        "claim_scope": [
            ...
            "Emit the resolvable subset immediately while recording unresolved reviewed roots as explicit blockers rather than failing the whole bundle.",
        ],
        "source_artifacts": {
        ...
        "metrics": {
            ...
            "skipped_specs": len(skipped_specs),
            "by_language": {
        ...
        "findings": [
            "MirrorMind, agentkernel C/C++, and parametergolf C/C++ resolve cleanly into concrete multilingual next-root artifacts.",
            "The code_assist web control root is still missing from the current source-row corpus, so it remains an explicit unresolved blocker rather than silently disappearing.",
