...
STAGE = 9318
NAME = "stage9318_combined_suffix_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9317_combined_suffix_probe_preexecution.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9316_combined_suffix_curriculum_manifest/combined_suffix_curriculum_manifest.jsonl"
RUN_DIR = ROOT / "runs/local/artifacts/stage9318_combined_suffix_probe"
AUDIT = RUN_DIR / "stage9318_combined_suffix_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "COMBINED_SUFFIX_PROBE_AUDIT_STAGE9318.md"
...
def _bucket(samples: list[dict[str, Any]], manifest_by_id: dict[str, dict[str, Any]], key: str) -> dict[str, dict[str, Any]]:
    ...
    for sample in samples:
        ...
        if key == "source":
            value = str(meta.get("combined_curriculum_source") or "unknown")
        elif key == "variant":
...
def audit_run() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9317_not_passed")
    ...
    failed_samples = []
    phrase_error_counts = {"patch_ins_fragment": 0, "verified_patch_contamination": 0, "pator_fragment": 0, "constant_to_localizhed": 0}
    for sample in samples:
        if sample.get("exact_match"):
            continue
        meta = manifest_by_id.get(str(sample.get("row_id")), {})
        generated_text = str(sample.get("generated_text") or "")
        if "patch ins" in generated_text:
            phrase_error_counts["patch_ins_fragment"] += 1
        if "verified patch" in generated_text and "dependency handle" in generated_text:
            phrase_error_counts["verified_patch_contamination"] += 1
        if "pator" in generated_text:
            phrase_error_counts["pator_fragment"] += 1
        if "localizhed" in generated_text:
            phrase_error_counts["constant_to_localizhed"] += 1
        failed_samples.append({
            "row_id": sample.get("row_id"),
            "source": meta.get("combined_curriculum_source"),
            "variant": meta.get("repair_task_type"),
            "split": sample.get("split"),
            "prefix_words": (meta.get("model_input") or {}).get("active_generation_prefix_words") if isinstance(meta.get("model_input"), dict) else None,
    ...
    quality_gate_passed = bool(generated == 44 and exact == generated and target_prefix == generated and boundary == generated and contentful == generated and repetition == 0)
    return {
        "passed": not failures,
        ...
        "by_source": _bucket(samples, manifest_by_id, "source"),
        "by_variant": _bucket(samples, manifest_by_id, "variant"),
        "by_prefix_words": _bucket(samples, manifest_by_id, "prefix"),
        "by_split": _bucket(samples, manifest_by_id, "split"),
        "phrase_error_counts": phrase_error_counts,
        "failed_samples": failed_samples,
        "diagnosis": "combined_suffix_curriculum_is_safe_but_quality_regressed_from_curriculum_interference_around_patch_inside_and_verified_patch_operator",
        "next_patch_target": "stage9319_source_balanced_phrase_completion_curriculum_for_patch_inside_and_patch_operator",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Safety passed, but the combined suffix curriculum did not preserve the isolated lexical bridge quality.",
        "next_best_step": "Build Stage9319 source-balanced phrase-completion curriculum focused on `patch inside`, `patch operator`, and constant-preserves continuations before rerunning a combined probe.",
    ...
    DOC.write_text("\n".join([
        ...
        f"Phrase error counts: `{audit['phrase_error_counts']}`",
    ...
    print(json.dumps({"stage": STAGE, "passed": summary["passed"], "metrics": {k: audit[k] for k in ["safety_gate_passed", "quality_gate_passed", "exact_match_rate", "target_prefix_match_rate", "boundary_next_token_match_rate", "contentful_rate", "phrase_error_counts"]}}, indent=2, sort_keys=True))
