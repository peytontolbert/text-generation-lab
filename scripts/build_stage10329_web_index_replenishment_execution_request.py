...
STAGE = 10329
NAME = 'stage10329_web_index_replenishment_execution_request'
OUT_DIR = ROOT / 'runs/local/artifacts' / NAME
TRAIN_ROWS_JSONL = OUT_DIR / 'web_index_replenishment_train_rows.jsonl'
MANIFEST_JSONL = OUT_DIR / 'web_index_replenishment_manifest.jsonl'
REQUEST_JSON = OUT_DIR / 'web_index_replenishment_execution_request.json'
COMMAND_JSON = OUT_DIR / 'web_index_replenishment_command.json'
SUMMARY = ROOT / 'runs/summaries' / f'{NAME}.json'
BUNDLE_JSON = OUT_DIR / 'web_index_replenishment_bundle_manifest.json'
...
RUN_ID = 'stage10330_web_index_replenishment_probe'
OUTPUT_DIR = 'runs/local/artifacts/stage10330_web_index_replenishment_probe/bounded_decoder_probe'
RUNTIME_MODEL_DIR = 'runs/local/artifacts/stage10330_web_index_replenishment_probe/runtime_model'
MAX_STEPS = 64
LEARNING_RATE = '8e-6'
BATCH_SIZE = 2
PERMUTATION_COUNT = 3
...
TASK_ROWS = [
    {
        ...
        'task': 'Choose the most likely entrypoint or edit target from the visible web maintenance evidence.',
    },
    {
    ...
    },
    {
        ...
        'answer_kind': 'candidate_path',
        'gold_value': 'index.html',
        'task': 'Compare candidate edits by likely behavior change and choose the one most directly tied to the visible page-entry surface.',
    },
    {
        'perspective': 'minimal_fix_selection',
        'answer_kind': 'candidate_path',
        'gold_value': 'index.html',
        'task': 'Choose the smallest maintainable intervention supported by the visible build and page-entry evidence.',
    },
]
...
def bundle_evidence() -> dict[str, list[dict[str, Any]]]:
    return {
        'candidate_change_surface': [
            {
                'path': 'index.html',
                'source_type': 'local_repo',
                'retrieval_reason': 'fresh_bddy_index_seed_change',
                'distance_from_seed': 0,
                'text': excerpt(REPO_ROOT / 'index.html', '<script', radius=22),
            },
        ],
        'verifier_and_test_constraint': [
            {
                'path': 'package.json',
                'source_type': 'local_repo',
                'retrieval_reason': 'build_entry_constraint',
                'distance_from_seed': 1,
                'text': excerpt(REPO_ROOT / 'package.json', '"build"', radius=16),
            },
            {
                'path': 'vite.config.js',
                'source_type': 'local_repo',
                'retrieval_reason': 'vite_build_constraint',
                'distance_from_seed': 1,
                'text': excerpt(REPO_ROOT / 'vite.config.js', 'defineConfig', radius=18),
            },
        ],
        'nearby_definition_or_usage_context': [
            {
                'path': 'src/main.js',
                'source_type': 'local_repo',
                'retrieval_reason': 'implementation_neighbor',
                'distance_from_seed': 1,
                'text': excerpt(REPO_ROOT / 'src/main.js', 'class App', radius=22),
            },
            {
                'path': 'package.json',
                'source_type': 'local_repo',
                'retrieval_reason': 'build_neighbor',
                'distance_from_seed': 1,
                'text': excerpt(REPO_ROOT / 'package.json', '"dev"', radius=16),
            },
        ],
    }
...
def compile_prompt(root_id: str, evidence: dict[str, list[dict[str, Any]]], task: dict[str, str], options: list[tuple[str, str]]) -> str:
    parts = [
        'Language: web_js_ts_html',
        f'Perspective: {task["perspective"]}',
        f'Task: {task["task"]}',
        f'SourceRoot: {root_id}',
        'Evidence:',
    ]
    for key in VISIBLE_EVIDENCE_KEYS:
        for item in evidence.get(key, [])[:2]:
...
def compile_train_rows(evidence: dict[str, list[dict[str, Any]]]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for root_id in SOURCE_ROOTS:
        bundle_id = f'stage10329::{root_id}::web_js_ts_html'
        for task in TASK_ROWS:
            values = build_options(task['answer_kind'], task['gold_value'])
            labels = list('ABCDE')[: len(values)]
            for shift in range(min(PERMUTATION_COUNT, len(values))):
                ...
                prompt = compile_prompt(root_id, evidence, task, options)
                rows.append({
                    'row_id': f"{bundle_id}::{task['perspective']}::compact_bounded::perm_{shift:02d}",
                    'semantic_key': f"web_index_replenishment::{root_id}::{task['perspective']}::perm_{shift:02d}",
                    'bundle_id': bundle_id,
                    ...
                    'query_text': f"compact_bounded::web_js_ts_html::{task['perspective']}::{task['answer_kind']}::perm_{shift:02d}",
                    'prompt_text': prompt,
                    'input_text': prompt,
...
def main() -> None:
    ...
    request = {
        ...
        'required_honesty_gates': [
            ...
            'fresh web rows are train-support-only and excluded from headline same-surface Gemma claims',
            'web support rows use concrete repo snippets from /data/bddy/bddy_website rather than abstract templates',
            'no verifier/test claims are added because these fresh web roots still have no real selected tests',
        ],
        'next_best_step': 'run one frontloaded warm-start probe and check whether the unresolved web index.html heldout family moves without regressing Python or C/C++',
        'command': command,
