...
STAGE = 10961
NAME = "stage10961_clean_explicit_ledger_postrun_audit"
OUT_DIR = ARTIFACTS / NAME
SUMMARY_JSON = OUT_DIR / "clean_explicit_ledger_postrun_audit.json"
...
OVERLAY_EVAL_JSON = ARTIFACTS / "stage10960_clean_explicit_ledger_support_probe" / "bounded_decoder_probe" / "bounded_choice_eval_audit_eval.json"
OVERLAY_STRICT_JSON = ARTIFACTS / "stage10960_clean_explicit_ledger_support_probe" / "bounded_decoder_probe" / "bounded_choice_eval_audit_strict_eval.json"
...
def main() -> None:
    ...
    for row in row_cards:
        ...
        new_match = row.get("constrained_choice_match")
        base_match = base.get("constrained_choice_match")
        if new_match != base_match:
            changed += 1
            if new_match is True and base_match is not True:
                improved += 1
            elif new_match is not True and base_match is True:
                regressed += 1
        combined.append(
            {
                ...
                "new_constrained_choice_match": new_match,
                ...
                "baseline_constrained_choice_match": base_match,
    ...
    summary = {
        ...
        "passed": True,
        "claim_scope": [
            "Audit the stage10960 runtime against the unchanged 23-row overlay and the fresh 3-row evidence successor slice.",
            "Measure whether the anti-cheat-clean explicit-ledger support improved the direct B-vs-F successor rows without introducing new overlay regressions.",
        ],
        "runtime_bundle": rel(RUNTIME_BUNDLE),
        "runtime_initialization": init_card,
        "anticheat_prerequisite": {
            "path": rel(ANTICHEAT_JSON),
            "passed": anticheat.get("passed"),
        },
        "overlay": {
        ...
        "findings": [
            "A real positive result requires fresh-slice improvement beyond the baseline while preserving the 22/23 strict overlay.",
            "If the fresh 3-row slice stays flat, the cleaned explicit-ledger geometry improved audit quality but not the remaining evidence decision boundary.",
        ],
        "outputs": {
