...
STAGE = 9937
NAME = "stage9937_weighted_winner_signoff_sheets"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
MANIFEST = OUT_DIR / "weighted_winner_signoff_sheets.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "WEIGHTED_WINNER_SIGNOFF_SHEETS_STAGE9937.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
CLOSEOUT = ROOT / "runs/local/artifacts/stage9936_v27_closeout_packet/v27_closeout_packet.json"
...
def _sheet_text(row: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {lang} Weighted Winner Signoff Sheet",
        "",
        f"Standalone weighted same-surface result: `100M {standalone.get('strict_exact_100m')} vs Gemma {standalone.get('strict_exact_gemma')}`",
...
def main() -> None:
    ...
    next_step = "Use the four signoff_sheet.md files to complete the remaining 8 human review tasks, then pass the referenced harness handoff bundles to the external runtime owner."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Materialized four plain-language signoff sheets, one per required language, so the remaining human review and external handoff work can be executed from local packet dirs without navigating raw JSON artifacts.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9937 Weighted Winner Signoff Sheets",
        "",
