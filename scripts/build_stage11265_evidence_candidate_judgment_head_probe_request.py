...
STAGE = 11265
NAME = "stage11265_evidence_candidate_judgment_head_probe_request"
OUT_DIR = ARTIFACTS / NAME
SUMMARY_JSON = OUT_DIR / "evidence_candidate_judgment_head_probe_request.json"
COMMAND_JSON = OUT_DIR / "evidence_candidate_judgment_head_probe_command.json"
MANIFEST = OUT_DIR / "evidence_candidate_judgment_head_probe_manifest.jsonl"
...
JUDGMENT_PACKAGE = ARTIFACTS / "stage11259_true_evidence_candidate_judgment_package"
JUDGMENT_SUMMARY = JUDGMENT_PACKAGE / "true_evidence_candidate_judgment_package.json"
JUDGMENT_AUDIT = ARTIFACTS / "stage11260_true_evidence_candidate_judgment_audit/true_evidence_candidate_judgment_audit.json"
JUDGMENT_TRAIN = JUDGMENT_PACKAGE / "true_evidence_candidate_judgment_train_rows.jsonl"
JUDGMENT_VALIDATION = JUDGMENT_PACKAGE / "true_evidence_candidate_judgment_validation_rows.jsonl"
JUDGMENT_STRICT = JUDGMENT_PACKAGE / "true_evidence_candidate_judgment_strict_rows.jsonl"
...
RUN_DIR = ARTIFACTS / "stage11266_evidence_candidate_judgment_head_probe"
...
def main() -> None:
    ...
    if not package.get("passed"):
        raise SystemExit("Stage11259 package did not pass")
    if not audit.get("passed"):
        raise SystemExit("Stage11260 audit did not pass")
    ...
    summary = {
        ...
        "passed": True,
        "decision": "evidence_candidate_judgment_head_probe_requested",
        "rationale": [
            "Stage11259 changes the supervision geometry to one candidate evidence item per row; this request tests a dedicated judgment-head scorer for that surface.",
            "Stage11260 found no target-label, target-value, role-alias, root-overlap, or source-row-overlap blockers.",
            "This is diagnostic: C/C++ supply is absent and Rust/Web supply is small. The new scorer head should learn Stage11259 while falling back to verifier-conditioned scoring for existing canary/residual rows.",
        ],
        "metrics": {
            ...
            "judgment_audit_counts": audit.get("counts"),
        },
        "promotion_gate_for_postrun": {
            ...
            "judgment_validation_or_strict_should_score_above_majority_baseline": True,
        },
        "source_artifacts": {
