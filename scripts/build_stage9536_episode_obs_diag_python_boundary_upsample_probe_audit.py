...
STAGE = 9536
NAME = "stage9536_episode_obs_diag_python_boundary_upsample_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9535_episode_obs_diag_python_boundary_upsample_preflight_audit.json"
RUN_DIR = ROOT / "runs/local/artifacts/stage9536_episode_obs_diag_python_boundary_upsample_target_100m_probe"
...
AUDIT = RUN_DIR / "stage9536_episode_obs_diag_python_boundary_upsample_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "EPISODE_OBS_DIAG_PYTHON_BOUNDARY_UPSAMPLE_PROBE_AUDIT_STAGE9536.md"
...
def main() -> None:
    ...
    if source.get("passed") is not True or not source.get("metrics", {}).get("execution_authorized_for_next_stage"):
        safety_failures.append("stage9535_not_authorized")
    ...
    audit = {
        ...
        "regressed_vs_stage9532": (final_eval.get("joint_proxy_exact") or 0.0) < 0.8333333333333334,
        "failures": failures,
        "safety_failures": safety_failures,
        "quality_failures": quality_failures,
        "source_stage": "stage9535_episode_obs_diag_python_boundary_upsample_preflight_audit",
        ...
        "diagnosis": "Safe severe regression. Train-only Python boundary upsampling overcorrected the failure-type head and should not be promoted.",
        "next_repair_target": "Revert to Stage9525/9528 residual-gate base. Do not use Stage9529 boundary-component or Stage9533 upsample branches as bases; use calibration/threshold analysis or collect genuinely new non-duplicated Python boundary examples.",
    ...
    summary = {
        ...
        "decision": "Python boundary upsample probe regressed severely; keep Stage9528 as the best current base and reject the upsample branch.",
        "next_best_step": "Return to Stage9525/9528 residual-gate base and perform calibration/error analysis before adding more data or features.",
