#!/usr/bin/env python3
"""Materialize hyper-connections Python selected-verifier support rows."""
...
STAGE = 11861
NAME = "stage11861_hyper_connections_python_support_rows"
OUT = ART / NAME
SUMMARY = OUT / "hyper_connections_python_support_rows.json"
ROWS = OUT / "hyper_connections_python_support_rows.jsonl"
FEASIBILITY = ART / "stage11860_hyper_connections_python_feasibility/hyper_connections_python_feasibility_results.jsonl"
...
def base_row(root_id: str, suffix: str, task_type: str, options: list[dict[str, Any]], target_label: str) -> dict[str, Any]:
    ...
    return {
        ...
        "source_snapshot_id": "hyper_connections::local_git_snapshot",
...
def main() -> None:
    feasible = [row for row in read_jsonl(FEASIBILITY) if row.get("repo_family") == "hyper_connections" and row.get("passed") is True]
    if not feasible:
        raise SystemExit("hyper-connections feasibility did not pass")
    probe = feasible[0]
    root_id = "stage11861::hyper_connections::python::manual_hyper_connection_support"
    evidence = [
        file_evidence("E01", "implementation_source", probe["source_path"], "Hyper-connections source defines stream expansion, learned residual weighting, and branch decoration behavior."),
        file_evidence("E02", "selected_test_anchor", probe["selected_test_path"], "Tests cover manual hyper-connection use and residual-transform/channel-first variants."),
        file_evidence("E03", "nearby_source_distractor", probe["distractor_source_path"], "Channel-first hyper-connection variant is related but not the selected manual verifier target."),
        log_evidence("V01", probe["selected_result"], "Selected manual hyper-connection pytest passed."),
        log_evidence("V02", probe["sibling_result"], "Sibling residual-transform pytest passed."),
    ]
    verifier_evidence = {
        "id": "V01",
        "kind": "actual_pytest_pass_log",
        "summary": "Selected and sibling hyper-connections pytest functions passed.",
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "tenth_python_verifier_support_root_materialized",
        "passed": True,
        "row_count": len(rows),
        "root_count": 1,
        "repo_family": "hyper_connections",
