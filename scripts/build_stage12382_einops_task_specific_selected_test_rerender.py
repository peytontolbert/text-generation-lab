...
STAGE = "stage12382_einops_task_specific_selected_test_rerender"
OUT = ROOT / "runs/local/artifacts" / STAGE
SUMMARY = ROOT / "runs/summaries" / f"{STAGE}.json"
SOURCE = ROOT / "runs/local/artifacts/stage12360_review_corrected_mixed_selected_test_decision/stage12360_admitted_einops_only_rows.jsonl"
TASKS = [
...
def refs(input_text: str) -> list[str]:
    ...
    try:
        ...
        values = [raw]
    return [value for value in values if isinstance(value, str) and "::None" not in value]


def task_specs(task: str, selected: str) -> tuple[list[tuple[str, str]], str, str]:
    if task == "transition_candidate_selection":
        return (
            [
                ("candidate_selected_test_backed", "candidate is supported by focused Python verifier evidence"),
                ("candidate_source_surface_only", "candidate relies on source hashes without verifier support"),
            ...
            "Select the candidate whose support is grounded by the focused selected-test record.",
        )
    if task == "transition_next_action":
        return (
            [
                ("action_run_selected_verifier", "run or rely on the focused Python verifier before claiming support"),
    ...
    if task == "transition_continue_or_stop":
        return (
            [
                ("policy_stop_selected_verifier_passed", f"stop/support is justified after focused verifier evidence is present: {selected}"),
    ...
    if task == "transition_verifier_transition":
        return (
            [
                ("status_pass_current_state", "focused local Python verifier passed in the current state"),
    ...
    if task == "transition_evidence_citation":
        return (
            [
                ...
                ("evidence_unselected_neighbor", "neighboring Python artifact is decisive"),
            ],
...
def options_for(task: str, root_id: str, selected: str) -> tuple[list[dict[str, Any]], str]:
    specs, target, _ = task_specs(task, selected)
    rotations = {
...
def render(row: dict[str, Any], task: str, options: list[dict[str, Any]], safe_refs: list[str]) -> str:
    original = row.get("input_text") or ""
    selected = field(original, "Selected-test evidence:")
    command_ref = field(original, "Verifier command class:").split("command_ref=")[-1].strip()
    ...
    return (
        ...
        f"Focused verifier class: focused local verifier; exit_code=0\n"
        f"Focused verifier command ref: command_sha256::{command_ref or digest(field(original, 'Verifier command class:'))[:16]}\n"
        ...
        f"Evidence refs: {json.dumps(safe_refs[:10], sort_keys=True)}\n"
        f"Question: {question}\n"
        "Choose the task-specific option. Labels are opaque.\n"
        "Options:\n"
        + "\n".join(f"- {option['label']}: Option {option['label']}" for option in options)
        + "\nAnswer:"
    )


def blockers(row: dict[str, Any], safe_refs: list[str]) -> list[str]:
    out = []
...
def main() -> None:
    ...
    for source_row in source_rows:
        ...
        options, target = options_for(task, source_row.get("root_id") or "", selected)
        target_semantic = next(option["semantic_id"] for option in options if option["label"] == target)
        safe_refs = refs(source_row.get("input_text") or "")
        row_blockers = blockers(source_row, safe_refs)
        ...
        rendered = {
            ...
            "source_stage": "stage12360_review_corrected_mixed_selected_test_decision",
            "repo_family": source_row.get("repo_family"),
            "language_family": "python",
            "task_family": task,
            "input_text": render(source_row, task, options, safe_refs),
            ...
            "admission": {
                ...
                "reason": "Task-specific einops selected-test train-support only; no strict/source-heldout/Level-3/patch/repair claim.",
            },
            "blocked_reasons": row_blockers,
        }
        (admitted if not row_blockers else blocked).append(rendered)
    write_jsonl(OUT / "einops_task_specific_selected_test_rows.jsonl", admitted)
    write_jsonl(OUT / "einops_task_specific_selected_test_blocked_rows.jsonl", blocked)
    summary = {
        "stage": STAGE,
        "decision": "einops_task_specific_selected_test_rerender_complete" if admitted else "einops_task_specific_selected_test_rerender_blocked",
        "training_allowed": bool(admitted),
        "claim_boundary": "Train-support-only einops task-specific selected-test re-render. No strict/source-heldout/Level-3/patch/repair claim.",
    ...
    (OUT / "einops_task_specific_selected_test_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
