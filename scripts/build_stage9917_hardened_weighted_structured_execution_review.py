...
STAGE = 9917
NAME = "stage9917_hardened_weighted_structured_execution_review"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9916_v27_hardened_edit_localization_weighted_compiler_refresh.json"
SOURCE_STRUCTURED = ROOT / "runs/local/artifacts/stage9916_v27_hardened_edit_localization_weighted_compiler_refresh/compiled/structured_state.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
TINY_DIR = OUT_DIR / "tiny_structured_manifests"
RUNS_DIR = OUT_DIR / "surface_runs"
AUDIT = OUT_DIR / "hardened_weighted_structured_execution_review_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "HARDENED_WEIGHTED_STRUCTURED_EXECUTION_REVIEW_STAGE9917.md"
...
SURFACES = {
    "symbol_binding": {"mode": "symbol_binding_probe", "loss": "symbol_binding_ce", "max_steps": 8, "eval_interval": 0, "restore_best": False},
    "edit_localization": {"mode": "edit_localization_probe", "loss": "edit_localization_ce", "max_steps": 32, "eval_interval": 1, "restore_best": True},
    "patch_operator_selection": {"mode": "patch_operator_probe", "loss": "patch_operator_ce", "max_steps": 8, "eval_interval": 0, "restore_best": False},
    "verifier_failure_repair_or_abstain": {"mode": "verifier_repair_probe", "loss": "verifier_repair_ce", "max_steps": 8, "eval_interval": 0, "restore_best": False},
}
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9916_not_passed")
    surface_results = []
    if not failures:
    ...
    next_step = "Compare this weighted hardened review against the Stage9914 hardened baseline to see whether doubled train pressure plus the longer edit-localization schedule finally lifts integrated exactness."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Executed a weighted hardened structured review that keeps the anti-cheat-safe edit-localization source, increases only its train pressure, and gives that surface the longer best-state schedule that earlier standalone wins needed.",
