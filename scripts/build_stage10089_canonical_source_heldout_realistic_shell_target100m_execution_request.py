...
STAGE = 10089
NAME = "stage10089_canonical_source_heldout_realistic_shell_target100m_execution_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
REQUEST = OUT_DIR / "canonical_source_heldout_realistic_shell_target100m_execution_request.json"
SURFACE_REQUEST = OUT_DIR / "surface_requests" / "edit_localization.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "CANONICAL_SOURCE_HELDOUT_REALISTIC_SHELL_TARGET100M_EXECUTION_REQUEST_STAGE10089.md"
...
SOURCE_SUMMARY = ROOT / "runs/summaries/stage10088_canonical_source_heldout_realistic_maintenance_shell.json"

FUTURE_RUN_ID = "stage10089_canonical_source_heldout_realistic_shell_target100m_probe"
FUTURE_OUTPUT_DIR = "runs/local/artifacts/stage10089_canonical_source_heldout_realistic_shell_target100m_probe/edit_localization_probe"
...
def build_request() -> dict[str, Any]:
    ...
    metrics = {"rows": len(rows), "language_counts": dict(sorted(language_counts.items())), "split_counts": dict(sorted(split_counts.items())), "heldout_compare_rows": heldout_rows, "l2_rows": l2_rows}
    if metrics["rows"] != 95:
    ...
    if l2_rows != 95:
        failures.append("l2_rows_not_95")
    if source_summary.get("passed") is not True:
        failures.append("stage10088_not_passed")
    req = {
        ...
        "required_contract_invariants": {
            "source_stage10088_passed": bool(source_summary.get("passed") is True),
            "same_compare_rows_preserved": heldout_rows,
            "l2_shell_rows_present": l2_rows,
        },
    ...
    return {"passed": not failures, "failures": failures, "metrics": metrics, "source_artifacts": {"stage10088_packet_summary": display(SOURCE_SUMMARY)}, "surface_requests": [{**req, "request_path": display(SURFACE_REQUEST)}], "authority": dict(AUTHORITY_CLOSED)}
...
def main() -> None:
    ...
    next_step = "Run one target-100m edit-localization probe on the stage10088 realistic-maintenance shell, then execute the matching same-manifest Gemma queue."
    summary = {"stage": STAGE, "stage_name": NAME, "name": NAME, "passed": built["passed"], "authority": dict(AUTHORITY_CLOSED), "metrics": {**dict(AUTHORITY_CLOSED), **built["metrics"], "failures": built["failures"]}, "artifacts": {"request": display(REQUEST), "surface_request": display(SURFACE_REQUEST), "doc": display(DOC)}, "decision": "Prepared the execution request for the stage10088 realistic-maintenance shell so the next probe can test whether the current 100M survives a richer L2-style same-manifest surface without changing row composition.", "next_best_step": next_step, "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage10089 Canonical Source-Heldout Realistic Shell Target100M Execution Request", "", f"Passed: `{summary['passed']}`", "", summary["decision"], "", f"Next: {next_step}", ""]), encoding="utf-8")
    if summary["passed"]:
