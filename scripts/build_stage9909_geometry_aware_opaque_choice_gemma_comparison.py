...
STAGE = 9909
NAME = "stage9909_geometry_aware_opaque_choice_gemma_comparison"
...
ROWS_100M = ROOT / "runs/local/artifacts/stage9908_direct_geometry_aware_opaque_choice_exec/row_field_logits.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
ROWS_OUT = OUT_DIR / "geometry_aware_opaque_choice_gemma_rows.jsonl"
AUDIT = OUT_DIR / "geometry_aware_opaque_choice_gemma_comparison.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "GEOMETRY_AWARE_OPAQUE_CHOICE_GEMMA_COMPARISON_STAGE9909.md"
...
_row_text = _load_symbol("stage9909_training_data", ROOT / "legacy_src/agentkernel_lite/training_data.py", "_row_text")
...
def main() -> None:
    ...
    next_step = "Use this hardened opaque-choice same-surface comparison as the truthful multilingual edit-localization benchmark. If the 100M margin survives, promote it; if not, change the head/objective before claiming multilingual superiority."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Compared local Gemma against the hardened geometry-aware opaque-choice packet using Stage9908 row-level 100M outputs and a prompt surface that no longer prints the valid-label list.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9909 Geometry-Aware Opaque Choice Gemma Comparison",
        "",
