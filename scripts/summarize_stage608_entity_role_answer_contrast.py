...
CAND = ROOT / "runs/local/artifacts/knowledge_compression_moe_residual_10k_stage608_stage602_entity_role_answer_contrast_lr1e6_steps200"
OUT = ROOT / "runs/local/artifacts/stage608_entity_role_answer_contrast_summary.json"
DOC = ROOT / "docs/stage608_entity_role_answer_contrast.md"
...
def build_summary() -> dict[str, Any]:
    ...
    return {
        "artifact_kind": "stage608_entity_role_answer_contrast_summary",
        "candidate": "stage608_stage602_entity_role_answer_contrast_lr1e6_steps200",
        ...
        "training": {
            ...
            "learning_rate": 1e-6,
            "retrieval_answer_contrastive_weight": 0.02,
        ...
        "candidate_stage608": compact(cand_no, cand_hard),
        ...
        "decision": "rejected_answer_contrast_on_role_tokens",
        "finding": (
            "Entity answer contrast on the role-token schema moves in the wrong direction: exact, answer, entity_context, "
            "direct_fact, and rule intersections all fall versus the Stage602 schema baseline, while hard-filter corrections "
            "increase. The answer-directed path should not use the existing broad answer-contrast loss on entity_context."
        ),
    }
...
def write_doc(summary: dict[str, Any]) -> None:
    ...
    cand = summary["candidate_stage608"]
    entity = summary["operation_deltas"]["entity_context"]
    doc = f"""# Stage608 Entity Role Answer Contrast
...
Stage608 exact/answer: `{cand['exact_top1']}` / `{cand['answer_top1']}`.
