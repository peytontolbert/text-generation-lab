...
STAGE = 9861
NAME = "stage9861_validity_weighted_disk_safe_cleanup_execution_readiness_gate"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9860_capped_validity_weighted_structured_target_100m_contract_preflight.json"
STAGE9859_TICKET = ROOT / "runs/local/artifacts/stage9859_validity_weighted_structured_tiny_execution_review/multisurface_structured_tiny_execution_review_inactive.json"
STAGE9860_RUNS = ROOT / "runs/local/artifacts/stage9860_capped_validity_weighted_structured_target_100m_contract_preflight/contract_runs"
...
DOC = ROOT / "docs" / "VALIDITY_WEIGHTED_DISK_SAFE_CLEANUP_EXECUTION_READINESS_GATE_STAGE9861.md"
...
FUTURE_STAGE = 9862
FUTURE_RUN_ID = "stage9862_symbol_binding_target_100m_structured_tiny_probe"
FUTURE_OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9862_symbol_binding_target_100m_structured_tiny_probe/symbol_binding_probe"
FUTURE_MANIFEST = "runs/local/artifacts/stage9859_validity_weighted_structured_tiny_execution_review/tiny_structured_manifests/symbol_binding_tiny.jsonl"
...
def runner_python() -> str:
    return str(PREFERRED_PYTHON if PREFERRED_PYTHON.exists() else Path("python"))
...
def safe_cleanup_dry_runs() -> tuple[list[dict[str, Any]], list[str]]:
    ...
    failures: list[str] = []
    for surface_dir in sorted(path for path in STAGE9860_RUNS.iterdir() if path.is_dir()):
...
def future_command_checks(cmd: list[str]) -> tuple[dict[str, Any], list[str]]:
    ...
    if f"stage{STAGE}_" in text or "stage9860_" in text:
        failures.append("future_command_reuses_readiness_or_contract_stage_namespace")
        checks["future_command_uses_fresh_stage9862_namespace"] = False
    else:
        checks["future_command_uses_fresh_stage9862_namespace"] = "stage9862_" in text
        if "stage9862_" not in text:
            failures.append("future_command_missing_stage9862_namespace")
    return checks, failures
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9860_not_passed")
    ...
    stale_stage9859_namespace_rows = []
    for surface, old_cmd in (ticket.get("surface_commands") or {}).items():
        if "stage9860_" in " ".join(str(part) for part in old_cmd):
            stale_stage9859_namespace_rows.append(surface)
    candidate = {
    ...
    readiness = {
        ...
        "stale_stage9859_future_command_surfaces": stale_stage9859_namespace_rows,
        "selected_first_surface_candidate": str(FIRST_SURFACE.relative_to(ROOT)),
        "source_stage9860_summary": str(SOURCE_SUMMARY.relative_to(ROOT)),
        "authority": dict(AUTHORITY_CLOSED),
    }
    READINESS.write_text(json.dumps(readiness, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    next_step = "With explicit confirmation, run Stage9862 symbol-binding target-100M tiny structured execution using the Stage9861 candidate command; otherwise stop before execution or inspect readiness artifacts."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Validated disk and cleanup readiness for one future tiny structured target-100M execution candidate on the validity-weighted package, but did not authorize or run execution.",
