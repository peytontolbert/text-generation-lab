...
STAGE = 9436
NAME = "stage9436_suffix_choice_prior_fusion_denoise_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9435_suffix_choice_prior_fusion_denoise_manifest_audit.json"
...
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9437_suffix_choice_prior_fusion_denoise_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "suffix_choice_prior_fusion_denoise_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9437_suffix_choice_prior_fusion_denoise_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "SUFFIX_CHOICE_PRIOR_FUSION_DENOISE_PREEXECUTION_STAGE9436.md"
...
COMMAND = [
    ...
    str(OUTPUT_DIR),
    "--run-id",
    "stage9437_suffix_choice_prior_fusion_denoise_probe",
]
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9435_not_passed")
    if len(rows) != 53:
        failures.append("bad_manifest_row_count")
    if dict(splits) != {"eval": 5, "strict_eval": 4, "train": 44}:
    ...
    if routes != Counter({"USE_FOR_DENOISE_REPAIR_WITH_SUFFIX_CHOICE_PRIOR": 53}):
    ...
    summary = {
        ...
        "decision": "Prepared a 53-row suffix-choice-prior fusion denoise probe; decoder CE remains closed.",
        "next_best_step": "Execute Stage9437 suffix-choice-prior fusion denoise probe under trellis and audit whether heldout suffix recovery improves.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    DOC.write_text("\n".join(["# Stage9436 Suffix Choice Prior Fusion Denoise Preexecution", "", f"Passed: `{audit['passed']}`", f"Rows: `{len(rows)}`", f"Splits: `{dict(sorted(splits.items()))}`", f"Prior sources: `{dict(sorted(prior_sources.items()))}`", "", "Only the Stage9437 suffix-choice-prior fusion denoise probe is authorized. Decoder CE, runtime, Gemma, harness, scoring, and promotion remain closed.", ""]))
