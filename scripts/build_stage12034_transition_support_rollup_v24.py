#!/usr/bin/env python3
"""Roll Stage12033 PASS_TO_PASS rows into transition support v24."""
...
ROOT = Path("runs/local/artifacts/stage12034_transition_support_rollup_v24")
BASE = Path("runs/local/artifacts/stage12032_transition_support_rollup_v23/transition_support_rows_v23.jsonl")
ADDED = Path("runs/local/artifacts/stage12033_pass_to_pass_expansion_rows/pass_to_pass_expansion_rows.jsonl")
ROWS_OUT = ROOT / "transition_support_rows_v24.jsonl"
SUMMARY = ROOT / "transition_support_rollup_v24.json"
SUMMARY_MIRROR = Path("runs/summaries/stage12034_transition_support_rollup_v24.json")
...
def main() -> None:
    ...
    duplicates = 0
    for source, path in [("stage12032", BASE), ("stage12033", ADDED)]:
        for row in read_jsonl(path):
            out = dict(row)
            out["stage12034_source"] = source
            out["split"] = "train"
            out["split_role"] = "stage12034_transition_support_v24_train_support_only"
    ...
    summary = {
        ...
        "decision": "support_inventory_v24_not_train_ready" if not train_ready else "support_inventory_v24_floor_ready_requires_anti_cheat_audit",
        "reason": "PASS_TO_PASS coverage improved and Python/Web language floors closed, but Transition-Root-250 status floors and C/C++/Rust floors remain unmet."
        if not train_ready
        else "Floor counts are met; run anti-cheat and split audits before training.",
        "claim_boundary": "Support inventory only. No model promotion or Gemma comparison should be claimed from v24.",
        "next_stage_recommendation": {
            "stage": "stage12035_transition_root_250_floor_closure",
            "action": "Prioritize FAIL_TO_PASS, PASS_CURRENT_BUILD_AND_RUN, PASS_CURRENT_BUILD, NOT_EXERCISED, and INSUFFICIENT_EVIDENCE rows while closing C/C++ and Rust root floors.",
        },
    }
