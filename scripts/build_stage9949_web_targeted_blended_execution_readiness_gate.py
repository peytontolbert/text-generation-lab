...
STAGE = 9949
NAME = "stage9949_web_targeted_blended_execution_readiness_gate"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9948_web_targeted_blended_target100m_execution_request.json"
REQUEST = ROOT / "runs/local/artifacts/stage9948_web_targeted_blended_target100m_execution_request/web_targeted_blended_target100m_execution_request.json"
STAGE9946_RUNS = ROOT / "runs/local/artifacts/stage9946_web_targeted_blended_target100m_contract_preflight/contract_runs"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
READINESS = OUT_DIR / "web_targeted_blended_execution_readiness.json"
FIRST_SURFACE = OUT_DIR / "first_surface_execution_candidate_edit_localization.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "WEB_TARGETED_BLENDED_EXECUTION_READINESS_GATE_STAGE9949.md"
...
FUTURE_STAGE = 9950
FUTURE_STAGE_PREFIX = "stage9950_"
...
def _rewrite_stage9948_to_stage9950(text: str) -> str:
    return text.replace("stage9948_", FUTURE_STAGE_PREFIX)


def future_command() -> list[str]:
    row = _selected_surface_request()
    command = [str(item) for item in (row.get("command") or [])]
    return [_rewrite_stage9948_to_stage9950(item) for item in command]


def future_output_dir() -> Path:
    row = _selected_surface_request()
    return ROOT / _rewrite_stage9948_to_stage9950(str(row.get("output_dir") or ""))


def future_run_id() -> str:
    row = _selected_surface_request()
    return _rewrite_stage9948_to_stage9950(str(row.get("run_id") or ""))
...
def safe_cleanup_dry_runs() -> tuple[list[dict[str, Any]], list[str]]:
    ...
    failures: list[str] = []
    for surface_dir in sorted(path for path in STAGE9946_RUNS.iterdir() if path.is_dir()):
...
def negative_safety_checks() -> tuple[dict[str, Any], list[str]]:
    ...
    failures: list[str] = []
    for name, kwargs in {
        "reject_arxiv_output": {"repo_root": ROOT, "output_dir": Path("/arxiv"), "run_id": "stage9950"},
        "reject_data_output": {"repo_root": ROOT, "output_dir": Path("/data"), "run_id": "stage9950"},
        "reject_root_output": {"repo_root": ROOT, "output_dir": Path("/"), "run_id": "stage9950"},
    }.items():
        try:
...
def future_command_checks(cmd: list[str]) -> tuple[dict[str, Any], list[str]]:
    ...
    checks["future_command_uses_fresh_stage9950_namespace"] = "stage9950_" in text
    if "stage9950_" not in text:
        failures.append("future_command_missing_stage9950_namespace")
    if "stage9948_" in text or "stage9949_" in text:
        failures.append("future_command_reuses_request_or_gate_namespace")
        checks["future_command_reuses_request_or_gate_namespace"] = True
    else:
        checks["future_command_reuses_request_or_gate_namespace"] = False
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9948_not_passed")
    if request.get("passed") is not True:
        failures.append("stage9948_request_packet_not_passed")
    if not row:
    ...
    blend_invariants = {
        ...
        "targeted_web_refresh_preserved": int((row.get("language_counts") or {}).get("web_js_ts_html", 0) or 0) == 27,
        "request_status": row.get("request_status"),
    }
    if blend_invariants["edit_localization_rows"] != 72:
        failures.append("edit_localization_rows_not_72")
    if blend_invariants["edit_localization_web_rows"] != 27:
    ...
    candidate = {
        ...
        "command": cmd,
        "reason": "Run the blended edit-localization surface first because it carries the targeted web recovery rows and directly measures the multilingual weakness we are trying to close against Gemma-12B.",
    ...
    readiness = {
        ...
        "source_stage9948_summary": str(SOURCE_SUMMARY.relative_to(ROOT)),
        "blend_invariants": blend_invariants,
        "authority": dict(AUTHORITY_CLOSED),
    }
    READINESS.write_text(json.dumps(readiness, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    next_step = "With explicit confirmation, run Stage9950 edit-localization blended target-100M execution using the Stage9949 candidate command; otherwise stop before execution and keep the blended request packet as the authoritative handoff."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Validated disk, cleanup, namespace, and blended-manifest readiness for a first real blended target-100M execution candidate on edit localization, without authorizing or running the probe.",
