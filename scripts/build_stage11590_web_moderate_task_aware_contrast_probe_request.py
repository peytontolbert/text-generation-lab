...
STAGE = 11590
NAME = "stage11590_web_moderate_task_aware_contrast_probe_request"
OUT = ART / NAME
SUMMARY = OUT / "web_moderate_task_aware_contrast_probe_request.json"
TRAIN_ROWS = OUT / "web_moderate_task_aware_contrast_train_rows.jsonl"
STRICT_ROWS = OUT / "web_moderate_task_aware_contrast_successor_strict_rows.jsonl"
MANIFEST = OUT / "web_moderate_task_aware_contrast_probe_manifest.jsonl"
COMMAND = OUT / "web_moderate_task_aware_contrast_probe_command.json"
...
READINESS = ART / "stage11585_web_task_aware_contrast_readiness_audit/contrast_readiness_summary.json"
PROBE = ART / "stage11590_web_moderate_task_aware_contrast_probe/bounded_decoder_probe"
RUNTIME_OUT = ART / "stage11590_web_moderate_task_aware_contrast_probe/runtime_model"
...
def main() -> None:
    ...
    readiness = load_json(READINESS) if READINESS.exists() else {}
    gates = {
        "stage11585_ready": readiness.get("decision") == "task_aware_contrast_ready",
        "source_training_authorized": len(train) == 156,
        "successor_strict_available": len(strict_successor) == 36,
        ...
        "protected_validation_and_strict_present": len(validation) == 22 and len(protected_strict) == 22,
        "command_uses_web_task_sampler": "web_task_family_balanced" in command,
        "command_uses_contrast_weight": "--bounded-choice-contrast-weight" in command and command[command.index("--bounded-choice-contrast-weight") + 1] == "0.5",
        "command_uses_contrast_margin": "--bounded-choice-contrast-margin" in command and command[command.index("--bounded-choice-contrast-margin") + 1] == "0.08",
        "command_uses_gpu2_mask": "CUDA_VISIBLE_DEVICES=2" in command and "NVIDIA_VISIBLE_DEVICES=2" in command,
    }
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        ...
        "decision": "web_moderate_task_aware_contrast_probe_ready" if all(gates.values()) else "web_moderate_task_aware_contrast_probe_blocked",
        "metrics": {
        ...
        "command": command,
        "claim_boundary": [
            "This is a guarded ablation after the stale Stage11586 request artifact mismatch.",
            "Stage11580 verifier labels are current-state PASS_TO_PASS selected-test relevance, not fail-to-pass repair supervision.",
            "Promotion requires preserving selected frontier gates and improving Web heldout/successor strict under the selected product scorer.",
        ],
        "source_artifacts": {
            ...
            "stage11585_readiness": rel(READINESS),
        },
        "outputs": {
    ...
    print(json.dumps({"decision": summary["decision"], "metrics": summary["metrics"], "objective_settings": summary["objective_settings"], "gates": gates}, indent=2, sort_keys=True))
