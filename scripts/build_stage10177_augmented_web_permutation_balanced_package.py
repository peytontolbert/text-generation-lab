...
STAGE = 10177
NAME = "stage10177_augmented_web_permutation_balanced_package"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
TRAIN_JSONL = OUT_DIR / "agentkernel_lite_encdec_train.jsonl"
EVAL_JSONL = OUT_DIR / "agentkernel_lite_encdec_eval.jsonl"
PACKAGE_JSON = OUT_DIR / "augmented_web_permutation_balanced_package.json"
...
WEB_TRAIN_BUNDLE_SUBSTRS = ["src_main_js_index_html_440e122a0f", "stage10176::localsess_code_assist"]
...
def build_variant_rows(bundle: dict[str, Any], row: dict[str, Any], *, split: str) -> list[dict[str, Any]]:
        ...
        variants.append(
            {
                ...
                "semantic_key": f"maintainer_bundle_compact_bounded_perm::{bundle.get('bundle_id')}::{row.get('perspective')}::perm_{shift:02d}",
                "anti_cheat": {
                    "opaque_labels": True,
...
def compile_bundle_rows(bundle: dict[str, Any]) -> list[dict[str, Any]]:
...
def choose_language_splits(grouped: dict[str, list[dict[str, Any]]]) -> tuple[dict[str, list[dict[str, Any]]], dict[str, dict[str, Any]], list[str]]:
    ...
    for language, bundles in sorted(grouped.items()):
        ...
        web_train = [
            ...
            if any(marker in str(bundle.get("bundle_id") or "") for marker in WEB_TRAIN_BUNDLE_SUBSTRS)
        ]
        if web_eval is None:
            failures.append("missing_web_eval_bundle")
            continue
        if len(web_train) < 2:
            failures.append("need_two_web_train_bundles")
            continue
        train_bundles[language] = web_train
...
def build_package() -> dict[str, Any]:
    ...
    for language in sorted(train_selection):
        ...
        for bundle in train_selection[language]:
            for row in compile_bundle_rows(bundle):
                train_compiled.extend(build_variant_rows(bundle, row, split="train"))
