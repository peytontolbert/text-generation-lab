...
STAGE = 11591
NAME = "stage11591_web_moderate_task_aware_contrast_postrun_audit"
OUT = ART / NAME
SUMMARY = OUT / "web_moderate_task_aware_contrast_postrun_audit.json"

RUNTIME = ART / "stage11590_web_moderate_task_aware_contrast_probe/runtime_model/runtime_model_bundle.json"
ROWSETS = {
    "web_heldout": ART / "stage11548_web_root_heldout_stage11507_score_audit/web_root_heldout_rows.jsonl",
    "web_successor_strict": ART / "stage11590_web_moderate_task_aware_contrast_probe_request/web_moderate_task_aware_contrast_successor_strict_rows.jsonl",
...
def main() -> None:
    ...
    gates = {
        ...
        "residual_at_least_7_of_10": (results["residual_bank"]["correct"] or 0) >= 7,
        "web_heldout_beats_stage11571_42_of_66": (results["web_heldout"]["correct"] or 0) > 42,
    ...
    summary = {
        ...
        "created_at_utc": now(),
        "passed": True,
        "decision": "promote_if_all_gates_pass" if all(gates.values()) else "do_not_promote_web_moderate_task_aware_contrast_runtime",
        "runtime": rel(RUNTIME),
        "device": str(DEVICE),
        "scorer": SCORER,
        "results": results,
    ...
    print(json.dumps({"decision": summary["decision"], "gates": gates, "compact": {k: {"correct": v["correct"], "rows": v["rows"]} for k, v in results.items()}}, indent=2, sort_keys=True))
