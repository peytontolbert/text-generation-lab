...
STAGE = 9410
NAME = "stage9410_structured_suffix_route_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9409_structured_suffix_route_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9409_structured_suffix_route_manifest/structured_suffix_route_manifest.jsonl"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9411_structured_suffix_route_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "structured_suffix_route_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9411_structured_suffix_route_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "STRUCTURED_SUFFIX_ROUTE_PREEXECUTION_STAGE9410.md"
...
COMMAND = [
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9411_structured_suffix_route_probe",
]
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9409_not_passed")
    ...
    summary = {
        ...
        "decision": "Prepared a denoise-only probe for structured suffix-route controls.",
        "next_best_step": "Execute Stage9411 and compare heldout target-pair recovery against Stage9399/9407.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9410 Structured Suffix Route Preexecution", "", f"Passed: `{card['passed']}`", f"Rows: `{card['manifest_rows']}`", f"Splits: `{card['split_counts']}`", "", "Only Stage9411 denoise CE is authorized. Decoder CE and external authority remain closed.", ""]), encoding="utf-8")
