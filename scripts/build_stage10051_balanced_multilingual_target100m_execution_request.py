...
STAGE = 10051
NAME = "stage10051_balanced_multilingual_target100m_execution_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
REQUEST = OUT_DIR / "balanced_multilingual_target100m_execution_request.json"
SURFACE_REQUEST = OUT_DIR / "surface_requests" / "edit_localization.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "BALANCED_MULTILINGUAL_TARGET100M_EXECUTION_REQUEST_STAGE10051.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
MANIFEST = ROOT / "runs/local/artifacts/stage10050_balanced_multilingual_successor_packet/balanced_multilingual_successor_manifest.jsonl"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage10050_balanced_multilingual_successor_packet.json"
BASELINE_RESULT = ROOT / "runs/local/artifacts/stage10048_python_symbol_test_target100m_probe/edit_localization_probe/execution_result.json"

FUTURE_RUN_ID = "stage10051_balanced_multilingual_target100m_probe"
FUTURE_OUTPUT_DIR = "runs/local/artifacts/stage10051_balanced_multilingual_target100m_probe/edit_localization_probe"
...
def build_request() -> dict[str, Any]:
    ...
    if language_counts.get("python") != 29:
        failures.append("python_rows_not_29")
    ...
    if source_summary.get("passed") is not True:
        failures.append("stage10050_not_passed")
    if baseline.get("runtime_executed") is not True:
        failures.append("stage10048_runtime_not_executed")
    ...
    req = {
        ...
        "required_contract_invariants": {
            "source_stage10050_passed": bool(source_summary.get("passed") is True),
            "source_stage10048_runtime_executed": bool(baseline.get("runtime_executed") is True),
...
def main() -> None:
    ...
    next_step = "Use this request to run one target-100m edit-localization probe on the balanced multilingual successor manifest, then compare the heldout rows directly against Gemma and the stage10048 successor run."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Materialized the target-100M execution request for the balanced multilingual successor manifest so the next run can test whether the Python fix can be retained without losing multilingual strength.",
