...
STAGE = 9900
NAME = "stage9900_geometry_aware_v27_execution_delta_audit"
CURRENT = ROOT / "runs/summaries/stage9899_geometry_aware_structured_tiny_execution_review.json"
BASELINE = ROOT / "runs/summaries/stage9876_margin_objective_structured_tiny_execution_review.json"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "geometry_aware_v27_execution_delta_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "GEOMETRY_AWARE_V27_EXECUTION_DELTA_AUDIT_STAGE9900.md"
...
def build_audit() -> dict[str, Any]:
    ...
    if baseline.get("passed") is not True:
        failures.append("stage9876_not_passed")
    ...
    decision = (
        ...
        if not failures
        else "The geometry-aware v2.7 execution delta audit is incomplete."
    )
    caveats = [
        "This is still a tiny capped review, not a broad standalone or Gemma comparison rerun.",
        "Edit localization is improved but still far below a credible multilingual Gemma-beating claim.",
        "The next execution should target the refreshed package in a broader same-surface comparison rather than reverting to the stale edit-localization packet.",
    ]
...
def main() -> None:
    ...
    next_step = "Promote Stage9896 as the default v2.7 package for future training and comparison runs, then rebuild the same-surface multilingual Gemma comparison on that refreshed edit-localization packet."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
