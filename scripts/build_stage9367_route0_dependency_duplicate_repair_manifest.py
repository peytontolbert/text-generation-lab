...
STAGE = 9367
NAME = "stage9367_route0_dependency_duplicate_repair_manifest"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9366_full_interference_rejoin_probe_audit.json"
SOURCE_MANIFEST = ROOT / "runs/local/artifacts/stage9364_full_mixture_with_interference_repairs_manifest/full_mixture_with_interference_repairs_manifest.jsonl"
SOURCE_RUN = ROOT / "runs/local/artifacts/stage9366_full_interference_rejoin_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
MANIFEST = OUT_DIR / "route0_dependency_duplicate_repair_manifest.jsonl"
AUDIT = OUT_DIR / "route0_dependency_duplicate_repair_manifest_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "ROUTE0_DEPENDENCY_DUPLICATE_REPAIR_MANIFEST_STAGE9367.md"
...
def build_rows() -> list[dict[str, Any]]:
    ...
    for idx, sample in enumerate(samples):
        ...
        if "patch ins the patch inside" not in generated:
            continue
        ...
        row_id = f"stage9367_route0_dup_{idx:03d}_{h(source_id + generated)}"
        mi.update(
            {
                "active_generation_prefix_span": prefix,
                "active_generation_prefix_words": len(prefix.split()),
                "bridge_error_family": "route0_dependency_patch_ins_duplicate_repair",
                "bridge_repair_objective": "route0_dependency_duplicate_repair",
                ...
                "route_schema_version": "stage9367_route0_dependency_duplicate_v1",
            }
        )
        ...
        state.update(
            {
                "source_stage9366_row_id": source_id,
                "source_stage9366_generated_sha256": hashlib.sha256(generated.encode("utf-8")).hexdigest(),
                "target_text_sha256": hashlib.sha256(target.encode("utf-8")).hexdigest(),
                "route0_dependency_duplicate_repair_required": True,
        ...
        anti.update(
            {
                ...
                "route0_dependency_duplicate_repair": True,
            }
        )
        row.update(
            {
                ...
                "merged_curriculum_source": "route0_dependency_duplicate_repair",
                "combined_curriculum_source": "route0_dependency_duplicate_repair",
                "repair_task_type": "route0_dependency_patch_ins_duplicate_repair",
                "clean_target": target,
...
def audit_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if source.get("passed") is not False or metrics.get("safety_gate_passed") is not True:
        failures.append("source_stage9366_not_safe_failed_audit")
    if len(rows) != 41:
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Built denoise-only rows for the Stage9366 route_0 dependency duplicate residual.",
        "next_best_step": "Build Stage9368 preexecution for a 41-row route_0 duplicate repair probe; keep decoder CE closed.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9367 Route0 Dependency Duplicate Repair Manifest", "", f"Passed: `{summary['passed']}`", f"Rows: `{audit['rows']}`", f"Splits: `{audit['split_counts']}`", f"Routes: `{audit['route_counts']}`", "", "This manifest isolates the Stage9366 route_0 `patch ins the patch inside` residual. Decoder CE and external authority remain closed.", ""]), encoding="utf-8")
