...
STAGE = 9302
NAME = "stage9302_causal_mask_boundary_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9301_transformer_attention_mask_patch_audit.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9290_one_next_token_suffix_manifest/one_next_token_suffix_manifest.jsonl"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9303_causal_mask_boundary_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "post_prefix_boundary_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9303_causal_mask_boundary_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "CAUSAL_MASK_BOUNDARY_PROBE_PREEXECUTION_STAGE9302.md"
...
COMMAND = [
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9303_causal_mask_boundary_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9301_not_passed")
    if metrics.get("uses_additive_future_mask") is not True or metrics.get("uses_additive_padding_mask") is not True:
        failures.append("causal_mask_patch_not_confirmed")
...
def main() -> None:
    ...
    summary = {
        ...
        "metrics": {**{key: bool(card["authority"].get(key, False)) for key in AUTHORITY_CLOSED}, "command_ready": card["command_ready"], "will_execute_now": False, "authorized_next_stage": 9303, "manifest_rows": card["manifest_rows"], "tmpdir": card["tmpdir"], "required_postrun_artifact": card["required_postrun_artifact"], "failures": len(card["failures"])},
        "artifacts": {"audit": str(AUDIT.relative_to(ROOT)), "command": str(COMMAND_JSON.relative_to(ROOT)), "doc": str(DOC.relative_to(ROOT))},
        "decision": "Prepared causal-mask boundary probe command with TMPDIR redirected to /data/tmp; not executed in this stage.",
        "next_best_step": "Execute Stage9303 causal-mask boundary probe under trellis, then audit boundary rank and generation effects.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9302 Causal-Mask Boundary Probe Preexecution", "", f"Passed: `{card['passed']}`", f"TMPDIR: `{card['tmpdir']}`", "Command artifact prepares Stage9303 but does not execute it.", "Required post-run artifact: `boundary_next_token_logits.jsonl`", ""]) , encoding="utf-8")
