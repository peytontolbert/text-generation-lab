...
STAGE = 9437
NAME = "stage9437_suffix_choice_prior_fusion_denoise_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9436_suffix_choice_prior_fusion_denoise_preexecution.json"
RUN_DIR = ROOT / "runs/local/artifacts/stage9437_suffix_choice_prior_fusion_denoise_probe"
AUDIT = RUN_DIR / "stage9437_suffix_choice_prior_fusion_denoise_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "SUFFIX_CHOICE_PRIOR_FUSION_DENOISE_PROBE_AUDIT_STAGE9437.md"
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9436_not_passed")
    if contract.get("passed") is not True:
        failures.append("contract_not_passed")
    if contract.get("loss_counts", {}).get("denoise_ce") != 53 or contract.get("loss_counts", {}).get("decoder_ce") != 0:
    ...
    safety_gate_passed = not failures and generated == 53 and leak_rows == 0 and short_rows == 0
    ...
    audit = {
        ...
        "diagnosis": (
            "Suffix-choice prior fusion is execution-safe and improves train exactness, but heldout exact remains "
            "2/9 and repetition reappears. The controller prior should not be widened to decoder CE."
        ),
        "next_patch_target": "heldout suffix-prior residual quarantine and train-support balance audit",
    ...
    summary = {
        ...
        "decision": "The suffix-choice-prior fusion denoise probe is safety-clean but not quality-passing; decoder CE remains closed.",
        "next_best_step": "Build a residual diagnosis for Stage9437: quarantine heldout prior-fusion failures and identify whether repetition comes from duplicated train support or weak suffix-prior confidence.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    DOC.write_text("\n".join(["# Stage9437 Suffix Choice Prior Fusion Denoise Probe Audit", "", f"Passed: `{audit['passed']}`", f"Safety gate passed: `{audit['safety_gate_passed']}`", f"Quality gate passed: `{audit['quality_gate_passed']}`", f"Exact rows: `{exact}` / `{generated}`", f"Heldout exact rows: `{heldout_exact}` / `{heldout_rows}`", f"Contentful rows: `{contentful}` / `{generated}`", f"Repetition rows: `{repetition_rows}`", f"Leak rows: `{leak_rows}`", "", "Decoder CE remains closed. The next step is residual diagnosis, not widening.", ""]))
    ...
    print(json.dumps({"stage": STAGE, "passed": summary["passed"], "metrics": {k: audit[k] for k in ["safety_gate_passed", "quality_gate_passed", "exact_match_rows", "heldout_exact_rows", "heldout_rows", "degenerate_repetition_rows"]}}, indent=2, sort_keys=True))
