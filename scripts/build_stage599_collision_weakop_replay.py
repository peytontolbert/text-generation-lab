...
OUT = ROOT / "runs/local/tmp/pocketpal_stage599_collision_weakop_replay_seed461"
ARTIFACT = ROOT / "runs/local/artifacts/stage599_collision_weakop_replay_dataset.json"
DOC = ROOT / "docs/stage599_collision_weakop_replay_dataset.md"
...
def main() -> None:
    ...
    train_rows = list(iter_jsonl(train_source))
    eval_rows = list(iter_jsonl(eval_source))

    replay_rows = list(train_rows)
    added = 0
    for copy_index in range(EXTRA_COPIES):
        for row in train_rows:
            if str(row.get("operation", "") or "") not in WEAK_OPS:
                continue
            clone = dict(row)
            clone["source_id"] = f"{row.get('source_id', '')}__weakop_replay_{copy_index}"
            clone["weakop_replay"] = True
    ...
    manifest.update(
        {
            "artifact_kind": "agentkernel_lite_encdec_stage599_collision_weakop_replay",
            ...
            "weakop_replay": True,
            "weakop_replay_ops": sorted(WEAK_OPS),
            "weakop_replay_extra_copies": EXTRA_COPIES,
            "weakop_replay_added_train_examples": added,
            "collision_goal": "Oversample the weak Stage598 collision families while leaving eval unchanged.",
        }
    )
    (OUT / "agentkernel_lite_encdec_dataset_manifest.json").write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    summary = {
        "artifact_kind": "stage599_collision_weakop_replay_dataset",
        ...
        "decision": "weakop_replay_dataset_ready",
        "finding": (
            "Stage599 weak-op replay oversamples direct_fact, entity_context, and two_hop_owner_region on the collision-conditioned "
            "training set while keeping the Stage596 eval unchanged. This targets the collision families that remained weakest after Stage598."
        ),
    }
    ARTIFACT.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text(
        f"""# Stage599 Collision Weak-Op Replay Dataset
...
- Weak ops: `{', '.join(sorted(WEAK_OPS))}`
- Extra copies per weak-op train row: `{EXTRA_COPIES}`
