...
STAGE = 9362
NAME = "stage9362_mixture_interference_residual_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9361_mixture_interference_residual_repair_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9361_mixture_interference_residual_repair_manifest/mixture_interference_residual_repair_manifest.jsonl"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9363_mixture_interference_residual_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "mixture_interference_residual_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9363_mixture_interference_residual_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "MIXTURE_INTERFERENCE_RESIDUAL_PROBE_PREEXECUTION_STAGE9362.md"
...
COMMAND = [
    ...
    str(OUTPUT_DIR),
    "--run-id",
    "stage9363_mixture_interference_residual_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9361_not_passed")
    if len(rows) != 55:
...
def main() -> None:
    ...
    summary = {
        ...
        "metrics": {**{key: bool(card["authority"].get(key, False)) for key in AUTHORITY_CLOSED}, "command_ready": card["command_ready"], "manifest_rows": card["manifest_rows"], "split_counts": card["split_counts"], "task_counts": card["task_counts"], "route_counts": card["route_counts"], "failures": len(card["failures"])},
        "artifacts": {"audit": str(AUDIT.relative_to(ROOT)), "command": str(COMMAND_JSON.relative_to(ROOT)), "doc": str(DOC.relative_to(ROOT))},
        "decision": "Prepared a closed-boundary 55-row mixture-interference residual denoise probe; decoder CE remains closed.",
        "next_best_step": "Execute Stage9363 mixture-interference residual probe under trellis and audit exactness before another full rejoin.",
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage9362 Mixture Interference Residual Probe Preexecution",
                "",
                ...
                "",
                "Only the Stage9363 55-row target-100M denoise probe is authorized. Decoder CE and external authority remain closed.",
                "",
            ]
        ),
        encoding="utf-8",
