...
STAGE = 11582
NAME = "stage11582_web_verifier_attached_postrun_audit"
OUT = ART / NAME
SUMMARY = OUT / "web_verifier_attached_postrun_audit.json"

RUNTIME = ART / "stage11581_web_verifier_attached_probe/runtime_model/runtime_model_bundle.json"
ROWSETS = {
    "web_heldout": ART / "stage11548_web_root_heldout_stage11507_score_audit/web_root_heldout_rows.jsonl",
    "web_successor_strict": ART / "stage11581_web_verifier_attached_probe_request/web_verifier_attached_strict_rows.jsonl",
...
def main() -> None:
    ...
    summary = {
        ...
        "created_at_utc": now(),
        "passed": True,
        "decision": "promote_if_all_gates_pass" if all(gates.values()) else "do_not_promote_web_verifier_attached_runtime",
        "runtime": rel(RUNTIME),
        "device": str(DEVICE),
        "scorer": SCORER,
        "results": results,
