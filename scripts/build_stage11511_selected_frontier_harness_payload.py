...
STAGE = 11511
NAME = "stage11511_selected_frontier_harness_payload"
OUT = ART / NAME
OUT_JSON = OUT / "selected_frontier_harness_payload.json"
SUMMARY_JSON = OUT / "selected_frontier_harness_payload_summary.json"

HANDOFF = ART / "stage10657_repaired_headline_harness_handoff_bundle/repaired_headline_harness_handoff_bundle.json"
MATCHED_ROWS = ART / "stage11510_selected_frontier_same_manifest_gemma_comparison/matched_comparison_rows.jsonl"
COMPARISON = ART / "stage11510_selected_frontier_same_manifest_gemma_comparison/selected_frontier_same_manifest_gemma_comparison.json"
...
def row_to_harness_row(row: dict[str, Any], matched: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "claim_role": row.get("claim_role") or "stage11510_matched_old_canary_strict",
...
def main() -> None:
    ...
    for cell in handoff.get("handoff_cells") or []:
        ...
        task_pack = {
            ...
            "blocked_training_reason": "stage11507_selected_frontier_harness_pack_never_train_eligible",
        ...
        runs.append(
            {
                ...
                "hundred_m_backend": {
                    ...
                    "bounded_choice_aux_source": "encoder_option_retrieval_evidence_judgment_head",
                    "device": "cuda",
    ...
    payload = {
        "runtime_payload_version": "stage11511.v1",
        "stage": STAGE,
        "stage_name": NAME,
        "created_at_utc": now(),
        "claim_scope": (
            "Stage11507 selected frontier harness payload for the matched old-canary strict bounded-choice slice. "
        ...
        "notes": [
            ...
            "Uses Stage11507 runtime bundle and encoder_option_retrieval_evidence_judgment_head scorer promoted by Stage11509.",
            "Includes the legacy singleton Rust verifier row because it is part of the current 23-row Stage11510 matched canary; downstream audits should keep flagging it as non-discriminative.",
        ],
        "runs": runs,
    }
