...
STAGE = 9946
NAME = "stage9946_web_targeted_blended_target100m_contract_preflight"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9945_web_targeted_blended_structured_mix.json"
STRUCTURED = ROOT / "runs/local/artifacts/stage9945_web_targeted_blended_structured_mix/web_targeted_blended_structured_state.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
MANIFEST_DIR = OUT_DIR / "manifests"
RUNS_DIR = OUT_DIR / "contract_runs"
AUDIT = OUT_DIR / "web_targeted_blended_target100m_contract_preflight_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "WEB_TARGETED_BLENDED_TARGET100M_CONTRACT_PREFLIGHT_STAGE9946.md"
...
def main() -> None:
    ...
    next_step = "Use these preflighted manifests as the next capped blended target-100M structured run contract; the edit-localization leg now carries the targeted web refresh rows inside the same contract-only-validated trainer path."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Ran target-100M contract-only trainer preflights for the stage9945 blended structured-state successor mix; this preserves the same no-execution boundary while validating the next run contract after the web-targeted refresh was blended in.",
