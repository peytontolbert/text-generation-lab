...
STAGE = 10632
NAME = "stage10632_repaired_long_context_successor_probe_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
REQUEST_JSON = OUT_DIR / "repaired_long_context_successor_probe_request.json"
COMMAND_JSON = OUT_DIR / "repaired_long_context_successor_probe_command.json"
MANIFEST_JSONL = OUT_DIR / "repaired_long_context_successor_probe_manifest.jsonl"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"

PACKAGE_JSON = ROOT / "runs/local/artifacts/stage10631_repaired_long_context_successor_package/repaired_long_context_successor_package.json"
TRAIN_ROWS = ROOT / "runs/local/artifacts/stage10631_repaired_long_context_successor_package/repaired_long_context_successor_train_rows.jsonl"
STRICT_ROWS = ROOT / "runs/local/artifacts/stage10631_repaired_long_context_successor_package/repaired_long_context_successor_strict_rows.jsonl"
...
RUN_ID = "stage10632_repaired_long_context_successor_probe"
OUTPUT_DIR = "runs/local/artifacts/stage10632_repaired_long_context_successor_probe/bounded_decoder_probe"
RUNTIME_MODEL_DIR = "runs/local/artifacts/stage10632_repaired_long_context_successor_probe/runtime_model"
...
def with_probe_fields(row: dict[str, Any], split: str) -> dict[str, Any]:
    ...
    updated["objective_family"] = "repaired_long_context_successor_decoder_ce"
...
def main() -> None:
    ...
    if len(train_rows) > TARGET_100M_MAX_TRAIN_ROWS:
        raise SystemExit(
            f"target_100m train cap exceeded: {len(train_rows)} > {TARGET_100M_MAX_TRAIN_ROWS}"
        )
    ...
    request = {
        ...
        "decision": "repaired_long_context_successor_probe_ready",
        "claim_scope": [
            "Launch the first clean 100M support probe from the repaired long-context successor package.",
            "Restrict promotable headline scoring to repaired decisive_evidence_option strict rows only.",
            "Keep verifier_outcome_option and retrieve_answer_abstain_option support-only until fresh heldout roots exist.",
            "Cap multilingual train support at a target_100m-safe size using root-level rather than row-level selection.",
        ],
        ...
        "required_honesty_gates": [
            ...
            "post-run headline must separate repaired long-context strict, repaired long-context support eval, and repaired v2.7 canary results",
        ],
        "post_run_required_artifacts": [
            "bounded_decoder_probe/execution_result.json",
            "strict comparison against the 18-row repaired long-context decisive-evidence strict slice",
            "support-eval slice accuracy by language and target subtype",
            "post-run repaired-v2.7 canary audit against the 24-row overlay",
            "generation audit validity and degeneration summary",
        ],
        "known_limits": [
            "This request does not make verifier_outcome_option a promotable heldout result yet.",
            ...
            "Language balance is still thin in Rust and web because current repaired root supply is limited.",
        ],
        "package_context": {
            ...
            "rows_by_language_before_caps": ((package.get("metrics") or {}).get("rows_by_language")),
            "quarantined_root_ratio": ((root_admission_audit.get("metrics") or {}).get("quarantined_root_ratio")),
        },
        "next_best_step": (
            "Launch this repaired successor probe, then evaluate whether decisive-evidence strict accuracy improves "
            "without regressing the reviewed v2.7 24-row canary."
        ),
        "command": command,
    }
