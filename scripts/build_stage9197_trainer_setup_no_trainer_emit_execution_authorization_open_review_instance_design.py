...
STAGE = 9197
NAME = "stage9197_trainer_setup_no_trainer_emit_execution_authorization_open_review_instance_design"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
SOURCE_9196 = ROOT / "runs/summaries/stage9196_trainer_setup_no_trainer_emit_execution_authorization_open_review_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "TRAINER_SETUP_NO_TRAINER_EMIT_EXECUTION_AUTHORIZATION_OPEN_REVIEW_INSTANCE_DESIGN_STAGE9197.md"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
DESIGN = OUT_DIR / "trainer_setup_no_trainer_emit_execution_authorization_open_review_instance_design.json"

REQUIRED_INSTANCE_FIELDS = [
    "source_stage9196_audit_ref",
    "emit_execution_authorization_open_review_ref",
    ...
    "authorization_open_review_instance_ticket_ref",
]

REQUIRED_INSTANCE_GUARDS = [
    "instance_requires_stage9196_passed",
...
def registry(latest: int = 9196) -> dict[str, Any]:
...
def build_design(registry_card: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    source = load_json(SOURCE_9196)
    return {
        "stage": STAGE,
        "stage_name": NAME,
        "design_type": "trainer_setup_no_trainer_emit_execution_authorization_open_review_instance_v1",
        "source_stage9196_passed": source.get("passed") is True,
        "registry_frontier_stage9196": int((registry_card.get("metrics") or {}).get("latest_stage", -1)) == 9196,
        ...
        "decision": (
            "Designed the deliberate no-trainer emit execution authorization-open review instance. The instance "
            "may pin the exact local materializer argv, output directory, and review-instance ticket while "
            "keeping trainer invocation and all model execution paths closed."
        ),
        "next_best_step": "Audit the no-trainer emit execution authorization-open review instance design; still do not emit outputs.",
    }
...
def validate_design(design: dict[str, Any]) -> list[str]:
    ...
    if design.get("source_stage9196_passed") is not True or metrics.get("source_stage9196_passed") is not True:
        failures.append("source_stage9196_not_passed")
    if design.get("registry_frontier_stage9196") is not True or metrics.get("registry_frontier_stage9196") is not True:
        failures.append("registry_frontier_stage9196")
...
def run_negative_cases() -> dict[str, Any]:
    ...
    for case in NEGATIVE_CASES:
        ...
        if case == "source_stage_missing":
            design["source_stage9196_passed"] = False
            design["metrics"]["source_stage9196_passed"] = False
        elif case == "registry_frontier_bad":
            design["registry_frontier_stage9196"] = False
            design["metrics"]["registry_frontier_stage9196"] = False
        elif case == "missing_instance_field":
            design["required_instance_fields"].remove("authorization_open_review_instance_ticket_ref")
...
def main() -> None:
    ...
    artifact = {
        ...
        "decision": design["decision"] if passed else "No-trainer emit execution authorization-open review instance design failed validation.",
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage9197 Trainer Setup No-Trainer Emit Execution Authorization-Open Review Instance Design",
                "",
                f"Passed: `{summary['passed']}`",
                "",
                "Designs the exact no-trainer emit authorization-open review instance without emitting outputs.",
                "",
