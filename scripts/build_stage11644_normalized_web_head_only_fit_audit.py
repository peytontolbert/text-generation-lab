...
STAGE = 11644
NAME = "stage11644_normalized_web_head_only_fit_audit"
OUT = ART / NAME
SUMMARY = OUT / "normalized_web_head_only_fit_audit.json"

RUNTIME = ART / "stage11643_normalized_web_head_only_fit_fixed/runtime_model/runtime_model_bundle.json"
ROWSETS = {
    ...
    "normalized_web_train_support": ART / "stage11638_web_support_schema_normalization/web_support_normalized_admitted_train_rows.jsonl",
...
def main() -> None:
    ...
    gates = {
        "head_only_support_fit_at_least_180_of_206": (results["normalized_web_train_support"]["correct"] or 0) >= 180
        and results["normalized_web_train_support"]["rows"] == 206,
        "head_only_support_fit_at_least_200_of_206": (results["normalized_web_train_support"]["correct"] or 0) >= 200
        and results["normalized_web_train_support"]["rows"] == 206,
        ...
        "normalized_support_full_coverage": results["normalized_web_train_support"]["coverage"] == 1.0
        and results["normalized_web_train_support"]["rows"] == 206,
    }
    preservation_gates = {
        key: gates[key]
    ...
    if gates["head_only_support_fit_at_least_200_of_206"] and not all(preservation_gates.values()):
        decision = "head_only_support_fit_confirmed_but_not_product_safe"
    elif gates["head_only_support_fit_at_least_180_of_206"]:
        decision = "head_only_support_fit_partial_no_promotion"
    else:
        decision = "head_only_support_fit_failed"
    ...
    summary = {
        ...
        "baseline_reference": {
            ...
            "stage11639_full_model_support_fit": "16/206",
        },
        "runtime_initialization": init_card,
        "source_artifacts": {name: rel(path) for name, path in ROWSETS.items()},
        "outputs": {"summary": rel(SUMMARY), "audit_dir": rel(OUT)},
        "claim_boundary": [
            "This is a representability audit for a head-only Web scorer fit.",
            "It must not be promoted as a product runtime unless protected gates and Web heldout also improve.",
            "If support fit is high but protected/Web heldout is poor, the head can memorize support but still lacks safe transfer.",
            "If support fit is low, the current scorer architecture/loss cannot fit normalized Web support even directly.",
        ],
    }
