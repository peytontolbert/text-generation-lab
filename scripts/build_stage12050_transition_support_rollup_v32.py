#!/usr/bin/env python3
"""Roll Stage12049 INSUFFICIENT_EVIDENCE rows into support v32."""
...
ROOT = Path("runs/local/artifacts/stage12050_transition_support_rollup_v32")
BASE = Path("runs/local/artifacts/stage12048_transition_support_rollup_v31/transition_support_rows_v31.jsonl")
ADDED = Path("runs/local/artifacts/stage12049_insufficient_evidence_floor_closure_rows/insufficient_evidence_floor_closure_rows.jsonl")
ROWS_OUT = ROOT / "transition_support_rows_v32.jsonl"
SUMMARY = ROOT / "transition_support_rollup_v32.json"
SUMMARY_MIRROR = Path("runs/summaries/stage12050_transition_support_rollup_v32.json")
...
def main() -> None:
    ...
    duplicates = 0
    for source, path in [("stage12048", BASE), ("stage12049", ADDED)]:
        for row in read_jsonl(path):
            out = dict(row)
            out["stage12050_source"] = source
            out["split"] = "train"
            out["split_role"] = "stage12050_transition_support_v32_train_support_only"
    ...
    summary = {
        ...
        "insufficient_evidence_floor_closure_added": len(read_jsonl(ADDED)),
        "decision": "support_inventory_v32_not_train_ready" if not train_ready else "support_inventory_v32_floor_ready_requires_anti_cheat_audit",
        "reason": "INSUFFICIENT_EVIDENCE floor is closed; PASS_TO_PASS and semantic FAIL_TO_PASS floors remain unmet." if not train_ready else "Floor counts are met; run anti-cheat and split audits before training.",
        "claim_boundary": "Support inventory only. No model promotion or Gemma comparison should be claimed from v32.",
        "next_stage_recommendation": {
            "stage": "stage12051_status_floor_closure",
            "action": "Close semantic FAIL_TO_PASS and PASS_TO_PASS floors; keep FAIL_TO_PASS majority semantic and avoid repo-family concentration.",
        },
    }
