...
STAGE = 9279
NAME = "stage9279_bridge_primed_denoise_manifest"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9278_continuation_bridge_denoise_probe_audit.json"
SOURCE_MANIFEST = ROOT / "runs/local/artifacts/stage9277_continuation_bridge_denoise_manifest/continuation_bridge_denoise_manifest.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
MANIFEST = OUT_DIR / "bridge_primed_denoise_manifest.jsonl"
AUDIT = OUT_DIR / "bridge_primed_denoise_manifest_audit.json"
CARD = OUT_DIR / "bridge_priming_card.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "BRIDGE_PRIMED_DENOISE_MANIFEST_STAGE9279.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"

AUTHORITY_FALSE = dict(AUTHORITY_CLOSED)
MAX_PRIMING_WORDS = 8
MAX_PRIMING_CHARS = 112
...
def priming_span(target: str) -> str:
    matches = list(re.finditer(r"[A-Za-z][A-Za-z0-9_-]*", target))
    if not matches:
        return ""
    end = matches[min(MAX_PRIMING_WORDS, len(matches)) - 1].end()
    return target[:end].strip()
...
def build_rows() -> list[dict[str, Any]]:
    ...
    for source in load_jsonl(SOURCE_MANIFEST):
        ...
        span = priming_span(target)
        row = dict(source)
        row["row_id"] = str(source.get("row_id", "row")).replace("stage9277_bridge_", "stage9279_bridgeprimed_")
        row["source_stage"] = STAGE
        row["source_manifest_stage"] = 9277
        row["source_repair_row_id"] = source.get("row_id")
        row["objective_family"] = "bridge_primed_bounded_decoder_output_repair"
        row["repair_task_type"] = f"bridge_primed_{source.get('repair_task_type', 'denoise_repair')}"
        state = dict(source.get("input_state") if isinstance(source.get("input_state"), dict) else {})
        state.update({
            "bridge_priming_required": True,
            "bridge_priming_span": span,
            "bridge_priming_word_count": len(words(span)),
            "bridge_priming_char_count": len(span),
        ...
        model_input.update({
            "target_grounding_mode": "bridge_primed_v1",
            "bridge_priming_span": span,
            "bridge_priming_word_count": len(words(span)),
        ...
        row["anti_cheat"] = {
            ...
            "target_remainder_hidden": True,
            "max_priming_words": MAX_PRIMING_WORDS,
            "max_priming_chars": MAX_PRIMING_CHARS,
...
def audit_rows(rows: list[dict[str, Any]], source_summary: dict[str, Any]) -> dict[str, Any]:
    ...
    if source_summary.get("passed") is not True:
        failures.append("source_stage9278_not_passed")
    if not rows:
    ...
    invalid_priming_rows = 0
    excessive_priming_rows = 0
    authority_rows = 0
    unsafe_loss_rows = 0
    ...
    for row in rows:
        ...
        span = str(model_input.get("bridge_priming_span") or "")
        ...
        if not span or not target.startswith(span):
            invalid_priming_rows += 1
        remainder = target[len(span):].strip() if target.startswith(span) else target
        if remainder and len(remainder) > 16 and remainder in visible:
            remainder_visible_rows += 1
        if len(words(span)) > MAX_PRIMING_WORDS or len(span) > MAX_PRIMING_CHARS or span == target:
            excessive_priming_rows += 1
        ...
        cards.append({"row_id": row.get("row_id"), "split": row.get("split"), "language_family": row.get("language_family"), "bridge_priming_span": span, "bridge_priming_word_count": len(words(span)), "target_sha256": sha(target)})
    ...
    if invalid_priming_rows:
        failures.append("invalid_priming_rows_nonzero")
    if excessive_priming_rows:
        failures.append("excessive_priming_rows_nonzero")
    if authority_rows:
    ...
    return {
        ...
        "invalid_priming_rows": invalid_priming_rows,
        "excessive_priming_rows": excessive_priming_rows,
...
def main() -> None:
    ...
    summary = {
        ...
        "artifacts": {"manifest": str(MANIFEST.relative_to(ROOT)), "priming_card": str(CARD.relative_to(ROOT)), "audit": str(AUDIT.relative_to(ROOT)), "doc": str(DOC.relative_to(ROOT))},
        "decision": "Built bridge-primed rows with an 8-word decoder priming span to isolate continuation beyond the first bridge fragment.",
        "next_best_step": "Run a contract-only preflight, then a tiny target-100M bridge-primed denoise probe using --generation-prefix-field model_input.bridge_priming_span.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text(
        "\n".join([
            "# Stage9279 Bridge-Primed Denoise Manifest",
            "",
            "Stage9279 exposes an 8-word target-start span for decoder-side priming while hiding the target remainder.",
            "",
            f"Rows: {audit['rows']}",
            f"Splits: {audit['split_counts']}",
            f"Priming visible rows: {audit['priming_visible_rows']}",
