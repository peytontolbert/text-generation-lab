...
STAGE = 9862
NAME = "stage9862_symbol_binding_target_100m_structured_tiny_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9861_validity_weighted_disk_safe_cleanup_execution_readiness_gate.json"
RUN_ROOT = ROOT / "runs/local/artifacts/stage9862_symbol_binding_target_100m_structured_tiny_probe"
FIRST_ATTEMPT_DIR = RUN_ROOT / "symbol_binding_probe"
SUCCESS_DIR = RUN_ROOT / "symbol_binding_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "symbol_binding_target_100m_structured_tiny_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "SYMBOL_BINDING_TARGET_100M_STRUCTURED_TINY_PROBE_STAGE9862.md"
...
def extract_metrics() -> dict[str, Any]:
    ...
    return {
        ...
        "high_confidence_wrong_rows": None,
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9861_readiness_gate_not_passed")
    ...
    quality_passed = (
        ...
        and float(metrics["strict_symbol_binding_exact"]) >= 0.4
    )
    next_step = (
        "Build Stage9863 post-run diagnostics and historical comparison for validity-weighted symbol binding: inspect confusion matrix, row logits, "
        "feature ablations, gradient norms, and target/evidence balance before deciding whether to open the same execution path for edit-localization."
    )
    audit = {
        "stage": STAGE,
        "name": NAME,
        ...
        "failures": failures,
        "source_stage9861_summary": str(SOURCE_SUMMARY.relative_to(ROOT)),
        ...
        "source_backed_field_alias_patch_required": False,
    ...
    summary = {
        ...
        "metrics": {
            ...
            "gemma_executed": metrics.get("gemma_executed"),
            "harness_executed": metrics.get("harness_executed"),
        },
        "authority": dict(AUTHORITY_CLOSED),
        "next_best_step": next_step,
    }
