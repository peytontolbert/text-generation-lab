...
STAGE = 9308
NAME = "stage9308_copy_prefix_multitoken_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9307_copy_prefix_multitoken_probe_preexecution.json"
BASELINE_SUMMARY = ROOT / "runs/summaries/stage9306_supported_suffix_boundary_probe_audit.json"
RUN_DIR = ROOT / "runs/local/artifacts/stage9308_copy_prefix_multitoken_probe"
AUDIT = RUN_DIR / "stage9308_copy_prefix_multitoken_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "COPY_PREFIX_MULTITOKEN_PROBE_AUDIT_STAGE9308.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
REQUIRED = ["probe_contract_audit.json", "execution_result.json", "sample_generation_audit.json", "boundary_next_token_logits.jsonl", "row_token_loss.jsonl", "cleanup_proof.json"]
...
def audit_run() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9307_not_passed")
    ...
    exact_rows = samples.get("exact_match_rows") or 0
    generated_rows = samples.get("generated_rows") or 0
    quality_gate_passed = bool(boundary["match_rate"] == 1.0 and samples.get("target_prefix_match_rate") == 1.0 and generated_rows and exact_rows == generated_rows)
    return {
        "passed": not failures,
        "failures": failures,
        ...
        "baseline_boundary_match_rate": baseline_metrics.get("boundary_match_rate"),
        "baseline_boundary_mean_expected_rank": baseline_metrics.get("boundary_mean_expected_rank"),
        "generation_prefix_field": contract.get("generation_prefix_field"),
        ...
        "diagnosis": "copy_prefix_multitoken_probe_tests_longer_supported_suffix_generation_after_boundary_success",
...
def main() -> None:
    ...
    if audit["quality_gate_passed"]:
        decision = "Safety passed and the copy-prefix probe generated every supported multi-token target exactly."
        next_step = "Design a non-duplicate supported-suffix contrast manifest before widening beyond micro-overfit denoise diagnostics."
    else:
        decision = "Safety result recorded; quality did not prove all supported multi-token suffixes."
        next_step = "Inspect sample_generation_audit and row_token_loss for the failing copy-prefix rows, then patch the suffix curriculum."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
