...
STAGE = 9297
NAME = "stage9297_boundary_next_token_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9296_boundary_next_token_probe_preexecution.json"
RUN_DIR = ROOT / "runs/local/artifacts/stage9297_boundary_next_token_probe"
AUDIT = RUN_DIR / "stage9297_boundary_next_token_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "BOUNDARY_NEXT_TOKEN_PROBE_AUDIT_STAGE9297.md"
...
def audit_run() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9296_not_passed")
    ...
    ranks = [int(row["expected_rank"]) for row in boundary_rows if row.get("expected_rank") is not None]
    matches = [row for row in boundary_rows if row.get("match")]
    ...
    return {
        ...
        "boundary_available_rows": samples.get("boundary_next_token_available_rows"),
        "boundary_match_rows": len(matches),
        "boundary_match_rate": samples.get("boundary_next_token_match_rate"),
        "boundary_mean_expected_rank": samples.get("boundary_next_token_mean_expected_rank"),
        "boundary_min_expected_rank": min(ranks) if ranks else None,
        "boundary_max_expected_rank": max(ranks) if ranks else None,
        "split_boundary_summary": split_summary,
        ...
        "diagnosis": "correct_suffix_token_not_top_ranked_at_forced_bridge_boundary_after_training",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Safety passed, but the correct first suffix token is not top-ranked at the forced bridge boundary after training.",
        "next_best_step": "Patch the denoise probe to train only the post-prefix suffix tokens or add a bridge-boundary loss mask; current loss is still dominated by copying prefix tokens.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9297 Boundary Next-Token Probe Audit", "", f"Safety gate passed: `{audit['safety_gate_passed']}`", f"Boundary match rate: `{audit['boundary_match_rate']}`", f"Boundary mean expected rank: `{audit['boundary_mean_expected_rank']}`", f"Boundary rank range: `{audit['boundary_min_expected_rank']}..{audit['boundary_max_expected_rank']}`", f"Train loss: `{audit['train_loss_start']} -> {audit['train_loss_end']}`", f"Diagnosis: `{audit['diagnosis']}`", "", "Authority is closed after the audit.", ""]) , encoding="utf-8")
