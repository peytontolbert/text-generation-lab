#!/usr/bin/env python3
"""Audit Stage12059 guarded transition training runtime."""
...
STAGE = 12060
NAME = "stage12060_guarded_transition_training_postrun_audit"
OUT = ART / NAME
SUMMARY = OUT / "guarded_transition_training_postrun_audit.json"
...
def main() -> None:
    ...
    postrun = results["stage12059_guarded_transition_training"]
    gates = {
        ...
        "transition_projection_retains_stage11924_baseline": (postrun["transition_projection_routed"].get("correct") or 0) >= (previous["transition_projection_routed"].get("correct") or 0),
        "transition_projection_beats_gemma_386": (postrun["transition_projection_routed"].get("correct") or 0) > 386,
        "transition_projection_at_least_364_of_640": (postrun["transition_projection_routed"].get("correct") or 0) >= 364,
    ...
    protected_preserved = all(gates[name] for name in protected_gate_names)
    if protected_preserved and gates["transition_projection_beats_gemma_386"]:
        decision = "guarded_transition_training_promotable_gemma_win_candidate"
    elif protected_preserved and gates["transition_projection_retains_stage11924_baseline"]:
        decision = "guarded_transition_training_retention_candidate_no_gemma_win"
    elif protected_preserved and gates["transition_projection_improved_vs_selected"]:
        decision = "guarded_transition_training_partial_transition_gain"
    else:
        decision = "guarded_transition_training_rejected"
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "created_at_utc": now(),
    ...
    print(json.dumps({"decision": decision, "gates": gates, "stage11924_transition": previous["transition_projection_routed"], "stage12059_transition": postrun["transition_projection_routed"]}, indent=2, sort_keys=True))
