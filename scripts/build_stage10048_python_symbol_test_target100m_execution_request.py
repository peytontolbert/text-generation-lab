...
STAGE = 10048
NAME = "stage10048_python_symbol_test_target100m_execution_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
REQUEST = OUT_DIR / "python_symbol_test_target100m_execution_request.json"
SURFACE_REQUEST = OUT_DIR / "surface_requests" / "edit_localization.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PYTHON_SYMBOL_TEST_TARGET100M_EXECUTION_REQUEST_STAGE10048.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
MANIFEST = ROOT / "runs/local/artifacts/stage10047_python_symbol_test_source_backed_successor_packet/python_symbol_test_successor_train_manifest.jsonl"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage10047_python_symbol_test_source_backed_successor_packet.json"
...
FUTURE_RUN_ID = "stage10048_python_symbol_test_target100m_probe"
FUTURE_OUTPUT_DIR = "runs/local/artifacts/stage10048_python_symbol_test_target100m_probe/edit_localization_probe"
...
def build_request() -> dict[str, Any]:
    ...
    stage10047_heldout_rows = sum(1 for row in rows if str(row.get("stage10047_role") or "") == "heldout_eval")
    ...
    metrics = {
        ...
        "stage10047_heldout_rows": stage10047_heldout_rows,
    }
    if metrics["rows"] != 103:
        failures.append("rows_not_103")
    if split_counts.get("train") != 48:
        failures.append("train_rows_not_48")
    ...
    if stage10047_heldout_rows != 0:
        failures.append("stage10047_heldout_rows_not_0_in_train_manifest")
    if heldout_rows != 55:
        failures.append("heldout_compare_rows_not_55")
    if source_summary.get("passed") is not True:
        failures.append("stage10047_not_passed")
    ...
    req = {
        ...
        "required_contract_invariants": {
            "source_stage10047_passed": bool(source_summary.get("passed") is True),
            ...
            "stage10047_train_rows": stage10047_train_rows,
            "stage10047_heldout_rows_in_train_manifest": stage10047_heldout_rows,
    ...
    return {
        ...
        "metrics": metrics,
        "source_artifacts": {
            "stage10047_packet_summary": display(SOURCE_SUMMARY),
        ...
        "execution_constraints": {
            ...
            "stage10047_review_gated_heldout_candidates_stay_outside_train_manifest": True,
        },
...
def main() -> None:
    ...
    next_step = "Use this request to run one target-100M edit-localization probe on the stage10047 Python successor manifest, then compare the resulting heldout rows directly against the existing same-manifest Gemma outputs."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Materialized the target-100M execution request for the Python symbol-vs-test source-backed successor manifest so the next run can test a fresh-data fix rather than replaying heldout rows.",
