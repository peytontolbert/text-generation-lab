...
STAGE = 9359
NAME = "stage9359_non_operator_rejoin_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9358_full_mixture_with_non_operator_residual_repairs_manifest.json"
...
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9360_non_operator_rejoin_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "non_operator_rejoin_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9360_non_operator_rejoin_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "NON_OPERATOR_REJOIN_PROBE_PREEXECUTION_STAGE9359.md"
...
COMMAND = [
    ...
    "--max-train-rows", "82", "--max-eval-rows", "44", "--max-strict-rows", "28",
    ...
    "--enable-generation-audit", "--max-generation-rows", "154", "--max-generation-tokens", "64",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9360_non_operator_rejoin_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9358_not_passed")
    if len(rows) != 154:
        failures.append("unexpected_manifest_row_count")
    if dict(split_counts) != {"train": 82, "eval": 44, "strict_eval": 28}:
    ...
    return {"passed": not failures, "failures": failures, "manifest_rows": len(rows), "split_counts": dict(sorted(split_counts.items())), "unsafe_rows": sorted(set(unsafe)), "command": COMMAND, "command_ready": not failures, "authority": AUTHORITY_RUN if not failures else dict(AUTHORITY_CLOSED)}
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared a closed-boundary 154-row full-mixture denoise probe; decoder CE remains closed.",
        "next_best_step": "Execute Stage9360 non-operator rejoin probe under trellis and audit whether the full mixture is clean enough to approach bounded decoder CE.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9359 Non-Operator Rejoin Probe Preexecution", "", f"Passed: `{card['passed']}`", f"Rows: `{card['manifest_rows']}`", f"Splits: `{card['split_counts']}`", "", "Only the Stage9360 154-row target-100M denoise probe is authorized. Decoder CE and external authority remain closed.", ""]), encoding="utf-8")
