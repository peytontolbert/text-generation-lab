...
STAGE = 9626
NAME = "stage9626_hard_phrase_tri_phase_contract_preflight"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9625_targeted_hard_phrase_continuation_manifest.json"
...
AUDIT = OUT_DIR / "tri_phase_reconnect_contract_preflight.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "HARD_PHRASE_TRI_PHASE_CONTRACT_PREFLIGHT_STAGE9626.md"
...
def command() -> list[str]:
    return [
        ...
        "--output-dir", str(RUN_DIR), "--run-id", "stage9626_hard_phrase_tri_phase_contract",
        "--contract-only",
    ]
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9625_not_passed")
    if len(phase1_rows) != 96:
        failures.append("phase1_rows_not_96")
    if len(phase2_rows) != 22:
        failures.append("phase2_rows_not_22")
    ...
    if card.get("phase2_rows") != 22:
        failures.append("contract_phase2_rows_not_22")
    ...
    next_step = "If execution is explicitly authorized, run Stage9627 hard-phrase tri-phase suffix-choice -> targeted phrase warm-up -> full residual reconnect tiny probe under the patched in-memory trainer."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Hard-phrase tri-phase reconnect command surface is contract-clean. No model execution occurred.",
