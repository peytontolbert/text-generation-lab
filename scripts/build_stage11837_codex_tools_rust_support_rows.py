#!/usr/bin/env python3
"""Materialize codex-tools Rust selected-verifier support rows."""
...
STAGE = 11837
NAME = "stage11837_codex_tools_rust_support_rows"
OUT = ART / NAME
SUMMARY = OUT / "codex_tools_rust_support_rows.json"
ROWS = OUT / "codex_tools_rust_support_rows.jsonl"
FEASIBILITY = ART / "stage11836_codex_tools_rust_feasibility/codex_tools_rust_feasibility_results.jsonl"
...
def main() -> None:
    ...
    if not probes:
        raise SystemExit("no materializable codex-tools Rust probe found")
    probe = probes[0]
    ...
    root_id = "stage11837::codex_tools::rust::request_user_input_tool_support"
    evidence = [
        file_evidence(repo, "E01", "candidate_change_surface", source_path, "Codex tools request_user_input implementation defines schema and mode-aware descriptions used by selected verifier tests."),
        file_evidence(repo, "E02", "nearby_source_distractor", distractor_source_path, "Tool config is nearby tools infrastructure but not the selected request_user_input verifier target."),
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "eighth_rust_verifier_support_root_materialized",
        "passed": True,
        "row_count": len(rows),
        "root_count": 1,
        "repo_family": repo_family,
        ...
        "claim_boundary": [
            ...
            "This expands Rust verifier support with a non-tokenizers Codex tools repo family.",
        ],
    }
