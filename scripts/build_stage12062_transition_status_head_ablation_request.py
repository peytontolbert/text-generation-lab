...
NAME = "stage12062_transition_status_head_ablation_request"
OUT = ART / NAME
SUMMARY = OUT / "transition_status_head_ablation_request.json"
MANIFEST = OUT / "transition_status_head_ablation_manifest.jsonl"
COMMAND_JSON = OUT / "transition_status_head_ablation_command.json"
...
RUNTIME_DIR = ART / "stage12063_transition_status_head_ablation_probe/runtime_model"
OUTPUT_DIR = ART / "stage12063_transition_status_head_ablation_probe/bounded_decoder_probe"
...
def main() -> None:
    ...
    for r0 in old_verifier:
        r = dict(r0)
        r["row_id"] = f"{r['row_id']}::stage12062_verifier_replay"
        r["stage12062_replay_source"] = "stage11923_old_transition_verifier_transition_only"
        normalize_mask(r)
        train.append(r)
    for r0 in v35:
        r = dict(r0)
        ...
        r["row_id"] = f"{r['row_id']}::stage12062_v35_status_support"
        r["stage12062_support_source"] = "stage12056_v35_verifier_status_support"
        r["stage12062_train_support_only"] = True
        normalize_mask(r)
        train.append(r)
    ...
    summary = {
        ...
        "decision": "transition_status_head_ablation_request_ready" if passed else "transition_status_head_ablation_request_blocked",
        "passed": passed,
        "execute_now": False,
        "hypothesis": "Stage12059 failed because v35 verifier-status support updated the shared semantic candidate head and shifted candidate/next-action priors. Train a separate transition-status head only on old verifier-transition replay plus v35 status rows, then audit with routed status-head use only for verifier-transition rows.",
        "claim_boundary": [
        ...
        "source_artifacts": {"base_stage11923_manifest": rel(BASE_MANIFEST), "v35_support_rows": rel(V35_ROWS), "v35_audit": rel(V35_AUDIT), "init_runtime": rel(INIT_RUNTIME), "preservation_runtime": rel(PRESERVE_RUNTIME), "stage12061_decision": "runs/summaries/stage12061_guarded_transition_training_decision.json"},
        "next_stage_if_executed": "stage12063_transition_status_head_ablation_probe",
        "next_stage_after_execution": "stage12064_transition_status_head_composite_routed_audit",
    }
