...
STAGE = 11356
NAME = "stage11356_web_executed_verifier_support_probe_request"
OUT = ART / NAME
SUMMARY = OUT / "web_executed_verifier_support_probe_request.json"
CMD = OUT / "web_executed_verifier_support_probe_command.json"
MAN = OUT / "web_executed_verifier_support_probe_manifest.jsonl"
TRAIN = ART / "stage11354_web_executed_verifier_support_rows/web_executed_verifier_train_support_rows.jsonl"
...
RUN = ART / "stage11357_web_executed_verifier_support_probe"
...
def main() -> None:
    ...
    protected = {root_key(r) for r in val + strict}
    overlaps = sorted({root_key(r) for r in train} & protected)
    if overlaps:
        raise SystemExit(f"root overlap with canary: {overlaps[:5]}")
    ...
    baseline = read_json(BASELINE).get("product_metrics", {}).get("web_executed_verifier_train_support", {})
    cmd = [
    ...
    summary = {
        ...
        "passed": True,
        "decision": "web_executed_verifier_support_probe_requested_diagnostic_only",
        "metrics": {
            "train_rows": len(train),
            "eval_rows": len(val),
            "strict_rows": len(strict),
            "root_overlaps": overlaps,
            "baseline_web_executed_verifier_correct": baseline.get("correct"),
            "baseline_web_executed_verifier_rows": baseline.get("rows"),
            "baseline_web_executed_verifier_accuracy": baseline.get("exact_accuracy"),
        },
        "source_artifacts": {
            "train": rel(TRAIN),
        ...
        "promotion_gate": {
            ...
            "web_static_verifier_train_score_should_improve": "above 6/12 current-runtime baseline",
            "canary_strict_must_remain": "22/22",
            "canary_validation_must_remain": ">=20/23",
            "strict_eval_claim_allowed": False,
        },
        "command": cmd,
    }
