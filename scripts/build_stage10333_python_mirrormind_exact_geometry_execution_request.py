...
STAGE = 10333
NAME = "stage10333_python_mirrormind_exact_geometry_execution_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
BUNDLE_JSON = OUT_DIR / "python_mirrormind_exact_geometry_bundle.json"
RUBRIC_JSON = OUT_DIR / "review_packets" / "stage10333__repository_library__python" / "expert_maintainer_rubric_review.json"
ANTI_JSON = OUT_DIR / "review_packets" / "stage10333__repository_library__python" / "anti_cheat_review_card.json"
GOLD_JSON = OUT_DIR / "review_packets" / "stage10333__repository_library__python" / "perspective_gold_adjudication.json"
TRAIN_ROWS_JSONL = OUT_DIR / "python_mirrormind_exact_geometry_train_rows.jsonl"
MANIFEST_JSONL = OUT_DIR / "python_mirrormind_exact_geometry_manifest.jsonl"
REQUEST_JSON = OUT_DIR / "python_mirrormind_exact_geometry_execution_request.json"
COMMAND_JSON = OUT_DIR / "python_mirrormind_exact_geometry_command.json"
...
RUN_ID = 'stage10334_python_mirrormind_exact_geometry_probe'
OUTPUT_DIR = 'runs/local/artifacts/stage10334_python_mirrormind_exact_geometry_probe/bounded_decoder_probe'
RUNTIME_MODEL_DIR = 'runs/local/artifacts/stage10334_python_mirrormind_exact_geometry_probe/runtime_model'
MAX_STEPS = 56
LEARNING_RATE = '8e-6'
BATCH_SIZE = 2
PERMUTATION_COUNT = 6
...
BUNDLE_ID = (
    'stage10333::localsess_repository_library_sessseed_codex_sessions_rollout_2025_11_29t02_41_34_019acd7c_b80e_7880_9f45_f500_'
...
CANDIDATE_PATHS = [
    'models/mirrormind/__init__.py',
    'models/mirrormind/context.py',
    'models/mirrormind/coordinator.py',
    'models/mirrormind/domain.py',
    'models/mirrormind/embeddings.py',
    'models/mirrormind/graph_client.py',
]
SELECTED_TESTS = [
    'models/mirrormind/tests/test_context_assembly.py',
...
TASK_ROWS = [
    {
        ...
        'task': 'Choose the most likely edit target from the visible failure and trace evidence.',
    },
    {
        'perspective': 'patch_impact',
        'answer_kind': 'candidate_path',
        'gold_value': 'models/mirrormind/coordinator.py',
        'task': 'Compare candidate edits by likely behavior change and risk.',
    },
    {
        'perspective': 'verifier_outcome',
        'answer_kind': 'selected_test',
        'gold_value': 'models/mirrormind/tests/test_coordinator.py',
        'task': 'Predict which visible test or verification constraint should change if the fix is correct.',
    },
    {
        'perspective': 'minimal_fix_selection',
        'answer_kind': 'candidate_path',
        'gold_value': 'models/mirrormind/coordinator.py',
        'task': 'Choose the smallest maintainable intervention supported by the evidence.',
    },
]
        ...
        'algorithmic_background_reference': [
            {
                ...
                'retrieval_reason': 'fresh_repository_library_algorithmic_grounding',
                'distance_from_seed': 2,
                'text': excerpt(REPO_ROOT / 'models/mirrormind/domain.py', 'class DomainGraph', radius=18),
            },
            {
                'path': 'models/mirrormind/embeddings.py',
                'source_type': 'local_repo',
                'retrieval_reason': 'fresh_repository_library_algorithmic_grounding',
                'distance_from_seed': 2,
                'text': excerpt(REPO_ROOT / 'models/mirrormind/embeddings.py', 'class TextEmbedder', radius=16),
            },
        ],
        'candidate_change_surface': [
            {
                ...
                'text': excerpt(REPO_ROOT / 'models/mirrormind/coordinator.py', 'class Coordinator', radius=24),
            },
            {
                'path': 'models/mirrormind/__init__.py',
                'source_type': 'local_repo',
                'retrieval_reason': 'fresh_repository_library_competing_candidate',
                'distance_from_seed': 1,
                'text': excerpt(REPO_ROOT / 'models/mirrormind/__init__.py', 'MirrorMind scaffolding', radius=16),
            },
        ],
        'external_analogue_reference': [
            {
                'path': 'models/paper.md',
                'source_type': 'local_repo',
                'retrieval_reason': 'fresh_repository_library_external_analogue',
                'distance_from_seed': 2,
                'text': excerpt(REPO_ROOT / 'models/paper.md', 'MirrorMind', radius=16),
            },
        ],
        'nearby_definition_or_usage_context': [
            {
        ...
        'symptom_or_call_path_analogue': [
            {
                'path': 'models/mirrormind/coordinator.py',
                'source_type': 'local_repo',
                'retrieval_reason': 'fresh_repository_library_call_path',
                'distance_from_seed': 0,
                'text': excerpt(REPO_ROOT / 'models/mirrormind/coordinator.py', 'def run_with_piano', radius=24),
            },
            {
                'path': 'models/mirrormind/context.py',
                'source_type': 'local_repo',
                'retrieval_reason': 'fresh_repository_library_call_path',
                'distance_from_seed': 1,
                'text': excerpt(REPO_ROOT / 'models/mirrormind/context.py', 'def build_repo_context', radius=22),
            },
        ],
        'verifier_and_test_constraint': [
            {
