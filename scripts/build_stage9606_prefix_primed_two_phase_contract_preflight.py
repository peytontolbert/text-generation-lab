...
STAGE = 9606
NAME = "stage9606_prefix_primed_two_phase_contract_preflight"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9605_prefix_primed_residual_denoise_manifest.json"
...
PHASE2_MANIFEST = ROOT / "runs/local/artifacts/stage9605_prefix_primed_residual_denoise_manifest/prefix_primed_residual_denoise_manifest.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
RUN_DIR = OUT_DIR / "two_phase_contract"
AUDIT = OUT_DIR / "prefix_primed_two_phase_contract_preflight.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PREFIX_PRIMED_TWO_PHASE_CONTRACT_PREFLIGHT_STAGE9606.md"
...
def command() -> list[str]:
    return [
        ...
        str(RUN_DIR),
        "--run-id",
        "stage9606_prefix_primed_two_phase_contract",
        "--contract-only",
    ]
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9605_not_passed")
    if len(phase1_rows) != 96:
        failures.append("phase1_rows_not_96")
    if len(phase2_rows) != 26:
        failures.append("phase2_rows_not_26")
    ...
    next_step = "If execution is explicitly authorized, run a tiny Stage9607 prefix-primed two-phase probe with phase2 caps 20/3/3 and generation-prefix-field enabled."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Validated the two-phase trainer contract against the prefix-primed residual-denoise manifest. No model execution occurred.",
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage9606 Prefix-Primed Two-Phase Contract Preflight",
                "",
                f"Passed: `{audit['passed']}`",
                f"Phase 1 rows: `{audit['phase1_rows']}`",
