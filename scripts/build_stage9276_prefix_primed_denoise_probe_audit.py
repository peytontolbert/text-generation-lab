...
STAGE = 9276
NAME = "stage9276_prefix_primed_denoise_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9275_prefix_copy_denoise_probe_audit.json"
RUN_DIR = ROOT / "runs/local/artifacts/stage9276_prefix_primed_denoise_probe/denoise_repair_probe"
AUDIT = RUN_DIR / "stage9276_prefix_primed_denoise_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PREFIX_PRIMED_DENOISE_PROBE_AUDIT_STAGE9276.md"
...
def audit_run() -> dict[str, Any]:
    ...
    if samples.get("generation_prefix_field") != "model_input.copy_prefix_span":
        failures.append("sample_generation_prefix_field_missing")
    ...
    return {
        ...
        "prefix_primed_rows": samples.get("prefix_primed_rows"),
        "prefix_primed_rate": samples.get("prefix_primed_rate"),
        "generation_prefix_start_rate": prefix_start,
        "previous_generation_prefix_start_rate": previous.get("anchor_prefix_start_rate"),
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prefix priming fixed decoder start control, but continuation quality still fails through repetition, unterminated output, and target-prefix divergence.",
        "next_best_step": "Build a continuation-bridge denoise objective or audit that supervises the first post-prefix continuation span and EOS boundary, while keeping decoder CE/runtime/Gemma closed.",
