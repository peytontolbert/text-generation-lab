...
STAGE = 9833
NAME = "stage9833_permuted_choice_execution_manifest"
SOURCE = ROOT / "runs/local/artifacts/stage9829_current_winner_counterfactual_execution_manifest/current_winner_counterfactual_execution_manifest.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
MANIFEST = OUT_DIR / "permuted_choice_execution_manifest.jsonl"
AUDIT = OUT_DIR / "permuted_choice_execution_manifest_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PERMUTED_CHOICE_EXECUTION_MANIFEST_STAGE9833.md"
...
def _order_to_mapping(order: list[str]) -> dict[str, str]:
    # order gives which original semantic occupies option A-E after permutation.
...
def _remap_labels(row: dict[str, Any], mapping: dict[str, str]) -> dict[str, Any]:
    ...
    if isinstance(input_state.get("task_observation"), str):
        input_state["task_observation"] = _remap_inline_options(str(input_state.get("task_observation") or ""), mapping)
    if isinstance(input_state.get("visible_locality_evidence"), str):
        input_state["visible_locality_evidence"] = _remap_inline_options(str(input_state.get("visible_locality_evidence") or ""), mapping)
...
def build_rows() -> list[dict[str, Any]]:
    ...
    for lang in LANGS:
        for split in SPLITS:
            ...
            if len(bucket) != 5:
                raise ValueError(f"expected 5 rows in {lang}:{split}, found {len(bucket)}")
            ...
            for idx, row in enumerate(bucket):
                ...
                output.append(_remap_labels(row, mapping))
    return output
...
def build_audit(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for lang in LANGS:
        for split in SPLITS:
            ...
            safe_signatures = {_safe_signature(row) for row in bucket}
            permutation_maps = {json.dumps(row.get("choice_permutation_map") or {}, sort_keys=True) for row in bucket}
            choice_lists = {
            ...
            if len(safe_signatures) != 5:
                failures.append(f"bucket_signature_mismatch:{key}:{len(safe_signatures)}")
            if len(permutation_maps) != 5:
                failures.append(f"bucket_permutation_mismatch:{key}:{len(permutation_maps)}")
            if len(choice_lists) != 5:
                failures.append(f"bucket_choice_list_mismatch:{key}:{len(choice_lists)}")
...
def main() -> None:
    ...
    next_step = "Run a fresh target-100M structured probe and same-surface Gemma comparison on the permuted-choice manifest so fixed A-E semantic shortcuts are no longer stable across rows."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Built a trainer-compatible multilingual execution manifest that preserves the current same-surface counterfactual packet while randomizing the option-to-semantic mapping per row, forcing models away from stable label-proxy shortcuts.",
