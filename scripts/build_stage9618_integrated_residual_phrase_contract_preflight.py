...
STAGE = 9618
NAME = "stage9618_integrated_residual_phrase_contract_preflight"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9617_integrated_residual_phrase_denoise_manifest.json"
...
PHASE2_MANIFEST = ROOT / "runs/local/artifacts/stage9617_integrated_residual_phrase_denoise_manifest/integrated_residual_phrase_denoise_manifest.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
RUN_DIR = OUT_DIR / "two_phase_contract"
AUDIT = OUT_DIR / "integrated_residual_phrase_contract_preflight.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PHRASE_SUFFIX_TWO_PHASE_CONTRACT_PREFLIGHT_STAGE9618.md"
...
def command() -> list[str]:
    return [
        ...
        "--phase2-max-train-rows", "58", "--phase2-max-eval-rows", "12", "--phase2-max-strict-rows", "10",
        ...
        "--output-dir", str(RUN_DIR), "--run-id", "stage9618_integrated_residual_phrase_contract",
        "--contract-only",
    ]
...
def main() -> None:
    ...
    if len(phase2_rows) != 80:
        failures.append("phase2_rows_not_80")
    ...
    next_step = "If execution is explicitly authorized, run a tiny Stage9619 integrated residual-plus-phrase two-phase probe with phase2 caps 58/12/10 and generation-prefix-field enabled."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Validated the two-phase trainer contract against the integrated residual-plus-phrase denoise manifest. No model execution occurred.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9618 Integrated Residual Phrase Contract Preflight",
        "",
