#!/usr/bin/env python3
"""Roll Stage12027 NOT_EXERCISED rows into transition support v21."""
...
ROOT = Path("runs/local/artifacts/stage12028_transition_support_rollup_v21")
BASE = Path("runs/local/artifacts/stage12026_transition_support_rollup_v20/transition_support_rows_v20.jsonl")
ADDED = Path("runs/local/artifacts/stage12027_not_exercised_real_filter_rows/not_exercised_real_filter_rows.jsonl")
ROWS_OUT = ROOT / "transition_support_rows_v21.jsonl"
SUMMARY = ROOT / "transition_support_rollup_v21.json"
SUMMARY_MIRROR = Path("runs/summaries/stage12028_transition_support_rollup_v21.json")
...
def main() -> None:
    ...
    duplicates = 0
    for source, path in [("stage12026", BASE), ("stage12027", ADDED)]:
        for row in read_jsonl(path):
            out = dict(row)
            out["stage12028_source"] = source
            out["split"] = "train"
            out["split_role"] = "stage12028_transition_support_v21_train_support_only"
    ...
    summary = {
        ...
        "decision": "support_inventory_v21_not_train_ready",
        "reason": "NOT_EXERCISED coverage improved, but Transition-Root-250 still lacks PASS_TO_PASS, PASS_CURRENT_BUILD_AND_RUN, FAIL_TO_PASS, and total independent roots.",
        "claim_boundary": "Support inventory only. No model promotion or Gemma comparison should be claimed from v21.",
        "next_stage_recommendation": {
            "stage": "stage12029_transition_root_250_build_run_and_pass_expansion",
            "action": "Prioritize paired build+run and PASS_TO_PASS rows from underrepresented Rust/C++/Python roots, while continuing real FAIL_TO_PASS mining.",
        },
    }
