...
STAGE = 10106
NAME = "stage10106_real_session_adjudicated_manifest_compiler"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "real_session_bootstrap_adjudication_audit.json"
ADJUDICATED_MANIFEST = OUT_DIR / "real_session_bootstrap_adjudicated_manifest.jsonl"
BLOCKED_ROWS = OUT_DIR / "real_session_bootstrap_adjudication_blocked_rows.jsonl"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "REAL_SESSION_ADJUDICATED_MANIFEST_COMPILER_STAGE10106.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"

BRIDGE = ROOT / "runs/local/artifacts/stage10105_real_session_bootstrap_claim_bridge_after_review_workbook/real_session_bootstrap_claim_bridge_after_review_workbook.json"
REVIEW_ROOT = ROOT / "runs/local/artifacts/stage10104_real_session_bootstrap_review_packets/review_packets"
...
def compile_adjudications(
    *,
    ...
    expected_bridge_rows: int | None = 33,
) -> dict[str, Any]:
    ...
    if bridge.get("passed") is not True:
        failures.append("stage10105_bridge_not_passed")
    ...
    if expected_bridge_rows is not None and len(bridge_rows) != expected_bridge_rows:
        failures.append(f"stage10105_bridge_rows_not_{expected_bridge_rows}")
...
def main() -> None:
    ...
    audit = built["audit"]
    next_step = "Complete the stage10104 rubric and anti-cheat review files, then rerun this compiler so only fully adjudicated rows enter the manifest used for shortcut reruns, training, or Gemma comparison."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Compiled the review-packet contract into a machine-usable adjudication gate so only rows with completed expert rubric, completed anti-cheat review, and an explicit gold answer can enter training or scoring.",
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage10106 Real Session Adjudicated Manifest Compiler",
                "",
