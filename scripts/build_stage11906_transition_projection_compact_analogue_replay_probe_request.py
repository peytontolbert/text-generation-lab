...
STAGE = 11906
NAME = "stage11906_transition_projection_compact_analogue_replay_probe_request"
OUT = ART / NAME
SUMMARY = OUT / "transition_projection_compact_analogue_replay_probe_request.json"
COMMAND_JSON = OUT / "transition_projection_compact_analogue_replay_command.json"
MANIFEST = OUT / "transition_projection_compact_analogue_replay_manifest.jsonl"
...
RUN_DIR = ART / "stage11907_transition_projection_compact_analogue_replay_probe"
...
def train_copy(row: dict[str, Any], *, suffix: str, replay_kind: str, preservation_exempt: bool) -> dict[str, Any]:
    ...
    out["stage11906_replay_kind"] = replay_kind
...
def main() -> None:
    ...
    for row in support:
        train_rows.append(train_copy(row, suffix="stage11906_transition_projection", replay_kind="transition_projection", preservation_exempt=True))
    ...
    for repeat in range(5):
        for row in compact_analogues:
            train_rows.append(
                train_copy(
                    row,
                    suffix=f"stage11906_compact_analogue_r{repeat}",
    ...
    gates = {
        ...
        "support_preservation_exempt": all(row.get("preservation_exempt") for row in train_rows if row.get("stage11906_replay_kind") == "transition_projection"),
    }
    artifact = {
        "stage": STAGE,
        "stage_name": NAME,
        ...
        "decision": "transition_projection_compact_analogue_replay_probe_request_ready" if all(gates.values()) else "transition_projection_compact_analogue_replay_probe_request_blocked",
        "command": command,
        ...
        "claim_boundary": [
            ...
            "If it preserves gates and improves projection fit, it is the first heldout-safe transition-curriculum candidate.",
        ],
    }
