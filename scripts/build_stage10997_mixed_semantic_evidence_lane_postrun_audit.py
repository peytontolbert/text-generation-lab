...
STAGE = 10997
NAME = "stage10997_mixed_semantic_evidence_lane_postrun_audit"
OUT_DIR = ARTIFACTS / NAME
SUMMARY_JSON = OUT_DIR / "mixed_semantic_evidence_lane_postrun_audit.json"
RUNTIME_BUNDLE = ARTIFACTS / "stage10996_mixed_semantic_evidence_lane_probe" / "runtime_model" / "runtime_model_bundle.json"
...
def main() -> None:
    ...
    overlay_val = _write_bounded_choice_eval_audit(OUT_DIR, model=model, rows=load_jsonl(OVERLAY_VAL), tokenizer=tokenizer, max_encoder_tokens=768, max_decoder_tokens=8, split_name="eval_sampler_contract", bounded_choice_aux_source=source, eval_batch_size=8)
    overlay_strict = _write_bounded_choice_eval_audit(OUT_DIR, model=model, rows=load_jsonl(OVERLAY_STRICT), tokenizer=tokenizer, max_encoder_tokens=768, max_decoder_tokens=8, split_name="strict_eval_sampler_contract", bounded_choice_aux_source=source, eval_batch_size=8)
    ...
    payload = {
        ...
        "passed": True,
        "claim_scope": [
            "Audit the mixed semantic evidence-lane runtime against the unchanged overlay and the two live residual slices.",
            "Measure whether semantic evidence-role supervision plus mapped option retrieval improves the residual boundaries without regressing the honest heldout frontier.",
        ],
        ...
        "next_best_step": "Compare these results against stage10986 and the standing overlay baseline to decide whether semantic role targets are a viable next frontier path.",
    }
