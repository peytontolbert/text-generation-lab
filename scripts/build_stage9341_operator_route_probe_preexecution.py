...
STAGE = 9341
NAME = "stage9341_operator_route_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9340_operator_route_separation_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9340_operator_route_separation_manifest/operator_route_separation_manifest.jsonl"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9342_operator_route_separation_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "targeted_repair_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9342_operator_route_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "OPERATOR_ROUTE_PROBE_PREEXECUTION_STAGE9341.md"
...
COMMAND = [
    ...
    "--max-train-rows", "9", "--max-eval-rows", "6", "--max-strict-rows", "3",
    ...
    "--enable-generation-audit", "--max-generation-rows", "18", "--max-generation-tokens", "64",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9342_operator_route_separation_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9340_not_passed")
    if len(rows) != 18:
        failures.append("manifest_row_count_not_21")
    if split_counts != {"train": 9, "eval": 6, "strict_eval": 3}:
    ...
    required_pairs = {
        "--mode": "denoise_repair_probe", "--probe-scale": "target_100m", "--max-train-rows": "9",
        "--max-eval-rows": "6", "--max-strict-rows": "3", "--max-steps": "80",
        "--max-generation-rows": "18", "--decoder-ce-weight": "0.0", "--structured-aux-weight": "0.0",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared the tiny operator-route separation target-100M denoise repair probe; decoder CE remains closed.",
        "next_best_step": "Execute Stage9342 operator-route separation probe under trellis and audit residual repair quality.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9341 Operator Route Probe Preexecution",
        "",
        f"Passed: `{card['passed']}`",
        f"Rows: `{card['manifest_rows']}`",
        f"Splits: `{card['split_counts']}`",
        f"Tasks: `{card['task_counts']}`",
        "Only the next tiny operator-route target-100M denoise probe is authorized. Decoder CE and external authority remain closed.",
        "",
    ]), encoding="utf-8")
