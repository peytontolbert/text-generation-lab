...
from scripts.bounded_choice_policy import choose_bounded_choice_label, policy_correct


STAGE = 10910
NAME = "stage10910_python_verifier_transition_policy_slice_comparison"
OUT_DIR = ROOT / "runs" / "local" / "artifacts" / NAME
SUMMARY_JSON = OUT_DIR / "python_verifier_transition_policy_slice_comparison.json"
...
POLICY = "decoder_on_transition_only"
...
def main() -> None:
    ...
    for row in rows:
        row_id = str(row.get("row_id") or "")
        hundred = hundred_by_id.get(row_id) or {}
        policy_label = choose_bounded_choice_label(
            row=row,
            constrained_label=str(hundred.get("constrained_choice_top1_label") or "") or None,
            decoder_label=str(hundred.get("full_vocab_top1_text") or "") or None,
            policy=POLICY,
        )
        combined.append(
            {
                ...
                **hundred,
                "policy_name": POLICY,
                "policy_predicted_label": policy_label,
                "policy_correct": policy_correct(target_label=str(row.get("target_text") or ""), predicted_label=policy_label),
    ...
    summary = {
        ...
        "passed": True,
        "claim_scope": [
            "Diagnostic same-slice comparison using the decoder-on-transition-only policy candidate for 100M verifier_outcome_semantic_transition rows.",
            "Gemma remains direct opaque-label generation; this artifact is for explicit policy-boundary comparison on the fresh Python verifier-transition slice.",
        ],
        "runtime_bundle": display(RUNTIME_BUNDLE),
        "runtime_initialization": init_card,
        "policy_name": POLICY,
        "hundred_m_current_retrieval": {
            "overall": metric_block(combined, "constrained_choice_match"),
            "by_repo": group_metrics(combined, "repo_id", "constrained_choice_match"),
        },
        "hundred_m_policy_candidate": {
            "overall": metric_block(combined, "policy_correct"),
            "by_repo": group_metrics(combined, "repo_id", "policy_correct"),
        },
        "gemma12b": {
