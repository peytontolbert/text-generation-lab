...
STAGE = 9103
NAME = "stage9103_current_frontier_reconciliation_after_contract_only_schema_graph"
...
DOC = ROOT / "docs" / "CURRENT_FRONTIER_AFTER_CONTRACT_ONLY_SCHEMA_GRAPH_STAGE9103.md"
SPINE = ROOT / "docs" / "MODEL_STACK_SPINE.md"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
CARD = OUT_DIR / "current_frontier_after_contract_only_schema_graph.json"

RECOVERED_RECENT_CONTROLS = [
    "stage9099_frontier_after_runtime_assertion_graph",
    "stage9100_contract_only_artifact_schema_design",
    "stage9101_contract_only_artifact_schema_negative_case_audit",
    "stage9102_contract_only_schema_graph_attachment",
    "graph_gate_contract_only_requires_no_model_forward_proof",
    "graph_gate_contract_only_requires_no_training_execution_proof",
    "graph_gate_contract_only_requires_runtime_assertion_plan",
    "graph_gate_contract_only_requires_telemetry_artifact_plan",
]
...
def build_card(registry: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "source_stage9102_passed": s9102.get("passed") is True,
        "stage9101_negative_cases_rejected": (s9101.get("metrics") or {}).get("negative_cases_rejected") == (s9101.get("metrics") or {}).get("negative_cases"),
        "stage9102_added_graph_nodes": (s9102.get("metrics") or {}).get("added_nodes", 0) >= 6,
        "stage9102_added_graph_edges": (s9102.get("metrics") or {}).get("added_edges", 0) >= 16,
        "stage9102_authority_rows_zero": (s9102.get("metrics") or {}).get("authority_rows") == 0,
        "stage9102_contract_only_closed": (s9102.get("metrics") or {}).get("contract_only_invoked_now") is False,
        "stage9102_cleanup_closed": (s9102.get("metrics") or {}).get("cleanup_authorized_now") is False,
        "recovered_recent_controls_recorded": len(RECOVERED_RECENT_CONTROLS) >= 8,
        "next_safe_branches_recorded": len(NEXT_SAFE_BRANCHES) >= 4,
        "registry_frontier_stage9102": int((registry.get("metrics") or {}).get("latest_stage", -1)) == 9102,
        "authority_counts_zero": not any(((registry.get("metrics") or {}).get("authority_counts") or {}).get(key, 0) for key in AUTHORITY_CLOSED),
    }
    return {
        "stage": STAGE,
        "stage_name": NAME,
        "status": "CURRENT_FRONTIER_RECONCILED_AFTER_CONTRACT_ONLY_SCHEMA_GRAPH",
        ...
        "metrics": {
            ...
            "stage9102_added_nodes": (s9102.get("metrics") or {}).get("added_nodes", 0),
            "stage9102_added_edges": (s9102.get("metrics") or {}).get("added_edges", 0),
        ...
        "decision": "Frontier reconciled after contract-only artifact schema graph attachment. Trainer command controls, runtime assertion controls, and contract-only artifact schema controls are graph-visible, but no data access, route-card materialization, route-to-loss translation, model input rows, compiler handoff, trainer dry run, trainer invocation, contract-only invocation, runtime assertion execution, model forward, decoder CE, denoise CE, runtime, cleanup, or training is authorized.",
    }
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": card["decision"] if not failures else "Current frontier reconciliation after contract-only schema graph attachment failed.",
        "next_best_step": "Refresh trainer execution-authorization review card without execution; do not invoke trainer contract-only mode.",
    ...
    DOC.write_text("\n".join([
        ...
        "",
        "Recent controls are reconciled through Stage9102. Trainer command controls, runtime assertion controls, and contract-only artifact schema controls are graph-visible and closed.",
        "",
        f"Stage9102 added nodes: `{card['metrics']['stage9102_added_nodes']}`",
        f"Stage9102 added edges: `{card['metrics']['stage9102_added_edges']}`",
        "",
        "Next safe branches:",
        "",
    ...
    marker = "## Stage9103 Current Frontier After Contract-Only Schema Graph"
