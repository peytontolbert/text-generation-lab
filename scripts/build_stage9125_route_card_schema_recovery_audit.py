...
STAGE = 9125
NAME = "stage9125_route_card_schema_recovery_audit"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
SOURCE_9124 = ROOT / "runs/summaries/stage9124_route_card_schema_recovery_design.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "ROUTE_CARD_SCHEMA_RECOVERY_AUDIT_STAGE9125.md"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "route_card_schema_recovery_audit.json"
...
def registry(latest: int = 9123) -> dict[str, Any]:
...
def run_negative_cases() -> dict[str, Any]:
    ...
    cases["missing_loss_mask_ref"] = missing_field
    missing_route = copy.deepcopy(base)
    missing_route["route_enum"].remove("KEEP_BOUNDED_DECODER")
    cases["missing_keep_bounded_decoder_route"] = missing_route
...
def build_audit() -> dict[str, Any]:
    source = load_json(SOURCE_9124)
    base = build_schema(registry())
    base_failures = validate_schema(base, registry())
    negatives = run_negative_cases()
    checks = {
        "source_stage9124_present": SOURCE_9124.exists(),
        "source_stage9124_passed": source.get("passed") is True,
    ...
    return {
        ...
        "decision": "Route-card schema recovery audit rejects missing required fields, missing route enums, materialization, compiler handoff, trainer/model paths, training, runtime, upload, cleanup, authority, and bad frontier.",
    }
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": audit["decision"] if audit["passed"] else "Route-card schema recovery audit failed.",
        "next_best_step": "Recover route-to-loss translation contract; do not materialize real route cards yet.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9125 Route-Card Schema Recovery Audit",
        "",
