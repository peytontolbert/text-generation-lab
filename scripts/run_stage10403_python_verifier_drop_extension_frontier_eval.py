...
NAME = "stage10403_python_verifier_drop_extension_frontier_eval"
...
def _variant_name(row: dict[str, object]) -> str:
    ...
    if language == "python" and perspective == "verifier_outcome":
...
def _variant_text(row: dict[str, object], value: str, variant_name: str) -> str:
    if variant_name == "raw_value":
        return value
    if variant_name == "drop_extension":
    ...
    if variant_name == "task_templated":
...
def _score_row(model: AgentKernelLiteTransformerSeq2Seq, tokenizer: AgentKernelBPETokenizer, row: dict[str, object]) -> dict[str, object]:
    ...
    variant_name = _variant_name(row)
    option_pairs = _option_pairs(row)
    texts = [_variant_text(row, value, variant_name) for _, value in option_pairs]
...
def main() -> None:
    ...
    out_path = OUT_DIR / "python_verifier_drop_extension_frontier_eval.json"
