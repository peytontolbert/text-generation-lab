...
STAGE = 9939
NAME = "stage9939_v27_completion_boundary"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
BOUNDARY = OUT_DIR / "v27_completion_boundary.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "V27_COMPLETION_BOUNDARY_STAGE9939.md"
...
def build_boundary() -> dict[str, Any]:
    ...
    finish_rows = {
        ...
        for row in (finish.get("language_rows") or [])
        if isinstance(row, dict)
    }
    acceptance_rows = {
        str(row.get("language_family") or ""): row
        for row in (acceptance.get("records") or [])
        if isinstance(row, dict)
    }
    ...
    for lang in LANGS:
        ...
        boundary_rows.append({
            ...
            "completion_state": (
                "complete"
                if acceptance_ready and int(finish_row.get("standalone_human_signoff_tasks_remaining") or 0) == 0
                else "standalone_win_plus_handoff_ready_waiting_for_real_harness_outputs"
            ),
            "remaining_blockers": [
                *list(finish_row.get("remaining_blockers") or []),
                *(
                    []
                    if acceptance_ready
                    else ["real_harness_outputs_still_missing"]
                ),
            ],
        })
    ...
    metrics = {
        ...
        "standalone_human_signoff_tasks_remaining": sum(
            int(row["standalone_human_signoff_tasks_remaining"] or 0) for row in boundary_rows
        ),
    ...
    return {
        ...
        "completion_rule": {
            "standalone_requirement": "all 4 language families retain 100m_better strict verdict and receive signed expert rubric plus anti-cheat review",
...
def main() -> None:
    ...
    next_step = (
        "Complete the 8 remaining standalone human signoff tasks, then execute the 4 harness backend handoff bundles "
        "until the 28 reserved stub artifacts are replaced by real same-task-pack runtime outputs."
    )
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        ...
        "decision": (
            "Materialized the live v2.7 completion boundary by joining the four-language standalone finish gate to the "
            "harness acceptance audit, proving that harness packaging is handoff-ready but still entirely stub-backed."
        ),
    ...
    DOC.write_text("\n".join([
        "# Stage9939 V27 Completion Boundary",
        "",
