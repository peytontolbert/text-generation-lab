...
STAGE = 9603
NAME = "stage9603_two_phase_suffix_denoise_tiny_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9602_two_phase_in_memory_wrapper_static_audit.json"
...
AUDIT = OUT_DIR / "two_phase_suffix_denoise_tiny_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "TWO_PHASE_SUFFIX_DENOISE_TINY_PROBE_STAGE9603.md"
...
def command() -> list[str]:
    return [
        ...
        "--output-dir", str(RUN_DIR), "--run-id", "stage9603_two_phase_suffix_denoise_probe",
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9602_not_passed")
    ...
    next_step = "Audit Stage9603 samples and phase2 failure buckets; if denoise quality is not passing, patch residual target rendering or phase2 curriculum before widening."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Executed the first in-memory two-phase suffix-choice plus residual-denoise reconnect probe. This is a measurement probe, not widening or promotion.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9603 Two-Phase Suffix/Denoise Tiny Probe",
        "",
    ...
    print(json.dumps({"stage": STAGE, "passed": audit["passed"], "quality_gate": quality_gate, "failures": failures, "phase1_eval": phase1_exact, "phase1_strict": phase1_strict, "phase2_contentful": audit['phase2_contentful_generation_rate'], "phase2_prefix": audit['phase2_target_prefix_match_rate']}, indent=2, sort_keys=True))
    if not audit["passed"]:
        raise SystemExit(1)
