...
STAGE = 9357
NAME = "stage9357_non_operator_residual_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9356_non_operator_residual_probe_preexecution.json"
...
RUN_DIR = ROOT / "runs/local/artifacts/stage9357_non_operator_residual_probe"
AUDIT = RUN_DIR / "stage9357_non_operator_residual_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "NON_OPERATOR_RESIDUAL_PROBE_AUDIT_STAGE9357.md"
...
def audit_run() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9356_not_passed")
    ...
    quality_gate_passed = bool(generated == 32 and exact == 32 and target_prefix == 32 and boundary == 32 and contentful == 32 and repetition_rows == 0 and short_rows == 0 and leak_rows == 0)
    return {
        ...
        "diagnosis": "non-operator residual repair rows solved dependency inside, numeric preserves, and file-path localized-edit residuals in isolation.",
        "next_patch_target": "stage9358_full_mixture_with_non_operator_residual_repairs_manifest",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "The non-operator residual repair probe passed exactly in isolation; decoder CE remains closed.",
        "next_best_step": "Merge Stage9355 rows back into the full mixture and rerun a closed rejoin probe before any bounded decoder CE step.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9357 Non-Operator Residual Probe Audit", "", f"Passed: `{audit['passed']}`", f"Safety gate passed: `{audit['safety_gate_passed']}`", f"Quality gate passed: `{audit['quality_gate_passed']}`", f"Exact match rows: `{audit['exact_match_rows']}` / `{audit['generated_rows']}`", f"Target-prefix rows: `{audit['target_prefix_match_rows']}` / `{audit['generated_rows']}`", f"Boundary next-token rows: `{audit['boundary_next_token_match_rows']}` / `{audit['generated_rows']}`", f"Contentful rows: `{audit['contentful_rows']}` / `{audit['generated_rows']}`", f"Degenerate repetition rows: `{audit['degenerate_repetition_rows']}`", "", "Dependency-inside, numeric-preserves, and file-path localized-edit residual repairs all passed in isolation. Decoder CE and external authority remain closed.", ""]), encoding="utf-8")
