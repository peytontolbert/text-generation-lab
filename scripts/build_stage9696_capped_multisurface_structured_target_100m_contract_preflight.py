...
STAGE = 9696
NAME = "stage9696_capped_multisurface_structured_target_100m_contract_preflight"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9695_multisurface_structured_tiny_execution_review.json"
MANIFEST_DIR = ROOT / "runs/local/artifacts/stage9695_multisurface_structured_tiny_execution_review/tiny_structured_manifests"
...
DOC = ROOT / "docs" / "CAPPED_MULTISURFACE_STRUCTURED_TARGET_100M_CONTRACT_PREFLIGHT_STAGE9696.md"
...
def run_surface(surface: str) -> dict[str, Any]:
    ...
    if len(rows) != sum(CAPS.values()):
        failures.append("manifest_row_count_not_64")
    if split_counts(rows) != {**CAPS, "other": 0}:
        failures.append("manifest_split_caps_mismatch")
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9695_not_passed")
    surface_results = []
    if not failures:
    ...
    next_step = "If disk/safe-cleanup preflight is clean and execution is explicitly confirmed, run one tiny target-100M structured probe surface first; otherwise build Stage9697 disk/safe-cleanup execution readiness gate."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Validated capped Stage9695 tiny structured manifests under target-100M contract-only preflight. This does not execute model training.",
