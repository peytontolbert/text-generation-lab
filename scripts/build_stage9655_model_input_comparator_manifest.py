...
STAGE = 9655
NAME = "stage9655_model_input_comparator_manifest"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9654_episode_collator_visibility_audit.json"
...
MANIFEST = OUT_DIR / "model_input_comparator_manifest.jsonl"
RUN_DIR = OUT_DIR / "episode_step_contract"
AUDIT = OUT_DIR / "model_input_comparator_manifest_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "MODEL_INPUT_COMPARATOR_MANIFEST_STAGE9655.md"
...
def relation_from_row(row: dict[str, Any]) -> str:
    ob = row["episode_transition"]["observation_t"]
    return "same" if bool(ob.get("boundary_next_token_match")) else "different"
...
def build_rows() -> list[dict[str, Any]]:
    ...
    for row in load_jsonl(BASE_MANIFEST):
        ...
        clone["row_id"] = str(clone["row_id"]).replace("stage9649", "stage9655")
        clone["objective_family"] = "model_input_comparator_boundary_prefix"
        relation = relation_from_row(clone)
        clone["model_input"] = {
...
def feature(row: dict[str, Any], name: str) -> str:
    if name == "model_boundary_token_relation":
        ...
        return str(model.get("boundary_token_relation"))
    if name == "prefix":
...
def contract_command() -> list[str]:
    return ["env", f"TMPDIR={TMPDIR}", f"TEMP={TMPDIR}", f"TMP={TMPDIR}", "conda", "run", "-n", "trellis", "python", str(TRAINER),
        ...
        "--output-dir", str(RUN_DIR), "--run-id", "stage9655_model_input_comparator_contract", "--contract-only"]
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9654_not_passed")
    ...
    audit = {"passed": not failures, "failures": failures, "source_summary": str(SOURCE_SUMMARY.relative_to(ROOT)), "base_manifest": str(BASE_MANIFEST.relative_to(ROOT)), "manifest": str(MANIFEST.relative_to(ROOT)), "rows": len(rows), "split_counts": split_counts, "prefix_baselines": prefix_baselines, "model_input_comparator_baselines": comparator_baselines, "contract_passed": card.get("passed"), "loss_counts": card.get("loss_counts"), "model_execution_attempted": card.get("model_execution_attempted"), "authority": dict(AUTHORITY_CLOSED)}
    AUDIT.write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    next_step = "If Stage9655 passes, run Stage9656 model-input comparator micro target-100M probe."
    summary = {"stage": STAGE, "stage_name": NAME, "name": NAME, "passed": audit["passed"], "authority": dict(AUTHORITY_CLOSED), "metrics": {**dict(AUTHORITY_CLOSED), **audit}, "artifacts": {"audit": str(AUDIT.relative_to(ROOT)), "doc": str(DOC.relative_to(ROOT)), "manifest": str(MANIFEST.relative_to(ROOT)), "run_dir": str(RUN_DIR.relative_to(ROOT))}, "decision": "Model-input comparator manifest passed contract preflight; no model execution occurred." if audit["passed"] else "Model-input comparator manifest failed; do not execute.", "next_best_step": next_step, "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9655 Model-Input Comparator Manifest", "", f"Passed: `{audit['passed']}`", f"Rows: `{audit['rows']}`", f"Splits: `{split_counts}`", f"Prefix baselines: `{prefix_baselines}`", f"Model-input comparator baselines: `{comparator_baselines}`", "", "`model_input.boundary_token_relation` is consumed by `_row_text`; `encoder_text` and `state_features` are not.", "", "Decoder CE, denoise CE, runtime, Gemma, harness, checkpoint export, and promotion remain closed.", "", f"Next: {next_step}", ""]), encoding="utf-8")
