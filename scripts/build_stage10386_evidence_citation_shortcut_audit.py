...
NAME = "stage10386_evidence_citation_shortcut_audit"
...
def _variant_name(row: dict[str, object]) -> str:
    if _perspective(row) == "evidence_citation":
        return "evidence_citation_strip_candidate_change_surface"
    return "task_role_templated" if _language(row) == "rust" else "raw_value"
...
def _variant_text(row: dict[str, object], value: str, variant_name: str) -> str:
    perspective = _perspective(row)
    header = _header_prefix(row)
    if variant_name == "raw_value":
        return value
    if variant_name == "task_role_templated":
        ...
        return f"Perspective={perspective}\nCandidate under review: {value}"
    if variant_name == "header_only":
        return f"{header}\nCandidate under review: {value}" if header else f"Candidate under review: {value}"
    raise ValueError(f"unsupported option variant: {variant_name}")


def _query_variant_row(row: dict[str, object], query_variant: str) -> dict[str, object]:
    clone = dict(row)
    prompt = str(clone.get("prompt_text") or "")
    if query_variant == "raw_prompt":
        return clone
    if query_variant == "strip_candidate_change_surface":
        clone["prompt_text"] = "\n".join(
            line for line in prompt.splitlines()
            if not line.startswith("candidate_change_surface ")
        )
        return clone
    if query_variant == "strip_candidate_and_external":
        clone["prompt_text"] = "\n".join(
            line for line in prompt.splitlines()
            if not line.startswith("candidate_change_surface ")
            and not line.startswith("external_analogue_reference ")
        )
        return clone
...
def _score_row(
    ...
    *,
    ...
    scorer_row = _query_variant_row(row, query_variant)
    batch = build_batch([scorer_row], max_encoder_tokens=768, max_decoder_tokens=8, tokenizer=tokenizer)
    with torch.no_grad():
    ...
    option_variant = _variant_name(row)
    option_pairs = _option_pairs(row)
    texts = [_variant_text(row, value, option_variant if option_variant != "evidence_citation_strip_candidate_change_surface" else "raw_value") for _, value in option_pairs]
...
def main() -> None:
    ...
    variants = ["raw_prompt", "strip_candidate_change_surface", "strip_candidate_and_external"]
    results: dict[str, object] = {}
    for variant in variants:
        row_cards = [_score_row(model, tokenizer, row, query_variant=variant) for row in rows]
    ...
    out_path = OUT_DIR / "evidence_citation_shortcut_audit.json"
    out_path.write_text(
        json.dumps(
            {
                "stage_name": NAME,
