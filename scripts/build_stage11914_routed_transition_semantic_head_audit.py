#!/usr/bin/env python3
"""Audit a routed policy: semantic head for transition rows, evidence head for compact gates."""
...
STAGE = 11914
NAME = "stage11914_routed_transition_semantic_head_audit"
OUT = ART / NAME
SUMMARY = OUT / "routed_transition_semantic_head_audit.json"
...
def main() -> None:
    ...
    postrun = results["stage11912_semantic_head_only"]
    gates = {
        "transition_projection_improved": (postrun["transition_projection_routed"].get("correct") or 0) > (selected["transition_projection_routed"].get("correct") or 0),
        "transition_projection_at_least_320_of_640": (postrun["transition_projection_routed"].get("correct") or 0) >= 320,
    ...
    if all(gates.values()):
        decision = "routed_transition_semantic_head_candidate_passes_all_gates"
    elif gates["transition_projection_improved"] and gates["filtered_strict_preserved"] and gates["old_canary_strict_preserved"]:
        decision = "routed_transition_semantic_head_partial_candidate"
    else:
        decision = "routed_transition_semantic_head_rejected"
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "created_at_utc": now(),
        ...
        "claim_boundary": [
            ...
            "Compact protected gates remain scored with the selected Stage11507 evidence-judgment scorer.",
        ],
    }
