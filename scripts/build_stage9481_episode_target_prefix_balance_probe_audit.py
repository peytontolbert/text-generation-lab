...
STAGE = 9481
NAME = "stage9481_episode_target_prefix_balance_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9480_episode_target_prefix_balance_preflight_audit.json"
RUN_DIR = ROOT / "runs/local/artifacts/stage9481_episode_target_prefix_balance_target_100m_probe"
...
AUDIT = RUN_DIR / "stage9481_target_prefix_balance_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "EPISODE_TARGET_PREFIX_BALANCE_PROBE_AUDIT_STAGE9481.md"
...
def main() -> None:
    ...
    if source.get("passed") is not True or not source.get("metrics", {}).get("execution_authorized_for_next_stage"):
        safety_failures.append("source_stage9480_not_authorized")
    ...
    audit = {
        ...
        "source_stage": "stage9480_episode_target_prefix_balance_preflight_audit",
        ...
        "diagnosis": "Focused target-prefix head is safe but not quality-passing. The remaining strict error is a high-confidence false prediction on the CPP positive target-prefix row, indicating sparse positive coverage for non-Rust languages rather than a generic optimizer issue.",
    ...
    summary = {
        ...
        "decision": "Target-prefix focused probe failed strict safely; mine or construct non-Rust positive target-prefix rows before another execution.",
        "next_best_step": "Build Stage9482 language-balanced positive target-prefix repair queue for CPP, Python, and web rows; keep decoder/runtime/checkpoint authority closed.",
    ...
    DOC.write_text("\n".join([
        ...
        f"High-confidence wrong rows: `{high_conf_wrong}`",
        "",
        audit["diagnosis"],
        "",
    ...
    print(json.dumps({"stage": STAGE, "passed": summary["passed"], "failures": failures, "high_confidence_wrong_rows": high_conf_wrong}, indent=2, sort_keys=True))
