...
STAGE = 11416
NAME = "stage11416_third_family_rust_verifier_log_capture"
OUT = ART / NAME
SUMMARY = OUT / "third_family_rust_verifier_log_capture.json"
QUEUE = ART / "stage11415_flexible_third_family_rust_verifier_atlas/flexible_third_family_rust_verifier_queue.jsonl"
TIMEOUT_SECONDS = 240
...
def main() -> None:
    ...
    candidates = [
        ...
        if row.get("promotion_eligible_family") is True
        and row.get("can_attempt_verifier_log_capture") is True
    ]
    results = [run_one(row) for row in candidates]
    materializable = [row for row in results if row["materialization_allowed_after_log"]]
    repo_families = {row["repo_family"] for row in materializable}
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "created_at_utc": now(),
        "passed": True,
        "decision": "third_family_rust_verifier_logs_captured",
        "counts": {
            ...
            "unique_repo_families": len(repo_families),
        },
        "quality_gate": {
            "actual_verifier_logs_present": bool(materializable),
            "third_family_materializable": len(repo_families) >= 1,
            "train_rows_emitted": False,
            "probe_ready": False,
        },
        "results": results,
        "recommended_next_action": "Materialize third-family verifier-log-backed support rows, then update the combined Rust support inventory gate.",
        "source_artifacts": {"stage11415_queue": rel(QUEUE)},
