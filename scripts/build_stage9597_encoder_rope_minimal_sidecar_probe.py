...
STAGE = 9597
NAME = "stage9597_encoder_rope_minimal_sidecar_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9596_encoder_rope_order_sensitivity_audit.json"
...
DOC = ROOT / "docs" / "ENCODER_ROPE_MINIMAL_SIDECAR_PROBE_STAGE9597.md"
...
def main() -> None:
    ...
    if not failures:
        contract = subprocess.run(command(output_dir=CONTRACT_DIR, run_id="stage9597_contract_preflight", contract_only=True), cwd=ROOT, text=True, capture_output=True, check=False)
    ...
    if not failures:
        run = subprocess.run(command(output_dir=RUN_DIR, run_id="stage9597_encoder_rope_minimal_sidecar_probe", contract_only=False), cwd=ROOT, text=True, capture_output=True, check=False)
        if run.returncode != 0:
    ...
    summary = {
        ...
        "decision": "Reran the minimal residual-denoise sidecar probe after enabling encoder RoPE, while keeping decoder/export buckets frozen.",
        "next_best_step": "Promote encoder RoPE/order sensitivity as a structured-head architecture contract, then return to the residual denoise repair path with the now-learnable suffix-choice controller.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9597 Encoder RoPE Minimal Sidecar Probe", "", f"Passed: `{audit['passed']}`", f"Eval suffix choice exact: `{eval_exact}`", f"Strict suffix choice exact: `{strict_exact}`", f"Quality pass gate: `{pass_gate}`", f"Structured optimizer isolated: `{audit['structured_optimizer_isolated']}`", f"Max frozen decoder/LM-head delta: `{audit['max_frozen_bucket_delta']}`", "", "This stage verifies trainer gradient/optimizer routing. Decoder, denoise, runtime, export, harness, and promotion remain closed.", ""]), encoding="utf-8")
