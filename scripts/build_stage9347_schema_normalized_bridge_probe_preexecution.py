...
STAGE = 9347
NAME = "stage9347_schema_normalized_bridge_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9346_schema_normalized_subword_bridge_repair_manifest.json"
...
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9348_schema_normalized_bridge_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "schema_normalized_bridge_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9348_schema_normalized_bridge_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "SCHEMA_NORMALIZED_BRIDGE_PROBE_PREEXECUTION_STAGE9347.md"
...
COMMAND = [
    ...
    "--max-train-rows", "17", "--max-eval-rows", "10", "--max-strict-rows", "4",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9348_schema_normalized_bridge_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9346_not_passed")
    if len(rows) != 31:
        failures.append("unexpected_manifest_row_count")
    if dict(split_counts) != {"train": 17, "eval": 10, "strict_eval": 4}:
    ...
    if set(task_counts) != {"dependency_handle_keeps_the_bridge", "patch_operator_subword_bridge", "localized_edit_file_path_bridge"}:
        failures.append("missing_required_bridge_tasks")
    if unsafe_rows:
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared a closed-boundary 31-row target-100M denoise probe for schema-normalized subword bridge repairs; decoder CE remains closed.",
        "next_best_step": "Execute Stage9348 schema-normalized bridge repair probe under trellis, then audit exactness before any full-mixture rejoin.",
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage9347 Schema-Normalized Bridge Probe Preexecution",
                "",
                ...
                "",
                "Only the Stage9348 31-row target-100M denoise probe is authorized. Decoder CE, runtime, harness, Gemma, scoring, checkpoint export, controller merge, and promotion remain closed.",
                "",
            ]
        ),
        encoding="utf-8",
