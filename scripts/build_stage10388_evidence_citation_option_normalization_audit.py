...
NAME = "stage10388_evidence_citation_option_normalization_audit"
...
def _default_variant(row: dict[str, object]) -> str:
...
def _option_text(row: dict[str, object], value: str, variant: str) -> str:
    ...
    if variant == "evidence_citation_desnake":
...
def _row_variant(row: dict[str, object], evidence_variant: str) -> str:
    if _perspective(row) == "evidence_citation":
        return evidence_variant
    return _default_variant(row)
...
def _score_row(
    ...
    *,
    ...
    option_variant = _row_variant(row, evidence_variant)
...
def main() -> None:
    ...
    variants = [
        "raw_value",
        "evidence_citation_desnake",
        "evidence_citation_role_map",
        "evidence_citation_role_map_templated",
    ]
    results: dict[str, object] = {}
    for variant in variants:
        row_cards = [_score_row(model, tokenizer, row, evidence_variant=variant) for row in rows]
    ...
    out_path = OUT_DIR / "evidence_citation_option_normalization_audit.json"
    out_path.write_text(
        json.dumps(
            {
                "stage_name": NAME,
