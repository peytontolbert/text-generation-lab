#!/usr/bin/env python3
"""Roll Stage12009 targeted Rust rows into transition support v12."""
...
ROOT = Path("runs/local/artifacts/stage12010_transition_support_rollup_v12")
BASE = Path("runs/local/artifacts/stage12008_transition_support_rollup_v11/transition_support_rows_v11.jsonl")
ADDED = Path("runs/local/artifacts/stage12009_rust_targeted_transition_rows/rust_targeted_transition_rows.jsonl")
ROWS_OUT = ROOT / "transition_support_rows_v12.jsonl"
SUMMARY = ROOT / "transition_support_rollup_v12.json"
SUMMARY_MIRROR = Path("runs/summaries/stage12010_transition_support_rollup_v12.json")
...
def main() -> None:
    ...
    duplicates = 0
    for source, path in [("stage12008", BASE), ("stage12009", ADDED)]:
        for row in read_jsonl(path):
            out = dict(row)
            out["stage12010_source"] = source
            out["split"] = "train"
            out["split_role"] = "stage12010_transition_support_v12_train_support_only"
    ...
    summary = {
        ...
        "decision": "support_inventory_v12_not_train_ready",
        "reason": "Rust targeted rows improve status diversity, but Transition-Root-250 floors remain far short, especially PASS_TO_PASS and NOT_EXERCISED.",
        "claim_boundary": "Support inventory only. No model promotion or Gemma comparison should be claimed from v12.",
        "next_stage_recommendation": {"stage": "stage12011_transition_root_250_pass_to_pass_expansion", "action": "Mine more independent executable PASS_TO_PASS roots across all languages before training."},
    }
