...
OUT = ROOT / "runs/local/tmp/pocketpal_stage614_raw_selector_pair_seed461"
ARTIFACT = ROOT / "runs/local/artifacts/stage614_raw_selector_pair_dataset.json"
DOC = ROOT / "docs/stage614_raw_selector_pair_dataset.md"
...
def rewrite_row(row: dict[str, Any]) -> tuple[dict[str, Any], bool]:
    ...
    for key, value in row.items():
        if isinstance(value, str) and key in TEXT_FIELDS:
            ...
            rewritten[key] = value
    rewritten["entity_context_raw_selector_pair"] = True
    rewritten["entity_context_selector_entity"] = entity
    rewritten["entity_context_selector_field"] = field
    rewritten["source_type"] = f"{row.get('source_type', 'retrieval')}_raw_selector_pair"
...
def selector_counts(rows: list[dict[str, Any]]) -> dict[str, int]:
    return dict(Counter(str(row.get("entity_context_selector_field", "") or "") for row in rows if row.get("entity_context_raw_selector_pair")))
...
def main() -> None:
    ...
    manifest.update(
        {
            "artifact_kind": "agentkernel_lite_encdec_stage614_raw_selector_pair",
            ...
            "entity_context_raw_selector_pair": True,
            "stage614_selector_changed_train_examples": changed_counts["train"],
            "stage614_selector_changed_eval_examples": changed_counts["eval"],
            "collision_goal": "Test whether a raw entity|field marker preserves the selector gain without any key name.",
        }
    )
    ...
    summary = {
        "artifact_kind": "stage614_raw_selector_pair_dataset",
        ...
        "decision": "raw_selector_pair_dataset_ready",
        "finding": "Stage614 removes the selector key entirely and tests a raw entity|field marker.",
    }
    ARTIFACT.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text(
        f"""# Stage614 Raw Selector Pair Dataset
