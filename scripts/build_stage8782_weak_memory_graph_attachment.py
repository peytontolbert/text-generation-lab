...
STAGE = 8782
NAME = "stage8782_weak_memory_graph_attachment"
BASE = ROOT / "runs/local/artifacts/stage8779_cost_security_graph_attachment/central_research_graph_with_cost_security.json"
SOURCES = [
    ROOT / "runs/summaries/stage8780_weak_supervision_label_model_readiness.json",
    ROOT / "runs/summaries/stage8781_knowledge_graph_memory_store_readiness.json",
]
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "WEAK_MEMORY_GRAPH_ATTACHMENT_STAGE8782.md"
AUTHORITY_CLOSED = {
...
def main() -> None:
    ...
    weak = "support_module:weak_supervision_label_model"
    memory = "support_module:knowledge_graph_memory_store"
    added_nodes = 0
    added_nodes += int(add_node(nodes, {
        "id": weak,
        "kind": "support_module",
        "node_type": "support_module",
        "name": "weak_supervision_label_model",
        "status": "ready_shadow_only_vote_combiner",
    ...
    added_nodes += int(add_node(nodes, {
        ...
        "kind": "support_module",
        "node_type": "support_module",
        "name": "knowledge_graph_memory_store",
        "status": "ready_no_authority_typed_memory_scaffold",
    ...
    for downstream in [
        ...
        add_edge(edges, weak, "provides_shadow_labels_for", downstream)
        add_edge(edges, memory, "provides_typed_memory_for", downstream)

    out = {"version": NAME, "nodes": list(nodes.values()), "edges": edges}
    graph_path = OUT_DIR / "central_research_graph_with_weak_memory.json"
    nodes_path = OUT_DIR / "central_research_graph_with_weak_memory_nodes.jsonl"
    edges_path = OUT_DIR / "central_research_graph_with_weak_memory_edges.jsonl"
    ...
    card = {
        ...
        "decision": "Attached weak supervision and typed graph memory as no-authority support modules." if not failures else "Weak/memory graph attachment failed.",
        "next_best_step": "Recover source-backed verifier-repair builder under gate_status_contract.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    (OUT_DIR / "weak_memory_graph_attachment_card.json").write_text(json.dumps(card, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    SUMMARY.write_text(json.dumps(card, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage8782 Weak Memory Graph Attachment",
        "",
        f"Passed: `{card['passed']}`",
        "",
        "Attached weak supervision and typed graph memory support modules to the central graph.",
        "",
        "Both modules are no-authority: weak labels are shadow-only, and memory upserts/retrieval do not authorize mining, training, tool execution, runtime, source/body emission, or promotion.",
        "",
    ]), encoding="utf-8")
