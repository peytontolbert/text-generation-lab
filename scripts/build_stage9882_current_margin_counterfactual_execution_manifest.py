...
STAGE = 9882
NAME = "stage9882_current_margin_counterfactual_execution_manifest"
...
MANIFEST = OUT_DIR / "current_margin_counterfactual_execution_manifest.jsonl"
AUDIT = OUT_DIR / "current_margin_counterfactual_execution_manifest_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "CURRENT_MARGIN_COUNTERFACTUAL_EXECUTION_MANIFEST_STAGE9882.md"
...
def build_rows() -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if split is None:
            ...
            cloned["counterfactual_expected_behavior"] = "evaluation_replay_only; trainer execution manifest keeps the separable original surface in eval while harder counterfactual siblings stay in the external audit bank"
        ...
        if obligation == "POSITIVE_ORIGINAL":
            ...
            eval_clone["counterfactual_expected_behavior"] = "evaluation_replay_only; trainer execution manifest keeps the separable original surface in eval while harder counterfactual siblings stay in the external audit bank"
...
def build_audit(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    dropped_obligations = Counter(
        str(row.get("obligation_type") or "")
        for row in load_jsonl(SOURCE)
        if str(row.get("obligation_type") or "") not in ROLE_TO_SPLIT
    )
    bucket = _bucket_index(rows)
    failures: list[str] = []
    if len(rows) != 48:
        failures.append(f"rows_not_48:{len(rows)}")
    if split_counts.get("train") != 16:
        failures.append(f"train_not_16:{split_counts.get('train', 0)}")
    if split_counts.get("eval") != 16:
        failures.append(f"eval_not_16:{split_counts.get('eval', 0)}")
    if split_counts.get("strict_eval") != 16:
        failures.append(f"strict_not_16:{split_counts.get('strict_eval', 0)}")
    ...
    if kept_obligations.get("POSITIVE_ORIGINAL_EVAL_REPLAY", 0) != 16:
        failures.append("positive_original_eval_replay_count_mismatch")
    if dropped_obligations.get("CONTRADICTORY_EVIDENCE_OR_UNSAFE_TWIN", 0) != 16:
        failures.append("dropped_contradictory_count_mismatch")
    if dropped_obligations.get("EVIDENCE_REMOVED", 0) != 16:
...
def main() -> None:
    ...
    next_step = "Run one target-100M structured probe on the Stage9882 execution manifest, then replay the full Stage9881 evidence-removed and contradictory bank as a frozen-model audit to measure whether the current frontier survives stronger shortcut probes."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Converted the Stage9881 stronger current-frontier counterfactual bank into a trainer-compatible execution manifest by using the separable positive-original surface for train and eval, keeping mixed-replay rows in strict_eval, and leaving evidence-removed plus contradictory siblings in the external audit bank because they intentionally break separability.",
