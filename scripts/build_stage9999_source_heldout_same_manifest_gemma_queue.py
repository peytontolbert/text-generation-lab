...
STAGE = 9999
NAME = "stage9999_source_heldout_same_manifest_gemma_queue"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
QUEUE = OUT_DIR / "source_heldout_same_manifest_gemma_queue.json"
PACKETS = OUT_DIR / "source_heldout_same_manifest_gemma_packets.jsonl"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "SOURCE_HELDOUT_SAME_MANIFEST_GEMMA_QUEUE_STAGE9999.md"
...
OUTPUT = ROOT / "runs/local/artifacts/stage10000_source_heldout_same_manifest_gemma_execution/same_prompt_surface_gemma12b_outputs.json"
...
def build_queue() -> dict[str, Any]:
    ...
    if len(rows) != 78:
        failures.append("rows_not_78")
    if split_counts.get("train") != 40:
        failures.append("train_rows_not_40")
    if split_counts.get("eval") != 21:
        failures.append("eval_rows_not_21")
    if split_counts.get("strict_eval") != 17:
        failures.append("strict_rows_not_17")
    packet = {
        "cell_key": "source_heldout_target100m::edit_localization::same_manifest_gemma12b",
        ...
        "queue_entries": [
            {
                ...
                "language_family": "multilingual",
                "priority_rank": 1,
                "priority_reason": "same-manifest Gemma comparison for the source-heldout stage9998 frontier",
...
def main() -> None:
    ...
    next_step = "Run the local Ollama Gemma comparator on this source-heldout same-manifest queue, then compare its rows directly against stage9998."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Materialized a same-manifest Gemma queue and packet set for the source-heldout stage9997 successor manifest so the stage9998 frontier can be compared fairly against Gemma3 12B.",
