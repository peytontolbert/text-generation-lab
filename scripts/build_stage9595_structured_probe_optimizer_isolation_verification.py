...
STAGE = 9595
NAME = "stage9595_structured_probe_optimizer_isolation_verification"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9594_residual_denoise_minimal_sidecar_extended_probe.json"
...
RUN_DIR = RUN_ROOT / "minimal_sidecar_probe"
AUDIT = RUN_ROOT / "residual_denoise_structured_decision_sidecar_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "STRUCTURED_PROBE_OPTIMIZER_ISOLATION_STAGE9595.md"
...
def command(*, output_dir: Path, run_id: str, contract_only: bool) -> list[str]:
    cmd = [
        ...
        "--max-steps", "80", "--batch-size", "2", "--learning-rate", "2e-4",
...
def main() -> None:
    ...
    if not failures:
        contract = subprocess.run(command(output_dir=CONTRACT_DIR, run_id="stage9595_contract_preflight", contract_only=True), cwd=ROOT, text=True, capture_output=True, check=False)
    ...
    if not failures:
        run = subprocess.run(command(output_dir=RUN_DIR, run_id="stage9595_optimizer_isolation_probe", contract_only=False), cwd=ROOT, text=True, capture_output=True, check=False)
        if run.returncode != 0:
    ...
    max_frozen_delta = max(frozen_bucket_deltas.values()) if frozen_bucket_deltas else 0.0
    if execution and execution.get("structured_optimizer_isolated") is not True:
        failures.append("structured_optimizer_isolation_telemetry_missing")
    if execution and int(execution.get("structured_optimizer_frozen_parameter_count") or 0) <= 0:
        failures.append("structured_optimizer_frozen_parameter_count_missing")
    if execution and int(execution.get("structured_optimizer_trainable_parameter_count") or 0) <= 0:
        failures.append("structured_optimizer_trainable_parameter_count_missing")
    if max_frozen_delta > 1e-12:
        failures.append("frozen_decoder_or_lm_head_delta_nonzero")
    ...
    audit = {
        ...
        "frozen_bucket_deltas": frozen_bucket_deltas,
        "max_frozen_bucket_delta": max_frozen_delta,
        "module_delta_norms": str((RUN_DIR / "module_delta_norms.json").relative_to(ROOT)),
    ...
    summary = {
        ...
        "decision": "Verified that structured-only target_100M probes freeze decoder and LM-head parameters while running the minimal residual-denoise sidecar surface.",
        "next_best_step": "If optimizer isolation holds, diagnose why minimal verifier evidence still learns at chance before reopening denoise or decoder CE.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9595 Structured Probe Optimizer Isolation Verification", "", f"Passed: `{audit['passed']}`", f"Eval suffix choice exact: `{eval_exact}`", f"Strict suffix choice exact: `{strict_exact}`", f"Quality pass gate: `{pass_gate}`", f"Structured optimizer isolated: `{audit['structured_optimizer_isolated']}`", f"Max frozen decoder/LM-head delta: `{audit['max_frozen_bucket_delta']}`", "", "This stage verifies trainer gradient/optimizer routing. Decoder, denoise, runtime, export, harness, and promotion remain closed.", ""]), encoding="utf-8")
