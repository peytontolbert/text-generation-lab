...
STAGE = 9355
NAME = "stage9355_non_operator_residual_repair_manifest"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9354_schema_terminal_rejoin_probe_audit.json"
SOURCE_MANIFEST = ROOT / "runs/local/artifacts/stage9352_full_mixture_with_schema_and_terminal_repairs_manifest/full_mixture_with_schema_and_terminal_repairs_manifest.jsonl"
SOURCE_RUN = ROOT / "runs/local/artifacts/stage9354_schema_terminal_rejoin_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
MANIFEST = OUT_DIR / "non_operator_residual_repair_manifest.jsonl"
AUDIT = OUT_DIR / "non_operator_residual_repair_manifest_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "NON_OPERATOR_RESIDUAL_REPAIR_MANIFEST_STAGE9355.md"
...
def residual_type(target: str, generated: str) -> str:
    if "patch ins" in generated:
        return "dependency_inside_terminal_repair"
    if "small constant that the expected" in generated:
        return "numeric_preserves_omission_repair"
    if "localized patch operatch operator" in generated:
        return "file_path_localized_edit_repair"
    return "non_operator_exact_residual_repair"
...
def build_rows() -> list[dict[str, Any]]:
    ...
    for idx, sample in enumerate(samples):
        ...
        row_id = f"stage9355_residual_{idx:03d}_{h(source_id + generated)}"
        mi.update(
            {
                "active_generation_prefix_span": prefix,
                "active_generation_prefix_words": len(prefix.split()),
                "bridge_error_family": task,
                "bridge_repair_objective": "non_operator_residual_repair",
                "corrupted_output_fragment": generated[-48:],
                ...
                "route_schema_version": "stage9355_non_operator_residual_v1",
            }
        )
        ...
        state.update(
            {
                "source_stage9354_row_id": source_id,
                "source_stage9354_generated_sha256": hashlib.sha256(generated.encode("utf-8")).hexdigest(),
                "target_text_sha256": hashlib.sha256(target.encode("utf-8")).hexdigest(),
                "non_operator_residual_repair_required": True,
        ...
        anti.update(
            {
                ...
                "non_operator_residual_repair": True,
            }
        )
        row.update(
            {
                ...
                "merged_curriculum_source": "non_operator_residual_repair",
                "combined_curriculum_source": "non_operator_residual_repair",
...
def audit_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if dict(split_counts) != {"train": 17, "eval": 8, "strict_eval": 7}:
        failures.append("unexpected_split_counts")
    if dict(task_counts) != {"dependency_inside_terminal_repair": 22, "file_path_localized_edit_repair": 4, "numeric_preserves_omission_repair": 6}:
...
def main() -> None:
    ...
    if source.get("passed") is not False or metrics.get("safety_gate_passed") is not True:
        audit["passed"] = False
        audit.setdefault("failures", []).append("source_stage9354_not_safe_failed_audit")
    ...
    summary = {
        ...
        "decision": "Built denoise-only rows for the Stage9354 non-operator residuals.",
        "next_best_step": "Build Stage9356 preexecution for a 32-row non-operator residual probe; keep decoder CE closed.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9355 Non-Operator Residual Repair Manifest", "", f"Passed: `{summary['passed']}`", f"Rows: `{audit['rows']}`", f"Splits: `{audit['split_counts']}`", f"Tasks: `{audit['task_counts']}`", f"Routes: `{audit['route_counts']}`", "", "This manifest isolates the Stage9354 route_0, route_2, and route_file_path residuals. Decoder CE and external authority remain closed.", ""]), encoding="utf-8")
