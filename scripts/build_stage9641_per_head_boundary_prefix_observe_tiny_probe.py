...
STAGE = 9641
NAME = "stage9641_per_head_boundary_prefix_observe_tiny_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9640_per_head_boundary_prefix_observe_manifest.json"
...
AUDIT = OUT_DIR / "per_head_boundary_prefix_observe_tiny_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PER_HEAD_BOUNDARY_PREFIX_OBSERVE_TINY_PROBE_STAGE9641.md"
...
def command() -> list[str]:
    return [
        ...
        str(RUN_DIR),
        "--run-id",
        "stage9641_per_head_boundary_prefix_observe_probe",
...
def field_exact(split_card: dict[str, Any]) -> dict[str, float]:
    ...
    return {
        key: float(value.get("exact") or 0.0)
        for key, value in field_card.items()
        if key in EXPECTED_FIELDS and isinstance(value, dict)
    }
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        safety_failures.append("stage9640_not_passed")
    if float(source_metrics.get("strongest_non_boundary_pair_single_feature_baseline", 1.0) or 1.0) >= 0.75:
        safety_failures.append("stage9640_non_pair_shortcut_baseline_too_high")
    enabled = set(source_metrics.get("enabled_losses") or [])
    if enabled != {"episode_boundary_match_ce", "episode_target_prefix_match_ce"}:
        safety_failures.append("stage9640_enabled_loss_mismatch")
    ...
    if missing_fields:
        safety_failures.append("missing_expected_field_logits")

    wrong_by_field = Counter(str(row.get("field")) for row in logits if row.get("correct") is False)
    pred_pairs = Counter(
        f"{row.get('field')}::{row.get('target')}=>{row.get('pred')}"
        for row in logits
    ...
    quality_pass = bool(
        strict_joint >= QUALITY_FLOOR
        and strict_min >= QUALITY_FLOOR
        and len(strict_exact) == len(EXPECTED_FIELDS)
        and not high_conf_wrong
    )
    if not quality_pass:
    ...
    next_step = (
        "If Stage9641 passes, add repair_outcome in a separate two-head follow-up; if it fails, inspect "
        "boundary/prefix wrong pairs and patch the per-head contrast set before recombining objectives."
    )
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        ...
        "artifacts": {
            ...
            "execution_result": str((RUN_DIR / "execution_result.json").relative_to(ROOT)),
        },
        "decision": (
            "Tiny target-100M per-head boundary/prefix observe probe executed safely."
            if not safety_failures
            else "Safety failed; do not use probe result."
        ),
    ...
    DOC.write_text(
        "\n".join(
            [
                ...
                f"Eval/strict joint: `{eval_joint}` / `{strict_joint}`",
                f"Eval field exact: `{eval_exact}`",
                f"Strict field exact: `{strict_exact}`",
        ...
        json.dumps(
            {
                ...
                "quality_passed": audit["quality_passed"],
                "strict_joint_proxy_exact": strict_joint,
                "strict_field_exact": strict_exact,
                "failures": failures,
                "next_best_step": next_step,
