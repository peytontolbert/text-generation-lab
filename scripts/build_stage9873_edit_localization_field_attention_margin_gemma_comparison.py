...
STAGE = 9873
NAME = 'stage9873_edit_localization_field_attention_margin_gemma_comparison'
MANIFEST = ROOT / 'runs/local/artifacts/stage9867_edit_localization_label_identity_probe/edit_localization_label_identity_manifest.jsonl'
MODEL_ROWS = ROOT / 'runs/local/artifacts/stage9872_edit_localization_field_attention_margin_best_state_target_100m_probe/edit_localization_probe/row_field_logits.jsonl'
OUT_DIR = ROOT / 'runs/local/artifacts' / NAME
AUDIT = OUT_DIR / 'edit_localization_field_attention_margin_gemma_comparison.json'
ROWS_OUT = OUT_DIR / 'edit_localization_field_attention_margin_gemma_rows.jsonl'
...
build_prompt = _load_symbol('stage9873_stage9748_runner', ROOT / 'scripts/run_stage9748_standalone_gemma_queue_via_ollama.py', 'build_prompt')
...
def main() -> None:
    ...
    next_step = 'If the same-surface Gemma rerun confirms the Stage9872 margin objective win, promote this objective into the broader v2.7 multilingual structured mix and rerun the full standalone comparison ledger.'
    summary = {
        'stage': STAGE,
        'stage_name': NAME,
        'name': NAME,
        ...
        'artifacts': {'audit': str(AUDIT.relative_to(ROOT)), 'rows': str(ROWS_OUT.relative_to(ROOT))},
        'decision': 'Compared local Gemma against the Stage9867 relabeled multilingual edit-localization packet using the improved Stage9872 100M row-level outputs on the same eval and strict-eval slices.',
