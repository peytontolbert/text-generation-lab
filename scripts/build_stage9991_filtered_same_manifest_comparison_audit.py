...
STAGE = 9991
NAME = "stage9991_filtered_same_manifest_comparison_audit"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "filtered_same_manifest_comparison_audit.json"
ROWS = OUT_DIR / "filtered_same_manifest_comparison_rows.jsonl"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "FILTERED_SAME_MANIFEST_COMPARISON_AUDIT_STAGE9991.md"
...
def main() -> None:
    ...
    next_step = "Use this filtered same-manifest comparison to decide whether the stage9987 frontier is now a truthful 100M-over-Gemma multilingual winner or whether remaining Gemma advantages still block that claim."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Materialized the actual filtered same-manifest 100M-versus-Gemma comparison on the stage9986 successor manifest so the stage9987 frontier can be judged fairly rather than inferred from baseline deltas.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9991 Filtered Same Manifest Comparison Audit",
        "",
