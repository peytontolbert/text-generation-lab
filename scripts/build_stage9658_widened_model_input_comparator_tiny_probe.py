...
STAGE = 9658
NAME = "stage9658_widened_model_input_comparator_tiny_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9657_widened_model_input_comparator_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9657_widened_model_input_comparator_manifest/widened_model_input_comparator_manifest.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
RUN_DIR = OUT_DIR / "episode_step_probe"
AUDIT = OUT_DIR / "widened_model_input_comparator_tiny_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "WIDENED_MODEL_INPUT_COMPARATOR_TINY_PROBE_STAGE9658.md"
...
def command() -> list[str]:
    return ["env", f"TMPDIR={TMPDIR}", f"TEMP={TMPDIR}", f"TMP={TMPDIR}", "conda", "run", "-n", "trellis", "python", str(TRAINER),
        ...
        "--max-train-rows", "16", "--max-eval-rows", "16", "--max-strict-rows", "16", "--max-steps", "384", "--batch-size", "4", "--learning-rate", "3e-4",
        ...
        "--output-dir", str(RUN_DIR), "--run-id", "stage9658_widened_model_input_comparator_probe", "--execution-authorized-for-recovery-probe"]
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        safety_failures.append("stage9657_not_passed")
    if int(source_metrics.get("rows") or 0) != 48:
        safety_failures.append("stage9657_unexpected_row_count")
