#!/usr/bin/env python3
"""Capture codex-utils-path Rust verifier feasibility."""
...
STAGE = 11852
NAME = "stage11852_codex_utils_path_rust_feasibility"
OUT = ART / NAME
LOGS = OUT / "logs"
SUMMARY = OUT / "codex_utils_path_rust_feasibility.json"
RESULTS = OUT / "codex_utils_path_rust_feasibility_results.jsonl"
REPO = Path("/data/agentkernel/other_repos/codex/codex-rs")
SELECTED_TEST = "wsl_mnt_drive_paths_lowercase"
SIBLING_TEST = "wsl_non_drive_paths_unchanged"
...
def run_cargo(test_filter: str, name: str) -> dict[str, Any]:
    ...
    command = ["cargo", "test", "-p", "codex-utils-path", test_filter, "--", "--nocapture"]
    env = os.environ.copy()
    env.update(
        {
...
def main() -> None:
    selected = run_cargo(SELECTED_TEST, "selected_wsl_mnt_drive_paths_lowercase")
    sibling = run_cargo(SIBLING_TEST, "sibling_wsl_non_drive_paths_unchanged")
    passed = bool(selected["passed"] and sibling["passed"])
    row = {
        "stage": STAGE,
        "stage_name": NAME,
        "created_at_utc": now(),
        "repo_family": "codex_utils_path",
        "repo_path": str(REPO),
        ...
        "source_path": "utils/path-utils/src/lib.rs",
        "distractor_source_path": "utils/path-utils/src/path_utils_tests.rs",
        "passed": passed,
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "codex_utils_path_rust_feasibility_recorded",
        "passed": passed,
