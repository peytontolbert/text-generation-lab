...
STAGE = 9321
NAME = "stage9321_phrase_completion_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9320_phrase_completion_probe_preexecution.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9319_phrase_completion_balance_manifest/phrase_completion_balance_manifest.jsonl"
RUN_DIR = ROOT / "runs/local/artifacts/stage9321_phrase_completion_probe"
AUDIT = RUN_DIR / "stage9321_phrase_completion_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PHRASE_COMPLETION_PROBE_AUDIT_STAGE9321.md"
...
def _bucket(samples: list[dict[str, Any]], manifest_by_id: dict[str, dict[str, Any]], key: str) -> dict[str, dict[str, Any]]:
    ...
    for sample in samples:
        ...
        if key == "phrase":
            value = str(target.get("phrase_id") or "unknown")
        elif key == "variant":
...
def audit_run() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9320_not_passed")
    ...
    phrase_error_counts = {"phrase_a_patch_ins": 0, "phrase_b_expected_assertion": 0, "phrase_b_operatch": 0, "phrase_c_failures": 0}
    failed_samples = []
    for sample in samples:
        if sample.get("exact_match"):
            continue
        meta = manifest_by_id.get(str(sample.get("row_id")), {})
        phrase = ((meta.get("target") or {}).get("phrase_id") if isinstance(meta.get("target"), dict) else None)
        generated_text = str(sample.get("generated_text") or "")
        if phrase == "phrase_a" and "patch ins" in generated_text:
            phrase_error_counts["phrase_a_patch_ins"] += 1
        if phrase == "phrase_b" and "expected assertion" in generated_text:
            phrase_error_counts["phrase_b_expected_assertion"] += 1
        if phrase == "phrase_b" and "operatch" in generated_text:
            phrase_error_counts["phrase_b_operatch"] += 1
        if phrase == "phrase_c":
            phrase_error_counts["phrase_c_failures"] += 1
    ...
    quality_gate_passed = bool(generated == 24 and exact == generated and target_prefix == generated and boundary == generated and contentful == generated and repetition == 0)
    return {
        "passed": not failures,
        ...
        "by_phrase": _bucket(samples, manifest_by_id, "phrase"),
        ...
        "diagnosis": "phrase_completion_balance_probe_safe_but_not_learnability_clean_for_phrase_a_and_phrase_b_under_current_format",
        "next_patch_target": "stage9322_phrase_family_separated_micro_overfit_or_add_explicit_nonlabel_semantic_discriminators_before_merge",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Safety passed, but phrase-completion quality failed for phrase A/B; phrase C was not the failure family.",
        "next_best_step": "Do not merge Stage9319 yet. Split phrase families or add non-label semantic discriminators, then rerun isolated phrase-family probes.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9321 Phrase Completion Probe Audit",
        "",
        ...
        "The run is safe, but phrase A/B remain unstable. Do not merge this manifest back into the combined curriculum yet.",
