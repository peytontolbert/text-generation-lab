...
STAGE = 11103
NAME = "stage11103_fresh_family_support_probe_request_encoder_option_retrieval"
OUT_DIR = ARTIFACTS / NAME
SUMMARY_JSON = OUT_DIR / "fresh_family_support_probe_request_encoder_option_retrieval.json"
...
PACKAGE_DIR = ARTIFACTS / "stage11099_fresh_family_support_package"
PACKAGE_JSON = PACKAGE_DIR / "fresh_family_support_package.json"
...
ADDED_ROWS_JSONL = PACKAGE_DIR / "added_fresh_family_support_rows.jsonl"
...
PROBE_OUT_DIR = ARTIFACTS / "stage11104_fresh_family_support_probe_encoder_option_retrieval" / "bounded_decoder_probe"
RUNTIME_OUT_DIR = ARTIFACTS / "stage11104_fresh_family_support_probe_encoder_option_retrieval" / "runtime_model"
...
def main() -> None:
    ...
    summary = {
        ...
        "passed": True,
        "decision": "fresh_family_support_probe_requested_encoder_option_retrieval",
        "claim_scope": [
            "Run one bounded probe from the stage11099 fresh-family support package using the last compatible non-pairwise runtime bundle.",
            "Measure whether fresh-family retrieve/verifier support changes the unchanged successor overlay or the reserved residual bank.",
        ],
        "source_artifacts": {
        ...
        "metrics": {
            ...
            "fresh_family_rows_added": len(added_rows),
            "fresh_family_repo_count": len({str(row.get("repo_family") or "") for row in added_rows}),
        },
        "required_honesty_gates": [
            "Validation and strict remain unchanged and must be reported separately from the fresh-family support rows.",
            "Reserved candidate reporting must remain separate from the successor overlay and from the new support rows.",
            "No claim may treat the stage11097 heuristic evidence candidates as heldout generalization because they are not in this train package.",
            "This fallback uses the last compatible encoder_option_retrieval runtime bundle and should be interpreted as a compatibility-preserving continuation.",
        ],
        "next_best_step": "Execute this request, then audit successor validation/strict plus the reserved residual bank in one postrun summary tied to the stage11099 package.",
        "outputs": {
