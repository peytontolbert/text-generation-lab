...
STAGE = 11432
NAME = "stage11432_selected_test_rust_support_postrun_audit"
OUT_DIR = ARTIFACTS / NAME
SUMMARY_JSON = OUT_DIR / "selected_test_rust_support_postrun_audit.json"
...
RUNTIME_BUNDLE = ARTIFACTS / "stage11431_selected_test_rust_support_probe" / "runtime_model" / "runtime_model_bundle.json"
VALIDATION_JSON = ARTIFACTS / "stage11431_selected_test_rust_support_probe" / "bounded_decoder_probe" / "bounded_choice_eval_audit_eval.json"
STRICT_JSON = ARTIFACTS / "stage11431_selected_test_rust_support_probe" / "bounded_decoder_probe" / "bounded_choice_eval_audit_strict_eval.json"
EXECUTION_JSON = ARTIFACTS / "stage11431_selected_test_rust_support_probe" / "bounded_decoder_probe" / "execution_result.json"
RESERVED_ROWS = ARTIFACTS / "stage11051_successor_residual_support_plus_priority_evidence" / "reserved_residual_candidates.jsonl"
SUPPORT_ROWS = ARTIFACTS / "stage11429_selected_test_rust_support_package" / "added_selected_test_rust_support_rows.jsonl"
PACKAGE_SUMMARY = ARTIFACTS / "stage11429_selected_test_rust_support_package" / "selected_test_rust_support_package.json"
BASELINE_POSTRUN = ARTIFACTS / "stage11425_rust_verifier_support_diagnostic_postrun_audit" / "rust_verifier_support_diagnostic_postrun_audit.json"
...
def miss_rows(card: dict[str, Any]) -> list[str]:
    return [str(row.get("row_id") or "") for row in card.get("row_cards", []) if row.get("constrained_choice_match") is False]
...
def overlap_report(strict_card: dict[str, Any], reserved_rows: list[dict[str, Any]], support_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "strict_singleton_option_rows": [str(row.get("row_id") or "") for row in strict_rows if len(list(row.get("option_labels") or [])) == 1],
    }
...
def main() -> None:
    ...
    validation_acc = audit_accuracy(validation)
    strict_acc = audit_accuracy(strict)
    reserved_block = metric_block(enriched_cards, "constrained_choice_match")
    reserved_acc = reserved_block["exact_accuracy"]
    baseline_validation = (baseline.get("successor_surface_result") or {}).get("validation_accuracy") if baseline else None
    baseline_strict = (baseline.get("successor_surface_result") or {}).get("strict_accuracy") if baseline else None
    baseline_reserved = ((baseline.get("reserved_candidate_result") or {}).get("overall") or {}).get("exact_accuracy") if baseline else None
    anti_cheat = overlap_report(strict, reserved_rows, support_rows)
    promoted = (
        strict_acc is not None and strict_acc >= (22 / 23)
        and validation_acc is not None and validation_acc >= (20 / 23)
        and reserved_acc is not None and reserved_acc > 0.5
    ...
    summary = {
        ...
        "passed": True,
        "decision": "selected_test_rust_support_probe_promotable" if promoted else "selected_test_rust_support_probe_not_promoted",
        "source_artifacts": {
            ...
            "selected_test_rust_support_rows": rel(SUPPORT_ROWS),
        ...
        "support_package_metrics": {
            ...
            "added_selected_test_rust_rows": package_summary.get("counts", {}).get("added_selected_test_rust_support"),
            "added_selected_test_rust_support": package_summary.get("added_selected_test_rust_support"),
        },
        ...
        "delta_vs_stage11425": {
            "successor_validation_accuracy": None if baseline_validation is None or validation_acc is None else validation_acc - float(baseline_validation),
            "successor_strict_accuracy": None if baseline_strict is None or strict_acc is None else strict_acc - float(baseline_strict),
            "reserved_candidate_accuracy": None if baseline_reserved is None or reserved_acc is None else reserved_acc - float(baseline_reserved),
        },
        "promotion_gate": {
            ...
            "reserved_residual_improved_above_5_of_10": reserved_acc is not None and reserved_acc > 0.5,
    ...
    print(json.dumps({
        ...
        "delta_vs_stage11425": summary["delta_vs_stage11425"],
