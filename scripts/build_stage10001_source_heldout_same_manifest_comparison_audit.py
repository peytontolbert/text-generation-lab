...
STAGE = 10001
NAME = "stage10001_source_heldout_same_manifest_comparison_audit"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "source_heldout_same_manifest_comparison_audit.json"
ROWS = OUT_DIR / "source_heldout_same_manifest_comparison_rows.jsonl"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "SOURCE_HELDOUT_SAME_MANIFEST_COMPARISON_AUDIT_STAGE10001.md"
...
def main() -> None:
    ...
    next_step = "Use this source-heldout same-manifest comparison to decide whether the stage9998 frontier is a truthful multilingual 100M-over-Gemma winner or whether Python/c_cpp still need new independent training roots."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Materialized the actual source-heldout same-manifest 100M-versus-Gemma comparison on the stage9997 successor manifest so the stage9998 frontier can be judged fairly.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage10001 Source-Heldout Same Manifest Comparison Audit",
        "",
