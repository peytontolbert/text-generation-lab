...
STAGE = 11363
NAME = "stage11363_web_llama_stack_same_manifest_gemma_comparison"
OUT = ART / NAME
SUMMARY = OUT / "web_llama_stack_same_manifest_gemma_comparison.json"
ROWS_OUT = OUT / "web_llama_stack_gemma_rows.jsonl"
ROWS = ART / "stage11361_web_llama_stack_executed_heldout_rows/web_llama_stack_executed_heldout_rows.jsonl"
HUNDRED = ART / "stage11362_web_llama_stack_heldout_score_audit/web_llama_stack_heldout_score_audit.json"
MODEL_ID = "gemma3:12b"
...
def prompt_for_gemma(row: dict[str, Any]) -> str:
    ...
    return (
        base
        + "\n\nReturn exactly one option label from the listed choices. Do not explain. "
...
def main() -> None:
    ...
    hundred_metric = (
        (hundred.get("product_metrics") or {})
        .get("stage11200_frontier", {})
        .get("llama_stack_web_heldout", {})
    )
    ...
    summary = {
        ...
        "created_at_utc": now(),
        "passed": True,
        "decision": "web_llama_stack_same_manifest_gemma_comparison_complete",
        "claim_scope": "single disjoint executed Web heldout root; diagnostic only, not a broad Web claim",
        "model_id": MODEL_ID,
        ...
        "recommended_next_action": "Use this hard Web root as sealed diagnostic heldout. Do not train on it until there are enough separate Web heldout roots or it is explicitly demoted to support.",
    }
    write_json(SUMMARY, summary)
    write_jsonl(ROWS_OUT, result_rows)
    print(json.dumps({
        "hundred_m": hundred_metric,
        "gemma12b": gemma_metric,
        "delta": summary["delta_hundred_m_minus_gemma"],
