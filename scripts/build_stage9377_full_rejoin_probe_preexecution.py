...
STAGE = 9377
NAME = "stage9377_full_rejoin_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9376_full_mixture_with_route2_filepath_repairs_manifest.json"
...
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9378_full_rejoin_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "full_rejoin_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9378_full_rejoin_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "FULL_REJOIN_PROBE_PREEXECUTION_STAGE9377.md"
...
COMMAND = [
    ...
    "--max-train-rows", "144", "--max-eval-rows", "76", "--max-strict-rows", "56",
    "--max-steps", "180", "--batch-size", "2", "--learning-rate", "1e-5",
    ...
    "--enable-generation-audit", "--max-generation-rows", "276", "--max-generation-tokens", "64",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9378_full_rejoin_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9376_not_passed")
    if len(rows) != 276:
...
def main() -> None:
    ...
    summary = {
        ...
        "metrics": {**{key: bool(card["authority"].get(key, False)) for key in AUTHORITY_CLOSED}, "command_ready": card["command_ready"], "manifest_rows": card["manifest_rows"], "split_counts": card["split_counts"], "route_counts": card["route_counts"], "source_counts": card["source_counts"], "failures": len(card["failures"])},
        "artifacts": {"audit": str(AUDIT.relative_to(ROOT)), "command": str(COMMAND_JSON.relative_to(ROOT)), "doc": str(DOC.relative_to(ROOT))},
        "decision": "Prepared a closed-boundary 276-row target-100M denoise rejoin probe; decoder CE remains closed.",
        "next_best_step": "Execute Stage9378 full rejoin probe under trellis and audit whether the mixed denoise objective is quality-passing.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9377 Full Rejoin Probe Preexecution", "", f"Passed: `{card['passed']}`", f"Rows: `{card['manifest_rows']}`", f"Splits: `{card['split_counts']}`", f"Routes: `{card['route_counts']}`", f"Sources: `{card['source_counts']}`", "", "Only the Stage9378 276-row target-100M denoise probe is authorized. Decoder CE and external authority remain closed.", ""]), encoding="utf-8")
