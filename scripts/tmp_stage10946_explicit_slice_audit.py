...
ROWS = ROOT/'runs/local/artifacts/stage10938_explicit_verifier_ledger_strict_candidates/strict_candidate_rows.jsonl'
OUT = ROOT/'runs/local/artifacts/stage10946_explicit_slice_margin_dump.json'
ROLE_MAP = {
...
def score(row, variant):
    ...
    with torch.no_grad(): out = model(batch.input_ids, batch.decoder_input_ids)
    ...
    texts = [value if variant == 'raw' else f"Visible fact role under review: {ROLE_MAP.get(value, value)}" for _, value in pairs]
