...
STAGE = 9370
NAME = "stage9370_full_mixture_with_route0_repairs_manifest"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9369_route0_dependency_duplicate_probe_audit.json"
BASE = ROOT / "runs/local/artifacts/stage9364_full_mixture_with_interference_repairs_manifest/full_mixture_with_interference_repairs_manifest.jsonl"
REPAIR = ROOT / "runs/local/artifacts/stage9367_route0_dependency_duplicate_repair_manifest/route0_dependency_duplicate_repair_manifest.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
MANIFEST = OUT_DIR / "full_mixture_with_route0_repairs_manifest.jsonl"
AUDIT = OUT_DIR / "full_mixture_with_route0_repairs_manifest_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "FULL_MIXTURE_WITH_ROUTE0_REPAIRS_MANIFEST_STAGE9370.md"
...
def remap(source_name: str, source_stage: int, row: dict[str, Any]) -> dict[str, Any]:
    ...
    new["row_id"] = f"stage9370_{source_name}_{old_id}"
    new["source_row_id"] = old_id
    new["source_stage"] = source_stage
    new["merged_curriculum_source"] = source_name
    new["objective_family"] = "full_mixture_with_route0_repairs_denoise"
    ...
    mi["full_mixture_with_route0_repairs"] = True
    mi["route_schema_version"] = mi.get("route_schema_version") or "stage9370_full_mixture_v1"
    new["model_input"] = mi
    anti = new.get("anti_cheat") if isinstance(new.get("anti_cheat"), dict) else {}
    anti.update({"decoder_ce_closed": True, "runtime_closed": True, "full_mixture_with_route0_repairs": True})
    new["anti_cheat"] = anti
    return new


def build_rows() -> list[dict[str, Any]]:
    return [remap("full_mixture_pre_route0_repair", 9364, row) for row in load_jsonl(BASE)] + [
        remap("route0_dependency_duplicate_repair", 9367, row) for row in load_jsonl(REPAIR)
    ]
...
def audit_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9369_not_passed")
    if len(rows) != 250:
        failures.append("unexpected_row_count")
    if dict(split_counts) != {"train": 133, "eval": 72, "strict_eval": 45}:
        failures.append("unexpected_split_counts")
    if dict(source_counts) != {"full_mixture_pre_route0_repair": 209, "route0_dependency_duplicate_repair": 41}:
        failures.append("unexpected_source_counts")
    if dict(route_counts) != {"route_0": 93, "route_1": 117, "route_2": 18, "route_file_path": 22}:
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Merged Stage9367 route_0 dependency duplicate repairs back into the full denoise mixture.",
        "next_best_step": "Build Stage9371 preexecution and run a 250-row closed rejoin probe before bounded decoder CE.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9370 Full Mixture With Route0 Repairs Manifest", "", f"Passed: `{audit['passed']}`", f"Rows: `{audit['rows']}`", f"Splits: `{audit['split_counts']}`", f"Sources: `{audit['source_counts']}`", f"Routes: `{audit['route_counts']}`", "", "Decoder CE, runtime, harness, Gemma, scoring, promotion, and checkpoint export remain closed.", ""]), encoding="utf-8")
