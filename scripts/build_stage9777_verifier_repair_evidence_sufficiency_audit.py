...
STAGE = 9777
NAME = "stage9777_verifier_repair_evidence_sufficiency_audit"
...
AUDIT = OUT_DIR / "verifier_repair_evidence_sufficiency_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "VERIFIER_REPAIR_EVIDENCE_SUFFICIENCY_STAGE9777.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
LANGS = ['python', 'rust', 'c_cpp', 'web_js_ts_html']
SPLITS = ['train', 'eval', 'strict_eval']
LEAKY_FIELDS = ['verifier_signal', 'action_sequence', 'file_plan']
SAFE_FIELDS = ['query_kind']


def _load_row_text():
    path = ROOT / 'legacy_src/agentkernel_lite/training_data.py'
    spec = importlib.util.spec_from_file_location('stage9777_training_data', path)
    module = importlib.util.module_from_spec(spec)
    assert spec and spec.loader
    sys.modules['stage9777_training_data'] = module
...
def label(row: dict[str, Any]) -> str:
    ...
    return str(clean.get('verifier_repair_action') or '')
...
def build_audit() -> dict[str, Any]:
    ...
    findings = [
        'Every current verifier-repair language/split bucket collapses to a single recovered encoder surface.',
        'The non-leaky observable evidence available in the current rows also collapses to a single signature per bucket.',
        'Distinct verifier-repair labels become separable only when using label-shaped fields such as verifier_signal, action_sequence, or file_plan.',
        'This means the current verifier-repair package is structurally underspecified for an honest multilingual 100M-vs-Gemma comparison.',
    ]
    return {
        'passed': True,
...
def main() -> None:
    ...
    next_step = 'Rebuild verifier-repair rows with non-label observable verifier evidence instead of action-plan aliases, then rerun target-100M and deterministic Gemma comparison on that repaired surface.'
    summary = {
        'stage': STAGE,
        'stage_name': NAME,
        'name': NAME,
        ...
        'decision': 'Audited the current multilingual verifier-repair surface and found that safe observable evidence does not separate the labels. The current package is structurally underspecified unless label-shaped fields are exposed.',
