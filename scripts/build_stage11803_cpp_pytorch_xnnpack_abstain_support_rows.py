#!/usr/bin/env python3
"""Materialize PyTorch C++ XNNPACK abstain-attractor support rows."""
...
STAGE = 11803
NAME = "stage11803_cpp_pytorch_xnnpack_abstain_support_rows"
OUT = ART / NAME
SUMMARY = OUT / "cpp_pytorch_xnnpack_abstain_support_rows.json"
ROWS = OUT / "cpp_pytorch_xnnpack_abstain_support_rows.jsonl"
FEASIBILITY = ART / "stage11802_cpp_pytorch_xnnpack_build_feasibility/cpp_pytorch_xnnpack_build_feasibility_results.jsonl"
...
def main() -> None:
    ...
    if not feasible:
        raise SystemExit("PyTorch XNNPACK feasibility did not pass")
    feasibility = feasible[0]
    evidence = [
        file_evidence("E01", "candidate_change_surface", ".ci/pytorch/test_example_code/check-torch-xnnpack.cpp", "Small C++ Torch sample checks torch::globalContext().qEngine() XNNPACK availability path."),
        file_evidence("E02", "nearby_mkl_distractor", ".ci/pytorch/test_example_code/check-torch-mkl.cpp", "Adjacent MKL sample is a plausible but different verifier surface."),
        log_evidence("V01", feasibility["configure"], "PyTorch XNNPACK sample configure passed against trellis Torch CMake package."),
        log_evidence("V02", feasibility["build"], "check-torch-xnnpack target built successfully."),
        log_evidence("V03", feasibility["execute"], "check-torch-xnnpack executable ran successfully."),
    ]
    verifier_evidence = {"id": "V03", "kind": "actual_cpp_build_run_pass_log", "summary": "PyTorch check-torch-xnnpack configured, built, and executed successfully.", "configure": feasibility["configure"], "build": feasibility["build"], "execute": feasibility["execute"], "result": "PASS_CURRENT_BUILD_AND_RUN", "generated_build_harness": feasibility.get("harness_dir")}
    answerable = [
        {"label": "A", "role": "candidate_change_surface", "value": ".ci/pytorch/test_example_code/check-torch-xnnpack.cpp", "evidence_ids": ["E01"]},
        {"label": "B", "role": "verifier_and_build_constraint", "value": "cmake build and run check-torch-xnnpack passed", "evidence_ids": ["V01", "V02", "V03"]},
        {"label": "C", "role": "nearby_mkl_surface", "value": ".ci/pytorch/test_example_code/check-torch-mkl.cpp", "evidence_ids": ["E02"]},
    ...
    root_id = "stage11803::pytorch::c_cpp::check_torch_xnnpack_support"
    rows = [
        base_row(root_id, "answerable_evidence_citation_build_run_constraint", "evidence_citation", answerable, "B", True),
        base_row(root_id, "answerable_patch_impact_xnnpack_surface", "patch_impact", answerable, "A", True),
