...
STAGE = 9644
NAME = "stage9644_balanced_positive_per_head_observe_tiny_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9643_balanced_positive_per_head_observe_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9643_balanced_positive_per_head_observe_manifest/balanced_positive_per_head_observe_manifest.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
RUN_DIR = OUT_DIR / "episode_step_probe"
AUDIT = OUT_DIR / "balanced_positive_per_head_observe_tiny_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "BALANCED_POSITIVE_PER_HEAD_OBSERVE_TINY_PROBE_STAGE9644.md"
...
def command() -> list[str]:
    return [
        ...
        "--max-train-rows", "28", "--max-eval-rows", "28", "--max-strict-rows", "28", "--max-steps", "128", "--batch-size", "2", "--learning-rate", "2e-4",
        ...
        "--output-dir", str(RUN_DIR), "--run-id", "stage9644_balanced_positive_per_head_observe_probe", "--execution-authorized-for-recovery-probe",
    ]
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        safety_failures.append("stage9643_not_passed")
    if float(source_metrics.get("strongest_single_feature_baseline", 1.0) or 1.0) >= 0.75:
        safety_failures.append("stage9643_shortcut_baseline_too_high")
    if set(source_metrics.get("enabled_losses") or ["episode_boundary_match_ce", "episode_target_prefix_match_ce"]) != {"episode_boundary_match_ce", "episode_target_prefix_match_ce"}:
        safety_failures.append("stage9643_enabled_loss_mismatch")
    ...
    strict_min = min(strict_exact.values()) if strict_exact else 0.0
    quality_pass = bool(strict_joint >= QUALITY_FLOOR and strict_min >= QUALITY_FLOOR and len(strict_exact) == len(EXPECTED_FIELDS) and not high_conf_wrong)
    if not quality_pass:
    ...
    audit = {
        ...
        "eval_field_exact": eval_exact,
        "strict_field_exact": strict_exact,
        "wrong_by_field": dict(sorted(wrong_by_field.items())),
        "top_wrong_pred_pairs": dict(pred_pairs.most_common(20)),
    ...
    next_step = "If Stage9644 passes, add repair_outcome as a separate follow-up; if it fails, audit whether row_id/source-family markers caused split memorization."
    summary = {"stage": STAGE, "stage_name": NAME, "name": NAME, "passed": audit["passed"], "authority": dict(AUTHORITY_CLOSED), "metrics": {**dict(AUTHORITY_CLOSED), **audit}, "artifacts": {"audit": str(AUDIT.relative_to(ROOT)), "doc": str(DOC.relative_to(ROOT)), "run_dir": str(RUN_DIR.relative_to(ROOT)), "execution_result": str((RUN_DIR / "execution_result.json").relative_to(ROOT))}, "decision": "Tiny target-100M balanced-positive per-head observe probe executed safely." if not safety_failures else "Safety failed; do not use probe result.", "next_best_step": next_step, "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9644 Balanced Positive Per-Head Observe Tiny Probe", "", f"Passed: `{audit['passed']}`", f"Safety passed: `{audit['safety_passed']}`", f"Quality passed: `{audit['quality_passed']}`", f"Eval/strict joint: `{eval_joint}` / `{strict_joint}`", f"Eval field exact: `{eval_exact}`", f"Strict field exact: `{strict_exact}`", f"Wrong by field: `{dict(sorted(wrong_by_field.items()))}`", f"High-confidence wrong rows: `{len(high_conf_wrong)}`", "", "Decoder CE, denoise CE, runtime, Gemma, harness, checkpoint export, and promotion remained closed.", "", f"Next: {next_step}", ""]), encoding="utf-8")
    update_registry(summary)
    print(json.dumps({"stage": STAGE, "passed": audit["passed"], "safety_passed": audit["safety_passed"], "quality_passed": audit["quality_passed"], "strict_joint_proxy_exact": strict_joint, "strict_field_exact": strict_exact, "failures": failures, "next_best_step": next_step}, indent=2, sort_keys=True))
    if safety_failures:
        raise SystemExit(1)
