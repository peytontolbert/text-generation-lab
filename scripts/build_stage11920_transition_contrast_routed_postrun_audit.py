#!/usr/bin/env python3
"""Audit Stage11919 with routed transition semantic head and compact evidence head."""
...
STAGE = 11920
NAME = "stage11920_transition_contrast_routed_postrun_audit"
OUT = ART / NAME
SUMMARY = OUT / "transition_contrast_routed_postrun_audit.json"
...
RUNTIMES = {
    ...
    "stage11912_semantic_head_only": ART / "stage11912_transition_projection_semantic_head_only_probe/runtime_model/runtime_model_bundle.json",
    "stage11919_transition_contrast_head_only": ART / "stage11919_transition_projection_contrast_head_only_probe/runtime_model/runtime_model_bundle.json",
}
PROTECTED_ROWSETS = {
...
def by_group(card_path: Path, group_key: str) -> dict[str, dict[str, Any]]:
    card = read_json(card_path)
    buckets: dict[str, list[dict[str, Any]]] = {}
    for row in card.get("row_cards") or []:
        buckets.setdefault(str(row.get(group_key)), []).append(row)
    out: dict[str, dict[str, Any]] = {}
    for key, rows in sorted(buckets.items()):
        correct = sum(1 for row in rows if row.get("constrained_choice_match") is True)
        out[key] = {"rows": len(rows), "correct": correct, "accuracy": correct / len(rows) if rows else None}
    return out
...
def main() -> None:
    ...
    transition_cards: dict[str, Path] = {}
    for runtime_name, runtime_path in RUNTIMES.items():
        ...
        transition_card_path = OUT / runtime_name / "bounded_choice_eval_audit_transition_projection__semantic_candidate_head.json"
        transition_cards[runtime_name] = transition_card_path
        runtime_results["transition_projection_routed"] = metric(card)
        runtime_results["transition_by_language"] = by_group(transition_card_path, "language_family")
        runtime_results["transition_by_task"] = by_group(transition_card_path, "task_type")
        for name, rows in protected.items():
            ...
            runtime_results[f"protected::{name}"] = metric(card)
    ...
    previous = results["stage11912_semantic_head_only"]
    postrun = results["stage11919_transition_contrast_head_only"]
    gates = {
        ...
        "transition_projection_improved_vs_stage11912": (postrun["transition_projection_routed"].get("correct") or 0) > (previous["transition_projection_routed"].get("correct") or 0),
        "transition_projection_at_least_340_of_640": (postrun["transition_projection_routed"].get("correct") or 0) >= 340,
    ...
    if all(gates.values()):
        decision = "transition_contrast_routed_candidate_ready_for_gemma_comparison"
    elif gates["transition_projection_improved_vs_stage11912"] and gates["filtered_strict_preserved"] and gates["old_canary_strict_preserved"]:
        decision = "transition_contrast_routed_partial_candidate"
    else:
        decision = "transition_contrast_routed_rejected"
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "created_at_utc": now(),
        ...
        "device": str(DEVICE),
        "route_policy": {
            "transition_projection_rows": TRANSITION_SCORER,
            "compact_protected_rows": COMPACT_SCORER,
        },
        "gates": gates,
        "row_counts": {
            "transition_rows": len(transition_rows),
            "transition_by_language": dict(Counter(str(row.get("language_family")) for row in transition_rows)),
            "protected_rows": {name: len(rows) for name, rows in protected.items()},
        },
        "results": results,
        ...
        "claim_boundary": [
            "This is a routed scorer audit, not a standalone benchmark promotion.",
            "The semantic head is used only for transition projection rows.",
            "Compact protected gates remain scored with the selected evidence-judgment scorer.",
        ],
    }
        ...
        json.dumps(
            {
                "decision": decision,
                "gates": gates,
                "selected_transition": selected["transition_projection_routed"],
                "stage11912_transition": previous["transition_projection_routed"],
