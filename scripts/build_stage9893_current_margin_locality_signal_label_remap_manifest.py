...
STAGE = 9893
NAME = "stage9893_current_margin_locality_signal_label_remap_manifest"
...
MANIFEST = OUT_DIR / "current_margin_locality_signal_label_remap_manifest.jsonl"
AUDIT = OUT_DIR / "current_margin_locality_signal_label_remap_manifest_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "CURRENT_MARGIN_LOCALITY_SIGNAL_LABEL_REMAP_MANIFEST_STAGE9893.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
LABEL_MAP = {"K": "M", "M": "R", "R": "T", "T": "Z"}
...
def remap_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    anti["stage9894_label_remap"] = LABEL_MAP
    anti["stage9894_removed_k_from_target_vocab"] = True
...
def build_rows(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    audit = {
        ...
        "label_map": LABEL_MAP,
        "removed_k_from_target_vocab": "K" not in target_labels,
    }
...
def main() -> None:
    ...
    if sorted(audit["target_labels"]) != ["M", "R", "T", "Z"]:
        failures.append("unexpected_target_labels_after_remap")
    if not audit["removed_k_from_target_vocab"]:
        failures.append("k_still_present_in_target_vocab")
    write_json(
        AUDIT,
        {
            "passed": not failures,
    ...
    next_step = "Run Stage9894 on the locality-lifted remapped packet and check whether the improved K semantics survive once K is removed from the target vocabulary."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Built a current-frontier control manifest that keeps the locality-signal lift and all row semantics intact while removing K from the target vocabulary, to test whether the remaining failure is dominated by class-token geometry.",
    ...
    DOC.write_text(
        "\n".join(
            [
                ...
                f"Removed K from target vocab: `{audit['removed_k_from_target_vocab']}`",
                "",
                summary["decision"],
                "",
                f"Next: {next_step}",
