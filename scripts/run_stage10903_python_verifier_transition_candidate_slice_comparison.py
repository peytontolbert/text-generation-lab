...
STAGE = 10903
NAME = "stage10903_python_verifier_transition_candidate_slice_comparison"
OUT_DIR = ROOT / "runs" / "local" / "artifacts" / NAME
SUMMARY_JSON = OUT_DIR / "python_verifier_transition_candidate_slice_comparison.json"
...
def main() -> None:
        ...
        gemma_rows.append(
            {
                ...
                "gemma12b_predicted_label": normalize_label(raw, options),
                "gemma12b_correct": normalize_label(raw, options) == str(row.get("target_text") or ""),
            }
        )
    ...
    summary = {
        ...
        "passed": True,
        "claim_scope": [
            "Diagnostic scoring only on the 2-row fresh Python verifier-transition candidate slice.",
            "This slice is non-promotable and exists to test whether the verifier-transition miss repeats across a second reviewed root.",
        ],
