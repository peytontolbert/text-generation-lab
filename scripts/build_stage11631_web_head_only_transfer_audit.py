...
STAGE = 11631
NAME = "stage11631_web_head_only_transfer_audit"
OUT = ART / NAME
SUMMARY = OUT / "web_head_only_transfer_audit.json"

RUNTIME = ART / "stage11625_web_fail_to_pass_stronger_head_overfit/runtime_model/runtime_model_bundle.json"
ROWSETS = {
...
def main() -> None:
    ...
    gates = {
        "support_fit_48_of_48": results["controlled_fail_to_pass_support"]["correct"] == 48 and results["controlled_fail_to_pass_support"]["rows"] == 48,
    ...
    decision = "head_only_transfer_candidate_needs_guarded_validation" if gates["web_heldout_beats_stage11507_35_of_66"] and gates["residual_at_least_7_of_10"] else "head_only_overfit_no_transfer"
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "created_at_utc": now(),
        ...
        "baseline_reference": {
            ...
            "stage11628_guarded_web_heldout": "32/66",
        },
        "runtime_initialization": init_card,
        "source_artifacts": {name: rel(path) for name, path in ROWSETS.items()},
        "outputs": {"summary": rel(SUMMARY), "audit_dir": rel(OUT)},
        "claim_boundary": [
            "Stage11625 is same-row head-only overfit; this audit checks transfer only.",
            "This cannot promote the selected frontier without a separate guarded training run.",
        ],
    }
