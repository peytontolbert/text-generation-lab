...
STAGE = 11541
NAME = "stage11541_mcp_typescript_sdk_web_gold_heldout_rows"
OUT = ART / NAME
SUMMARY = OUT / "mcp_typescript_sdk_web_gold_heldout_rows.json"
ROWS = OUT / "mcp_typescript_sdk_web_gold_heldout_rows.jsonl"
PACKETS = OUT / "mcp_typescript_sdk_web_gold_heldout_packets.jsonl"

STAGE11540_PACKETS = ART / "stage11540_mcp_typescript_sdk_web_heldout_execution_artifact/mcp_typescript_sdk_web_heldout_execution_packets.jsonl"
...
def option_bank(packet: dict[str, Any], perspective: str) -> list[dict[str, str]]:
    ...
    if perspective == "abstention_insufficient_evidence":
        return [
            ...
            {"value": "needs_external_web_research", "text": "NEEDS_EXTERNAL_WEB_RESEARCH: version-matched external docs are required."},
    ...
    return [
        ...
        {"value": "symptom_or_call_path_analogue", "text": f"{packet['secondary_source_path']} adjacent protocol/call-path context."},
        {"value": "dependency_or_test_environment_surface", "text": "package manager, build, and Vitest execution environment surface."},
        {"value": "abstain_insufficient_evidence", "text": "ABSTAIN_INSUFFICIENT_EVIDENCE"},
    ]
...
def main() -> None:
    ...
    packets = load_jsonl(STAGE11540_PACKETS)
    admitted_packets = [
        {
            **packet,
    ...
    rows = [row for packet in admitted_packets for row in rows_for_packet(packet)]
    leakage_flags = [
        row["row_id"]
        for row in rows
    ...
    summary = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "passed": len(admitted_packets) == 5 and len(rows) == 30 and not leakage_flags,
        "decision": "mcp_typescript_sdk_web_heldout_rows_admitted" if not leakage_flags else "blocked_prompt_target_leak",
        "counts": {
            "admitted_heldout_roots": len(admitted_packets),
            "strict_eval_rows": len(rows),
            "train_rows": 0,
            "unique_repo_families": len({packet["git_repo_family"] for packet in admitted_packets}),
        ...
        "claim_boundary": [
            ...
            "They add verifier-backed TypeScript/Web roots but do not by themselves prove model improvement.",
            "They should only enter a same-manifest comparison after root-overlap audit with train packages.",
        ],
        "outputs": {
            "summary": rel(SUMMARY),
            "packets": rel(PACKETS),
            "rows": rel(ROWS),
            "source_execution_artifact": "runs/summaries/stage11540_mcp_typescript_sdk_web_heldout_execution_artifact.json",
        },
    }
    write_jsonl(PACKETS, admitted_packets)
