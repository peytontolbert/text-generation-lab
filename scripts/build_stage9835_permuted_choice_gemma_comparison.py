...
STAGE = 9835
NAME = "stage9835_permuted_choice_gemma_comparison"
MANIFEST = ROOT / "runs/local/artifacts/stage9833_permuted_choice_execution_manifest/permuted_choice_execution_manifest.jsonl"
ROWS_100M = ROOT / "runs/local/artifacts/stage9834_permuted_choice_counterfactual_exec/row_field_logits.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
ROWS_OUT = OUT_DIR / "permuted_choice_gemma_rows.jsonl"
AUDIT = OUT_DIR / "permuted_choice_gemma_comparison.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PERMUTED_CHOICE_GEMMA_COMPARISON_STAGE9835.md"
...
def load_100m_split_metrics() -> dict[str, float]:
    payload = load_json(ROOT / "runs/local/artifacts/stage9834_permuted_choice_counterfactual_exec/execution_result.json")
    eval_card = payload.get("eval") if isinstance(payload.get("eval"), dict) else {}
    out: dict[str, float] = {}
    for split in COMPARE_SPLITS:
        split_card = eval_card.get(split) if isinstance(eval_card.get(split), dict) else {}
        out[split] = float(split_card.get("joint_proxy_exact") or 0.0)
    return out


def build_audit(gemma_rows: list[dict[str, Any]], hundred_m_split_metrics: dict[str, float], manifest_rows: list[dict[str, Any]]) -> dict[str, Any]:
    gemma = summarize_accuracy(gemma_rows)
    comparisons = {}
    wins_100m = wins_gemma = ties = 0
    for split in COMPARE_SPLITS:
        a = float(hundred_m_split_metrics.get(split) or 0.0)
        b = float(gemma['macro_by_split'][split])
        verdict = 'tie'
        if a > b:
            verdict = '100m_win'
            wins_100m += 1
            ...
            ties += 1
        comparisons[split] = {'hundred_m_exact': a, 'gemma_macro_exact': b, 'verdict': verdict}
    return {
        'passed': True,
        'rows_compared': len(gemma_rows),
        'same_surface_hash': prompt_surface_hash(manifest_rows),
        'model_id': MODEL_ID,
        'hundred_m_split_metrics': hundred_m_split_metrics,
        'hundred_m_per_language_artifact_available': False,
        'gemma': gemma,
        'comparisons': comparisons,
        'wins_100m': wins_100m,
        'wins_gemma': wins_gemma,
        'ties': ties,
        'macro_delta_by_split': {split: float(hundred_m_split_metrics.get(split) or 0.0) - gemma['macro_by_split'][split] for split in COMPARE_SPLITS},
        'authority': dict(AUTHORITY_CLOSED),
    }
...
def main() -> None:
    ...
    hundred_m_split_metrics = load_100m_split_metrics()
    write_jsonl(ROWS_OUT, gemma_rows)
    audit = build_audit(gemma_rows, hundred_m_split_metrics, manifest_rows)
    write_json(AUDIT, audit)
    next_step = 'If the 100M win survives the permuted-choice surface, use this as the default same-surface comparison packet for future multilingual claims and only promote after the blind expert-maintainer review is filled in.'
    summary = {
        'stage': STAGE,
        'stage_name': NAME,
        'name': NAME,
        ...
        'decision': 'Ran local Gemma-12B on the permuted-choice same-surface counterfactual packet and compared it against the Stage9834 100M split-level exactness, while explicitly marking the missing 100M per-language artifact gap.',
