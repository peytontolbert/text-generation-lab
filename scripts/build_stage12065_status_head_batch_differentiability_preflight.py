...
NAME = "stage12065_status_head_batch_differentiability_preflight"
OUT = ART / NAME
SUMMARY = OUT / "status_head_batch_differentiability_preflight.json"
MANIFEST = ART / "stage12062_transition_status_head_ablation_request/transition_status_head_ablation_manifest.jsonl"
...
def main() -> None:
    ...
    payload = {
        ...
        "interpretation": "Stage12063 can be rerun only if all planned batches have a differentiable weighted loss." if passed else "Stage12063 should not be rerun unchanged; the first bad batch must be filtered, repaired, or the training objective must use a differentiable fallback.",
        "source_artifacts": {"manifest": rel(MANIFEST), "init_runtime": rel(INIT_RUNTIME), "failed_stage12064": "runs/summaries/stage12064_transition_status_head_ablation_failure_decision.json"},
