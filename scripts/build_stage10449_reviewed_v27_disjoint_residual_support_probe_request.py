...
STAGE = 10449
NAME = "stage10449_reviewed_v27_disjoint_residual_support_probe_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
REQUEST_JSON = OUT_DIR / "reviewed_v27_disjoint_residual_support_probe_request.json"
MANIFEST_JSONL = OUT_DIR / "reviewed_v27_disjoint_residual_support_probe_manifest.jsonl"
...
PYTHON_SUPPORT_JSONL = ROOT / "runs/local/artifacts/stage10447_python_disjoint_residual_support_package/python_disjoint_residual_support_rows.jsonl"
...
RUN_ID = "stage10450_reviewed_v27_disjoint_residual_support_probe"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage10450_reviewed_v27_disjoint_residual_support_probe/bounded_decoder_probe"
RUNTIME_MODEL_DIR = ROOT / "runs/local/artifacts/stage10450_reviewed_v27_disjoint_residual_support_probe/runtime_model"
...
def main() -> None:
    train_rows = load_jsonl(PYTHON_SUPPORT_JSONL)
    ...
    request = {
        ...
        "decision": "reviewed_v27_disjoint_residual_support_probe_ready",
        "claim_scope": [
            "Diagnostic-only preserved-runtime probe against the repaired v2.7 strict overlay.",
            "Train only on disjoint Python support rows from stage10447; do not replay repaired strict rows into train.",
            "Measure whether the two remaining repaired-overlay residuals move without regressing the broader overlay.",
        ],
        "source_artifacts": {
            "python_support_package_rows": display(PYTHON_SUPPORT_JSONL),
        ...
        "command": command,
        "success_criteria": [
            "At least one of the remaining repaired-overlay misses flips to correct.",
            "No more than one new strict regression elsewhere on the repaired overlay.",
            "Post-run reporting keeps the repaired-overlay claim boundary separate from the live stage10424 package.",
        ],
        "required_honesty_gates": [
            "No repaired strict overlay row is copied into train.",
            "Train rows remain stage10447 disjoint support only.",
            "Any improvement claim remains diagnostic-only until tested on disjoint strict roots.",
        ],
        "outputs": {
