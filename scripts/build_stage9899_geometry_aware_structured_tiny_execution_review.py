...
STAGE = 9899
NAME = "stage9899_geometry_aware_structured_tiny_execution_review"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9898_capped_geometry_aware_structured_target_100m_contract_preflight.json"
SOURCE_MANIFEST_DIR = ROOT / "runs/local/artifacts/stage9897_geometry_aware_structured_tiny_execution_review/tiny_structured_manifests"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
RUNS_DIR = OUT_DIR / "surface_runs"
AUDIT = OUT_DIR / "geometry_aware_structured_tiny_execution_review_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "GEOMETRY_AWARE_STRUCTURED_TINY_EXECUTION_REVIEW_STAGE9899.md"
...
def run_surface(surface: str) -> dict[str, Any]:
    manifest = SOURCE_MANIFEST_DIR / f"{surface}_tiny.jsonl"
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9898_not_passed")
    surface_results = []
    if not failures:
    ...
    next_step = "Use this geometry-aware structured review to decide whether the refreshed v2.7 package is ready for a broader standalone Gemma rerun or still needs a head/objective intervention."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Executed a capped target-100M structured tiny review on the Stage9896 geometry-aware v2.7 surfaces to verify whether the integrated edit-localization packet improvements carry through the actual multisurface execution path.",
