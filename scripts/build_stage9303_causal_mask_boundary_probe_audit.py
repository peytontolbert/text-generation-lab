...
STAGE = 9303
NAME = "stage9303_causal_mask_boundary_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9302_causal_mask_boundary_probe_preexecution.json"
BASELINE_SUMMARY = ROOT / "runs/summaries/stage9300_post_prefix_boundary_probe_audit.json"
RUN_DIR = ROOT / "runs/local/artifacts/stage9303_causal_mask_boundary_probe"
AUDIT = RUN_DIR / "stage9303_causal_mask_boundary_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "CAUSAL_MASK_BOUNDARY_PROBE_AUDIT_STAGE9303.md"
...
def audit_run() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9302_not_passed")
    ...
    return {
        ...
        "diagnosis": "corrected_causal_mask_enables_train_and_strict_generation_but_eval_suffix_class_remains_unlearned",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Safety passed and corrected causal masking enabled exact generation for all train rows and the strict row, but the eval operator suffix remains unlearned.",
        "next_best_step": "Add a balanced operator-suffix support row or rotate heldout suffix classes, then rerun the tiny boundary probe; do not reopen decoder CE or widen beyond controlled suffix diagnostics yet.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9303 Causal-Mask Boundary Probe Audit", "", f"Safety gate passed: `{audit['safety_gate_passed']}`", f"Quality gate passed: `{audit['quality_gate_passed']}`", f"Boundary match rate: `{audit['boundary_match_rate']}`", f"Boundary mean expected rank: `{audit['boundary_mean_expected_rank']}`", f"Baseline boundary match rate: `{audit['baseline_boundary_match_rate']}`", f"Baseline mean expected rank: `{audit['baseline_boundary_mean_expected_rank']}`", f"Target prefix match rate: `{audit['target_prefix_match_rate']}`", f"Diagnosis: `{audit['diagnosis']}`", "", "Authority is closed after the audit.", ""]) , encoding="utf-8")
