...
STAGE = 9630
NAME = "stage9630_tri_phase_repetition_guard_tiny_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9629_tri_phase_repetition_guard_contract_preflight.json"
...
PHASE3_MANIFEST = ROOT / "runs/local/artifacts/stage9609_suffix_continuation_ladder_manifest/suffix_continuation_ladder_manifest.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
RUN_DIR = OUT_DIR / "tri_phase_probe"
AUDIT = OUT_DIR / "tri_phase_repetition_guard_tiny_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "TRI_PHASE_REPETITION_GUARD_TINY_PROBE_STAGE9630.md"
...
def command() -> list[str]:
    return [
        ...
        "--phase3-manifest", str(PHASE3_MANIFEST),
        ...
        "--phase2-max-steps", "48", "--phase2-max-decoder-tokens", "96",
        "--phase3-max-train-rows", "40", "--phase3-max-eval-rows", "6", "--phase3-max-strict-rows", "6",
        "--phase3-max-steps", "80", "--phase3-max-decoder-tokens", "96",
        ...
        "--generation-repetition-guard", "--generation-repetition-guard-top-k", "32",
        "--require-loss-mask-enforcement-audit", "--no-final-checkpoint-export",
        "--cleanup-checkpoints-after-probe", "--skip-final-model-save", "1",
        "--output-dir", str(RUN_DIR), "--run-id", "stage9630_tri_phase_repetition_guard_probe",
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9629_not_passed")
    ...
    phase3_quality = result.get("phase3_quality") if isinstance(result.get("phase3_quality"), dict) else {}
    ...
    quality_gate = (
        ...
        and phase3_quality.get("contentful_generation_rate") == 1.0
        and (phase3_quality.get("short_or_junk_rate") in {0, 0.0, None})
        and (phase3_quality.get("degenerate_repetition_rate") in {0, 0.0, None})
        and (phase3_quality.get("generated_internal_token_rows") in {0, None})
        and phase3_quality.get("generation_prefix_start_rate") == 1.0
        and (phase3_quality.get("target_prefix_match_rate") or 0.0) >= 0.80
    )
    audit = {
        "passed": not failures,
        "failures": failures,
        ...
        "phase3_generated_rows": phase3_quality.get("generated_rows"),
        "phase3_contentful_generation_rate": phase3_quality.get("contentful_generation_rate"),
        "phase3_short_or_junk_rate": phase3_quality.get("short_or_junk_rate"),
        "phase3_degenerate_repetition_rate": phase3_quality.get("degenerate_repetition_rate"),
        "phase3_generated_internal_token_rows": phase3_quality.get("generated_internal_token_rows"),
        "phase3_target_prefix_match_rate": phase3_quality.get("target_prefix_match_rate"),
        "phase3_generation_prefix_field": phase3_quality.get("generation_prefix_field"),
        "phase3_generation_prefix_start_rate": phase3_quality.get("generation_prefix_start_rate"),
        "phase3_generation_repetition_guard": phase3_quality.get("generation_repetition_guard"),
        "phase3_generation_repetition_guard_top_k": phase3_quality.get("generation_repetition_guard_top_k"),
        "phase3_generation_repetition_guard_events": phase3_quality.get("generation_repetition_guard_events"),
        "phase3_generation_repetition_guard_event_rows": phase3_quality.get("generation_repetition_guard_event_rows"),
    ...
    next_step = "Audit Stage9630 guarded samples against Stage9623 baseline; if quality still fails, move from decoding guard to non-generative anti-repetition/EOS continuation objective."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Ran tiny tri-phase reconnect probe under patched in-memory trainer. Quality gate is recorded separately from safety pass.",
    ...
    DOC.write_text("\n".join([
        ...
        f"Quality gate: `{audit['quality_gate']}`",
        f"Phase1 eval/strict suffix exact: `{audit['phase1_eval_suffix_choice_exact']}` / `{audit['phase1_strict_suffix_choice_exact']}`",
        f"Phase3 contentful rate: `{audit['phase3_contentful_generation_rate']}`",
        f"Phase3 target prefix match rate: `{audit['phase3_target_prefix_match_rate']}`",
        f"Phase3 generation prefix start rate: `{audit['phase3_generation_prefix_start_rate']}`",
    ...
    print(json.dumps({"stage": STAGE, "passed": audit["passed"], "quality_gate": quality_gate, "failures": failures, "next_best_step": next_step}, indent=2, sort_keys=True))
    if failures:
        raise SystemExit(1)
