#!/usr/bin/env python3
"""Capture codex-protocol Rust verifier feasibility."""
...
STAGE = 11840
NAME = "stage11840_codex_protocol_rust_feasibility"
OUT = ART / NAME
LOGS = OUT / "logs"
SUMMARY = OUT / "codex_protocol_rust_feasibility.json"
RESULTS = OUT / "codex_protocol_rust_feasibility_results.jsonl"
REPO = Path("/data/agentkernel/other_repos/codex/codex-rs")
SELECTED_TEST = "resolve_supports_relative_and_absolute_references"
SIBLING_TEST = "join_builds_child_paths"
...
def run_cargo(test_filter: str, name: str) -> dict[str, Any]:
    ...
    command = ["cargo", "test", "-p", "codex-protocol", test_filter, "--", "--nocapture"]
    env = os.environ.copy()
    env.update(
        {
...
def main() -> None:
    selected = run_cargo(SELECTED_TEST, "selected_agent_path_resolve")
    sibling = run_cargo(SIBLING_TEST, "sibling_agent_path_join")
    passed = bool(selected["passed"] and sibling["passed"])
    row = {
        "stage": STAGE,
        "stage_name": NAME,
        "created_at_utc": now(),
        "repo_family": "codex_protocol",
        "repo_path": str(REPO),
        ...
        "source_path": "protocol/src/agent_path.rs",
        "distractor_source_path": "protocol/src/thread_id.rs",
        "passed": passed,
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "codex_protocol_rust_feasibility_recorded",
        "passed": passed,
