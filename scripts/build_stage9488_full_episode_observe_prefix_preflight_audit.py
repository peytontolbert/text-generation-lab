...
STAGE = 9488
NAME = "stage9488_full_episode_observe_prefix_preflight_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9487_full_episode_with_observe_prefix_manifest.json"
PREFLIGHT_DIR = ROOT / "runs/local/artifacts/stage9488_full_episode_observe_prefix_target_100m_contract_preflight"
CONTRACT = PREFLIGHT_DIR / "probe_contract_audit.json"
AUDIT = PREFLIGHT_DIR / "stage9488_preflight_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "FULL_EPISODE_OBSERVE_PREFIX_PREFLIGHT_AUDIT_STAGE9488.md"
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9487_not_passed")
    ...
    expected_splits = {"eval": 7, "other": 0, "strict_eval": 7, "train": 102}
    if contract.get("rows") != 116 or contract.get("split_counts") != expected_splits:
    ...
    for key, expected in EXPECTED_LOSS_COUNTS.items():
        if loss_counts.get(key) != expected:
            failures.append(f"loss_count_mismatch:{key}")
    for key, value in loss_counts.items():
        if key not in EXPECTED_LOSS_COUNTS and value:
    ...
    audit = {
        "passed": not failures,
        "failures": failures,
        "source_stage": "stage9487_full_episode_with_observe_prefix_manifest",
    ...
    summary = {
        ...
        "decision": "Authorized only Stage9489 full episode-step observe-prefix target-100M structured execution if this preflight passes.",
        "next_best_step": "Run Stage9489 full episode-step observe-prefix best-state-restored probe under trellis, then audit eval/strict exactness and safety artifacts.",
