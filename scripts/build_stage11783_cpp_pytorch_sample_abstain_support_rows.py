#!/usr/bin/env python3
"""Materialize PyTorch C++ sample abstain-attractor support rows."""
...
STAGE = 11783
NAME = "stage11783_cpp_pytorch_sample_abstain_support_rows"
OUT = ART / NAME
SUMMARY = OUT / "cpp_pytorch_sample_abstain_support_rows.json"
ROWS = OUT / "cpp_pytorch_sample_abstain_support_rows.jsonl"

FEASIBILITY = ART / "stage11782_cpp_pytorch_sample_build_feasibility/cpp_pytorch_sample_build_feasibility_results.jsonl"
...
def main() -> None:
    ...
    if not feasible:
        raise SystemExit("PyTorch C++ sample feasibility did not pass")
    feasibility = feasible[0]
    evidence = [
        file_evidence("E01", "candidate_change_surface", ".ci/pytorch/test_example_code/simple-torch-test.cpp", "Small C++ Torch sample executable emits a pass warning from main."),
        file_evidence("E02", "build_target_constraint", ".ci/pytorch/test_example_code/CMakeLists.txt", "CMake target links Torch and builds simple-torch-test."),
        file_evidence("E03", "nearby_cuda_check_distractor", ".ci/pytorch/test_example_code/check-torch-cuda.cpp", "CUDA check source is adjacent but not the selected simple executable target."),
        log_evidence("V01", feasibility["configure"], "PyTorch C++ sample configure passed against trellis Torch CMake package."),
        log_evidence("V02", feasibility["build"], "simple-torch-test target built successfully."),
        log_evidence("V03", feasibility["execute"], "simple-torch-test executable ran and emitted Simple test passed."),
    ]
    verifier_evidence = {"id": "V03", "kind": "actual_cpp_build_run_pass_log", "summary": "PyTorch simple-torch-test configured, built, and executed successfully.", "configure": feasibility["configure"], "build": feasibility["build"], "execute": feasibility["execute"], "result": "PASS_CURRENT_BUILD_AND_RUN"}
    answerable = [
        {"label": "A", "role": "candidate_change_surface", "value": ".ci/pytorch/test_example_code/simple-torch-test.cpp", "evidence_ids": ["E01"]},
        {"label": "B", "role": "verifier_and_build_constraint", "value": "cmake build and run simple-torch-test passed", "evidence_ids": ["E02", "V01", "V02", "V03"]},
        {"label": "C", "role": "nearby_cuda_check_surface", "value": ".ci/pytorch/test_example_code/check-torch-cuda.cpp", "evidence_ids": ["E03"]},
    ...
    root_id = "stage11783::pytorch::c_cpp::simple_torch_test_support"
    rows = [
        base_row(root_id, "answerable_evidence_citation_build_run_constraint", "evidence_citation", answerable, "B", True),
        base_row(root_id, "answerable_patch_impact_simple_test_surface", "patch_impact", answerable, "A", True),
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "third_cpp_abstain_attractor_support_root_materialized",
        "passed": True,
        "row_count": len(rows),
        "root_count": 1,
        "repo_family": "pytorch",
