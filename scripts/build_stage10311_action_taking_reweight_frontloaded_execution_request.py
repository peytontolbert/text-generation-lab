...
STAGE = 10311
NAME = "stage10311_action_taking_reweight_frontloaded_execution_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
INVENTORY_JSON = OUT_DIR / "action_taking_reweight_frontloaded_inventory.json"
MANIFEST_JSONL = OUT_DIR / "action_taking_reweight_frontloaded_manifest.jsonl"
REQUEST_JSON = OUT_DIR / "action_taking_reweight_frontloaded_execution_request.json"
COMMAND_JSON = OUT_DIR / "action_taking_reweight_frontloaded_command.json"
...
RUN_ID = "stage10312_action_taking_reweight_frontloaded_probe"
OUTPUT_DIR = "runs/local/artifacts/stage10312_action_taking_reweight_frontloaded_probe/bounded_decoder_probe"
RUNTIME_MODEL_DIR = "runs/local/artifacts/stage10312_action_taking_reweight_frontloaded_probe/runtime_model"
MAX_STEPS = 48
LEARNING_RATE = "8e-6"
...
def focus_duplicates_for(base_rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for row in base_rows:
        ...
        for index in range(duplicates):
            ...
            clone["row_id"] = f"{row['row_id']}::action_reweight_frontloaded::{index:02d}"
            clone["semantic_key"] = f"{row.get('semantic_key')}::action_reweight_frontloaded::{index:02d}"
            ...
            clone["reweight_metadata"] = {
                ...
                "reweight_family": "action_taking_frontier_frontloaded",
            }
...
def build() -> dict[str, Any]:
    ...
    merged = list(focus_rows) + train_rows + strict_rows + other_rows
    ...
    inventory = {
        ...
        "frontloaded_focus_rows": len(focus_rows),
    ...
    request = {
        ...
        "created_at_utc": now_utc(),
        "passed": True,
        "decision": "warm_start_action_taking_reweight_frontloaded_ready",
        ...
        "required_honesty_gates": [
            "strict eval rows remain identical to stage10307 and stage10310",
            "only train rows are duplicated and frontloaded; no admitted or strict eval rows are moved into train",
            "the change fixes capped-probe reachability rather than changing the benchmark itself",
            "focus remains restricted to observed failure slices: Python and web action-taking plus rust evidence citation",
        ],
        "next_best_step": "run one focused warm-start probe and check whether frontloaded action-taking pressure moves the unchanged strict frontier",
        "command": cmd,
    }
...
def main() -> None:
    ...
    print(json.dumps({
        ...
        "inventory": display(INVENTORY_JSON),
        "request": display(REQUEST_JSON),
        "frontloaded_focus_rows": result["frontloaded_focus_rows"],
        "rows": result["rows"],
