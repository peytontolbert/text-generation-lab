#!/usr/bin/env python3
"""Materialize codex-utils-string Rust selected-verifier support rows."""
...
STAGE = 11849
NAME = "stage11849_codex_utils_string_rust_support_rows"
OUT = ART / NAME
SUMMARY = OUT / "codex_utils_string_rust_support_rows.json"
ROWS = OUT / "codex_utils_string_rust_support_rows.jsonl"
FEASIBILITY = ART / "stage11848_codex_utils_string_rust_feasibility/codex_utils_string_rust_feasibility_results.jsonl"
...
def file_evidence(repo: Path, eid: str, kind: str, rel_path: str, summary: str, start: int = 1, end: int = 190) -> dict[str, Any]:
...
def main() -> None:
    ...
    if not probes:
        raise SystemExit("no materializable codex-utils-string Rust probe found")
    probe = probes[0]
    ...
    root_id = "stage11849::codex_utils_string::rust::find_uuids_support"
    evidence = [
        file_evidence(repo, "E01", "candidate_change_surface", source_path, "Codex string utilities source implements UUID finding and metric/tag normalization behavior covered by focused unit tests."),
        file_evidence(repo, "E02", "nearby_source_distractor", distractor_source_path, "String truncation utilities are nearby text helpers but not the selected UUID verifier target."),
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "eleventh_rust_verifier_support_root_materialized",
        "passed": True,
        "row_count": len(rows),
        "root_count": 1,
        "repo_family": repo_family,
        ...
        "claim_boundary": ["Rows are train-support-only.", "This expands Rust verifier support with a non-tokenizers Codex string utilities repo family."],
    }
