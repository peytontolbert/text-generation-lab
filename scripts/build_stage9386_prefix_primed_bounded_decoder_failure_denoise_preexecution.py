...
STAGE = 9386
NAME = "stage9386_prefix_primed_bounded_decoder_failure_denoise_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9385_prefix_primed_bounded_decoder_failure_denoise_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9385_prefix_primed_bounded_decoder_failure_denoise_manifest/prefix_primed_bounded_decoder_failure_denoise_manifest.jsonl"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9387_prefix_primed_bounded_decoder_failure_denoise_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "prefix_primed_bounded_decoder_failure_denoise_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9387_prefix_primed_bounded_decoder_failure_denoise_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PREFIX_PRIMED_BOUNDED_DECODER_FAILURE_DENOISE_PREEXECUTION_STAGE9386.md"
...
COMMAND = [
    ...
    "--enable-generation-audit", "--max-generation-rows", "23", "--max-generation-tokens", "128",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9387_prefix_primed_bounded_decoder_failure_denoise_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if not prefix or not target.startswith(prefix):
            missing_prefix.append(str(row.get("row_id")))
    failures: list[str] = []
    if source.get("passed") is not True:
        failures.append("source_stage9385_not_passed")
    if len(rows) != 23:
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared a prefix-primed 23-row denoise probe for bounded decoder failure repairs; decoder CE remains closed.",
        "next_best_step": "Execute Stage9387 prefix-primed denoise repair probe under trellis and audit target recovery.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9386 Prefix-Primed Bounded Decoder Failure Denoise Preexecution", "", f"Passed: `{card['passed']}`", f"Rows: `{card['manifest_rows']}`", f"Splits: `{card['split_counts']}`", f"Routes: `{card['route_counts']}`", "", "Only the Stage9387 prefix-primed denoise probe is authorized. Decoder CE and external authority remain closed.", ""]), encoding="utf-8")
