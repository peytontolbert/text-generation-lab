...
STAGE = 9647
NAME = "stage9647_micro_overfit_boundary_prefix_tiny_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9646_micro_overfit_boundary_prefix_manifest.json"
...
AUDIT = OUT_DIR / "micro_overfit_boundary_prefix_tiny_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "MICRO_OVERFIT_BOUNDARY_PREFIX_TINY_PROBE_STAGE9647.md"
...
def command() -> list[str]:
    return [
        ...
        "--output-dir", str(RUN_DIR), "--run-id", "stage9647_micro_overfit_boundary_prefix_probe", "--execution-authorized-for-recovery-probe",
    ]
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        safety_failures.append("stage9646_not_passed")
    if float(source_metrics.get("strongest_metadata_baseline", 1.0) or 1.0) >= 0.75:
        safety_failures.append("stage9646_metadata_shortcut_baseline_too_high")
    if int(source_metrics.get("rows") or 0) != 12:
        safety_failures.append("stage9646_unexpected_row_count")
    ...
    train_exact_from_logits = {
        field: 1.0 - (sum(1 for row in logits if row.get("split") == "train" and row.get("field") == field and row.get("correct") is False) / max(1, sum(1 for row in logits if row.get("split") == "train" and row.get("field") == field)))
        for field in EXPECTED_FIELDS
    }
    quality_pass = bool(
        eval_joint == 1.0
        and all(value == 1.0 for value in eval_exact.values())
        and len(eval_exact) == len(EXPECTED_FIELDS)
    ...
    audit = {
        ...
        "train_field_exact_from_logits": train_exact_from_logits,
