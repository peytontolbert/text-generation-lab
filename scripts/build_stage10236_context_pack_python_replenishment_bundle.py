...
STAGE = 10236
NAME = "stage10236_context_pack_python_replenishment_bundle"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
BUNDLE_JSON = OUT_DIR / "context_pack_python_replenishment_bundle.json"
ADMITTED_JSON = OUT_DIR / "context_pack_python_replenishment_admitted_manifest.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"

BASE_ADMITTED = ROOT / "runs/local/artifacts/stage10176_code_assist_web_replenishment_bundle/code_assist_web_replenishment_admitted_manifest.json"
REVIEW_DIR = OUT_DIR / "review_packets" / "stage10236__code_assist__python"

REPO_ROOT = Path('/data/code_assist')
CONTEXT_PACK = REPO_ROOT / 'src/code_assist/orchestrator/context_pack.py'
...
TEST_DOMAIN = REPO_ROOT / 'tests/integration/test_domain_knowledge_context_pack.py'
TEST_INCIDENT = REPO_ROOT / 'tests/integration/test_incident_loop_in_context_pack.py'
TEST_TASK_MESSAGE = REPO_ROOT / 'tests/integration/test_task_message_in_context_pack.py'
TEST_LOG_REF = REPO_ROOT / 'tests/unit/test_context_pack_log_ref_path_safety.py'

BUNDLE_ID = (
    'stage10236::localsess_code_assist_sessseed_codex_sessions_rollout_2026_01_27t02_21_48_019bfd41_d2a6_7672_9873_81f8_'
    'src_code_assist_orchestrator_config_py_src_code_assist_orchestrator_context_pack_acd5ccc9c1_aug_1500000_8b46e7f662::'
    'python'
)
SEED_PATHS = [
    'src/code_assist/orchestrator/config.py',
    'src/code_assist/orchestrator/context_pack.py',
    'src/code_assist/orchestrator/contract_compiler.py',
    'src/code_assist/orchestrator/event_log.py',
    'src/code_assist/orchestrator/metrics.py',
    'src/code_assist/orchestrator/node_contract_compiler.py',
    'src/code_assist/orchestrator/oracle_synthesis.py',
...
SELECTED_TESTS = [
    ...
    'tests/integration/test_incident_loop_in_context_pack.py',
    'tests/integration/test_task_message_in_context_pack.py',
    'tests/unit/test_context_pack_log_ref_path_safety.py',
]
PERSPECTIVES = [
...
def evidence() -> dict[str, list[dict[str, Any]]]:
    return {
        'candidate_change_surface': [
            {
                'path': 'src/code_assist/orchestrator/context_pack.py',
                'source_type': 'local_repo',
                'retrieval_reason': 'seed_change',
                'distance_from_seed': 0,
                'text': excerpt(CONTEXT_PACK, anchor='for ref in log_refs[:5]:', radius=20),
            },
            {
                'path': 'src/code_assist/orchestrator/context_pack.py',
                'source_type': 'local_repo',
                'retrieval_reason': 'seed_change',
                'distance_from_seed': 0,
                'text': excerpt(CONTEXT_PACK, anchor='"domain_knowledge": dk_artifacts,', radius=16),
            },
            {
                'path': 'src/code_assist/orchestrator/context_pack.py',
                'source_type': 'local_repo',
                'retrieval_reason': 'seed_change',
                'distance_from_seed': 0,
                'text': excerpt(CONTEXT_PACK, anchor='"chat_history": chat_history or [],', radius=14),
            },
            {
                ...
                'retrieval_reason': 'seed_change',
                'distance_from_seed': 0,
                'text': excerpt(CONFIG, anchor='class DomainKnowledgeSettings', radius=24),
            },
        ],
        'verifier_and_test_constraint': [
            {
                'path': 'tests/integration/test_domain_knowledge_context_pack.py',
                'source_type': 'local_repo',
                'retrieval_reason': 'targeted_test_selection',
                'distance_from_seed': 1,
                'text': excerpt(TEST_DOMAIN, anchor='def test_domain_knowledge_docs_and_dependency_manifests_in_context_pack', radius=28),
            },
            {
                'path': 'tests/unit/test_context_pack_log_ref_path_safety.py',
                'source_type': 'local_repo',
                'retrieval_reason': 'targeted_test_selection',
                'distance_from_seed': 1,
                'text': excerpt(TEST_LOG_REF, anchor='def test_context_pack_does_not_read_log_refs_outside_artifacts_dir', radius=24),
            },
        ],
        'symptom_or_call_path_analogue': [
            {
                'path': 'tests/integration/test_incident_loop_in_context_pack.py',
                'source_type': 'local_repo',
                'retrieval_reason': 'targeted_test_selection',
                'distance_from_seed': 1,
                'text': excerpt(TEST_INCIDENT, anchor='compiler.compile(task=task', radius=18),
            },
            {
                'path': 'tests/integration/test_task_message_in_context_pack.py',
                'source_type': 'local_repo',
                'retrieval_reason': 'targeted_test_selection',
                'distance_from_seed': 1,
                'text': excerpt(TEST_TASK_MESSAGE, anchor='path = compiler.compile(', radius=20),
            },
        ],
        'nearby_definition_or_usage_context': [
            {
                ...
                'source_type': 'local_repo',
                'retrieval_reason': 'context_pack_owner_instantiation',
                'distance_from_seed': 1,
                'text': excerpt(ORCHESTRATOR, anchor='self.context = ContextPackCompiler(', radius=16),
            },
            {
                'path': 'src/code_assist/orchestrator/orchestrator.py',
                'source_type': 'local_repo',
                'retrieval_reason': 'context_pack_owner_usage',
                'distance_from_seed': 1,
                'text': excerpt(ORCHESTRATOR, anchor='chat_history=self._recent_task_messages(task_id),', radius=18),
            },
        ],
...
def prompt_contract(perspective: str, visible_keys: list[str]) -> dict[str, Any]:
    tasks = {
        'symptom_localization': 'Choose the most likely edit target from the visible context-pack behavior and verifier evidence.',
        'evidence_citation': 'Name the visible fact that best supports the chosen edit target.',
        'alternative_hypothesis_elimination': 'Explain why a plausible competing orchestrator/config surface is less justified.',
...
def bundle_payload() -> dict[str, Any]:
    ...
    return {
        ...
        'repo_id': 'code_assist',
        'language_family': 'python',
        'source_route': 'PATCH_ONLY_FILTERED_PYTHON_REPLENISHMENT',
...
def rubric_review() -> dict[str, Any]:
    rationale = (
        'Admit. The visible tests directly exercise ContextPackCompiler behavior for domain knowledge, incident artifacts, task-message inclusion, and log-ref path safety. '
        'That makes context_pack.py the clear behavior-owning surface, with orchestrator.py and config.py remaining plausible but weaker alternatives.'
    )
    return {
        'bundle_id': BUNDLE_ID,
        ...
        'reviewer_notes': 'AI maintainer adjudication performed in-repo on a filtered Python packet. Polluted repo-graph neighbors and unrelated changed surfaces from the raw session row were intentionally excluded.',
        'rubric_lines': {
...
def anti_cheat_review() -> dict[str, Any]:
    return {
        ...
        'decision_rationale': (
            'Admit. This repaired packet removes the raw row\'s noisy venv/site-packages neighbors and limits visible evidence to local source plus directly relevant tests. '
            'The main residual shortcut risk is that the selected tests explicitly name context-pack behavior, so future harder variants should include additional candidate-competition roots rather than relying on this one packet alone.'
        ),
        'language_family': 'python',
        'passed': True,
        'repo_id': 'code_assist',
        ...
        'reviewer_id': 'codex-gpt5-ai-review',
        'reviewer_notes': 'Strongest remaining shortcut risk is explicit context_pack naming in tests; that is acceptable for same-surface training support but should be balanced with additional fresh roots before broad claim upgrades.',
        'status': 'completed',
    }
    ...
    answers = [
        {
            ...
            'gold_answer_value': 'src/code_assist/orchestrator/context_pack.py',
            'perspective': 'symptom_localization',
            'reviewer_rationale': 'The visible tests import or compile ContextPackCompiler directly and assert fields that are assembled inside context_pack.py.',
            'selected_tests': list(SELECTED_TESTS),
            'visible_evidence_keys': visible_keys,
        },
        {
            ...
            'reviewer_rationale': 'The selected tests directly check domain_knowledge, incident artifacts, chat_history, and log-ref path safety, making verifier evidence the strongest visible support for the context_pack target.',
            'selected_tests': list(SELECTED_TESTS),
            'visible_evidence_keys': visible_keys,
        },
        {
            ...
            'gold_answer_value': 'orchestrator.py wires the compiler into the larger system and config.py defines domain-knowledge settings, but the visible tests assert fields emitted by ContextPackCompiler.compile itself, so context_pack.py is the more justified edit surface.',
            'perspective': 'alternative_hypothesis_elimination',
            'reviewer_rationale': 'The competing files are plausible supporting surfaces, but the visible assertions land on the compiler output rather than the higher-level orchestrator wiring.',
            'selected_tests': list(SELECTED_TESTS),
            'visible_evidence_keys': visible_keys,
        },
        {
            ...
            'reviewer_rationale': 'Editing the compiler is the most direct way to change the emitted context-pack fields and path-safety behavior with less unrelated blast radius than altering the full orchestrator.',
            'selected_tests': list(SELECTED_TESTS),
            'visible_evidence_keys': visible_keys,
        },
        {
            ...
            'gold_answer_value': 'tests/integration/test_domain_knowledge_context_pack.py',
            'perspective': 'verifier_outcome',
            'reviewer_rationale': 'That integration test is the clearest single visible verifier for the compiler\'s structured output, while the other selected tests provide supporting coverage for incident, chat-history, and path-safety cases.',
            'selected_tests': list(SELECTED_TESTS),
            'visible_evidence_keys': visible_keys,
        },
        {
            ...
            'reviewer_rationale': 'It is the narrowest visible behavior-owning surface that can satisfy all four focused context-pack tests.',
            'selected_tests': list(SELECTED_TESTS),
            'visible_evidence_keys': visible_keys,
        },
        {
            ...
            'gold_answer_value': 'A context_pack.py change could regress domain-knowledge injection, incident artifact carry-through, task-message inclusion, and log-ref path sanitization across the orchestrator.',
            'perspective': 'regression_risk',
            'reviewer_rationale': 'The same compiler builds multiple evidence-bearing sections, so a local repair can still have broad downstream prompt and safety effects.',
            'selected_tests': list(SELECTED_TESTS),
            'visible_evidence_keys': visible_keys,
        },
        {
            ...
            'reviewer_rationale': 'This perspective remains the explicit honesty row even though the other perspectives are sufficiently grounded by local compiler and test evidence.',
            'selected_tests': list(SELECTED_TESTS),
            'visible_evidence_keys': visible_keys,
        },
    ]
...
def augmented_admitted_manifest(bundle: dict[str, Any]) -> dict[str, Any]:
    ...
    metrics['python_replenishment_added'] = True
    return {
        'stage': STAGE,
        'stage_name': NAME,
        ...
        'passed': True,
        'decision': (
            'Added a reviewed Python context-pack replenishment bundle from code_assist so the bounded standalone path can train on a second honest Python maintainer root '
            'with direct verifier/test constraints and filtered candidate competition.'
        ),
...
def main() -> None:
    ...
    summary = {
        ...
        'next_best_step': 'Use this admitted Python replenishment bundle when compiling the next bounded maintainer package, then build the filtered code_assist web bundle and mine fresh c_cpp/rust roots.',
    }
    write_json(SUMMARY, summary)
    update_registry(summary)
    print(json.dumps({'stage': STAGE, 'passed': True, 'bundle': display(BUNDLE_JSON), 'admitted_manifest': display(ADMITTED_JSON)}, indent=2, sort_keys=True))
