...
STAGE = 9669
NAME = "stage9669_prefix_primed_sidecar_residual_denoise_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9668_prefix_primed_sidecar_residual_denoise_preexecution.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9668_prefix_primed_sidecar_residual_denoise_preexecution/prefix_primed_sidecar_residual_denoise_manifest.jsonl"
RUN_DIR = ROOT / "runs/local/artifacts/stage9669_prefix_primed_sidecar_residual_denoise_probe"
AUDIT = RUN_DIR / "stage9669_prefix_primed_sidecar_residual_denoise_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PREFIX_PRIMED_SIDECAR_RESIDUAL_DENOISE_PROBE_AUDIT_STAGE9669.md"
...
def first_mismatch(target: str, generated: str) -> dict[str, Any]:
    target_words = target.split()
    generated_words = generated.split()
    prefix_len = 0
    for target_word, generated_word in zip(target_words, generated_words):
        if target_word != generated_word:
            break
        prefix_len += 1
    return {
        "matching_words": prefix_len,
        "target_next": target_words[prefix_len] if prefix_len < len(target_words) else "",
        "generated_next": generated_words[prefix_len] if prefix_len < len(generated_words) else "",
    }
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9668_not_passed")
    ...
    mismatch_next: Counter[str] = Counter()
    failed_samples: list[dict[str, Any]] = []
    for sample in samples:
        row = rows_by_id.get(str(sample.get("row_id")), {})
        bucket = str((row.get("residual_repair_route") or {}).get("repair_bucket") or "unknown")
        ...
        counter["prefix_start"] += int(bool(sample.get("generation_prefix_start_match")))
        if not sample.get("exact_match"):
            mismatch = first_mismatch(str(sample.get("target_text") or ""), str(sample.get("generated_text") or ""))
            if mismatch["target_next"] or mismatch["generated_next"]:
                mismatch_next[f"{mismatch['target_next']}=>{mismatch['generated_next']}"] += 1
    ...
    bucket_card = {
        key: {
            ...
            "prefix_start": c["prefix_start"],
            "exact_rate": rate(c["exact"], c["rows"]),
            "target_prefix_rate": rate(c["target_prefix"], c["rows"]),
            "prefix_start_rate": rate(c["prefix_start"], c["rows"]),
        }
        for key, c in sorted(by_bucket.items())
    }
    safety_passed = not failures
    quality_passed = bool(
        generated == 26
        and exact == 26
        and target_prefix == 26
        and prefix_start == 26
        and contentful == 26
        and short_rows == 0
    ...
    audit = {
        ...
        "denoise_delta_norm": module_delta.get("denoise_delta_norm"),
        ...
        "by_repair_bucket": bucket_card,
        "top_first_mismatch_pairs": dict(mismatch_next.most_common(12)),
        "failed_sample_count": len(failed_samples),
        "failed_samples": failed_samples[:20],
        "diagnosis": "prefix_scaffold_works_but_post_prefix_discriminating_suffix_tokens_fail",
        "next_patch_target": "post_prefix_suffix_choice_or_slot_discriminator_manifest",
        "authority": dict(AUTHORITY_CLOSED),
    }
    AUDIT.write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    next_step = "Build Stage9670 post-prefix suffix-choice/slot discriminator manifest from Stage9669 mismatches; keep decoder/runtime/Gemma/harness closed."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Stage9669 executed safely and prefix priming started every generation correctly, but post-prefix suffix discrimination failed; do not continue generic denoise tuning.",
    ...
    DOC.write_text(
        "\n".join(
            [
                ...
                "",
                f"Top first mismatches: `{audit['top_first_mismatch_pairs']}`",
                "",
                f"Next: {next_step}",
                "",
            ]
        ...
        json.dumps(
            {
                ...
                "generation_prefix_start_rate": audit["generation_prefix_start_rate"],
                "next_best_step": next_step,
            },
            indent=2,
            sort_keys=True,
