#!/usr/bin/env python3
"""Bind deep repo/code knowledge rows to a trainer-shaped adapter and shortcut policy.

This resolves the Stage12679 adapter/shortcut materialization blocker only. It
does not admit training, loss execution, optimizer steps, runtime, or eval use.
"""
...
STAGE = "stage12680_deep_repo_code_knowledge_adapter_and_shortcut_preflight_only"
OUT = ROOT / "runs/local/artifacts" / STAGE
SUMMARY = ROOT / "runs/summaries" / f"{STAGE}.json"

S12679_SUMMARY = ROOT / "runs/summaries/stage12679_deep_repo_code_knowledge_independent_review_only.json"
S12679_AUDIT = ROOT / "runs/local/artifacts/stage12679_deep_repo_code_knowledge_independent_review_only/deep_repo_code_knowledge_independent_review_audit.json"
...
EXPECTED_HASHES = {
    "stage12679_summary": "9f38b80bb6be0137717b9f54c9b02cdfd310d0354600ce094b15e75eea48aad8",
    "stage12679_audit": "52f785ee294c13e4946c544c8b9d9363cdea8bf38343b9b3ceed5623ee75823d",
    "stage12678_rows": "009af0a174e5c00327aeededff7f11e9c08144bd420bddba413ba0d19fbaca3f",
}

FALSE_FIELDS = (
    "implementation_ready", "stage12681_allowed", "training_allowed", "training_run_allowed",
...
UPSTREAM_FALSE_FIELDS = tuple(field for field in FALSE_FIELDS if field != "stage12681_allowed") + ("stage12680_allowed",)
...
CANONICAL_REQUIRED_FIELDS = {
    "row_id", "source_row_id", "split", "language_family", "input_state", "target", "loss_mask",
    "loss_weight", "authority", "source_provenance", "anti_cheat_contract", "adapter_status",
}
...
ULTRA_COMMON_CROSS_SPLIT_TARGET_THRESHOLD = 1000
MIN_DUPLICATE_TARGET_WEIGHT = 0.05


class Stage12680AdapterError(RuntimeError):
    pass
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise Stage12680AdapterError("json_object_required:" + path.name)
    return value
...
def read_jsonl(path: Path) -> list[dict[str, Any]]:
    ...
    with path.open(encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            if not isinstance(value, dict):
                raise Stage12680AdapterError(f"row_object_required:{line_number}")
            rows.append(value)
    return rows
...
def check_false(record: Mapping[str, Any], label: str, fields: tuple[str, ...]) -> None:
    for field in fields:
        if record.get(field) is not False:
            raise Stage12680AdapterError(f"{label}_gate_drift:{field}")
...
def assert_no_forbidden(value: Any, label: str) -> None:
    ...
    for needle in FORBIDDEN_SUBSTRINGS:
        if needle in encoded:
            raise Stage12680AdapterError(f"{label}_forbidden_substring:{needle}")
...
def anti_cheat_contract() -> dict[str, bool]:
    return {
        ...
        "file_split_repair_still_requires_review": True,
    }


def load_inputs() -> tuple[dict[str, Any], dict[str, Any], list[dict[str, Any]]]:
    pins = {
        "stage12679_summary": S12679_SUMMARY,
        "stage12679_audit": S12679_AUDIT,
        "stage12678_rows": S12678_ROWS,
    }
    for label, path in pins.items():
        if sha256_bytes(path.read_bytes()) != EXPECTED_HASHES[label]:
            raise Stage12680AdapterError("pin_drift:" + label)
    summary = read_json(S12679_SUMMARY)
    audit = read_json(S12679_AUDIT)
    check_false(summary, "stage12679_summary", UPSTREAM_FALSE_FIELDS)
    check_false(audit, "stage12679_audit", UPSTREAM_FALSE_FIELDS)
    if summary.get("recommended_next_stage") != STAGE:
        raise Stage12680AdapterError("stage12679_next_action_drift")
    if summary.get("training_admission_review_passed") is not False:
        raise Stage12680AdapterError("stage12679_training_admission_drift")
    rows = read_jsonl(S12678_ROWS)
    if len(rows) != 120000 or count(rows, "objective_family") != EXPECTED_OBJECTIVE_COUNTS or count(rows, "split") != EXPECTED_SPLIT_COUNTS:
        raise Stage12680AdapterError("stage12678_row_count_drift")
...
def language_family(row: Mapping[str, Any]) -> str:
    input_state = row.get("input_state") if isinstance(row.get("input_state"), dict) else {}
    return str(input_state.get("language_family") or "mixed")
...
def adapt_row(row: Mapping[str, Any], signature: str, signature_count: int) -> dict[str, Any]:
    ...
    if any(original_authority.values()):
        raise Stage12680AdapterError("source_row_authority_open")
    input_state = row.get("input_state") if isinstance(row.get("input_state"), dict) else {}
    ...
    if not input_state or not expected_output:
        raise Stage12680AdapterError("source_row_missing_deep_fields")
    adapted_input = dict(input_state)
    adapted_input["objective_family"] = row.get("objective_family")
    adapted = {
        "row_id": "stage12680_" + str(row["row_id"]).removeprefix("stage12678_"),
        "source_row_id": row["row_id"],
        "split": row["split"],
        "language_family": language_family(row),
        "input_state": adapted_input,
        "target": {
            "deep_repo_code_knowledge_target": expected_output,
            "target_signature": signature[:24],
        },
        "loss_mask": {"deep_repo_code_knowledge_ce": True},
        "loss_weight": duplicate_weight(signature_count),
        "authority": authority(),
        "source_provenance": {
            "source_stage": "stage12678_code_doc_build_semantic_knowledge_materialization_preflight_only",
            "source_row_digest": stable_hash(row),
            "source_target_signature": signature[:24],
            "source_file_digest": evidence.get("source_file_digest"),
            "repo_digest": evidence.get("repo_digest"),
        },
        "anti_cheat_contract": anti_cheat_contract(),
        "adapter_status": "deep_repo_code_knowledge_adapter_bound_shortcut_weighted_review_required",
        "shortcut_baseline": {
    ...
    if missing:
        raise Stage12680AdapterError("canonical_fields_missing:" + ",".join(sorted(missing)))
    if any(adapted["authority"].values()):
        raise Stage12680AdapterError("adapted_authority_open")
...
def build_adapter(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    ...
    duplicate_bearing_rows = sum(len(group) for group in groups.values() if len(group) > 1)
    duplicate_excess_rows = sum(len(group) - 1 for group in groups.values() if len(group) > 1)
    cross_split_signatures = sum(1 for group in groups.values() if len({row["split"] for row in group}) > 1)
    effective_weighted_rows = round(sum(float(row["loss_weight"]) for row in adapter_rows), 6)
    shortcut_audit = {
        "record_type": "stage12680_shortcut_baseline_audit_v1",
        "stage": STAGE,
        "source_rows_reviewed": len(rows),
        "target_signature_count": len(groups),
        "target_duplicate_instances_before_policy": duplicate_bearing_rows,
        "target_duplicate_excess_rows_before_policy": duplicate_excess_rows,
        "target_duplicate_signatures_cross_split_before_policy": cross_split_signatures,
        ...
        "objective_counts": dict(sorted(collections.Counter(row["input_state"]["objective_family"] for row in adapter_rows).items())),
        "training_source_rows_admitted": 0,
        **false_fields(),
    }
    if len(adapter_rows) != 97462:
        raise Stage12680AdapterError("adapter_row_count_drift")
    if len(quarantine_rows) != 22538:
        raise Stage12680AdapterError("quarantine_policy_count_drift")
    if abs(effective_weighted_rows - 52073.566783) > 0.000001:
        raise Stage12680AdapterError("effective_weighted_rows_drift")
...
def build_packet() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], list[dict[str, Any]], list[dict[str, Any]]]:
    summary79, audit79, rows = load_inputs()
    ...
    audit = {
        "record_type": "stage12680_deep_repo_code_knowledge_adapter_and_shortcut_audit_v1",
        "stage": STAGE,
        "decision": "DEEP_REPO_CODE_KNOWLEDGE_ADAPTER_AND_SHORTCUT_POLICY_MATERIALIZED_REVIEW_REQUIRED",
        "stage12679_blockers_addressed": {
            "trainer_adapter_fit": "resolved_canonical_deep_repo_code_knowledge_ce_adapter_candidates_materialized",
            "shortcut_baseline_status": "resolved_ultra_common_quarantine_and_duplicate_target_weighting_materialized",
            "semantic_duplicate_repair_status": "mitigated_by_duplicate_target_weighting_review_required",
            "file_split_repair_status": "not_resolved_independent_link_materialization_and_split_review_required",
        },
        "source_rows_reviewed": len(rows),
        "adapter_candidate_rows": len(adapter_rows),
        "quarantined_rows": len(quarantine_rows),
        ...
        "trainer_adapter_fit": "deep_repo_code_knowledge_ce_adapter_bound_review_required",
        "shortcut_baseline_status": "materialized_review_required",
        "training_source_rows_admitted": 0,
        "next_required_action": "stage12681_precise_symbol_doc_test_link_materialization_preflight_only",
        "upstream_review_decision": summary79["decision"],
        "upstream_review_cross_split_source_file_digest_groups": audit79["cross_split_source_file_digest_groups"],
        **false_fields(),
    }
    summary = {
        "record_type": "stage12680_public_deep_repo_code_knowledge_adapter_shortcut_summary_v1",
        "stage": STAGE,
...
def build(out: Path = OUT, summary_path: Path = SUMMARY) -> dict[str, Any]:
    ...
    private = {
        "record_type": "stage12680_private_deep_repo_code_knowledge_adapter_packet_v1",
        "stage": STAGE,
    ...
    contract = {
        "record_type": "stage12680_deep_repo_code_knowledge_adapter_contract_v1",
        "stage": STAGE,
    ...
    pointer = {
        "record_type": "stage12680_digest_pointer_v1",
        "stage": STAGE,
    ...
    for label, record in (("private", private), ("contract", contract), ("pointer", pointer)):
        assert_no_forbidden(record, label)
    write_jsonl(out / "private/deep_repo_code_knowledge_adapter_candidates.jsonl", adapter_rows)
    write_jsonl(out / "private/deep_repo_code_knowledge_shortcut_quarantine.jsonl", quarantine_rows)
    write_json(out / "summary.json", summary)
    write_json(out / "deep_repo_code_knowledge_adapter_audit.json", audit)
    write_json(out / "deep_repo_code_knowledge_shortcut_baseline_audit.json", shortcut_audit)
    write_json(out / "private/deep_repo_code_knowledge_adapter_packet.json", private)
