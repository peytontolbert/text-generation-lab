...
STAGE = 9638
NAME = "stage9638_counternegative_upsample_observe_tiny_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9637_counternegative_upsample_observe_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9637_counternegative_upsample_observe_manifest/counternegative_upsample_observe_manifest.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
RUN_DIR = OUT_DIR / "episode_step_probe"
AUDIT = OUT_DIR / "counternegative_upsample_observe_tiny_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "COUNTERNEGATIVE_UPSAMPLE_OBSERVE_TINY_PROBE_STAGE9638.md"
...
def command() -> list[str]:
    return ["env", f"TMPDIR={TMPDIR}", f"TEMP={TMPDIR}", f"TMP={TMPDIR}", "conda", "run", "-n", "trellis", "python", str(TRAINER),
        ...
        "--output-dir", str(RUN_DIR), "--run-id", "stage9638_counternegative_upsample_observe_probe", "--execution-authorized-for-recovery-probe"]
...
def main() -> None:
    ...
    if source.get("passed") is not True: safety_failures.append("stage9637_not_passed")
    if (source.get("metrics") or {}).get("strongest_single_feature_baseline", 1.0) >= 0.80: safety_failures.append("stage9637_shortcut_baseline_too_high")
