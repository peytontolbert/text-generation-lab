...
STAGE = 9725
NAME = "stage9725_multilingual_edit_localization_target100m_contract_preflight"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9724_multilingual_edit_localization_tiny_package.json"
...
AUDIT = OUT_DIR / "multilingual_edit_localization_target100m_contract_preflight_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "MULTILINGUAL_EDIT_LOCALIZATION_TARGET100M_CONTRACT_PREFLIGHT_STAGE9725.md"
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9724_not_passed")
    if run.returncode != 0:
    ...
    next_step = "If explicit execution is confirmed, run one multilingual target-100M edit-localization structured probe from the Stage9724 package; otherwise widen the same multilingual pattern to patch_operator_selection."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Validated the multilingual edit-localization package under target-100M contract-only preflight. This does not execute model training.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9725 Multilingual Edit Localization Target-100M Contract Preflight",
        "",
        f"Passed: `{summary['passed']}`",
        f"Rows: `{len(rows)}`",
        f"Loss counts: `{losses}`",
        "",
        "This is contract-only and validates a real multilingual edit-localization package under target-100M settings without opening model execution claims.",
        "",
        f"Next: {next_step}",
        "",
    ]), encoding="utf-8")
