...
STAGE = 9360
NAME = "stage9360_non_operator_rejoin_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9359_non_operator_rejoin_probe_preexecution.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9358_full_mixture_with_non_operator_residual_repairs_manifest/full_mixture_with_non_operator_residual_repairs_manifest.jsonl"
RUN_DIR = ROOT / "runs/local/artifacts/stage9360_non_operator_rejoin_probe"
AUDIT = RUN_DIR / "stage9360_non_operator_rejoin_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "NON_OPERATOR_REJOIN_PROBE_AUDIT_STAGE9360.md"
...
def classify(sample: dict[str, Any]) -> list[str]:
    generated = str(sample.get("generated_text") or "")
    target = str(sample.get("target_text") or "")
    labels: list[str] = []
    if "pator" in generated:
        labels.append("operator_subword_pator")
    if target.endswith("verified patch operator") and "verified" not in generated:
        labels.append("operator_verified_omission")
    if target.endswith("verified patch operator") and "patch operator" in generated and "verified" not in generated:
        labels.append("operator_unverified_terminal")
    if target.endswith("localized edit") and "patch inside" in generated:
        labels.append("file_path_patch_inside_intrusion")
    if target.endswith("localized edit") and "localized edit" not in generated:
        labels.append("file_path_localized_edit_omission")
...
def audit_run() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9359_not_passed")
    ...
    if contract.get("probe_scale") != "target_100m":
        failures.append("wrong_probe_scale")
    ...
    if execution.get("denoise_ce_rows") != len(rows) or contract.get("loss_counts", {}).get("denoise_ce") != len(rows):
        ...
        failed_samples.append(
            {
                ...
                "labels": labels,
                "source": row.get("merged_curriculum_source"),
                "combined": row.get("combined_curriculum_source"),
    ...
    quality_gate_passed = bool(
        generated == len(rows)
        and exact == len(rows)
        and target_prefix == len(rows)
        and boundary == len(rows)
        and contentful == len(rows)
    ...
    return {
        ...
        "diagnosis": "The non-operator residual rows rejoined safely, with zero leaks, zero short outputs, and zero repetition, but full-mixture exactness still fails on route_1 operator wording and route_file_path localized-edit wording.",
        "next_patch_target": "operator_verified_terminal_and_file_path_localized_edit_mixture_repairs",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "The non-operator residual rejoin probe executed safely but did not pass exactness; decoder CE remains closed.",
        "next_best_step": "Build a targeted mixture-interference residual manifest for route_1 `verified patch operator` and route_file_path `localized edit`, then isolate and rejoin before bounded decoder CE.",
