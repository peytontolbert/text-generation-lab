#!/usr/bin/env python3
"""Roll Stage12023 build-status rows into transition support v19."""
...
ROOT = Path("runs/local/artifacts/stage12024_transition_support_rollup_v19")
BASE = Path("runs/local/artifacts/stage12022_transition_support_rollup_v18/transition_support_rows_v18.jsonl")
ADDED = Path("runs/local/artifacts/stage12023_build_status_expansion_rows/build_status_expansion_rows.jsonl")
ROWS_OUT = ROOT / "transition_support_rows_v19.jsonl"
SUMMARY = ROOT / "transition_support_rollup_v19.json"
SUMMARY_MIRROR = Path("runs/summaries/stage12024_transition_support_rollup_v19.json")
...
def main() -> None:
    ...
    duplicates = 0
    for source, path in [("stage12022", BASE), ("stage12023", ADDED)]:
        for row in read_jsonl(path):
            out = dict(row)
            out["stage12024_source"] = source
            out["split"] = "train"
            out["split_role"] = "stage12024_transition_support_v19_train_support_only"
    ...
    summary = {
        ...
        "decision": "support_inventory_v19_not_train_ready",
        "reason": "Build-status coverage improved, but Transition-Root-250 still lacks enough FAIL_TO_PASS, NOT_EXERCISED, PASS_CURRENT_BUILD_AND_RUN, and overall language roots.",
        "claim_boundary": "Support inventory only. No model promotion or Gemma comparison should be claimed from v19.",
        "next_stage_recommendation": {
            "stage": "stage12025_transition_root_250_fail_not_exercised_expansion",
            "action": "Prioritize real FAIL_TO_PASS mutations and non-synthetic NOT_EXERCISED rows across Python/Rust/C++/Web.",
        },
    }
