#!/usr/bin/env python3
"""Roll Stage12039 build-only rows into support v27."""
...
ROOT = Path("runs/local/artifacts/stage12040_transition_support_rollup_v27")
BASE = Path("runs/local/artifacts/stage12038_transition_support_rollup_v26/transition_support_rows_v26.jsonl")
ADDED = Path("runs/local/artifacts/stage12039_python_build_only_rows/python_build_only_rows.jsonl")
ROWS_OUT = ROOT / "transition_support_rows_v27.jsonl"
SUMMARY = ROOT / "transition_support_rollup_v27.json"
SUMMARY_MIRROR = Path("runs/summaries/stage12040_transition_support_rollup_v27.json")
...
def main() -> None:
    ...
    duplicates = 0
    for source, path in [("stage12038", BASE), ("stage12039", ADDED)]:
        for row in read_jsonl(path):
            out = dict(row)
            out["stage12040_source"] = source
            out["split"] = "train"
            out["split_role"] = "stage12040_transition_support_v27_train_support_only"
    ...
    summary = {
        "stage": "stage12040_transition_support_rollup_v27",
        ...
        "python_build_only_added": len(read_jsonl(ADDED)),
        "decision": "support_inventory_v27_not_train_ready" if not train_ready else "support_inventory_v27_floor_ready_requires_anti_cheat_audit",
        "reason": "PASS_CURRENT_BUILD floor is closed, but PASS_TO_PASS, FAIL_TO_PASS, PASS_CURRENT_BUILD_AND_RUN, INSUFFICIENT_EVIDENCE, and NOT_EXERCISED floors remain unmet." if not train_ready else "Floor counts are met; run anti-cheat and split audits before training.",
        "claim_boundary": "Support inventory only. No model promotion or Gemma comparison should be claimed from v27.",
        "next_stage_recommendation": {"stage": "stage12041_status_floor_closure", "action": "Prioritize PASS_TO_PASS, FAIL_TO_PASS, PASS_CURRENT_BUILD_AND_RUN, INSUFFICIENT_EVIDENCE, and NOT_EXERCISED; avoid overconcentrating benchmark/MCP/tokenizers."},
    }
