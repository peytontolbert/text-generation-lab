#!/usr/bin/env python3
"""Postrun audit for Stage11983 transition support replay diagnostic."""
...
STAGE = 11984
NAME = "stage11984_transition_support_replay_postrun_audit"
OUT = ART / NAME
SUMMARY = OUT / "transition_support_replay_postrun_audit.json"

OLD_ROWS = ART / "stage11897_transition_record_projection_rows/transition_projection_rows.jsonl"
REPLAY_MANIFEST = ART / "stage11982_transition_support_replay_probe_request/transition_support_replay_manifest.jsonl"
EXECUTION_RESULT = ART / "stage11983_transition_support_replay_probe/bounded_decoder_probe/execution_result.json"
...
RUNTIMES = {
    ...
    "stage11983_support_replay": {
        "path": ART / "stage11983_transition_support_replay_probe/runtime_model/runtime_model_bundle.json",
        "transition_scorer": "encoder_option_retrieval_semantic_candidate_head",
    },
}
...
def normalize_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(out.get("target"), dict):
        label = out.get("bounded_choice_target_label") or out.get("target_label") or out.get("target_text") or out.get("decoder_text")
...
def main() -> None:
    ...
    replay_all = [normalize_row(row) for row in read_jsonl(REPLAY_MANIFEST)]
    replay_splits = {
        "train": [row for row in replay_all if str(row.get("split")) == "train"],
        "eval": [row for row in replay_all if str(row.get("split")) == "eval"],
        "strict_eval": [row for row in replay_all if str(row.get("split")) == "strict_eval"],
    }
    protected = {name: [normalize_row(row) for row in read_jsonl(path)] for name, path in PROTECTED_ROWSETS.items()}
    row_meta = {"old_transition_640": by_id(old_rows), **{f"replay_{k}": by_id(v) for k, v in replay_splits.items()}}

    execution_result = read_json(EXECUTION_RESULT) if EXECUTION_RESULT.exists() else {"runtime_executed": False}
    results: dict[str, Any] = {}
    init_cards: dict[str, Any] = {}
    for runtime_name, runtime_spec in RUNTIMES.items():
        model, tokenizer, init_card = load_runtime(Path(runtime_spec["path"]))
        init_cards[runtime_name] = init_card
        transition_scorer = str(runtime_spec["transition_scorer"])
        runtime_out = OUT / runtime_name
        runtime_result: dict[str, Any] = {"transition_scorer": transition_scorer, "runtime_path": rel(Path(runtime_spec["path"]))}
        ...
        for split, rows in replay_splits.items():
            m, c = audit_rows(runtime_out / f"replay_{split}", model, tokenizer, rows, f"replay_{split}", transition_scorer)
            runtime_result[f"replay_{split}"] = {
                "metric": m,
                "by_task_type": group_from_card(c, row_meta[f"replay_{split}"], "task_type"),
                "by_language_family": group_from_card(c, row_meta[f"replay_{split}"], "language_family"),
            }
    ...
    postrun = results["stage11983_support_replay"]
    gates = {
        ...
        "replay_train_improves_stage11924": postrun["replay_train"]["metric"]["correct"] > baseline["replay_train"]["metric"]["correct"],
        "replay_eval_at_least_stage11924": postrun["replay_eval"]["metric"]["correct"] >= baseline["replay_eval"]["metric"]["correct"],
        "replay_strict_at_least_stage11924": postrun["replay_strict_eval"]["metric"]["correct"] >= baseline["replay_strict_eval"]["metric"]["correct"],
    ...
    promote = all(gates.values()) and postrun["old_transition_640"]["metric"]["correct"] > baseline["old_transition_640"]["metric"]["correct"]
    decision = "promote_transition_support_replay_candidate" if promote else "reject_transition_support_replay_candidate"
    scoreboard = {
        runtime: {
            "transition_scorer": data["transition_scorer"],
            "old_transition_640": data["old_transition_640"]["metric"],
            "replay_train": data["replay_train"]["metric"],
            "replay_eval": data["replay_eval"]["metric"],
            "replay_strict_eval": data["replay_strict_eval"]["metric"],
    ...
    summary = {
        ...
        "decision": decision,
        "device": str(DEVICE),
        "claim_boundary": "Stage11983 is diagnostic replay over 19 support rows; not a frontier/source-heldout claim unless gates and old-transition score improve.",
        "execution_result_path": rel(EXECUTION_RESULT),
        "stage11983_runtime_executed": bool(execution_result.get("runtime_executed")),
        "gates": gates,
        "scoreboard": scoreboard,
        "results": results,
        "runtime_initialization": init_cards,
        "next_recommendation": "Keep Stage11924 selected unless this audit promotes Stage11983; regardless, build broader Transition-Root-250/5K data rather than more tiny replay probes.",
    }
