...
STAGE = 9393
NAME = "stage9393_bounded_decoder_short_suffix_target_resolved_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9392_bounded_decoder_short_suffix_target_resolved_preexecution.json"
...
RUN_DIR = ROOT / "runs/local/artifacts/stage9393_bounded_decoder_short_suffix_target_resolved_probe"
AUDIT = RUN_DIR / "stage9393_bounded_decoder_short_suffix_target_resolved_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "BOUNDED_DECODER_SHORT_SUFFIX_TARGET_RESOLVED_PROBE_AUDIT_STAGE9393.md"
...
def audit_run() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9392_not_passed")
    ...
    if execution.get("denoise_ce_rows") != 23 or contract.get("loss_counts", {}).get("denoise_ce") != 23:
    ...
    quality_gate_passed = bool(generated == 23 and exact == 23 and target_prefix == 23 and boundary == 23 and contentful == 23 and repetition_rows == 0 and short_rows == 0 and leak_rows == 0 and unterminated_rows == 0)
    return {
        ...
        "diagnosis": "target_resolved_short_suffix_probe_completed" if quality_gate_passed else "target_resolved_short_suffix_quality_not_passing",
        "next_patch_target": "rejoin_target_resolved_short_suffix_bridge" if quality_gate_passed else "inspect_target_resolved_short_suffix_residuals",
...
def main() -> None:
    ...
    next_step = "Merge target-resolved short-suffix bridge into the bounded decoder repair path." if audit["quality_gate_passed"] else "Inspect target-resolved short-suffix residual buckets before any wider decoder work."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
