...
STAGE = 11139
NAME = "stage11139_evidence_item_conditioned_gated_probe_request"
OUT_DIR = ARTIFACTS / NAME
SUMMARY_JSON = OUT_DIR / "evidence_item_conditioned_gated_probe_request.json"
COMMAND_JSON = OUT_DIR / "evidence_item_conditioned_gated_probe_command.json"
MANIFEST_JSONL = OUT_DIR / "evidence_item_conditioned_gated_probe_manifest.jsonl"

PACKAGE_DIR = ARTIFACTS / "stage11131_evidence_item_support_package"
PACKAGE_JSON = PACKAGE_DIR / "evidence_item_support_package.json"
...
ADDED_ROWS_JSONL = PACKAGE_DIR / "added_evidence_item_option_rows.jsonl"
...
PROBE_OUT_DIR = ARTIFACTS / "stage11140_evidence_item_conditioned_gated_probe" / "bounded_decoder_probe"
RUNTIME_OUT_DIR = ARTIFACTS / "stage11140_evidence_item_conditioned_gated_probe" / "runtime_model"
...
def main() -> None:
    ...
    summary = {
        ...
        "passed": True,
        "decision": "evidence_item_conditioned_gated_probe_requested",
        "claim_scope": [
            "Run one isolated encoder_option_retrieval probe using stage11131 evidence-item option support rows.",
            "Test whether scoring evidence-gated conditioned option retrieval improves residual evidence behavior without applying conditioned scoring to non-evidence tasks.",
        ],
        "source_artifacts": {
        ...
        "metrics": {
            ...
            "evidence_item_rows_added": len(added_rows),
            "evidence_item_repo_count": len({str(row.get("repo_family") or "") for row in added_rows}),
        },
        "training_contract": {
            "bounded_choice_aux_source": "encoder_option_retrieval_evidence_conditioned_gated",
            ...
            "purpose": "test evidence-only conditioned option geometry while preserving raw scorer behavior on non-evidence tasks",
        },
        "promotion_gates": [
            "Strict successor accuracy must stay at or above 22/23.",
            "Validation must stay at or above 20/23 unless reserved residual improves enough to justify a diagnostic tradeoff.",
            "Reserved residual bank must improve above 5/10 and reserved evidence above 4/9 before this is called a frontier gain.",
            "If reserved remains flat, move to fresh non-aliased evidence roots or a true semantic candidate scorer objective.",
        ],
        "next_best_step": "Execute this request, then audit successor validation/strict and the reserved residual bank.",
        "outputs": {
