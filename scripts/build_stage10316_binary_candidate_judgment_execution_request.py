...
STAGE = 10316
NAME = "stage10316_binary_candidate_judgment_execution_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
INVENTORY_JSON = OUT_DIR / "binary_candidate_judgment_inventory.json"
MANIFEST_JSONL = OUT_DIR / "binary_candidate_judgment_manifest.jsonl"
REQUEST_JSON = OUT_DIR / "binary_candidate_judgment_execution_request.json"
COMMAND_JSON = OUT_DIR / "binary_candidate_judgment_command.json"
...
RUN_ID = "stage10317_binary_candidate_judgment_probe"
OUTPUT_DIR = "runs/local/artifacts/stage10317_binary_candidate_judgment_probe/bounded_decoder_probe"
RUNTIME_MODEL_DIR = "runs/local/artifacts/stage10317_binary_candidate_judgment_probe/runtime_model"
MAX_STEPS = 48
LEARNING_RATE = "8e-6"
...
def binary_prompt(base_prompt: str, option_label: str, option_value: str) -> str:
    return (
        base_prompt.rstrip()
        + "\nBinary candidate judgment:\n"
        + f"Candidate under review: {option_label}. {option_value}\n"
        + "Question: Is this the best-supported answer from the visible evidence?\n"
        + "Return YES or NO only.\n"
        + "Answer:\n"
    )


def build_binary_rows(base_rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for row in base_rows:
        ...
        for option_label, option_value in options:
            target = "YES" if option_label == gold else "NO"
            anti_cheat = json.loads(json.dumps(row.get("anti_cheat") or {}))
            anti_cheat["binary_candidate_judgment_train_only"] = True
            anti_cheat["strict_eval_unchanged"] = True
            new_rows.append({
                "row_id": f"{row['row_id']}::binary_candidate_judgment::{option_label}",
                "semantic_key": f"{row.get('semantic_key')}::binary_candidate_judgment::{option_label}",
                "language_family": row.get("language_family"),
                "route": "KEEP_BOUNDED_DECODER",
                "objective_family": "bounded_decoder_ce",
                "surface": "maintainer_bundle_binary_candidate_judgment",
                "task_type": f"{row.get('task_type')}__binary_candidate_judgment",
                "split": "train",
                "prompt_text": binary_prompt(prompt, option_label, option_value),
                "input_text": binary_prompt(prompt, option_label, option_value),
                "query_text": f"{row.get('query_text')}::binary_candidate_judgment::{option_label}",
                "target_text": target,
                "decoder_text": target,
                ...
                "disable_losses": [],
                "source_skill_area": "maintainer_bundle_binary_candidate_judgment",
                "source_stage": STAGE,
                "source_row_id": row.get("row_id"),
                "source_bundle_id": row.get("source_bundle_id"),
                "standalone_projection_source": {
                    "projection_mode": "binary_candidate_judgment_auxiliary",
                    "projection_stage": STAGE,
                    "source_target_label": gold,
                    "candidate_under_review": {"label": option_label, "value": option_value},
...
def build() -> dict[str, Any]:
    ...
    aux_rows, inventory_rows = build_binary_rows(base_rows)
    ...
    request = {
        ...
        "created_at_utc": now_utc(),
        "passed": True,
        "decision": "binary_candidate_judgment_aux_ready",
        ...
        "required_honesty_gates": [
            "strict eval rows remain identical to the current frontier packet",
            "binary candidate-judgment rows are train-only and derived from existing train rows",
            "the reformulation changes supervision structure rather than benchmark content",
            "anti-cheat and admitted-bundle boundaries remain unchanged",
        ],
        "next_best_step": "run one focused warm-start probe and check whether binary candidate-judgment supervision lifts Python/web action-taking without harming the preserved slices",
        "command": cmd,
    }
...
def main() -> None:
    ...
    print(json.dumps({
        ...
        "request": display(REQUEST_JSON),
        "rows": result["rows"],
        "binary_aux_rows": result["binary_aux_rows"],
