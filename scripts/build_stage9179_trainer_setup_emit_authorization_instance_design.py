...
STAGE = 9179
NAME = "stage9179_trainer_setup_emit_authorization_instance_design"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
SOURCE_9178 = ROOT / "runs/summaries/stage9178_trainer_setup_emit_instance_opening_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "TRAINER_SETUP_EMIT_AUTHORIZATION_INSTANCE_DESIGN_STAGE9179.md"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
DESIGN = OUT_DIR / "trainer_setup_emit_authorization_instance_design.json"

REQUIRED_INSTANCE_FIELDS = [
    "source_stage9178_audit_ref",
    "emit_instance_opening_design_ref",
...
REQUIRED_INSTANCE_GUARDS = [
    "emit_authorization_requires_stage9178_passed",
    "emit_authorization_requires_repo_local_output_dir",
    "emit_authorization_requires_expected_output_set_lock",
    "emit_authorization_requires_expected_output_count_lock",
    "emit_authorization_requires_output_hash_lock",
    "emit_authorization_requires_output_provenance_join_lock",
    "emit_authorization_requires_post_emit_cardinality_audit",
    "emit_authorization_requires_post_emit_provenance_join_audit",
    "emit_authorization_forbids_trainer_invocation",
    "emit_authorization_forbids_model_forward",
    "emit_authorization_forbids_optimizer_or_backward",
    "emit_authorization_forbids_arxiv_reads",
    "emit_authorization_forbids_source_body_reads",
]
...
def registry(latest: int = 9178) -> dict[str, Any]:
...
def build_design(registry_card: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    source = load_json(SOURCE_9178)
    return {
        "stage": STAGE,
        "stage_name": NAME,
        "design_type": "trainer_setup_emit_authorization_instance_v1",
        "source_stage9178_passed": source.get("passed") is True,
        "registry_frontier_stage9178": int((registry_card.get("metrics") or {}).get("latest_stage", -1)) == 9178,
        ...
        "decision": (
            "Designed the deliberate emit-authorization instance. The instance may define the exact "
            "repo-local output path and exact trainer-setup artifact set under hash/provenance locks, "
            "but it may not emit outputs or invoke trainer execution in this stage."
        ),
        "next_best_step": "Audit the emit-authorization instance design; still do not emit outputs.",
    }
...
def validate_design(design: dict[str, Any]) -> list[str]:
    ...
    if design.get("source_stage9178_passed") is not True or metrics.get("source_stage9178_passed") is not True:
        failures.append("source_stage9178_not_passed")
    if design.get("registry_frontier_stage9178") is not True or metrics.get("registry_frontier_stage9178") is not True:
        failures.append("registry_frontier_stage9178")
...
def run_negative_cases() -> dict[str, Any]:
    ...
    for case in NEGATIVE_CASES:
        ...
        if case == "source_stage_missing":
            design["source_stage9178_passed"] = False
            design["metrics"]["source_stage9178_passed"] = False
        elif case == "registry_frontier_bad":
            design["registry_frontier_stage9178"] = False
            design["metrics"]["registry_frontier_stage9178"] = False
        elif case == "missing_instance_field":
            design["required_instance_fields"].remove("repo_local_output_dir_ref")
        elif case == "missing_instance_guard":
            design["required_instance_guards"].remove("emit_authorization_forbids_trainer_invocation")
...
def main() -> None:
    ...
    artifact = {
        ...
        "decision": design["decision"] if passed else "Emit-authorization instance design failed validation.",
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage9179 Trainer Setup Emit-Authorization Instance Design",
                "",
                f"Passed: `{summary['passed']}`",
                "",
                "Designs the exact emit-authorization instance without emitting outputs.",
                "",
