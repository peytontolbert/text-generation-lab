#!/usr/bin/env python3
"""Emit a diagnostic transition-projection + protected-replay probe request.

This stage intentionally includes protected canary/residual rows as train replay
copies. It is not promotable. It tests whether the transition projection
interface can be fit while retaining selected frontier geometry.
"""
...
STAGE = 11902
NAME = "stage11902_transition_projection_protected_replay_probe_request"
OUT = ART / NAME
SUMMARY = OUT / "transition_projection_protected_replay_probe_request.json"
COMMAND_JSON = OUT / "transition_projection_protected_replay_command.json"
MANIFEST = OUT / "transition_projection_protected_replay_manifest.jsonl"
...
RUN_DIR = ART / "stage11903_transition_projection_protected_replay_probe"
...
def train_copy(row: dict[str, Any], *, suffix: str, replay_kind: str, preservation_exempt: bool) -> dict[str, Any]:
    ...
    out["stage11902_replay_kind"] = replay_kind
...
def main() -> None:
    ...
    for row in support:
        train_rows.append(train_copy(row, suffix="stage11902_transition_projection", replay_kind="transition_projection", preservation_exempt=True))
        ...
        for name, rows in protected_sources.items():
            for row in rows:
                train_rows.append(
                    train_copy(
                        row,
                        suffix=f"stage11902_{name}_replay_r{repeat}",
                        replay_kind=name,
    ...
    gates = {
        ...
        "train_rows_expected": len(train_rows) == 640 + (5 * sum(len(rows) for rows in protected_sources.values())),
        ...
        "support_preservation_exempt": all(row.get("preservation_exempt") for row in train_rows if row.get("stage11902_replay_kind") == "transition_projection"),
    }
    artifact = {
        "stage": STAGE,
        "stage_name": NAME,
        ...
        "decision": "transition_projection_protected_replay_probe_request_ready" if all(gates.values()) else "transition_projection_protected_replay_probe_request_blocked",
        "command": command,
        ...
        "claim_boundary": [
            ...
            "This tests transition-projection mechanism, not benchmark standing.",
        ],
    }
