...
try:
    from scripts.build_stage9179_trainer_setup_emit_authorization_instance_design import (
        EXPECTED_OUTPUTS,
    ...
    from build_stage9179_trainer_setup_emit_authorization_instance_design import (  # type: ignore
        EXPECTED_OUTPUTS,
        REQUIRED_INSTANCE_FIELDS,
        REQUIRED_INSTANCE_GUARDS,
        STILL_BLOCKED_OUTPUTS,
...
STAGE = 9180
NAME = "stage9180_trainer_setup_emit_authorization_instance_audit"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
SOURCE_9179 = ROOT / "runs/summaries/stage9179_trainer_setup_emit_authorization_instance_design.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "TRAINER_SETUP_EMIT_AUTHORIZATION_INSTANCE_AUDIT_STAGE9180.md"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "trainer_setup_emit_authorization_instance_audit.json"
...
def registry(latest: int = 9179) -> dict[str, Any]:
...
def build_audit(registry_card: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    source = load_json(SOURCE_9179)
    design = build_design(registry(9178))
    design_failures = validate_design(design)
    negatives = run_negative_cases()
    checks = {
        "source_stage9179_passed": source.get("passed") is True,
        ...
        "registry_frontier_stage9179": int((registry_card.get("metrics") or {}).get("latest_stage", -1)) == 9179,
    ...
    return {
        ...
        "decision": "Audited the emit-authorization instance design and kept output emission and all execution paths closed.",
        "next_best_step": "Stop at the emit-authorization instance boundary until a deliberate emission action is intentionally opened.",
    }
...
def main() -> None:
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage9180 Trainer Setup Emit-Authorization Instance Audit",
                "",
                f"Passed: `{summary['passed']}`",
                "",
                "Audits the emit-authorization instance design without emitting outputs.",
                "",
