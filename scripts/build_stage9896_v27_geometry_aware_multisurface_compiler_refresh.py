...
STAGE = 9896
NAME = "stage9896_v27_geometry_aware_multisurface_compiler_refresh"
...
PREPARED = OUT_DIR / "geometry_aware_multisurface_prepared_rows.jsonl"
COMPILED_DIR = OUT_DIR / "compiled"
AUDIT = OUT_DIR / "geometry_aware_multisurface_compiler_refresh_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "V27_GEOMETRY_AWARE_MULTISURFACE_COMPILER_REFRESH_STAGE9896.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"

_stage9693_path = ROOT / "scripts/build_stage9693_locked_guarded_source_backed_multisurface_compiler_refresh.py"
_stage9693_spec = importlib.util.spec_from_file_location("stage9693_for_9896", _stage9693_path)
_stage9693 = importlib.util.module_from_spec(_stage9693_spec)
assert _stage9693_spec and _stage9693_spec.loader
sys.modules["stage9693_for_9896"] = _stage9693
...
SOURCES = {
    ...
    "edit_localization": {
        ...
        "path": ROOT / "runs/local/artifacts/stage9893_current_margin_locality_signal_label_remap_manifest/current_margin_locality_signal_label_remap_manifest.jsonl",
        "target_loss": "edit_localization_ce",
        "source_audit": ROOT / "runs/summaries/stage9895_current_margin_locality_signal_label_remap_probe_audit.json",
    },
...
def _normalize_ready_row(row: dict[str, Any], skill: str, target_loss: str, source_audit: Path) -> dict[str, Any]:
    ...
    out["row_id"] = f"stage9896_{skill}_{row.get('row_id')}"
    ...
    anti["stage9896_geometry_aware_refresh"] = True
    out["anti_cheat"] = anti
    return out
...
def main() -> None:
    ...
    next_step = "Use this geometry-aware compiled package as the next v2.7 training mix: locality-signal evidence is in, K-specific label collapse is neutralized in the edit-localization source packet, and the remaining work moves to head/objective redesign rather than packet hygiene."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Built a geometry-aware successor to the v2.7 multisurface compiler refresh: edit localization now sources from the locality-lifted, K-free current-frontier packet that survived the Stage9895 control audit, while the other multisurface validity-weighted sources remain unchanged.",
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage9896 V2.7 Geometry-Aware Multisurface Compiler Refresh",
                "",
                ...
                "",
                "This stage refreshes the v2.7 multisurface mix with the strongest current edit-localization packet evidence: locality resolution is visible, the old K-specific collapse is neutralized in the source packet, and patch/verifier remain abstention-only guardrails.",
                "",
                f"Next: {next_step}",
                "",
            ]
