...
STAGE = 10072
NAME = "stage10072_canonical_label_aligned_same_manifest_comparison_audit"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "canonical_label_aligned_same_manifest_comparison_audit.json"
ROWS = OUT_DIR / "canonical_label_aligned_same_manifest_comparison_rows.jsonl"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "CANONICAL_LABEL_ALIGNED_SAME_MANIFEST_COMPARISON_AUDIT_STAGE10072.md"
...
def _language_metrics(rows: list[dict[str, Any]], key: str) -> dict[str, dict[str, Any]]:
    ...
    for row in rows:
        ...
        bucket[1] += 1
    return {
        language: {"exact": correct / total if total else 0.0, "rows": total}
...
def build_audit() -> dict[str, Any]:
    ...
    failures: list[str] = []
    shared_ids = sorted(set(manifest) & set(hundred_m) & set(gemma))
    ...
    for row_id in shared_ids:
        ...
        comparisons.append({
            ...
            "hundred_m_correct": bool(hm.get("correct")),
            "gemma_pred": gm.get("predicted_label"),
            "gemma_correct": bool(gm.get("correct")),
        })
    hundred_m_by_language = _language_metrics(comparisons, "hundred_m_correct")
    gemma_by_language = _language_metrics(comparisons, "gemma_correct")
    per_language: dict[str, dict[str, Any]] = {}
    wins_100m = wins_gemma = ties = 0
    for language in sorted(hundred_m_by_language):
        hm_exact = hundred_m_by_language[language]["exact"]
        gemma_exact = gemma_by_language.get(language, {}).get("exact", 0.0)
        if hm_exact > gemma_exact:
            verdict = "100m_better"
            wins_100m += 1
        elif gemma_exact > hm_exact:
        ...
        per_language[language] = {
            ...
            "delta_hundred_m_minus_gemma": hm_exact - gemma_exact,
            "rows": hundred_m_by_language[language]["rows"],
            "verdict": verdict,
        }
    macro_hundred_m = sum(v["hundred_m_exact"] for v in per_language.values()) / len(per_language) if per_language else 0.0
    ...
    return {
        ...
        "failures": failures,
        "metrics": {
            "rows_hundred_m_present": len(hundred_m),
            "rows_gemma_present": len(gemma),
            "shared_rows": len(shared_ids),
            "comparison_rows": len(comparisons),
            "macro_exact_hundred_m": macro_hundred_m,
            "macro_exact_gemma": macro_gemma,
            "macro_delta_hundred_m_minus_gemma": macro_hundred_m - macro_gemma,
            "wins_100m": wins_100m,
...
def main() -> None:
    ...
    next_step = "Use this canonical-label same-manifest audit to decide whether label-semantic alignment repairs the multilingual gap against Gemma before attempting any broader v2.7 training or harness work."
    summary = {"stage": STAGE, "stage_name": NAME, "name": NAME, "passed": built["passed"], "metrics": {**built["metrics"], "failures": built["failures"]}, "artifacts": {"audit": display(AUDIT), "rows": display(ROWS), "doc": display(DOC)}, "decision": "Materialized the same-manifest audit for the canonical-label multilingual 100M and Gemma runs so the effect of label-semantic alignment can be measured directly on the heldout rows.", "next_best_step": next_step, "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage10072 Canonical Label Aligned Same Manifest Comparison Audit", "", f"Passed: `{summary['passed']}`", "", summary["decision"], "", f"Next: {next_step}", ""]), encoding="utf-8")
    if summary["passed"]:
