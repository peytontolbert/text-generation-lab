...
STAGE = 9656
NAME = "stage9656_model_input_comparator_tiny_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9655_model_input_comparator_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9655_model_input_comparator_manifest/model_input_comparator_manifest.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
RUN_DIR = OUT_DIR / "episode_step_probe"
AUDIT = OUT_DIR / "model_input_comparator_tiny_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "MODEL_INPUT_COMPARATOR_TINY_PROBE_STAGE9656.md"
...
def command() -> list[str]:
    return ["env", f"TMPDIR={TMPDIR}", f"TEMP={TMPDIR}", f"TMP={TMPDIR}", "conda", "run", "-n", "trellis", "python", str(TRAINER),
        ...
        "--output-dir", str(RUN_DIR), "--run-id", "stage9656_model_input_comparator_probe", "--execution-authorized-for-recovery-probe"]
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        safety_failures.append("stage9655_not_passed")
    comparator = source_metrics.get("model_input_comparator_baselines") if isinstance(source_metrics.get("model_input_comparator_baselines"), dict) else {}
    if comparator and min(float(value) for value in comparator.values()) != 1.0:
        safety_failures.append("stage9655_model_input_comparator_not_predictive")
