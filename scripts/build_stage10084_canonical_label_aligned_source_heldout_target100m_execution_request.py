...
STAGE = 10084
NAME = "stage10084_canonical_label_aligned_source_heldout_target100m_execution_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
REQUEST = OUT_DIR / "canonical_label_aligned_source_heldout_target100m_execution_request.json"
SURFACE_REQUEST = OUT_DIR / "surface_requests" / "edit_localization.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "CANONICAL_LABEL_ALIGNED_SOURCE_HELDOUT_TARGET100M_EXECUTION_REQUEST_STAGE10084.md"
...
SOURCE_SUMMARY = ROOT / "runs/summaries/stage10083_canonical_label_aligned_source_heldout_successor_packet.json"

FUTURE_RUN_ID = "stage10084_canonical_label_aligned_source_heldout_target100m_probe"
FUTURE_OUTPUT_DIR = "runs/local/artifacts/stage10084_canonical_label_aligned_source_heldout_target100m_probe/edit_localization_probe"
...
def build_request() -> dict[str, Any]:
    ...
    if source_summary.get("passed") is not True:
        failures.append("stage10083_not_passed")
    ...
    req = {
        ...
        "required_contract_invariants": {
            "source_stage10083_passed": bool(source_summary.get("passed") is True),
    ...
    return {
        ...
        "failures": failures,
        "metrics": metrics,
        "source_artifacts": {"stage10083_packet_summary": display(SOURCE_SUMMARY)},
...
def main() -> None:
    ...
    next_step = "Run one target-100m edit-localization probe on the canonical source-heldout manifest, then execute the matching same-manifest Gemma queue."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Prepared the execution request for the canonical source-heldout manifest so the next probe can test whether label-semantic alignment repairs the honest multilingual heldout frontier without changing row composition.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage10084 Canonical Label Aligned Source-Heldout Target100M Execution Request",
        "",
