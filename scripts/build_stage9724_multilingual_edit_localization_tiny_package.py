...
STAGE = 9724
NAME = "stage9724_multilingual_edit_localization_tiny_package"
...
MANIFEST = OUT_DIR / "multilingual_edit_localization_tiny.jsonl"
AUDIT = OUT_DIR / "multilingual_edit_localization_tiny_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "MULTILINGUAL_EDIT_LOCALIZATION_TINY_PACKAGE_STAGE9724.md"
...
def main() -> None:
    ...
    next_step = "Run Stage9725 target-100M contract-only preflight on the Stage9724 multilingual edit-localization package, then decide whether to execute that multilingual structured probe."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Materialized a balanced multilingual edit-localization tiny package with current rows from python, rust, c_cpp, and web_js_ts_html.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9724 Multilingual Edit Localization Tiny Package",
        "",
        f"Passed: `{summary['passed']}`",
        f"Rows: `{audit['rows']}`",
        f"Language counts: `{audit['language_counts']}`",
        "",
        "This stage packages a balanced multilingual edit-localization structured surface from current compiled rows: python, rust, c_cpp, and web_js_ts_html.",
        "",
        f"Next: {next_step}",
        "",
    ]), encoding="utf-8")
