...
STAGE = 9660
NAME = "stage9660_visible_observe_repair_boundary_prefix_tiny_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9659_visible_observe_repair_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9659_visible_observe_repair_manifest/visible_observe_repair_manifest.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
RUN_DIR = OUT_DIR / "episode_step_probe"
AUDIT = OUT_DIR / "visible_observe_repair_boundary_prefix_tiny_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "VISIBLE_OBSERVE_REPAIR_BOUNDARY_PREFIX_TINY_PROBE_STAGE9660.md"
...
def command() -> list[str]:
    return ["env", f"TMPDIR={TMPDIR}", f"TEMP={TMPDIR}", f"TMP={TMPDIR}", "conda", "run", "-n", "trellis", "python", str(TRAINER),
        ...
        "--max-train-rows", "54", "--max-eval-rows", "6", "--max-strict-rows", "6", "--max-steps", "384", "--batch-size", "4", "--learning-rate", "3e-4",
        ...
        "--output-dir", str(RUN_DIR), "--run-id", "stage9660_visible_observe_repair_boundary_prefix_probe", "--execution-authorized-for-recovery-probe"]
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        safety_failures.append("stage9659_not_passed")
    if int(source_metrics.get("rows") or 0) != 66:
        safety_failures.append("stage9659_unexpected_row_count")
