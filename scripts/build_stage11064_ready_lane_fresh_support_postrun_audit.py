...
STAGE = 11064
NAME = "stage11064_ready_lane_fresh_support_postrun_audit"
OUT_DIR = ARTIFACTS / NAME
SUMMARY_JSON = OUT_DIR / "ready_lane_fresh_support_postrun_audit.json"
...
VALIDATION_JSON = ARTIFACTS / "stage11063_ready_lane_fresh_support_probe" / "bounded_decoder_probe" / "bounded_choice_eval_audit_eval.json"
STRICT_JSON = ARTIFACTS / "stage11063_ready_lane_fresh_support_probe" / "bounded_decoder_probe" / "bounded_choice_eval_audit_strict_eval.json"
EXECUTION_JSON = ARTIFACTS / "stage11063_ready_lane_fresh_support_probe" / "bounded_decoder_probe" / "execution_result.json"
RESERVED_ROWS = ARTIFACTS / "stage11051_successor_residual_support_plus_priority_evidence" / "reserved_residual_candidates.jsonl"
HEURISTIC_ROWS = ARTIFACTS / "stage11061_ready_lane_fresh_support_package" / "heuristic_support_rows.jsonl"
BASELINE_POSTRUN = ARTIFACTS / "stage11054_successor_residual_support_plus_priority_postrun_audit" / "successor_residual_support_plus_priority_postrun_audit.json"
PACKAGE_SUMMARY = ARTIFACTS / "stage11061_ready_lane_fresh_support_package" / "ready_lane_fresh_support_package.json"
...
def overlap_report(strict_rows: list[dict[str, Any]], reserved_rows: list[dict[str, Any]], heuristic_rows: list[dict[str, Any]]) -> dict[str, Any]:
    strict_ids = {str(row.get("row_id") or "") for row in strict_rows}
    reserved_ids = {str(row.get("row_id") or "") for row in reserved_rows}
    heuristic_ids = {str(row.get("row_id") or "") for row in heuristic_rows}
    ...
    heuristic_source_roots = {str(row.get("source_root_id") or "") for row in heuristic_rows if row.get("source_root_id")}
    return {
        "heuristic_row_ids_overlap_strict": sorted(heuristic_ids & strict_ids),
        "heuristic_row_ids_overlap_reserved": sorted(heuristic_ids & reserved_ids),
        "heuristic_source_roots_overlap_strict": sorted(heuristic_source_roots & strict_source_roots),
        "heuristic_source_roots_overlap_reserved": sorted(heuristic_source_roots & reserved_source_roots),
...
def main() -> None:
    ...
    heuristic_rows = load_jsonl(HEURISTIC_ROWS)
    ...
    anti_cheat = overlap_report(
        list((strict.get("row_cards") or [])),
        reserved_rows,
        heuristic_rows,
    )
    ...
    summary = {
        ...
        "passed": True,
        "claim_scope": [
            "Audit the stage11063 runtime against the unchanged successor validation/strict overlay and the untouched reserved residual candidate bank.",
            "Check anti-cheat overlap between the stage11061 heuristic support rows and the scored strict/reserved surfaces before making any claim.",
        ],
        "source_artifacts": {
            ...
            "heuristic_rows": rel(HEURISTIC_ROWS),
        ...
        "headline_findings": [
            "This audit measures whether the stage11061 ready-lane support moved the untouched successor overlay or only changed the newly trained support geometry.",
            "Reserved candidate rows remain the cleanest off-overlay signal because they are still not part of the trained strict surface.",
            "Heuristic Rust evidence rows are explicitly checked for overlap against strict and reserved surfaces before interpreting any gain.",
        ],
        "runtime_bundle": {
