...
OUT = ROOT / "runs/local/tmp/pocketpal_stage613_short_selector_pair_seed461"
ARTIFACT = ROOT / "runs/local/artifacts/stage613_short_selector_pair_dataset.json"
DOC = ROOT / "docs/stage613_short_selector_pair_dataset.md"
...
def rewrite_row(row: dict[str, Any]) -> tuple[dict[str, Any], bool]:
    ...
    for key, value in row.items():
        if isinstance(value, str) and key in TEXT_FIELDS:
            ...
            rewritten[key] = value
    rewritten["entity_context_short_selector_pair"] = True
    rewritten["entity_context_selector_entity"] = entity
    rewritten["entity_context_selector_field"] = field
    rewritten["source_type"] = f"{row.get('source_type', 'retrieval')}_short_selector_pair"
...
def selector_counts(rows: list[dict[str, Any]]) -> dict[str, int]:
    return dict(Counter(str(row.get("entity_context_selector_field", "") or "") for row in rows if row.get("entity_context_short_selector_pair")))
...
def main() -> None:
    ...
    manifest.update(
        {
            "artifact_kind": "agentkernel_lite_encdec_stage613_short_selector_pair",
            ...
            "entity_context_short_selector_pair": True,
            "stage613_selector_changed_train_examples": changed_counts["train"],
            "stage613_selector_changed_eval_examples": changed_counts["eval"],
            "collision_goal": "Test whether selector_pair can be shortened to sp while preserving selector gain.",
        }
    )
    ...
    summary = {
        "artifact_kind": "stage613_short_selector_pair_dataset",
        ...
        "decision": "short_selector_pair_dataset_ready",
        "finding": "Stage613 tests whether the bare selector key can be shortened from selector_pair to sp without losing the Stage612 gain.",
    }
    ARTIFACT.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text(
        f"""# Stage613 Short Selector Pair Dataset
