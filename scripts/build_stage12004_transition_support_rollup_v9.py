#!/usr/bin/env python3
"""Roll Stage12003 C++ real-source FAIL_TO_PASS rows into support v9."""
...
ROOT = Path("runs/local/artifacts/stage12004_transition_support_rollup_v9")
BASE = Path("runs/local/artifacts/stage12002_transition_support_rollup_v8/transition_support_rows_v8.jsonl")
ADDED = Path("runs/local/artifacts/stage12003_cpp_real_source_fail_to_pass_mutations/cpp_real_source_fail_to_pass_rows.jsonl")
ROWS_OUT = ROOT / "transition_support_rows_v9.jsonl"
SUMMARY = ROOT / "transition_support_rollup_v9.json"
SUMMARY_MIRROR = Path("runs/summaries/stage12004_transition_support_rollup_v9.json")
...
def main() -> None:
    ...
    for src, path in [("stage12002", BASE), ("stage12003", ADDED)]:
        for row in read_jsonl(path):
            out=dict(row)
            out["stage12004_source"] = src
            out["split"] = "train"
            out["split_role"] = "stage12004_transition_support_v9_train_support_only"
    ...
    summary={
        "stage":"stage12004_transition_support_rollup_v9",
        ...
        "decision":"support_inventory_v9_not_train_ready",
        "reason":"C++ FAIL_TO_PASS improved, but inventory is still too root-thin and label/status-imbalanced for a serious transition training run.",
        "claim_boundary":"Support inventory only. No model promotion or Gemma comparison should be claimed from v9.",
        "next_stage_recommendation":{"stage":"stage12005_independent_cpp_web_root_mining","action":"Mine more independent C++/Web executable roots; avoid over-concentrating benchmark and MCP repo families."},
    }
