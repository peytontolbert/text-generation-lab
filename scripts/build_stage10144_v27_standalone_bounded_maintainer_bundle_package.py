...
STAGE = 10144
NAME = 'stage10144_v27_standalone_bounded_maintainer_bundle_package'
OUT_DIR = ROOT / 'runs/local/artifacts' / NAME
TRAIN_JSONL = OUT_DIR / 'agentkernel_lite_encdec_train.jsonl'
EVAL_JSONL = OUT_DIR / 'agentkernel_lite_encdec_eval.jsonl'
PACKAGE_JSON = OUT_DIR / 'standalone_bounded_maintainer_bundle_package.json'
SUMMARY = ROOT / 'runs/summaries' / f'{NAME}.json'
SOURCE = ROOT / 'runs/local/artifacts/stage10141_standalone_bounded_bundle_projection/standalone_bounded_bundle_projection.json'
...
def grouped_runs(payload: dict[str, Any]) -> dict[str, list[dict[str, Any]]]:
    ...
    for run in payload.get('runs') or []:
        ...
        out.setdefault(language, []).append(run)
    for language in out:
        out[language] = sorted(out[language], key=lambda row: str((row.get('task_pack') or {}).get('bundle_id') or ''))
    return out
...
def trainer_row(source_run: dict[str, Any], row: dict[str, Any], *, split: str) -> dict[str, Any]:
    ...
    return {
        ...
        'surface': 'maintainer_bundle_bounded_choice',
        ...
        'target_text': str(row.get('target_text') or row.get('expected_label') or ''),
        'decoder_text': str(row.get('target_text') or row.get('expected_label') or ''),
        'loss_mask': {'decoder_ce': True},
        'expected_enabled_loss': 'decoder_ce',
        'disable_losses': ['denoise_ce', 'runtime_reward'],
        'source_skill_area': 'maintainer_bundle_bounded_choice',
        'source_stage': STAGE,
        'source_row_id': str(row.get('row_id') or ''),
        'source_bundle_id': str(task_pack.get('bundle_id') or ''),
        'semantic_key': f"maintainer_bundle_bounded::{task_pack.get('bundle_id')}::{row.get('perspective')}",
        'anti_cheat': {
            'opaque_labels': True,
        ...
        'standalone_projection_source': {
            'projection_stage': 10141,
            'projection_mode': str((task_pack.get('projection_mode') or 'bounded_choice_auxiliary')),
...
def build_package() -> dict[str, Any]:
    ...
    for language, runs in sorted(by_language.items()):
        ...
        for row in ((train_run.get('task_pack') or {}).get('rows') or []):
            if isinstance(row, dict):
                train_rows.append(trainer_row(train_run, row, split='train'))
        for row in ((eval_run.get('task_pack') or {}).get('rows') or []):
            if isinstance(row, dict):
                eval_rows.append(trainer_row(eval_run, row, split='strict_eval'))
        split_manifest.append({
            ...
            'train_rows': len([r for r in ((train_run.get('task_pack') or {}).get('rows') or []) if isinstance(r, dict)]),
            'strict_eval_rows': len([r for r in ((eval_run.get('task_pack') or {}).get('rows') or []) if isinstance(r, dict)]),
        })
    ...
    package = {
        ...
        'fit_for': {
            ...
            'bounded_auxiliary_projection_only': True,
        },
        'required_honesty_gates': [
            'stage10142_standalone_decoder_contract_audit must pass before score claims',
            'bundle-level heldout split must remain intact',
            'freeform maintainer rows remain excluded from standalone package',
        ],
    }
