...
OUT = ROOT / "runs/local/tmp/pocketpal_stage615_raw_selector_no_role_tokens_seed461"
ARTIFACT = ROOT / "runs/local/artifacts/stage615_raw_selector_no_role_tokens_dataset.json"
DOC = ROOT / "docs/stage615_raw_selector_no_role_tokens_dataset.md"
...
def rewrite_row(row: dict[str, Any]) -> tuple[dict[str, Any], bool]:
    ...
    for key, value in row.items():
        if isinstance(value, str) and key in TEXT_FIELDS:
            ...
            rewritten[key] = value
    rewritten["entity_context_raw_selector_no_role_tokens"] = True
    rewritten["entity_context_selector_entity"] = entity
    rewritten["entity_context_selector_field"] = field
    rewritten["source_type"] = f"{row.get('source_type', 'retrieval')}_raw_selector_no_role_tokens"
...
def selector_counts(rows: list[dict[str, Any]]) -> dict[str, int]:
    return dict(Counter(str(row.get("entity_context_selector_field", "") or "") for row in rows if row.get("entity_context_raw_selector_no_role_tokens")))
...
def main() -> None:
    ...
    manifest.update(
        {
            "artifact_kind": "agentkernel_lite_encdec_stage615_raw_selector_no_role_tokens",
            ...
            "entity_context_raw_selector_no_role_tokens": True,
            "stage615_selector_changed_train_examples": changed_counts["train"],
            "stage615_selector_changed_eval_examples": changed_counts["eval"],
            "collision_goal": "Test whether raw entity|field selector works without Stage607 FIELD_ROLE tokens.",
        }
    )
    ...
    summary = {
        "artifact_kind": "stage615_raw_selector_no_role_tokens_dataset",
        ...
        "decision": "raw_selector_no_role_tokens_dataset_ready",
        "finding": "Stage615 tests the raw entity|field selector without FIELD_ROLE tokens to isolate the minimal selector signal.",
    }
    ARTIFACT.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text(
        f"""# Stage615 Raw Selector No Role Tokens Dataset
