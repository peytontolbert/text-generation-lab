...
STAGE = 9593
NAME = "stage9593_residual_denoise_minimal_sidecar_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9592_residual_denoise_minimal_sidecar_shortcut_audit.json"
...
DOC = ROOT / "docs" / "RESIDUAL_DENOISE_MINIMAL_SIDECAR_PROBE_STAGE9593.md"
...
def main() -> None:
    ...
    if not failures:
        contract = subprocess.run(command(output_dir=CONTRACT_DIR, run_id="stage9593_contract_preflight", contract_only=True), cwd=ROOT, text=True, capture_output=True, check=False)
    ...
    if not failures:
        run = subprocess.run(command(output_dir=RUN_DIR, run_id="stage9593_minimal_sidecar_probe", contract_only=False), cwd=ROOT, text=True, capture_output=True, check=False)
        if run.returncode != 0:
    ...
    summary = {
        ...
        "next_best_step": "If the minimal sidecar passes, compile sidecar-gated denoise rendering rows; otherwise inspect row-level logits and add harder verifier-evidence rows.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9593 Residual Denoise Minimal Sidecar Probe", "", f"Passed: `{audit['passed']}`", f"Eval suffix choice exact: `{eval_exact}`", f"Strict suffix choice exact: `{strict_exact}`", f"Pass gate: `{pass_gate}`", "", "Decoder, denoise, runtime, export, harness, and promotion remain closed.", ""]), encoding="utf-8")
