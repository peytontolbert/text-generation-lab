...
STAGE = 9244
NAME = "stage9244_source_backed_arg_context_balanced_bounded_decoder_tiny_package"
...
MANIFEST = OUT_DIR / "source_backed_arg_context_balanced_bounded_decoder_tiny_manifest.jsonl"
AUDIT = OUT_DIR / "source_backed_arg_context_balanced_bounded_decoder_tiny_package_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "SOURCE_BACKED_ARG_CONTEXT_BALANCED_BOUNDED_DECODER_TINY_PACKAGE_STAGE9244.md"
...
ARG_TYPES = ("ARG_CALL", "ARG_IMPORT", "ARG_LITERAL", "ARG_NAME", "ARG_PATH")
CONTEXT_GROUPS = ("add_import_plan", "callsite_plan", "hold_plan", "literal_plan", "name_plan", "path_plan")
...
def valid_source(row: dict[str, Any]) -> bool:
    ...
    return (
        split_name(row.get("split")) in CAPS
        and language_family(row.get("language")) in {"python", "rust", "cpp", "web_js_ts_html"}
        and row.get("quality", {}).get("decoder_budget_ok") is True
        and str(row.get("bounded_argument_type") or "") in ARG_TYPES
        and str(row.get("context_group") or "") in CONTEXT_GROUPS
        and bool(text)
        and token_len <= MAX_DECODER_TOKENS
    )


def source_sort_key(row: dict[str, Any]) -> tuple[str, str, str]:
    return (str(row.get("bounded_argument_type")), str(row.get("context_group")), str(row.get("target_ref")))
...
def choose_source(
    ...
    family: str,
    used_hashes: set[str],
    split_arg_counts: Counter[tuple[str, str]],
    split_context_counts: Counter[tuple[str, str]],
    split_language_arg_counts: Counter[tuple[str, str, str]],
    split_language_context_counts: Counter[tuple[str, str, str]],
) -> dict[str, Any] | None:
    available = []
    for source in candidates:
        text = str(source.get("decoder_text") or "").strip()
        target_hash = str(source.get("decoder_text_sha256") or stable_hash(text))
        if target_hash not in used_hashes:
            available.append(source)
    if not available:
        return None

    def score(row: dict[str, Any]) -> tuple[int, int, int, int, str]:
        arg = str(row.get("bounded_argument_type"))
        context = str(row.get("context_group"))
        return (
            split_arg_counts[(split, arg)],
            split_context_counts[(split, context)],
            split_language_arg_counts[(split, family, arg)],
            split_language_context_counts[(split, family, context)],
            "|".join(map(str, source_sort_key(row))),
        )
...
def build_rows(target_store: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for row in target_store:
        if valid_source(row):
            by_cell[(split_name(row.get("split")), language_family(row.get("language")))].append(row)
    by_cell = {cell: sorted(rows, key=source_sort_key) for cell, rows in by_cell.items()}
    ...
    split_arg_counts: Counter[tuple[str, str]] = Counter()
    split_context_counts: Counter[tuple[str, str]] = Counter()
    split_language_arg_counts: Counter[tuple[str, str, str]] = Counter()
    split_language_context_counts: Counter[tuple[str, str, str]] = Counter()

    for split, language_caps in LANGUAGE_TARGETS.items():
        for round_idx in range(max(language_caps.values())):
            for family, cap in language_caps.items():
                if round_idx >= cap:
                    continue
                source = choose_source(
                    by_cell.get((split, family), []),
                    split,
                    family,
                    used_hashes,
                    split_arg_counts,
                    split_context_counts,
                    split_language_arg_counts,
                    split_language_context_counts,
                )
                if source is None:
                    continue
                text = str(source.get("decoder_text") or "").strip()
                ...
                arg = str(source.get("bounded_argument_type"))
                context = str(source.get("context_group"))
                selected.append(
                    {
                        "row_id": f"stage9244_source_backed_{split}_{family}_{idx:03d}_{stable_hash(source_ref)[:12]}",
                        "split": split,
                        ...
                        "anti_cheat": {
                            ...
                            "argument_type_balance_required": True,
                            "context_group_balance_required": True,
                            "no_execution_in_this_stage": True,
                        },
                    }
                )
                selected_counts[split][family] += 1
                split_arg_counts[(split, arg)] += 1
                split_context_counts[(split, context)] += 1
                split_language_arg_counts[(split, family, arg)] += 1
                split_language_context_counts[(split, family, context)] += 1
...
def _counter_by(rows: list[dict[str, Any]], *keys: str) -> Counter[tuple[str, ...]]:
    values: Counter[tuple[str, ...]] = Counter()
    for row in rows:
        state = row.get("input_state") if isinstance(row.get("input_state"), dict) else {}
        item = []
        for key in keys:
            if key in row:
                item.append(str(row.get(key)))
            else:
                item.append(str(state.get(key)))
        values[tuple(item)] += 1
    return values


def _max_min_delta(counts: Counter[tuple[str, ...]], split: str, expected_values: tuple[str, ...], index: int = 1) -> int:
    values = [counts.get((split, value), 0) for value in expected_values]
...
def audit_rows(rows: list[dict[str, Any]], selection_card: dict[str, Any]) -> dict[str, Any]:
    ...
    split_arg_counts = _counter_by(rows, "split", "bounded_argument_type")
    split_context_counts = _counter_by(rows, "split", "context_group")
    split_language_arg_counts = _counter_by(rows, "split", "language_family", "bounded_argument_type")
    split_language_context_counts = _counter_by(rows, "split", "language_family", "context_group")
    ...
    per_cell_arg_unique = {
        f"{split}:{language}": len({arg for arg in ARG_TYPES if split_language_arg_counts.get((split, language, arg), 0) > 0})
        for split, languages in LANGUAGE_TARGETS.items()
        for language in languages
    }
    per_cell_context_unique = {
        f"{split}:{language}": len({context for context in CONTEXT_GROUPS if split_language_context_counts.get((split, language, context), 0) > 0})
        for split, languages in LANGUAGE_TARGETS.items()
        for language in languages
    }
    split_arg_deltas = {split: _max_min_delta(split_arg_counts, split, ARG_TYPES) for split in CAPS}
    split_context_deltas = {split: _max_min_delta(split_context_counts, split, CONTEXT_GROUPS) for split in CAPS}
    train_arg_unique_ok = all(per_cell_arg_unique[f"train:{language}"] >= 5 for language in LANGUAGE_TARGETS["train"])
    eval_arg_unique_ok = all(per_cell_arg_unique[f"{split}:{language}"] >= 4 for split in ("eval", "strict_eval") for language in LANGUAGE_TARGETS[split])
    train_context_unique_ok = all(per_cell_context_unique[f"train:{language}"] >= 6 for language in LANGUAGE_TARGETS["train"])
    eval_context_unique_ok = all(per_cell_context_unique[f"{split}:{language}"] >= 4 for split in ("eval", "strict_eval") for language in LANGUAGE_TARGETS[split])
    checks = {
        ...
        "split_arg_balance_delta_le_1": max(split_arg_deltas.values(), default=999) <= 1,
        "split_context_balance_delta_le_1": max(split_context_deltas.values(), default=999) <= 1,
        "train_language_cells_cover_all_arg_types": train_arg_unique_ok,
        "eval_strict_language_cells_cover_four_arg_types": eval_arg_unique_ok,
        "train_language_cells_cover_all_context_groups": train_context_unique_ok,
        "eval_strict_language_cells_cover_four_context_groups": eval_context_unique_ok,
    ...
    return {
        ...
        "split_arg_counts": {f"{split}:{arg}": count for (split, arg), count in sorted(split_arg_counts.items())},
        "split_context_counts": {f"{split}:{context}": count for (split, context), count in sorted(split_context_counts.items())},
        "per_split_language_arg_unique": per_cell_arg_unique,
        "per_split_language_context_unique": per_cell_context_unique,
        "split_arg_balance_deltas": split_arg_deltas,
        "split_context_balance_deltas": split_context_deltas,
...
def main() -> None:
    ...
    summary = {
        ...
        "passed": audit["passed"],
        "supersedes": [
            "stage9240_source_backed_multilang_bounded_decoder_tiny_package",
            "stage9241_multilang_source_backed_target_100m_contract_only_preflight",
            "stage9242_multilang_source_backed_bounded_decoder_execution_review",
        ],
        "authority": dict(AUTHORITY_CLOSED),
        ...
        "decision": "Built an argument/context-balanced source-backed tiny bounded decoder CE manifest; no execution authorized." if audit["passed"] else "Argument/context-balanced bounded decoder package failed audit.",
        "next_best_step": "Run target-100M contract-only preflight on Stage9244, then create a fresh inactive execution review if it passes.",
    ...
    DOC.write_text("\n".join([
        ...
        f"Split argument counts: `{audit['split_arg_counts']}`",
        f"Split context counts: `{audit['split_context_counts']}`",
        f"Split argument balance deltas: `{audit['split_arg_balance_deltas']}`",
        f"Split context balance deltas: `{audit['split_context_balance_deltas']}`",
