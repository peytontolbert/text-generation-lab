...
STAGE = 11672
NAME = "stage11672_web_canonical_verifier_value_listwise_probe_request"
OUT = ART / NAME
SUMMARY = OUT / "web_canonical_verifier_value_listwise_probe_request.json"
...
RUN_DIR = ART / "stage11672_web_canonical_verifier_value_listwise_probe"
...
def main() -> None:
    ...
    gates = {
        ...
        "contrast_weight_nonzero": "--bounded-choice-contrast-weight" in command
        and float(command[command.index("--bounded-choice-contrast-weight") + 1]) > 0.0,
        "contrast_margin_nonzero": "--bounded-choice-contrast-margin" in command
        and float(command[command.index("--bounded-choice-contrast-margin") + 1]) > 0.0,
        "verifier_value_listwise_weight_nonzero": "--bounded-choice-verifier-value-listwise-weight" in command
        and float(command[command.index("--bounded-choice-verifier-value-listwise-weight") + 1]) > 0.0,
    }
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "web_canonical_verifier_value_listwise_probe_ready" if all(gates.values()) else "web_canonical_verifier_value_listwise_probe_blocked",
        ...
        "claim_boundary": [
            "This is a head-only diagnostic over the canonical Web renderer with same-role verifier-transition listwise loss.",
