...
STAGE = 9344
NAME = "stage9344_operator_route_rejoin_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9343_full_mixture_with_operator_route_repairs_manifest.json"
...
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9345_operator_route_rejoin_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "targeted_repair_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9345_operator_route_rejoin_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "OPERATOR_ROUTE_REJOIN_PROBE_PREEXECUTION_STAGE9344.md"
...
COMMAND = [
    ...
    "--max-train-rows", "40", "--max-eval-rows", "19", "--max-strict-rows", "15",
    ...
    "--enable-generation-audit", "--max-generation-rows", "74", "--max-generation-tokens", "64",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9345_operator_route_rejoin_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9343_not_passed")
    if len(rows) != 74:
        failures.append("manifest_row_count_not_21")
    if split_counts != {"train": 40, "eval": 19, "strict_eval": 15}:
        failures.append("unexpected_split_counts")
    if not {"operator_route_separated_repair", "repair_file_path_associated_with", "prefix_ladder_to_remaining_suffix"}.issubset(task_counts):
    ...
    required_pairs = {
        "--mode": "denoise_repair_probe", "--probe-scale": "target_100m", "--max-train-rows": "40",
        "--max-eval-rows": "19", "--max-strict-rows": "15", "--max-steps": "80",
        "--max-generation-rows": "74", "--decoder-ce-weight": "0.0", "--structured-aux-weight": "0.0",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared the 74-row operator-route rejoin target-100M denoise interference probe; decoder CE remains closed.",
        "next_best_step": "Execute Stage9345 operator-route rejoin probe under trellis and audit whether route separation survives the full mixture.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9344 Operator Route Rejoin Probe Preexecution",
        "",
        f"Passed: `{card['passed']}`",
        f"Rows: `{card['manifest_rows']}`",
        f"Splits: `{card['split_counts']}`",
        f"Tasks: `{card['task_counts']}`",
        "Only the next 74-row operator-route rejoin target-100M denoise probe is authorized. Decoder CE and external authority remain closed.",
        "",
    ]), encoding="utf-8")
