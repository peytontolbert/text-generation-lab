...
STAGE = 10039
NAME = "stage10039_expanded_source_heldout_same_manifest_gemma_queue"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
QUEUE = OUT_DIR / "expanded_source_heldout_same_manifest_gemma_queue.json"
PACKETS = OUT_DIR / "expanded_source_heldout_same_manifest_gemma_packets.jsonl"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "EXPANDED_SOURCE_HELDOUT_SAME_MANIFEST_GEMMA_QUEUE_STAGE10039.md"
...
OUTPUT = ROOT / "runs/local/artifacts/stage10041_expanded_source_heldout_gemma_execution/same_prompt_surface_gemma12b_outputs.json"
...
def build_queue() -> dict[str, Any]:
    ...
    packet = {
        ...
        "same_surface_packet": {
            ...
            "heldout_compare_rows": split_counts.get("eval", 0) + split_counts.get("strict_eval", 0),
        ...
        "queue_entries": [
            {
                ...
                "language_family": "multilingual",
                "priority_rank": 1,
                "priority_reason": "same-manifest Gemma comparison for the expanded 95-row source-heldout frontier",
...
def main() -> None:
    ...
    next_step = "Run the local Ollama Gemma comparator on this expanded source-heldout same-manifest queue, then compare its rows directly against the matching 100M rerun on the exact 95-row manifest."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Materialized a same-manifest Gemma queue and packet set for the expanded 95-row source-heldout manifest so the next rerun can answer whether the 100M still beats Gemma after adding fresh Python and c_cpp heldout rows.",
