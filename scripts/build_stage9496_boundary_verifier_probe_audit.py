...
STAGE = 9496
NAME = "stage9496_boundary_verifier_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9495_boundary_verifier_preflight_audit.json"
RUN_DIR = ROOT / "runs/local/artifacts/stage9496_boundary_verifier_isolated_target_100m_probe"
...
AUDIT = RUN_DIR / "stage9496_boundary_verifier_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "BOUNDARY_VERIFIER_PROBE_AUDIT_STAGE9496.md"
...
def main() -> None:
    ...
    if source.get("passed") is not True or not source.get("metrics", {}).get("execution_authorized_for_next_stage"):
        safety_failures.append("source_stage9495_not_authorized")
    ...
    wrong_by_split_target_pred = Counter(
        f"{row.get('split')}::{row.get('target')}=>{row.get('pred')}" for row in wrong_rows
    )
    ...
    audit = {
        ...
        "source_stage": "stage9495_boundary_verifier_preflight_audit",
        ...
        "wrong_by_split_target_pred": dict(sorted(wrong_by_split_target_pred.items())),
        "high_confidence_wrong_rows": high_conf_wrong,
        "best_state_selection": best,
        "checkpoint_like_artifacts": checkpoint_like,
        "diagnosis": "Safe quality failure. Isolated boundary verifier matched the 4/6 majority baseline on eval and strict, with all false boundary examples predicted true at high confidence. The next data patch should counterbalance boundary-negative rows and/or expose deterministic generated/reference boundary-comparison features.",
    ...
    summary = {
        ...
        "decision": "Isolated boundary verifier probe failed quality safely; do not rejoin verifier heads. Build balanced boundary-negative repair rows first.",
        "next_best_step": "Build Stage9497 boundary-negative counterbalance manifest with eval/strict true/false balance and explicit deterministic boundary-comparison features.",
