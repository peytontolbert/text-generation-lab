#!/usr/bin/env python3
"""Materialize bitnet_wasm Rust selected-verifier support rows."""
...
STAGE = 11799
NAME = "stage11799_rust_bitnet_wasm_support_rows"
OUT = ART / NAME
SUMMARY = OUT / "rust_bitnet_wasm_support_rows.json"
ROWS = OUT / "rust_bitnet_wasm_support_rows.jsonl"
FEASIBILITY = ART / "stage11798_rust_bitnet_wasm_verifier_feasibility/rust_bitnet_wasm_verifier_feasibility_results.jsonl"
...
def file_evidence(repo: Path, eid: str, kind: str, rel_path: str, summary: str, start: int = 1, end: int = 260) -> dict[str, Any]:
...
def main() -> None:
    ...
    if not probes:
        raise SystemExit("no materializable bitnet_wasm Rust probe found")
    probe = probes[0]
    ...
    root_id = "stage11799::model_stack_bitnet_wasm::rust::lib"
    evidence = [
        file_evidence(repo, "E01", "candidate_change_surface", source_path, "Rust BitNet WASM implementation containing inline tests for tiny linear decode behavior."),
        file_evidence(repo, "E02", "nearby_source_distractor", distractor_source_path, "Cargo manifest is adjacent build metadata, not the selected inline verifier target."),
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "fourth_rust_verifier_support_root_materialized",
        "passed": True,
        "row_count": len(rows),
        "root_count": 1,
        "repo_family": repo_family,
        "anti_leak_status": {"strict_smoke_root_replay": False, "train_support_only": True, "tokenizers_overlap": False, "selected_test_identifier_only_inside_candidate_options": True},
        "source_artifacts": {"feasibility": rel(FEASIBILITY)},
        "outputs": {"summary": rel(SUMMARY), "rows": rel(ROWS)},
        "claim_boundary": ["Rows are train-support-only.", "This expands Rust verifier support with a non-tokenizers BitNet WASM repo family."],
    }
