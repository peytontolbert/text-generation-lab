...
STAGE = 10929
NAME = 'stage10929_semantic_evidence_postrun_audit'
OUT_DIR = ARTIFACTS / NAME
SUMMARY_JSON = OUT_DIR / 'semantic_evidence_postrun_audit.json'
SLICE_AUDIT_JSON = OUT_DIR / 'fresh_slice_bounded_choice_eval.json'
SLICE_ROWS_JSONL = OUT_DIR / 'fresh_slice_rows.jsonl'

RUNTIME_BUNDLE = ARTIFACTS / 'stage10928_semantic_evidence_multilingual_support_probe' / 'runtime_model' / 'runtime_model_bundle.json'
OVERLAY_EVAL_JSON = ARTIFACTS / 'stage10928_semantic_evidence_multilingual_support_probe' / 'bounded_decoder_probe' / 'bounded_choice_eval_audit_eval.json'
OVERLAY_STRICT_JSON = ARTIFACTS / 'stage10928_semantic_evidence_multilingual_support_probe' / 'bounded_decoder_probe' / 'bounded_choice_eval_audit_strict_eval.json'
...
def main() -> None:
    ...
    for row in row_cards:
        ...
        combined.append({
            ...
            'language_family': next((r.get('language_family') for r in fresh_rows if r.get('row_id') == row.get('row_id')), None),
    ...
    summary = {
        ...
        'passed': True,
        'claim_scope': [
            'Audit the stage10928 runtime against the unchanged 23-row overlay and the fresh 3-row evidence successor slice.',
            'Keep Gemma fixed from the prior slice comparison; this artifact measures only whether the new 100M runtime moved the evidence lane.',
        ],
        ...
        'findings': [
            'The unchanged overlay remains at the same miss geometry if the same Python verifier strict row is still the only strict miss and the same Python/C++ evidence rows remain the eval misses.',
            'The fresh 3-row successor slice is the decisive evidence-lane check because it survives reviewed rematerialization and option-order changes.',
        ],
        'outputs': {
