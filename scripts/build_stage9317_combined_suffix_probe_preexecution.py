...
STAGE = 9317
NAME = "stage9317_combined_suffix_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9316_combined_suffix_curriculum_manifest.json"
...
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9318_combined_suffix_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "combined_suffix_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9318_combined_suffix_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "COMBINED_SUFFIX_PROBE_PREEXECUTION_STAGE9317.md"
...
COMMAND = [
    ...
    "--max-train-rows", "27", "--max-eval-rows", "9", "--max-strict-rows", "8",
    ...
    "--enable-generation-audit", "--max-generation-rows", "44", "--max-generation-tokens", "64",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9318_combined_suffix_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9316_not_passed")
    if len(rows) != 44:
        failures.append("manifest_row_count_not_44")
    ...
    required_pairs = {
        "--mode": "denoise_repair_probe", "--probe-scale": "target_100m", "--max-train-rows": "27",
        "--max-eval-rows": "9", "--max-strict-rows": "8", "--max-steps": "80",
        "--max-generation-rows": "44", "--decoder-ce-weight": "0.0", "--structured-aux-weight": "0.0",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared the combined 44-row target-100M denoise probe; decoder CE remains closed.",
        "next_best_step": "Execute Stage9318 combined suffix probe under trellis and audit source/bridge/ladder quality.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9317 Combined Suffix Probe Preexecution",
        "",
