...
STAGE = 10937
NAME = "stage10937_explicit_verifier_ledger_postrun_audit"
OUT_DIR = ARTIFACTS / NAME
SUMMARY_JSON = OUT_DIR / "explicit_verifier_ledger_postrun_audit.json"
...
OVERLAY_EVAL_JSON = ARTIFACTS / "stage10936_explicit_verifier_ledger_support_probe" / "bounded_decoder_probe" / "bounded_choice_eval_audit_eval.json"
OVERLAY_STRICT_JSON = ARTIFACTS / "stage10936_explicit_verifier_ledger_support_probe" / "bounded_decoder_probe" / "bounded_choice_eval_audit_strict_eval.json"
...
def main() -> None:
    ...
    summary = {
        ...
        "passed": True,
        "claim_scope": [
            "Audit the stage10936 runtime against the unchanged 23-row overlay and the fresh 3-row evidence successor slice.",
            "Keep Gemma fixed from the prior slice comparison; this artifact measures only whether explicit verifier-ledger support moved the evidence lane.",
        ],
        ...
        "findings": [
            "This postrun audit is successful only if the fresh 3-row successor slice improves beyond the stage10916 baseline without introducing new overlay regressions.",
            "If the explicit verifier ledger improves Python/C++ fresh rows while preserving the overlay, it is the first evidence that missing visible verifier material rather than support volume was the dominant blocker.",
        ],
        "outputs": {
