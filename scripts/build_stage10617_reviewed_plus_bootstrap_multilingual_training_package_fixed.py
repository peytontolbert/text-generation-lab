...
STAGE = 10617
NAME = "stage10617_reviewed_plus_bootstrap_multilingual_training_package_fixed"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
PACKAGE_JSON = OUT_DIR / "reviewed_plus_bootstrap_multilingual_training_package_fixed.json"
...
PACKAGE_V1 = ROOT / "runs/local/artifacts/stage10616_reviewed_plus_bootstrap_multilingual_training_package/reviewed_plus_bootstrap_multilingual_training_package.json"
...
def main() -> None:
    ...
    package_v1 = load_json(PACKAGE_V1)
    ...
    payload = {
        ...
        "passed": True,
        "decision": "reviewed_plus_bootstrap_training_package_fixed",
        "claim_scope": [
            "Correct the stage10616 package by allowing bootstrap rows to flow by admitted root role even when row-level split is null.",
        ...
        "delta_vs_stage10616": {
            "prior_train_rows": ((package_v1.get("splits") or {}).get("train") or {}).get("rows"),
            "new_train_rows": len(train_rows),
            "prior_validation_rows": ((package_v1.get("splits") or {}).get("validation") or {}).get("rows"),
            "new_validation_rows": len(validation_rows),
            "prior_strict_rows": ((package_v1.get("splits") or {}).get("strict_eval") or {}).get("rows"),
        ...
        "headline_findings": [
            "The corrected package now includes the admitted bootstrap train and validation rows rather than only the reviewed compact rows.",
            "Python still dominates the train pool and therefore requires explicit repo-family caps and per-language balancing in the next probe request.",
            "The package is now a real multilingual training base, but Rust and web still need more leak-clean roots for a stronger headline path.",
        ],
        "required_next_actions": [
            "Use this corrected package as the base for the next multilingual probe request, with repo caps and per-language balancing.",
        ...
        "recommended_next_stage": "stage10618_reviewed_plus_bootstrap_multilingual_probe_request",
        "outputs": {
