#!/usr/bin/env python3
"""Emit a support-only learnability probe for transition projection rows."""
...
STAGE = 11899
NAME = "stage11899_transition_projection_support_only_probe_request"
OUT = ART / NAME
SUMMARY = OUT / "transition_projection_support_only_probe_request.json"
COMMAND_JSON = OUT / "transition_projection_support_only_command.json"
MANIFEST = OUT / "transition_projection_support_only_manifest.jsonl"

SOURCE_ROWS = ART / "stage11897_transition_record_projection_rows/transition_projection_rows.jsonl"
INIT_RUNTIME = ART / "stage11507_preservation_strengthened_evidence_judgment_probe/runtime_model/runtime_model_bundle.json"
RUN_DIR = ART / "stage11900_transition_projection_support_only_learnability_probe"
...
def copy_row(row: dict[str, Any], *, split: str, suffix: str) -> dict[str, Any]:
    out = dict(row)
    ...
    out["stage11899_transition_projection_support_only"] = True
...
def main() -> None:
    ...
    train_rows = [copy_row(row, split="train", suffix="stage11899_train") for row in source_rows]
    eval_rows = [copy_row(row, split="eval", suffix="stage11899_echo_eval") for row in source_rows[:24]]
    strict_rows = [copy_row(row, split="strict_eval", suffix="stage11899_echo_strict") for row in source_rows[24:48]]
    ...
    gates = {
        "source_rows_640": len(source_rows) == 640,
        "manifest_rows_688": len(manifest_rows) == 688,
    ...
    artifact = {
        ...
        "decision": "transition_projection_support_only_probe_request_ready" if all(gates.values()) else "transition_projection_support_only_probe_request_blocked",
        "command": command,
        "gates_before_execution": gates,
        "row_counts": {
            "train_rows": len(train_rows),
            "eval_echo_rows": len(eval_rows),
            "strict_echo_rows": len(strict_rows),
        ...
        "claim_boundary": [
            "Diagnostic only: trains on transition projection support rows and echo eval rows.",
            "A good result proves the projection interface can be fit; it is not promotable.",
            "Protected canary/residual gates must be audited separately after the run.",
        ],
    }
