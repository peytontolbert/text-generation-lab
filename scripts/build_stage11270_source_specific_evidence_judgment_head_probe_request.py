...
STAGE = 11270
NAME = "stage11270_source_specific_evidence_judgment_head_probe_request"
OUT_DIR = ARTIFACTS / NAME
SUMMARY_JSON = OUT_DIR / "source_specific_evidence_judgment_head_probe_request.json"
COMMAND_JSON = OUT_DIR / "source_specific_evidence_judgment_head_probe_command.json"
MANIFEST = OUT_DIR / "source_specific_evidence_judgment_head_probe_manifest.jsonl"
...
JUDGMENT_PACKAGE = ARTIFACTS / "stage11269_source_specific_evidence_item_materialization"
JUDGMENT_SUMMARY = JUDGMENT_PACKAGE / "source_specific_evidence_item_materialization.json"
JUDGMENT_AUDIT = JUDGMENT_SUMMARY
JUDGMENT_TRAIN = JUDGMENT_PACKAGE / "source_specific_evidence_item_train_rows.jsonl"
JUDGMENT_VALIDATION = JUDGMENT_PACKAGE / "source_specific_evidence_item_validation_rows.jsonl"
JUDGMENT_STRICT = JUDGMENT_PACKAGE / "source_specific_evidence_item_strict_rows.jsonl"
...
RUN_DIR = ARTIFACTS / "stage11271_source_specific_evidence_judgment_head_probe"
...
def main() -> None:
    ...
    if not package.get("passed"):
        raise SystemExit("Stage11269 package did not pass")
    if not audit.get("passed"):
        raise SystemExit("Stage11269 audit did not pass")
    ...
    summary = {
        ...
        "passed": True,
        "decision": "source_specific_evidence_judgment_head_probe_requested",
        "rationale": [
            "Stage11269 replaces generic path-list evidence with materialized source chunk excerpts; this request tests whether the dedicated judgment-head scorer generalizes on source-specific evidence items.",
            "Stage11269 found no target-label, role-alias, internal-support-reason, root-overlap, or duplicate-row blockers.",
            "This is diagnostic: C/C++ supply is absent and all admitted Stage11269 rows are Python. The scorer head should learn source-specific evidence judgment while falling back to verifier-conditioned scoring for existing canary/residual rows.",
        ],
        "metrics": {
        ...
        "promotion_gate_for_postrun": {
            ...
            "source_specific_judgment_validation_or_strict_should_score_above_majority_baseline": True,
        },
        "source_artifacts": {
