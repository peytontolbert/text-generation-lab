...
OUT = ROOT / "runs/local/tmp/pocketpal_stage612_bare_selector_pair_seed461"
ARTIFACT = ROOT / "runs/local/artifacts/stage612_bare_selector_pair_dataset.json"
DOC = ROOT / "docs/stage612_bare_selector_pair_dataset.md"
...
def inject_after_operation(text: str, marker: str) -> str:
    if not text or "selector_pair=" in text:
        return text
...
def rewrite_row(row: dict[str, Any]) -> tuple[dict[str, Any], bool]:
    ...
    if not (entity and field):
        return dict(row), False
    marker = f"selector_pair={entity}|{field}"
    ...
    for key, value in row.items():
        if isinstance(value, str) and key in TEXT_FIELDS:
            ...
            rewritten[key] = value
    rewritten["entity_context_bare_selector_pair"] = True
    rewritten["entity_context_selector_entity"] = entity
    rewritten["entity_context_selector_field"] = field
    rewritten["source_type"] = f"{row.get('source_type', 'retrieval')}_bare_selector_pair"
...
def selector_counts(rows: list[dict[str, Any]]) -> dict[str, int]:
    return dict(Counter(str(row.get("entity_context_selector_field", "") or "") for row in rows if row.get("entity_context_bare_selector_pair")))
...
def main() -> None:
    ...
    manifest.update(
        {
            "artifact_kind": "agentkernel_lite_encdec_stage612_bare_selector_pair",
            ...
            "entity_context_bare_selector_pair": True,
            "stage612_selector_changed_train_examples": changed_counts["train"],
            "stage612_selector_changed_eval_examples": changed_counts["eval"],
            "collision_goal": "Test whether the compact selector_pair marker works without an explicit ENTITY_SELECTOR label.",
        }
    )
    ...
    summary = {
        "artifact_kind": "stage612_bare_selector_pair_dataset",
        ...
        "decision": "bare_selector_pair_dataset_ready",
        "finding": (
            "Stage612 removes the explicit ENTITY_SELECTOR label from Stage611. It tests whether the compact pair key alone "
            "is enough to preserve selector weighting while reducing token cost."
        ),
    }
    ARTIFACT.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text(
        f"""# Stage612 Bare Selector Pair Dataset
