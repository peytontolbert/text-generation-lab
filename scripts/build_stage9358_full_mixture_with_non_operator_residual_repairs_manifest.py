...
STAGE = 9358
NAME = "stage9358_full_mixture_with_non_operator_residual_repairs_manifest"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9357_non_operator_residual_probe_audit.json"
BASE = ROOT / "runs/local/artifacts/stage9352_full_mixture_with_schema_and_terminal_repairs_manifest/full_mixture_with_schema_and_terminal_repairs_manifest.jsonl"
REPAIR = ROOT / "runs/local/artifacts/stage9355_non_operator_residual_repair_manifest/non_operator_residual_repair_manifest.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
MANIFEST = OUT_DIR / "full_mixture_with_non_operator_residual_repairs_manifest.jsonl"
AUDIT = OUT_DIR / "full_mixture_with_non_operator_residual_repairs_manifest_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "FULL_MIXTURE_WITH_NON_OPERATOR_RESIDUAL_REPAIRS_MANIFEST_STAGE9358.md"
...
def remap(source_name: str, source_stage: int, row: dict[str, Any]) -> dict[str, Any]:
    ...
    new["row_id"] = f"stage9358_{source_name}_{old_id}"
    new["source_row_id"] = old_id
    new["source_stage"] = source_stage
    new["merged_curriculum_source"] = source_name
    new["objective_family"] = "full_mixture_with_non_operator_residual_repairs_denoise"
    ...
    mi["full_mixture_with_non_operator_residual_repairs"] = True
    mi["route_schema_version"] = mi.get("route_schema_version") or "stage9358_full_mixture_v1"
    new["model_input"] = mi
    anti = new.get("anti_cheat") if isinstance(new.get("anti_cheat"), dict) else {}
    anti.update({"decoder_ce_closed": True, "runtime_closed": True, "full_mixture_with_non_operator_residual_repairs": True})
    new["anti_cheat"] = anti
    return new


def build_rows() -> list[dict[str, Any]]:
    return [remap("schema_terminal_mixture", 9352, row) for row in load_jsonl(BASE)] + [remap("non_operator_residual_repair", 9355, row) for row in load_jsonl(REPAIR)]
...
def audit_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9357_not_passed")
    if len(rows) != 154:
    ...
    if dict(source_counts) != {"schema_terminal_mixture": 122, "non_operator_residual_repair": 32}:
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Merged Stage9355 non-operator residual repairs back into the schema-terminal full mixture.",
        "next_best_step": "Build Stage9359 preexecution and run a 154-row closed rejoin probe before bounded decoder CE.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9358 Full Mixture With Non-Operator Residual Repairs Manifest", "", f"Passed: `{audit['passed']}`", f"Rows: `{audit['rows']}`", f"Splits: `{audit['split_counts']}`", f"Sources: `{audit['source_counts']}`", f"Routes: `{audit['route_counts']}`", "", "Decoder CE, runtime, harness, Gemma, scoring, promotion, and checkpoint export remain closed.", ""]), encoding="utf-8")
