...
STAGE = 9912
NAME = "stage9912_hardened_multilingual_review_workbook"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
WORKBOOK = OUT_DIR / "hardened_multilingual_review_workbook.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "HARDENED_MULTILINGUAL_REVIEW_WORKBOOK_STAGE9912.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
PACKETS = ROOT / "runs/local/artifacts/stage9911_hardened_multilingual_winner_review_packets/hardened_multilingual_winner_review_packets.jsonl"
...
def main() -> None:
    ...
    next_step = "Work the Stage9912 workbook top to bottom and replace the remaining human-review blockers on the four hardened multilingual winner cells with signed rubric and anti-cheat judgments."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Materialized a reviewer-facing workbook for the four hardened multilingual winner cells, with direct links to the refreshed rubric stubs, anti-cheat cards, and recommendation drafts tied to Stage9909 and Stage9910.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9912 Hardened Multilingual Review Workbook",
        "",
