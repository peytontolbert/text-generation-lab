...
STAGE = 9314
NAME = "stage9314_lexical_bridge_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9313_lexical_bridge_contrast_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9313_lexical_bridge_contrast_manifest/lexical_bridge_contrast_manifest.jsonl"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9315_lexical_bridge_contrast_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "lexical_bridge_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9315_lexical_bridge_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "LEXICAL_BRIDGE_PROBE_PREEXECUTION_STAGE9314.md"
...
COMMAND = [
    ...
    "--max-train-rows", "4", "--max-eval-rows", "4", "--max-strict-rows", "4",
    "--max-steps", "48", "--batch-size", "2", "--learning-rate", "1e-5",
    ...
    "--enable-generation-audit", "--max-generation-rows", "12", "--max-generation-tokens", "64",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9315_lexical_bridge_contrast_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9313_not_passed")
    if len(rows) != 12:
        failures.append("manifest_row_count_not_12")
    if split_counts != {"train": 4, "eval": 4, "strict_eval": 4}:
    ...
    required_pairs = {
        ...
        "--max-eval-rows": "4", "--max-strict-rows": "4", "--max-steps": "48",
        "--max-generation-rows": "12", "--decoder-ce-weight": "0.0", "--denoise-weight": "1.0",
    ...
    return {"passed": not failures, "failures": failures, "manifest_rows": len(rows), "split_counts": split_counts, "unsafe_rows": unsafe_rows, "command": COMMAND, "command_ready": not failures, "authority": AUTHORITY_RUN if not failures else dict(AUTHORITY_CLOSED)}
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared a 12-row lexical bridge target-100M denoise probe; decoder CE remains closed.",
        "next_best_step": "Execute Stage9315 lexical bridge contrast probe under trellis and audit whether bridge failures are repaired.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9314 Lexical Bridge Probe Preexecution", "", f"Passed: `{card['passed']}`", f"Rows: `{card['manifest_rows']}`", f"Splits: `{card['split_counts']}`", "Only the next tiny target-100M denoise probe is authorized. Decoder CE and external authority remain closed.", ""]), encoding="utf-8")
