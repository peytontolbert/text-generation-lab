...
STAGE = 9252
NAME = "stage9252_semantic_target_repair_bounded_decoder_execution_review"
SOURCE_PACKAGE = ROOT / "runs/summaries/stage9249_semantic_bounded_decoder_target_repair_package.json"
SOURCE_PREFLIGHT = ROOT / "runs/summaries/stage9251_semantic_target_repair_target_100m_contract_only_preflight.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "SEMANTIC_TARGET_REPAIR_BOUNDED_DECODER_EXECUTION_REVIEW_STAGE9252.md"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
TICKET = OUT_DIR / "semantic_target_repair_bounded_decoder_execution_review_inactive.json"
AUDIT = OUT_DIR / "semantic_target_repair_bounded_decoder_execution_review_audit.json"
...
OUTPUT_DIR = "runs/local/artifacts/stage9253_semantic_target_repair_target_100m_bounded_tiny_probe/bounded_decoder_probe"
RUN_ID = "stage9253_semantic_target_repair_target_100m_bounded_tiny_probe"
...
REQUIRED_BEFORE_EXECUTION = [
    ...
    "manifest_sha256_matches_stage9251",
...
def build_ticket(source_preflight: dict[str, Any]) -> dict[str, Any]:
    ticket = {
        "ticket_id": "stage9252_semantic_target_repair_bounded_decoder_execution_review__inactive",
        "ticket_status": "DESIGN_ONLY_INACTIVE",
        "requested_stage": 9253,
        "requested_stage_name": RUN_ID,
        "requested_capability": "tiny_semantic_target_repair_target_100m_bounded_decoder_ce_probe",
        "source_stage": 9251,
...
def audit_ticket(ticket: dict[str, Any], source_package: dict[str, Any], source_preflight: dict[str, Any]) -> dict[str, Any]:
    ...
    if source_package.get("passed") is not True:
        failures.append("source_stage9249_not_passed")
    if source_preflight.get("passed") is not True:
        failures.append("source_stage9251_not_passed")
    ...
    if (source_preflight.get("metrics") or {}).get("model_execution_attempted") is not False:
        failures.append("source_stage9251_attempted_execution")
    return {
        "passed": not failures,
        "failures": failures,
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Built an inactive execution review for the Stage9249/9251 semantic target-repaired target-100M bounded tiny probe. It records the future command but authorizes no execution." if audit["passed"] else "Argument/context-balanced bounded decoder execution review failed.",
        "next_best_step": "If the user explicitly authorizes one tiny execution, run the reviewed Stage9253 command; otherwise continue no-execution data/compiler work.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9252 Semantic Target Repair Bounded Decoder Execution Review",
        "",
        f"Passed: `{summary['passed']}`",
        "",
        "This stage records an inactive future one-run command for the Stage9249 semantic target-repaired bounded decoder manifest after the Stage9251 contract-only preflight. It does not execute trainer or authorize model execution now.",
        "",
