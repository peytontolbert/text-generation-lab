...
STAGE = 11420
NAME = "stage11420_perftree_rust_build_verifier_backed_support"
OUT = ART / NAME
SUMMARY = OUT / "perftree_rust_build_verifier_backed_support.json"
ROWS = OUT / "perftree_rust_build_verifier_backed_support_rows.jsonl"
AUDIT = OUT / "perftree_rust_build_verifier_backed_support_audit.json"

CAPTURE = ART / "stage11419_perftree_rust_build_verifier_log_capture/perftree_rust_build_verifier_log_capture.json"
OPTIONS = [
...
def candidate_id(seed: str) -> str:
    return "BV-" + hashlib.sha256(seed.encode()).hexdigest()[:8]
...
def row_for(result: dict[str, Any], kind: str, target: str, path: str, excerpt: str) -> dict[str, Any]:
    ...
    evidence_item = (
        ...
        f"Source path: {path}\n"
        "Evidence note: materialized Rust build-verifier-backed evidence excerpt.\n"
        f"Excerpt:\n{excerpt.strip()}"
    )
    prompt = (
        "Language: rust\n"
        ...
        "build-verifier constraint, observed verifier pass log, or background context.\n"
        ...
        "- Selected-test anchor: absent\n"
        "- Verifier-anchor type: Cargo build/test command and manifest\n"
        "- Available local evidence roles: changed, build_verifier_constraint, verifier_log, background\n\n"
    ...
    return {
        "row_id": f"stage11420::{root_id}::{kind}",
        "root_id": root_id,
        ...
        "surface": "maintainer_rust_build_verifier_log_backed_evidence_candidate_judgment_bounded_choice",
        "split": "train",
        "package_split": "train",
...
def main() -> None:
    ...
    for result in capture.get("results", []):
        ...
        manifest_excerpt = safe_read(result.get("verifier_anchor_path"))
        log_excerpt = safe_read(str(ROOT / result["log_path"]), limit=1800)
        if not (source_excerpt.strip() and manifest_excerpt.strip() and log_excerpt.strip()):
            continue
        ...
        rows.append(row_for(result, "cargo_manifest_build_verifier_constraint", "DECISIVE_BUILD_VERIFIER_CONSTRAINT", result["verifier_anchor_path"], manifest_excerpt))
    ...
    leak_checks = {
        ...
        "all_build_verifier_anchored": all((row.get("anti_cheat") or {}).get("build_verifier_anchor_present") for row in rows),
        "all_train_support_only": all(row.get("train_support_only") and not row.get("strict_eval_eligible") for row in rows),
        "all_selected_test_anchor_absent_tagged": all(row.get("selected_test_anchor_present") is False and row.get("build_verifier_only") is True for row in rows),
    }
    audit = {
        "rows": len(rows),
    ...
    summary = {
        ...
        "created_at_utc": now(),
        "passed": True,
        "decision": "perftree_rust_build_verifier_backed_support_materialized_not_selected_test_support",
        "counts": {
        ...
        "quality_gate": {
            ...
            "build_verifier_anchor_present": bool(rows),
            "selected_test_anchor_present": False,
            "probe_ready": False,
        },
        "limitations": [
            "These rows have exact Cargo verifier commands and logs but no selected-test source anchors.",
            "Use as Rust build-verifier/evidence support only; do not headline as selected-test verifier-transition support.",
        ],
        "recommended_next_action": "Update the combined Rust inventory and keep selected-test Rust replenishment as a separate future requirement.",
        "source_artifacts": {"stage11419_capture": rel(CAPTURE)},
