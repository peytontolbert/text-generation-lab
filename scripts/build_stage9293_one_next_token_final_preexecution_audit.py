...
STAGE = 9293
NAME = "stage9293_one_next_token_final_preexecution_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9292_one_next_token_execution_review.json"
REVIEW = ROOT / "runs/local/artifacts/stage9292_one_next_token_execution_review/one_next_token_execution_review_card.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9290_one_next_token_suffix_manifest/one_next_token_suffix_manifest.jsonl"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9294_one_next_token_suffix_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "one_next_token_final_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9294_one_next_token_suffix_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "ONE_NEXT_TOKEN_FINAL_PREEXECUTION_STAGE9293.md"
...
COMMAND = [
    ...
    "--run-id", "stage9294_one_next_token_suffix_probe",
]
...
def audit_preexecution() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9292_not_passed")
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Final pre-execution audit passed for the one tiny one-next-token denoise probe; command is ready but not executed in this stage." if audit["passed"] else "Final pre-execution audit failed; do not execute.",
        "next_best_step": "Execute Stage9294 one-next-token suffix denoise probe under trellis using the audited command, then immediately audit outputs." if audit["passed"] else "Fix final pre-execution failures before running.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text(
        "\n".join([
            "# Stage9293 One-Next-Token Final Pre-Execution Audit",
            "",
            ...
            "",
            "The command is written to the command artifact and must be executed exactly for Stage9294.",
        ]) + "\n",
        encoding="utf-8",
    )
