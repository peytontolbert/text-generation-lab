...
STAGE = 9527
NAME = "stage9527_episode_obs_diag_residual_gate_preflight_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9526_episode_obs_diag_residual_gate_audit.json"
RUN_DIR = ROOT / "runs/local/artifacts/stage9527_episode_obs_diag_residual_gate_target_100m_contract_preflight"
CONTRACT = RUN_DIR / "probe_contract_audit.json"
AUDIT = RUN_DIR / "stage9527_preflight_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "EPISODE_OBS_DIAG_RESIDUAL_GATE_PREFLIGHT_STAGE9527.md"
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9526_not_passed")
    ...
    audit = {
        "passed": not failures,
        "failures": failures,
        "source_stage": "stage9526_episode_obs_diag_residual_gate_audit",
    ...
    summary = {
        ...
        "decision": "Contract-only target-100M preflight passed for the residual-gate diagnosis manifest.",
        "next_best_step": "Run Stage9528 target-100M residual-gate diagnosis probe under trellis, then audit whether strict residuals are fixed without introducing new high-confidence wrong rows.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    DOC.write_text("\n".join([
        "# Stage9527 Episode Observation Diagnosis Residual-Gate Preflight",
        "",
