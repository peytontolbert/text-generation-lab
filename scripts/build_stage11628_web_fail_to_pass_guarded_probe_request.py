...
STAGE = 11628
NAME = "stage11628_web_fail_to_pass_guarded_probe_request"
OUT = ART / NAME
SUMMARY = OUT / "web_fail_to_pass_guarded_probe_request.json"
MANIFEST = OUT / "web_fail_to_pass_guarded_probe_manifest.jsonl"
COMMAND = OUT / "web_fail_to_pass_guarded_probe_command.json"

TRAIN_ROWS = ART / "stage11621_web_fail_to_pass_row_geometry_repair/web_fail_to_pass_geometry_repaired_train_support.jsonl"
...
PROBE = ART / "stage11628_web_fail_to_pass_guarded_probe/bounded_decoder_probe"
RUNTIME_OUT = ART / "stage11628_web_fail_to_pass_guarded_probe/runtime_model"
...
def option_count(row: dict[str, Any]) -> int:
    return len((row.get("standalone_projection_source") or {}).get("opaque_options") or row.get("opaque_options") or [])
...
def main() -> None:
    ...
    overlaps = {
        ...
        "train_vs_filtered_strict": sorted(root_ids(train) & root_ids(strict)),
    ...
    gates = {
        "train_rows_48": len(train) == 48,
        "train_roots_8": len(root_ids(train)) == 8,
        "filtered_validation_rows_22": len(validation) == 22,
        "filtered_strict_rows_22": len(strict) == 22,
        "no_root_overlap_with_protected_sets": all(not vals for vals in overlaps.values()),
        "all_train_rows_have_options": all(option_count(row) >= 3 for row in train),
        "all_train_rows_train_support_only": all(row.get("strict_eval_eligible_now") is False for row in train),
        ...
        "preservation_kl_high": "--preservation-kl-weight" in command and float(command[command.index("--preservation-kl-weight") + 1]) >= 10.0,
    }
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        ...
        "metrics": {
            ...
            "train_by_target_role": dict(Counter(str(row.get("semantic_target_role")) for row in train)),
        ...
        "promotion_gates_for_postrun": {
            ...
            "residual_bank": ">=7/10",
            "web_heldout": ">35/66",
            "controlled_support": "diagnostic only",
        },
        "command": command,
        "claim_boundary": [
            "This is a guarded training probe using repaired controlled FAIL_TO_PASS rows as train support only.",
            "Controlled rows are not strict eval or headline Web benchmark evidence.",
            "Promotion requires Web heldout improvement plus Stage11507 preservation gates.",
        ],
        "source_artifacts": {
        ...
        "outputs": {"manifest": rel(MANIFEST), "command": rel(COMMAND), "summary": rel(SUMMARY), "runtime_model": rel(RUNTIME_OUT), "probe_output": rel(PROBE)},
    }
