...
STAGE = 9859
NAME = "stage9859_validity_weighted_structured_tiny_execution_review"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9858_validity_weighted_target_100m_contract_only_preflight.json"
SOURCE_MANIFEST_DIR = ROOT / "runs/local/artifacts/stage9858_validity_weighted_target_100m_contract_only_preflight/manifests"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
TINY_DIR = OUT_DIR / "tiny_structured_manifests"
TICKET = OUT_DIR / "validity_weighted_structured_tiny_execution_review_inactive.json"
AUDIT = OUT_DIR / "validity_weighted_structured_tiny_execution_review_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "VALIDITY_WEIGHTED_STRUCTURED_TINY_EXECUTION_REVIEW_STAGE9859.md"
...
REQUIRED_BEFORE_EXECUTION = [
    "fresh_stage9860_capped_contract_only_preflight_passed",
    "explicit_one_run_execution_authorization_after_stage9860",
...
def future_command(surface: str, manifest: Path) -> list[str]:
    ...
    out_dir = f"runs/local/artifacts/stage9860_{surface}_target_100m_structured_tiny_probe/{surface}_probe"
    return [
        "python",
        ...
        "--output-dir", out_dir,
        "--run-id", f"stage9860_{surface}_target_100m_structured_tiny_probe",
...
def build_ticket(surface_cards: list[dict[str, Any]]) -> dict[str, Any]:
    ticket = {
        ...
        "requested_stage": 9860,
        "requested_stage_name": "stage9860_validity_weighted_capped_structured_target_100m_contract_preflight_or_probe_series",
        "requested_capability": "tiny_target_100m_validity_weighted_structured_heads_probe",
        "source_stage": 9858,
...
def audit_ticket(ticket: dict[str, Any], source: dict[str, Any], surface_cards: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("stage9858_not_passed")
...
def main() -> None:
    ...
    next_step = "Run Stage9860 capped contract-only preflight on the Stage9859 tiny structured manifests; only then decide whether to execute one tiny target-100M structured probe series for the validity-weighted package."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Built an inactive execution-review ticket for a tiny target-100M structured-head probe series over the Stage9857 validity-weighted package; bounded decoder remains excluded and no execution is authorized now.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9859 Validity-Weighted Structured Tiny Execution Review",
        "",
