#!/usr/bin/env python3
"""Materialize PyTorch C++ MKL abstain-attractor support rows."""
...
STAGE = 11787
NAME = "stage11787_cpp_pytorch_mkl_abstain_support_rows"
OUT = ART / NAME
SUMMARY = OUT / "cpp_pytorch_mkl_abstain_support_rows.json"
ROWS = OUT / "cpp_pytorch_mkl_abstain_support_rows.jsonl"

FEASIBILITY = ART / "stage11786_cpp_pytorch_mkl_build_feasibility/cpp_pytorch_mkl_build_feasibility_results.jsonl"
...
def main() -> None:
    ...
    if not feasible:
        raise SystemExit("PyTorch MKL feasibility did not pass")
    feasibility = feasible[0]
    evidence = [
        file_evidence("E01", "candidate_change_surface", ".ci/pytorch/test_example_code/check-torch-mkl.cpp", "Small C++ Torch sample checks torch::hasMKL()."),
        ...
        log_evidence("V01", feasibility["configure"], "PyTorch MKL sample configure passed against trellis Torch CMake package."),
        log_evidence("V02", feasibility["build"], "check-torch-mkl target built successfully."),
        log_evidence("V03", feasibility["execute"], "check-torch-mkl executable ran successfully."),
    ]
    verifier_evidence = {"id": "V03", "kind": "actual_cpp_build_run_pass_log", "summary": "PyTorch check-torch-mkl configured, built, and executed successfully.", "configure": feasibility["configure"], "build": feasibility["build"], "execute": feasibility["execute"], "result": "PASS_CURRENT_BUILD_AND_RUN", "generated_build_harness": feasibility.get("harness_dir")}
    answerable = [
        {"label": "A", "role": "candidate_change_surface", "value": ".ci/pytorch/test_example_code/check-torch-mkl.cpp", "evidence_ids": ["E01"]},
        {"label": "B", "role": "verifier_and_build_constraint", "value": "cmake build and run check-torch-mkl passed", "evidence_ids": ["V01", "V02", "V03"]},
    ...
    root_id = "stage11787::pytorch::c_cpp::check_torch_mkl_support"
    rows = [
        base_row(root_id, "answerable_evidence_citation_build_run_constraint", "evidence_citation", answerable, "B", True),
        base_row(root_id, "answerable_patch_impact_mkl_surface", "patch_impact", answerable, "A", True),
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "fourth_cpp_abstain_attractor_support_root_materialized",
        "passed": True,
        "row_count": len(rows),
        "root_count": 1,
        "repo_family": "pytorch",
        ...
        "claim_boundary": [
            ...
            "A generated build harness is declared; source evidence remains repo-backed.",
