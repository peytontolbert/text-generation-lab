...
STAGE = 10174
NAME = 'stage10174_compact_bounded_encoder_option_retrieval_gemma_comparison'
...
HUNDRED_M_AUDIT = ROOT / 'runs/local/artifacts/stage10172_choice_aux_encoder_option_retrieval_target100m_probe/bounded_decoder_probe/bounded_choice_eval_audit_strict_eval.json'
GEMMA_ROWS = OUT_DIR / 'compact_bounded_gemma12b_rows.jsonl'
COMPARISON = OUT_DIR / 'compact_bounded_encoder_option_retrieval_gemma_comparison.json'
MODEL_ID = 'gemma3:12b'
...
def main() -> None:
    ...
    hundred_m_audit = load_json(HUNDRED_M_AUDIT)
    ...
    for gemma_row in gemma_rows:
        ...
        hundred_m = hundred_m_rows[row_id]
    ...
    payload = {
        ...
        'claim_scope': 'same-manifest compact bounded maintainer projection using frozen stage10172 encoder_option_retrieval 100M outputs against live gemma3:12b same-prompt outputs; diagnostic standalone surface only, not full maintainer leaderboard',
    }
    COMPARISON.write_text(json.dumps(payload, indent=2, sort_keys=True) + '\n', encoding='utf-8')
    SUMMARY.write_text(json.dumps({'stage': STAGE, 'passed': True, 'comparison': display(COMPARISON), 'rows': len(manifest_rows), 'delta_exact_accuracy': payload['delta_exact_accuracy'], 'wins': payload['wins']}, indent=2, sort_keys=True) + '\n', encoding='utf-8')
    print(json.dumps({'stage': STAGE, 'passed': True, 'comparison': display(COMPARISON), 'rows': len(manifest_rows), 'delta_exact_accuracy': payload['delta_exact_accuracy'], 'wins': payload['wins']}, indent=2, sort_keys=True))
