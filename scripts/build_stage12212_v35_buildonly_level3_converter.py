#!/usr/bin/env python3
"""Convert v35 PASS_CURRENT_BUILD rows into Level-3 train-support records.

These rows are rendered support rows, not raw replay logs. We admit only rows
with explicit PASS_CURRENT_BUILD, non-singleton shuffled options, command
...
STAGE = "stage12212_v35_buildonly_level3_converter"
...
def convert(line_no: int, row: dict[str, Any]) -> tuple[dict[str, Any] | None, str | None]:
    if row.get("observed_verifier_transition") != "PASS_CURRENT_BUILD":
    ...
    episode_id = stable_id("stage12212_episode", row.get("row_id"), root_id, selected, command)
    command_result_id = stable_id("stage12212_command", command, output_tail, root_id)
    record = {
        ...
        "candidate_action_set": {
            "candidate_set_id": stable_id("stage12212_candidates", row.get("row_id"), row.get("opaque_options")),
        ...
        "observed_action": {
            ...
            "semantic_value": "PASS_CURRENT_BUILD",
        },
        "command_result": {
            "command_result_id": command_result_id,
            "command": command,
        ...
        "verifier_status": "PASS_CURRENT_BUILD",
        "verifier_transition": "PASS_CURRENT_BUILD",
        "state_update": {
            "state_update_id": stable_id("stage12212_state_update", episode_id),
            "state_update_type": "observed_build_or_collection_pass_without_run",
            "new_facts": ["build/typecheck and runnable verifier evidence both passed"],
            "invalidated_claims": [],
            "remaining_blockers": [],
        },
        "stop_decision": {
            "stop_decision_id": stable_id("stage12212_stop", episode_id),
            "continue_or_stop": "CONTINUE",
            "reason": "build/collection evidence is useful but does not prove runnable verifier success; continue toward selected verifier execution",
        },
        "anti_cheat": {
        ...
        "projection_permissions": {
            ...
            "claim_boundary": "Rendered v35 build-only verifier evidence converted to Level-3 train support only.",
        },
    }
    return record, None
...
def main() -> int:
    ...
    write_jsonl(OUT_DIR / "v35_buildonly_level3_records.jsonl", rows)
    write_jsonl(OUT_DIR / "blocked_v35_buildonly_rows.jsonl", blocked)
    summary = {
        "stage": STAGE,
        "decision": "v35_buildonly_records_materialized" if rows else "blocked_no_new_v35_buildonly_records",
        ...
        "train_support_only": True,
        "strict_eval_eligible": False,
        "claim_boundary": "Rendered v35 PASS_CURRENT_BUILD command-output rows converted to Level-3 train support only; no eval claim.",
        "artifact_paths": {
            "records": str(OUT_DIR / "v35_buildonly_level3_records.jsonl"),
            "blocked": str(OUT_DIR / "blocked_v35_buildonly_rows.jsonl"),
        },
    }
