...
STAGE = 9722
NAME = "stage9722_multilingual_verifier_repair_tiny_package"
...
MANIFEST = OUT_DIR / "multilingual_verifier_repair_tiny.jsonl"
AUDIT = OUT_DIR / "multilingual_verifier_repair_tiny_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "MULTILINGUAL_VERIFIER_REPAIR_TINY_PACKAGE_STAGE9722.md"
...
def select_rows(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for row in rows:
        if row.get("expected_enabled_loss") != "verifier_repair_ce":
            continue
...
def main() -> None:
    ...
    next_step = "Run Stage9723 target-100M contract-only preflight on the Stage9722 multilingual verifier-repair package, then decide whether to execute that multilingual structured probe."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Materialized a balanced multilingual verifier-repair tiny package with current rows from python, rust, c_cpp, and web_js_ts_html.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9722 Multilingual Verifier Repair Tiny Package",
        "",
        f"Passed: `{summary['passed']}`",
        f"Rows: `{audit['rows']}`",
        f"Language counts: `{audit['language_counts']}`",
        "",
        "This stage packages a balanced multilingual verifier-repair structured surface from current compiled rows: python, rust, c_cpp, and web_js_ts_html.",
        "",
        f"Next: {next_step}",
        "",
    ]), encoding="utf-8")
