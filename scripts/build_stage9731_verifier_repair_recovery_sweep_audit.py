...
STAGE = 9731
NAME = "stage9731_verifier_repair_recovery_sweep_audit"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "verifier_repair_recovery_sweep_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "VERIFIER_REPAIR_RECOVERY_SWEEP_AUDIT_STAGE9731.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
BASELINE = ROOT / "runs/local/artifacts/stage9729_probe_smoke_verifier_repair_exec/execution_result.json"
RUNS = {
    "steps64_lr5e5": ROOT / "runs/local/artifacts/stage9731_verifier_repair_recovery_sweep/steps64_lr5e5/execution_result.json",
    "steps128_lr1e4": ROOT / "runs/local/artifacts/stage9731_verifier_repair_recovery_sweep/steps128_lr1e4/execution_result.json",
}
...
def _field_exact(result: dict[str, Any], split: str) -> float | None:
...
def _run_metrics(result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "eval_exact": _field_exact(result, "eval"),
        "strict_exact": _field_exact(result, "strict_eval"),
        ...
        "restored_best_state": best.get("restored") is True,
...
def build_audit() -> dict[str, Any]:
    baseline_result = load_json(BASELINE)
    baseline = _run_metrics(baseline_result)
    sweep = {name: _run_metrics(load_json(path)) for name, path in RUNS.items()}
    failures: list[str] = []
    if baseline["probe_scale"] != "target_100m":
        failures.append("baseline_not_target_100m")
    if baseline["eval_exact"] != 0.25 or baseline["strict_exact"] != 0.25:
    ...
    if improved:
        failures.append("unexpected_improvement_relative_to_current_recorded_outputs")
    ...
    return {
        ...
        "regressed_runs": [
            name for name, metrics in sweep.items()
            if (metrics["eval_exact"] or 0.0) < (baseline["eval_exact"] or 0.0)
        ],
        "authority": dict(AUTHORITY_CLOSED),
    }
...
def main() -> None:
    ...
    next_step = (
        "Stop spending more step budget on the current multilingual verifier-repair tiny package without changing data or objective structure. "
        "Next work should target label-collapse repair, richer curriculum, or same-surface comparison packaging rather than longer identical sweeps."
    )
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        ...
        "metrics": {
            **dict(AUTHORITY_CLOSED),
            ...
            "regressed_runs": audit["regressed_runs"],
        },
        "artifacts": {
            "audit": str(AUDIT.relative_to(ROOT)),
            "doc": str(DOC.relative_to(ROOT)),
        },
        "decision": "Audited a small multilingual verifier-repair target-100M recovery sweep and found no improvement over the 8-step executed baseline.",
    ...
    DOC.write_text("\n".join([
        ...
        f"Regressed runs: `{audit['regressed_runs']}`",
        "",
