...
STAGE = 9913
NAME = "stage9913_v27_hardened_multisurface_compiler_refresh"
...
PREPARED = OUT_DIR / "hardened_multisurface_prepared_rows.jsonl"
COMPILED_DIR = OUT_DIR / "compiled"
AUDIT = OUT_DIR / "hardened_multisurface_compiler_refresh_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "V27_HARDENED_MULTISURFACE_COMPILER_REFRESH_STAGE9913.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"

_stage9693_path = ROOT / "scripts/build_stage9693_locked_guarded_source_backed_multisurface_compiler_refresh.py"
_stage9693_spec = importlib.util.spec_from_file_location("stage9693_for_9913", _stage9693_path)
_stage9693 = importlib.util.module_from_spec(_stage9693_spec)
assert _stage9693_spec and _stage9693_spec.loader
sys.modules["stage9693_for_9913"] = _stage9693
...
SOURCES = {
    ...
    "edit_localization": {
        ...
        "path": ROOT / "runs/local/artifacts/stage9907_geometry_aware_opaque_choice_manifest/geometry_aware_opaque_choice_manifest.jsonl",
        "target_loss": "edit_localization_ce",
        "source_audit": ROOT / "runs/summaries/stage9910_geometry_aware_opaque_choice_shortcut_audit.json",
    },
...
def _normalize_ready_row(row: dict[str, Any], skill: str, target_loss: str, source_audit: Path) -> dict[str, Any]:
    ...
    out["row_id"] = f"stage9913_{skill}_{row.get('row_id')}"
    ...
    anti["stage9913_hardened_refresh"] = True
    out["anti_cheat"] = anti
    return out
...
def main() -> None:
    ...
    next_step = "Use this hardened compiled package as the next real v2.7 training mix: edit localization now comes from the no-label-list opaque-choice packet that beat Gemma on all 8 same-surface buckets."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        "passed": audit["passed"],
        "authority": dict(AUTHORITY_CLOSED),
        "metrics": {**dict(AUTHORITY_CLOSED), "failures": failures, "rows": audit["rows"], "skill_counts": audit["skill_counts"], "split_counts": audit["split_counts"], "language_counts": audit["language_counts"], "loss_counts": audit["actual_loss_counts"]},
        "artifacts": {
        ...
        "decision": "Built a hardened successor to the geometry-aware v2.7 multisurface compiler refresh: edit localization now sources from the Stage9907 opaque-choice packet backed by the Stage9910 anti-cheat pass, while the other multisurface sources remain unchanged.",
