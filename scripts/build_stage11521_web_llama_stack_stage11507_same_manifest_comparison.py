...
STAGE = 11521
NAME = "stage11521_web_llama_stack_stage11507_same_manifest_comparison"
OUT = ART / NAME
SUMMARY = OUT / "web_llama_stack_stage11507_same_manifest_comparison.json"

RUNTIME = ART / "stage11507_preservation_strengthened_evidence_judgment_probe/runtime_model/runtime_model_bundle.json"
WEB_ROWS = ART / "stage11361_web_llama_stack_executed_heldout_rows/web_llama_stack_executed_heldout_rows.jsonl"
GEMMA_ROWS = ART / "stage11363_web_llama_stack_same_manifest_gemma_comparison/web_llama_stack_gemma_rows.jsonl"
...
def score_100m(model: Any, tokenizer: Any, rows: list[dict[str, Any]], scorer: str) -> dict[str, Any]:
    card = _write_bounded_choice_eval_audit(
        ...
        split_name=f"web_llama_stack_{scorer}",
...
def anti_cheat_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    keys = [
        ...
        "diagnostic_heldout_not_train",
        "executed_verifier_output_attached",
        "gold_value_not_used_as_label",
        "root_not_used_in_stage11347_or_stage11354_training",
...
def main() -> None:
    ...
    summary = {
        ...
        "claim_boundary": [
            "This is a Web/JS/TS/HTML-only executed-heldout standalone smoke comparison.",
            "Gemma outputs are reused from Stage11363; this script does not start Ollama.",
            "This does not establish the multilingual source-heldout smoke gate, because Python, C/C++, and Rust rows remain unavailable or blocked.",
