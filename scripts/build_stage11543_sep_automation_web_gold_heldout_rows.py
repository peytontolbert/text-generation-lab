...
STAGE = 11543
NAME = "stage11543_sep_automation_web_gold_heldout_rows"
OUT = ART / NAME
SUMMARY = OUT / "sep_automation_web_gold_heldout_rows.json"
ROWS = OUT / "sep_automation_web_gold_heldout_rows.jsonl"
PACKETS = OUT / "sep_automation_web_gold_heldout_packets.jsonl"

REPO = Path("/data/repositories/modelcontextprotocol__modelcontextprotocol/tools/sep-automation")
GIT_HEAD = "e085d3523d9280cf082a60383693a5f3d7e5f672"
LOG_DIR = ART / "stage11543_sep_automation_web_heldout_execution/logs"
LABELS = list("ABCDE")
PERSPECTIVES = [
...
ROOT_SPECS = [
    {
        "slug": "transition_handler_state_rules",
        "task": "SEP automation verifier checks valid/invalid state transitions and dry-run versus real label/comment effects.",
        "source": "src/actions/transition.ts",
        "secondary": "src/types.ts",
        "test": "test/unit/transition.test.ts",
        "log": "transition_test.log",
        "status": "transition_test.status",
    },
    {
        "slug": "hook_registry_dispatch_resilience",
        "task": "SEP automation verifier checks enabled hook registration, dispatch, and resilience to failing hooks.",
        "source": "src/hooks/registry.ts",
        "secondary": "src/hooks/types.ts",
        "test": "test/unit/hooks.test.ts",
        "log": "hooks_test.log",
        "status": "hooks_test.status",
    },
]
...
def explicit_label_leak(pre_options: str, target_label: str) -> bool:
    ...
    return any(re.search(pattern, pre_options) for pattern in patterns)
...
def packet(spec: dict[str, str]) -> dict[str, Any]:
    ...
    return {
        "root_id": f"stage11543::sep_automation::{spec['slug']}",
        "language_family": "web_js_ts_html",
        "repo_family": "sep_automation",
        "git_repo_family": "modelcontextprotocol_modelcontextprotocol",
        "repo_path": str(REPO),
        "git_head": GIT_HEAD,
        ...
        "verifier_execution": {
            "dependency_hydration_command": "npm ci --no-audit --no-fund",
            "focused_verifier_command": f"npm test -- {spec['test']}",
            "focused_verifier_exit_code": code,
            "observed_transition": "TARGETED_TEST_PASS" if passed else "TARGETED_TEST_FAILED_OR_BLOCKED",
            "raw_log_excerpt": log[-2200:],
...
def option_bank(packet: dict[str, Any], perspective: str) -> list[dict[str, str]]:
    ...
    return [
        ...
        {"value": "symptom_or_call_path_analogue", "text": f"{packet['secondary_source_path']} adjacent call-path or type context."},
        {"value": "dependency_or_test_environment_surface", "text": "package-lock, npm, and Vitest execution environment surface."},
        {"value": "abstain_insufficient_evidence", "text": "ABSTAIN_INSUFFICIENT_EVIDENCE"},
    ]
...
def rows_for_packet(packet: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for perspective in PERSPECTIVES:
        row_id = f"{packet['root_id']}::{perspective}::heldout_v1"
        ...
        rows.append({
            ...
            "split_component": "sealed_web_heldout",
            ...
            "anti_cheat": {
                ...
                "root_not_in_train": True,
                "do_not_train": True,
            },
            "review": {
                "rubric_complete": True,
                "anti_cheat_complete": True,
                "gold_adjudication_complete": True,
                "reviewer": "codex_ai_maintainer_review",
...
def main() -> None:
    ...
    packets = [
        {
            **packet(spec),
            "admission_status": "sealed_web_heldout_gold_adjudicated",
            "trainable_now": False,
            "strict_eval_eligible_now": True,
        }
        for spec in ROOT_SPECS
    ]
    packets = [
        packet for packet in packets
        if packet["verifier_execution"]["focused_verifier_exit_code"] == 0
        and packet["visible_source_evidence"]["primary"].get("exists")
        and packet["visible_verifier_evidence"]["test"].get("exists")
    ]
    ...
    summary = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "passed": len(packets) == 2 and len(rows) == 12 and not leakage_flags,
        "decision": "sep_automation_web_heldout_rows_admitted" if not leakage_flags else "blocked_prompt_target_leak",
        "counts": {
            "admitted_heldout_roots": len(packets),
            "strict_eval_rows": len(rows),
            "train_rows": 0,
        ...
        "claim_boundary": [
            "These rows are sealed Web heldout candidates, not train support.",
            "They close the Stage11527 Web heldout-root count but do not by themselves prove model improvement.",
        ],
