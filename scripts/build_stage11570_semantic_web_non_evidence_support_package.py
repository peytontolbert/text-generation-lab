...
STAGE = 11570
NAME = "stage11570_semantic_web_non_evidence_support_package"
OUT = ART / NAME
SUMMARY = OUT / "semantic_web_non_evidence_support_package.json"
MANIFEST = OUT / "semantic_web_non_evidence_support_manifest.jsonl"
TRAIN_ROWS = OUT / "semantic_web_non_evidence_train_rows.jsonl"
REJECTED = OUT / "semantic_web_non_evidence_rejected_rows.jsonl"
COMMAND = OUT / "semantic_web_non_evidence_probe_command.json"
...
PROBE = ART / "stage11570_semantic_web_non_evidence_support_probe/bounded_decoder_probe"
RUNTIME_OUT = ART / "stage11570_semantic_web_non_evidence_support_probe/runtime_model"
...
def semantic_role_for_option(task_name: str, option: dict[str, Any]) -> str:
    value = str(option.get("value") or "").lower()
    text = str(option.get("text") or value).lower()
    joined = f"{value} {text}"
    if "abstain" in joined or "insufficient" in joined or "retrieve" in joined or "external" in joined or "runtime_reexecution" in joined:
        return "external_analogue_reference"
    if "test" in joined or "verifier" in joined or "assert" in joined or "pass_targeted" in joined or "fail" in joined:
        return "verifier_and_test_constraint"
    if "symptom" in joined or "call" in joined or "import" in joined or "not_imported" in joined or "exercised" in joined:
        return "symptom_or_call_path_analogue"
    if "config" in joined or "package" in joined or "metadata" in joined or "dependency" in joined:
        return "nearby_definition_or_usage_context"
    if "edit" in joined or "rewrite" in joined or "implementation" in joined or "utils" in joined or "src/" in joined or ".ts" in joined or ".js" in joined or ".html" in joined:
        return "candidate_change_surface"
    if task_name in {"symptom_localization", "minimal_fix_selection", "patch_impact"}:
        return "candidate_change_surface"
    if task_name == "verifier_outcome":
        return "verifier_and_test_constraint"
    return "nearby_definition_or_usage_context"


def semantic_transition_for_option(task_name: str, option: dict[str, Any]) -> str:
    value = str(option.get("value") or "").upper()
    text = str(option.get("text") or "").upper()
    joined = f"{value} {text}"
    if "FAIL_TO_PASS" in joined or "PASS_TARGETED" in joined or "ALL" in joined and "PASS" in joined:
        return "FAIL_TO_PASS"
    if "PASS_TO_PASS" in joined:
        return "PASS_TO_PASS"
    if "FAIL_TO_FAIL" in joined or "FAILS_BEFORE" in joined:
        return "FAIL_TO_FAIL"
    if "NOT_EXERCISED" in joined or "DOES NOT IMPORT" in joined:
        return "NOT_EXERCISED"
    if "ABSTAIN" in joined or "INSUFFICIENT" in joined or "RETRIEVE" in joined or "NEEDS_" in joined:
...
def add_semantic_candidate_metadata(row: dict[str, Any]) -> dict[str, Any]:
    out = dict(row)
    task_name = task(out)
    src = dict(out.get("standalone_projection_source") or {})
    opts = []
    for option in list(src.get("opaque_options") or out.get("opaque_options") or []):
        if not isinstance(option, dict):
            opts.append(option)
            continue
        item = dict(option)
        metadata = dict(item.get("semantic_candidate") or {})
        metadata.setdefault("task_type", task_name)
        metadata.setdefault("evidence_role", semantic_role_for_option(task_name, item))
        metadata.setdefault("verifier_transition", semantic_transition_for_option(task_name, item))
        metadata.setdefault("option_value", str(item.get("value") or ""))
        item["semantic_candidate"] = metadata
        opts.append(item)
    src["opaque_options"] = opts
    src["stage11570_semantic_candidate_metadata"] = True
    out["standalone_projection_source"] = src
    out["opaque_options"] = opts
    ac = dict(out.get("anti_cheat") or {})
    ac["stage11570_semantic_candidate_metadata"] = True
    out["anti_cheat"] = ac
    return out
...
def main() -> None:
    ...
    for row in sorted(selected, key=lambda r: (task_counts[task(r)], task(r), repo(r), root_id(r))):
        ...
        if task_counts[t] >= TASK_CAP:
            ...
            continue
        task_counts[t]+=1
        train.append(normalize_row(add_semantic_candidate_metadata(row), "train"))
    ...
    summary={
        "stage":STAGE,"stage_name":NAME,"created_at_utc":now(),"passed":all(gates.values()),"decision":"semantic_web_non_evidence_support_probe_ready" if all(gates.values()) else "semantic_web_non_evidence_support_probe_blocked",
        "metrics":{
        ...
        "claim_boundary":["Diagnostic semantic-candidate Web support package only.","Rows are deduplicated by root/task and root-disjoint from Web heldout plus protected canaries.","Promotion requires postrun audit against Stage11507 residual/strict gates and same-manifest Gemma Web baseline."],
