...
def main() -> None:
    ...
    for cell in handoff.get("handoff_cells") or []:
        ...
        task_pack = {
            ...
            "thresholds": {"exact_accuracy_min": 0.0, "same_manifest_delta_vs_gemma_min": 0.0},
            "blocked_training_reason": "repaired_headline_harness_pack_never_train_eligible",
        ...
        runs.append(
            {
                ...
                "gemma_backend": {"kind": "ollama_generate", "model": "gemma3:12b", "seed": 0, "temperature": 0.0},
            }
        )

    payload = {
        "runtime_payload_version": "stage10658.v1",
        "created_at_utc": None,
        "notes": [
            "Repaired-headline maintainer-choice harness payload compiled from the 24-row repaired same-manifest headline slice.",
            "The four repaired evidence_citation rows use opaque evidence IDs in the prompt contract.",
            "100M uses preserved bounded-choice scoring from the frozen stage10422 runtime bundle.",
        ],
        "runs": runs,
    }

    out_dir = ARTIFACTS / "stage10658_repaired_headline_harness_runtime_payload"
    out_dir.mkdir(parents=True, exist_ok=True)
    write_json(out_dir / "repaired_headline_harness_runtime_payload.json", payload)
    print(out_dir / "repaired_headline_harness_runtime_payload.json")
