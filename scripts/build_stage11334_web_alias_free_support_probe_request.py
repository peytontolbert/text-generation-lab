...
STAGE=11334; NAME='stage11334_web_alias_free_support_probe_request'
OUT=ART/NAME; OUT.mkdir(parents=True,exist_ok=True)
SUMMARY=OUT/'web_alias_free_support_probe_request.json'; CMD=OUT/'web_alias_free_support_probe_command.json'; MAN=OUT/'web_alias_free_support_probe_manifest.jsonl'
TRAIN=ART/'stage11332_web_alias_free_evidence_materialization/web_alias_free_evidence_rows.jsonl'
...
RUN=ART/'stage11335_web_alias_free_support_probe'; PROBE=RUN/'bounded_decoder_probe'; RUNTIME=RUN/'runtime_model'
...
summary={'stage':STAGE,'stage_name':NAME,'created_at_utc':time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()),'passed':True,'decision':'web_alias_free_support_probe_requested_diagnostic_only','metrics':{'train_rows':len(train),'eval_rows':len(val),'strict_rows':len(strict),'root_overlaps':over},'source_artifacts':{'train':rel(TRAIN),'canary_validation':rel(VAL),'canary_strict':rel(STRICT),'init_runtime':rel(INIT)},'outputs':{'summary':rel(SUMMARY),'command':rel(CMD),'manifest':rel(MAN),'probe_dir':rel(PROBE),'runtime_model':rel(RUNTIME)},'promotion_gate':{'diagnostic_only':True,'web_support_train_score_should_improve':'above 7/21 current-runtime baseline','canary_strict_must_remain':'22/22','canary_validation_must_remain':'>=20/23'},'command':cmd}
