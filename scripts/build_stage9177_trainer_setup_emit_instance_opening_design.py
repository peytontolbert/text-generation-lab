...
STAGE = 9177
NAME = "stage9177_trainer_setup_emit_instance_opening_design"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
SOURCE_9176 = ROOT / "runs/summaries/stage9176_trainer_setup_emit_instance_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "TRAINER_SETUP_EMIT_INSTANCE_OPENING_DESIGN_STAGE9177.md"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
DESIGN = OUT_DIR / "trainer_setup_emit_instance_opening_design.json"

REQUIRED_OPENING_REFS = [
    "source_stage9176_audit_ref",
    "emit_instance_design_ref",
...
REQUIRED_OPENING_GUARDS = [
    "opening_requires_stage9176_passed",
    "opening_requires_repo_local_output_dir",
    "opening_requires_expected_output_set_lock",
    "opening_requires_output_hash_lock",
    "opening_requires_output_provenance_join_lock",
    "opening_requires_post_emit_cardinality_audit",
    "opening_requires_post_emit_provenance_join_audit",
    "opening_forbids_trainer_invocation",
    "opening_forbids_model_forward",
    "opening_forbids_optimizer_or_backward",
    "opening_forbids_arxiv_reads",
    "opening_forbids_source_body_reads",
]
...
NEGATIVE_CASES = [
    ...
    "missing_opening_ref",
    "missing_opening_guard",
    "missing_expected_output",
    "missing_blocked_output",
    "opening_authorized_now",
    "outputs_emitted_now",
    "trainer_invoked_now",
    "model_forward",
    "optimizer_created",
...
def registry(latest: int = 9176) -> dict[str, Any]:
...
def build_design(registry_card: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    source = load_json(SOURCE_9176)
    return {
        "stage": STAGE,
        "stage_name": NAME,
        "design_type": "trainer_setup_emit_instance_opening_v1",
        "source_stage9176_passed": source.get("passed") is True,
        "registry_frontier_stage9176": int((registry_card.get("metrics") or {}).get("latest_stage", -1)) == 9176,
        "required_opening_refs": list(REQUIRED_OPENING_REFS),
        "required_opening_guards": list(REQUIRED_OPENING_GUARDS),
        "expected_emitted_outputs": list(EXPECTED_EMITTED_OUTPUTS),
        ...
        "metrics": {
            ...
            "source_stage9176_passed": source.get("passed") is True,
            "registry_frontier_stage9176": int((registry_card.get("metrics") or {}).get("latest_stage", -1)) == 9176,
            "required_opening_refs": len(REQUIRED_OPENING_REFS),
            "required_opening_guards": len(REQUIRED_OPENING_GUARDS),
            "expected_emitted_outputs": len(EXPECTED_EMITTED_OUTPUTS),
            "still_blocked_outputs": len(STILL_BLOCKED_OUTPUTS),
            "opening_authorized_now": False,
        ...
        "decision": (
            "Designed the deliberate emit-instance opening stage. If later instantiated and audited, it "
            "may authorize exact trainer-setup artifact emission under output-set/hash/provenance locks, "
            "while trainer invocation and model execution remain closed."
        ),
        "next_best_step": "Audit the emit-instance opening design; still do not emit outputs.",
    }
...
def validate_design(design: dict[str, Any]) -> list[str]:
    ...
    if design.get("source_stage9176_passed") is not True or metrics.get("source_stage9176_passed") is not True:
        failures.append("source_stage9176_not_passed")
    if design.get("registry_frontier_stage9176") is not True or metrics.get("registry_frontier_stage9176") is not True:
        failures.append("registry_frontier_stage9176")
    for item in REQUIRED_OPENING_REFS:
        if item not in design.get("required_opening_refs", []):
            failures.append(f"missing_opening_ref:{item}")
    for item in REQUIRED_OPENING_GUARDS:
        if item not in design.get("required_opening_guards", []):
            failures.append(f"missing_opening_guard:{item}")
    for item in EXPECTED_EMITTED_OUTPUTS:
        if item not in design.get("expected_emitted_outputs", []):
...
def run_negative_cases() -> dict[str, Any]:
    ...
    for case in NEGATIVE_CASES:
        ...
        if case == "source_stage_missing":
            design["source_stage9176_passed"] = False
            design["metrics"]["source_stage9176_passed"] = False
        elif case == "registry_frontier_bad":
            design["registry_frontier_stage9176"] = False
            design["metrics"]["registry_frontier_stage9176"] = False
        elif case == "missing_opening_ref":
            design["required_opening_refs"].remove("repo_local_output_dir_ref")
        elif case == "missing_opening_guard":
            design["required_opening_guards"].remove("opening_forbids_trainer_invocation")
        elif case == "missing_expected_output":
            design["expected_emitted_outputs"].remove("trainer_rows.jsonl")
        elif case == "missing_blocked_output":
            design["still_blocked_outputs"].remove("model_input_rows.jsonl")
        elif case == "opening_authorized_now":
            design["metrics"]["opening_authorized_now"] = True
...
def main() -> None:
    ...
    artifact = {
        ...
        "decision": design["decision"] if passed else "Emit-instance opening design failed validation.",
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage9177 Trainer Setup Emit-Instance Opening Design",
                "",
                f"Passed: `{summary['passed']}`",
                "",
                "Designs the deliberate emit-instance opening stage without emitting outputs.",
                "",
                f"Expected emitted outputs: `{artifact['metrics']['expected_emitted_outputs']}`",
