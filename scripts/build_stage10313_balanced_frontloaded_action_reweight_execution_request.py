...
STAGE = 10313
NAME = "stage10313_balanced_frontloaded_action_reweight_execution_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
INVENTORY_JSON = OUT_DIR / "balanced_frontloaded_action_reweight_inventory.json"
MANIFEST_JSONL = OUT_DIR / "balanced_frontloaded_action_reweight_manifest.jsonl"
REQUEST_JSON = OUT_DIR / "balanced_frontloaded_action_reweight_execution_request.json"
COMMAND_JSON = OUT_DIR / "balanced_frontloaded_action_reweight_command.json"
...
RUN_ID = "stage10314_balanced_frontloaded_action_reweight_probe"
OUTPUT_DIR = "runs/local/artifacts/stage10314_balanced_frontloaded_action_reweight_probe/bounded_decoder_probe"
RUNTIME_MODEL_DIR = "runs/local/artifacts/stage10314_balanced_frontloaded_action_reweight_probe/runtime_model"
MAX_STEPS = 48
LEARNING_RATE = "8e-6"
...
def focus_duplicates_for(base_rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for row in base_rows:
        ...
        for index in range(duplicates):
            ...
            clone["row_id"] = f"{row['row_id']}::balanced_action_reweight::{index:02d}"
            clone["semantic_key"] = f"{row.get('semantic_key')}::balanced_action_reweight::{index:02d}"
            ...
            anti_cheat["frontloaded_for_capped_probe"] = True
            anti_cheat["balanced_evidence_anchor_prefix"] = True
            ...
            clone["reweight_metadata"] = {
                ...
                "reweight_family": "balanced_frontloaded_action_reweight",
            }
            focus_rows.append(clone)
    return focus_rows, inventory_rows
...
def build() -> dict[str, Any]:
    ...
    focus_rows, inventory_rows = focus_duplicates_for(base_rows)
    ...
    anchor_ids = {row["row_id"] for row in evidence_anchor_rows}
    remaining_train_rows = [row for row in train_rows if row["row_id"] not in anchor_ids]
    merged = list(evidence_anchor_rows) + list(focus_rows) + list(remaining_train_rows) + list(strict_rows) + list(other_rows)
    ...
    inventory = {
        ...
        "evidence_anchor_rows": len(evidence_anchor_rows),
    ...
    request = {
        ...
        "created_at_utc": now_utc(),
        "passed": True,
        "decision": "warm_start_balanced_frontloaded_action_reweight_ready",
        ...
        "required_honesty_gates": [
            "strict eval rows remain identical to stage10307, stage10310, and stage10312",
            "only train rows are duplicated or reordered; no admitted or strict eval rows are moved into train",
            "the prefix now balances evidence-citation anchors with Python/web action-taking pressure inside the capped probe budget",
            "focus remains restricted to observed failure slices and anchor preservation, not benchmark mutation",
        ],
        "next_best_step": "run one balanced frontloaded probe and check whether Python/web action-taking pressure can improve without collapsing c_cpp evidence citation",
        "command": cmd,
    }
