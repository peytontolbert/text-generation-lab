...
STAGE = 9305
NAME = "stage9305_supported_suffix_boundary_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9304_operator_suffix_support_manifest.json"
...
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9306_supported_suffix_boundary_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "supported_suffix_boundary_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9306_supported_suffix_boundary_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "SUPPORTED_SUFFIX_BOUNDARY_PROBE_PREEXECUTION_STAGE9305.md"
...
COMMAND = [
    ...
    "--max-steps", "24", "--batch-size", "2", "--learning-rate", "1e-5",
    ...
    "--enable-generation-audit", "--max-generation-rows", "7", "--max-generation-tokens", "32",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9306_supported_suffix_boundary_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    failures: list[str] = []
    if source.get("passed") is not True or manifest_audit.get("passed") is not True:
        failures.append("source_stage9304_not_passed")
    if len(rows) != 7:
        failures.append("manifest_row_count_not_7")
    if manifest_audit.get("unsupported_eval_suffixes") or manifest_audit.get("unsupported_strict_suffixes"):
        failures.append("suffix_support_not_confirmed")
    ...
    required_pairs = {
        ...
        "--max-eval-rows": "1", "--max-strict-rows": "1", "--max-steps": "24",
...
def main() -> None:
    ...
    summary = {
        ...
        "metrics": {**{key: bool(card["authority"].get(key, False)) for key in AUTHORITY_CLOSED}, "command_ready": card["command_ready"], "will_execute_now": False, "authorized_next_stage": 9306, "manifest_rows": card["manifest_rows"], "tmpdir": card["tmpdir"], "required_postrun_artifact": card["required_postrun_artifact"], "failures": len(card["failures"])},
        "artifacts": {"audit": str(AUDIT.relative_to(ROOT)), "command": str(COMMAND_JSON.relative_to(ROOT)), "doc": str(DOC.relative_to(ROOT))},
        "decision": "Prepared supported-suffix target-100M boundary probe command with TMPDIR redirected to /data/tmp; not executed in this stage.",
        "next_best_step": "Execute Stage9306 supported-suffix boundary probe under trellis, then audit whether all supported suffixes generate exactly.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9305 Supported-Suffix Boundary Probe Preexecution", "", f"Passed: `{card['passed']}`", f"TMPDIR: `{card['tmpdir']}`", "Command artifact prepares Stage9306 but does not execute it.", "Required post-run artifact: `boundary_next_token_logits.jsonl`", ""]) , encoding="utf-8")
