...
STAGE = 9374
NAME = "stage9374_route2_filepath_residual_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9373_route2_filepath_residual_repair_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9373_route2_filepath_residual_repair_manifest/route2_filepath_residual_repair_manifest.jsonl"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9375_route2_filepath_residual_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "route2_filepath_residual_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9375_route2_filepath_residual_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "ROUTE2_FILEPATH_RESIDUAL_PROBE_PREEXECUTION_STAGE9374.md"
...
COMMAND = [
    ...
    "--max-train-rows", "11", "--max-eval-rows", "4", "--max-strict-rows", "11",
    ...
    "--enable-generation-audit", "--max-generation-rows", "26", "--max-generation-tokens", "64",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9375_route2_filepath_residual_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9373_not_passed")
    if len(rows) != 26:
        failures.append("unexpected_manifest_row_count")
    if dict(split_counts) != {"train": 11, "eval": 4, "strict_eval": 11}:
    ...
    if dict(task_counts) != {"file_path_verified_to_localized_repair": 14, "numeric_preserves_omission_repair": 12}:
        failures.append("unexpected_task_counts")
    if unsafe:
    ...
    return {"passed": not failures, "failures": failures, "manifest_rows": len(rows), "split_counts": dict(sorted(split_counts.items())), "route_counts": dict(sorted(route_counts.items())), "task_counts": dict(sorted(task_counts.items())), "unsafe_rows": sorted(set(unsafe)), "command": COMMAND, "command_ready": not failures, "authority": AUTHORITY_RUN if not failures else dict(AUTHORITY_CLOSED)}
...
def main() -> None:
    ...
    summary = {
        ...
        "metrics": {**{key: bool(card["authority"].get(key, False)) for key in AUTHORITY_CLOSED}, "command_ready": card["command_ready"], "manifest_rows": card["manifest_rows"], "split_counts": card["split_counts"], "route_counts": card["route_counts"], "task_counts": card["task_counts"], "failures": len(card["failures"])},
        "artifacts": {"audit": str(AUDIT.relative_to(ROOT)), "command": str(COMMAND_JSON.relative_to(ROOT)), "doc": str(DOC.relative_to(ROOT))},
        "decision": "Prepared a closed-boundary 26-row target-100M denoise probe for route_2 numeric and route_file_path residual repairs; decoder CE remains closed.",
        "next_best_step": "Execute Stage9375 route2/filepath residual probe under trellis, then audit before rejoining.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9374 Route2/Filepath Residual Probe Preexecution", "", f"Passed: `{card['passed']}`", f"Rows: `{card['manifest_rows']}`", f"Splits: `{card['split_counts']}`", f"Routes: `{card['route_counts']}`", f"Tasks: `{card['task_counts']}`", "", "Only the Stage9375 26-row target-100M denoise probe is authorized. Decoder CE and external authority remain closed.", ""]), encoding="utf-8")
