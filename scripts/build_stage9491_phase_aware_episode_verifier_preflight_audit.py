...
STAGE = 9491
NAME = "stage9491_phase_aware_episode_verifier_preflight_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9490_phase_aware_episode_verifier_manifest.json"
PREFLIGHT_DIR = ROOT / "runs/local/artifacts/stage9491_phase_aware_episode_verifier_target_100m_contract_preflight"
CONTRACT = PREFLIGHT_DIR / "probe_contract_audit.json"
AUDIT = PREFLIGHT_DIR / "stage9491_preflight_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "PHASE_AWARE_EPISODE_VERIFIER_PREFLIGHT_AUDIT_STAGE9491.md"
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9490_not_passed")
    ...
    audit = {
        "passed": not failures,
        "failures": failures,
        "source_stage": "stage9490_phase_aware_episode_verifier_manifest",
    ...
    summary = {
        ...
        "decision": "Authorized only Stage9492 phase-aware episode verifier target-100M structured execution if this preflight passes.",
        "next_best_step": "Run Stage9492 phase-aware episode verifier best-state-restored probe under trellis, then audit eval/strict exactness and safety artifacts.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    DOC.write_text("\n".join([
        "# Stage9491 Phase-Aware Episode Verifier Preflight Audit",
        "",
        ...
        "",
        "Only structured observe-phase verifier losses are open. Decoder CE, denoise CE, runtime, source/body emission, Gemma, harness, scoring, checkpoint export, controller merge, and promotion remain closed.",
        "",
    ]))
