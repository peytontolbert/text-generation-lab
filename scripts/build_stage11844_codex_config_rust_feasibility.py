#!/usr/bin/env python3
"""Capture codex-config Rust verifier feasibility."""
...
STAGE = 11844
NAME = "stage11844_codex_config_rust_feasibility"
OUT = ART / NAME
LOGS = OUT / "logs"
SUMMARY = OUT / "codex_config_rust_feasibility.json"
RESULTS = OUT / "codex_config_rust_feasibility_results.jsonl"
REPO = Path("/data/agentkernel/other_repos/codex/codex-rs")
SELECTED_TEST = "merge_toml_values_normalizes_legacy_key_from_base_layer"
SIBLING_TEST = "merge_toml_values_normalizes_legacy_key_from_overlay_layer"
...
def run_cargo(test_filter: str, name: str) -> dict[str, Any]:
    ...
    command = ["cargo", "test", "-p", "codex-config", test_filter, "--", "--nocapture"]
    env = os.environ.copy()
    env.update(
        {
    ...
    proc = subprocess.run(command, cwd=REPO, env=env, text=True, capture_output=True, timeout=300)
...
def main() -> None:
    selected = run_cargo(SELECTED_TEST, "selected_merge_legacy_base")
    sibling = run_cargo(SIBLING_TEST, "sibling_merge_legacy_overlay")
    passed = bool(selected["passed"] and sibling["passed"])
    row = {
        "stage": STAGE,
        "stage_name": NAME,
        "created_at_utc": now(),
        "repo_family": "codex_config",
        "repo_path": str(REPO),
        ...
        "source_path": "config/src/merge.rs",
        "distractor_source_path": "config/src/types.rs",
        "passed": passed,
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "codex_config_rust_feasibility_recorded",
        "passed": passed,
