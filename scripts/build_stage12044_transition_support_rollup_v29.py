#!/usr/bin/env python3
"""Roll Stage12043 build+run rows into support v29."""
...
ROOT = Path("runs/local/artifacts/stage12044_transition_support_rollup_v29")
BASE = Path("runs/local/artifacts/stage12042_transition_support_rollup_v28/transition_support_rows_v28.jsonl")
ADDED = Path("runs/local/artifacts/stage12043_buildrun_expansion_rows/buildrun_expansion_rows.jsonl")
ROWS_OUT = ROOT / "transition_support_rows_v29.jsonl"
SUMMARY = ROOT / "transition_support_rollup_v29.json"
SUMMARY_MIRROR = Path("runs/summaries/stage12044_transition_support_rollup_v29.json")
...
def main() -> None:
    ...
    duplicates = 0
    for source, path in [("stage12042", BASE), ("stage12043", ADDED)]:
        for row in read_jsonl(path):
            out = dict(row)
            out["stage12044_source"] = source
            out["split"] = "train"
            out["split_role"] = "stage12044_transition_support_v29_train_support_only"
    ...
    summary = {
        "stage": "stage12044_transition_support_rollup_v29",
        ...
        "buildrun_added": len(read_jsonl(ADDED)),
        "decision": "support_inventory_v29_not_train_ready" if not train_ready else "support_inventory_v29_floor_ready_requires_anti_cheat_audit",
        "reason": "Build+run coverage improved, but PASS_TO_PASS, FAIL_TO_PASS, INSUFFICIENT_EVIDENCE, NOT_EXERCISED, and remaining build+run floors remain unmet." if not train_ready else "Floor counts are met; run anti-cheat and split audits before training.",
        "claim_boundary": "Support inventory only. No model promotion or Gemma comparison should be claimed from v29.",
        "next_stage_recommendation": {"stage": "stage12045_status_floor_closure", "action": "Close remaining PASS_CURRENT_BUILD_AND_RUN, PASS_TO_PASS, FAIL_TO_PASS, INSUFFICIENT_EVIDENCE, and NOT_EXERCISED floors."},
    }
