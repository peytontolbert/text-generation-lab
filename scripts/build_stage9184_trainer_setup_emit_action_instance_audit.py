...
try:
    from scripts.build_stage9183_trainer_setup_emit_action_instance_design import (
        EXPECTED_OUTPUTS,
    ...
    from build_stage9183_trainer_setup_emit_action_instance_design import (  # type: ignore
        EXPECTED_OUTPUTS,
        REQUIRED_INSTANCE_FIELDS,
        REQUIRED_INSTANCE_GUARDS,
        STILL_BLOCKED_OUTPUTS,
...
STAGE = 9184
NAME = "stage9184_trainer_setup_emit_action_instance_audit"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
SOURCE_9183 = ROOT / "runs/summaries/stage9183_trainer_setup_emit_action_instance_design.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "TRAINER_SETUP_EMIT_ACTION_INSTANCE_AUDIT_STAGE9184.md"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "trainer_setup_emit_action_instance_audit.json"
...
def registry(latest: int = 9183) -> dict[str, Any]:
...
def build_audit(registry_card: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    source = load_json(SOURCE_9183)
    design = build_design({"metrics": {"latest_stage": 9182, "authority_counts": {key: 0 for key in AUTHORITY_CLOSED}}})
    design_failures = validate_design(design)
    negatives = run_negative_cases()
    checks = {
        "source_stage9183_passed": source.get("passed") is True,
        ...
        "registry_frontier_stage9183": int((registry_card.get("metrics") or {}).get("latest_stage", -1)) == 9183,
    ...
    return {
        ...
        "decision": "Audited the emit-action instance design and kept artifact emission and all execution paths closed.",
        "next_best_step": "Stop at the emit-action instance boundary until a deliberate local materializer execution review is intentionally opened.",
    }
...
def main() -> None:
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage9184 Trainer Setup Emit-Action Instance Audit",
                "",
                f"Passed: `{summary['passed']}`",
                "",
                "Audits the exact local emit-action instance without emitting outputs.",
                "",
