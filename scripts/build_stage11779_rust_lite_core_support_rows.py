#!/usr/bin/env python3
"""Materialize agent_kernel_lite_core Rust selected-verifier support rows."""
...
STAGE = 11779
NAME = "stage11779_rust_lite_core_support_rows"
OUT = ART / NAME
SUMMARY = OUT / "rust_lite_core_support_rows.json"
ROWS = OUT / "rust_lite_core_support_rows.jsonl"

FEASIBILITY = ART / "stage11778_rust_lite_core_verifier_feasibility/rust_lite_core_verifier_feasibility_results.jsonl"
...
def main() -> None:
    ...
    if not probes:
        raise SystemExit("no materializable agent_kernel_lite_core Rust probe found")
    probe = probes[0]
    ...
    root_id = "stage11779::agent_kernel_lite_core::rust::policy"
    evidence = [
        file_evidence(repo, "E01", "candidate_change_surface", source_path, "Rust policy source containing the selected inline tests and implementation surface."),
        file_evidence(repo, "E02", "nearby_source_distractor", distractor_source_path, "Nearby Rust context module used as a source distractor."),
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "third_rust_verifier_support_root_materialized",
        "passed": True,
        "row_count": len(rows),
        "root_count": 1,
        "repo_family": repo_family,
        ...
        "claim_boundary": [
            ...
            "This expands Rust verifier support beyond the prior single agent_kernel_rust_wasm repo family.",
        ],
    }
