...
STAGE = 9874
NAME = "stage9874_v27_margin_objective_multisurface_compiler_refresh"
...
DOC = ROOT / "docs" / "V27_MARGIN_OBJECTIVE_MULTISURFACE_COMPILER_REFRESH_STAGE9874.md"
...
SOURCES = {
    ...
    "edit_localization": {
        ...
        "path": ROOT / "runs/local/artifacts/stage9867_edit_localization_label_identity_probe/edit_localization_label_identity_manifest.jsonl",
        "target_loss": "edit_localization_ce",
        "source_audit": ROOT / "runs/summaries/stage9873_edit_localization_field_attention_margin_gemma_comparison.json",
    },
...
def _normalize_ready_row(row: dict[str, Any], skill: str, target_loss: str, source_audit: Path) -> dict[str, Any]:
    ...
    out["row_id"] = f"stage9874_{skill}_{row.get('row_id')}"
    ...
    anti["stage9874_margin_objective_refresh"] = True
    out["anti_cheat"] = anti
    return out
...
def main() -> None:
    ...
    next_step = "Use this refreshed compiled package as the next v2.7 training mix: the Stage9872 field-attention-plus-margin edit-localization objective is now the canonical multilingual edit-localization surface for broader standalone reruns."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Built a margin-objective successor to the v2.7 multisurface compiler refresh: the relabeled multilingual edit-localization packet with a verified Stage9872/9873 win replaces the older Stage9833 packet, while symbol binding and abstention-only patch/verifier guardrails stay unchanged.",
    ...
    DOC.write_text(
        "\n".join(
            [
                ...
                "",
                "This stage upgrades the v2.7 multisurface mix again: the stale opaque-choice edit-localization bank is replaced with the Stage9867 relabeled packet backed by the Stage9872 objective win and Stage9873 same-surface Gemma comparison, while the abstention guardrails remain intact.",
                "",
