#!/usr/bin/env python3
"""Capture einops Python verifier feasibility."""
...
STAGE = 11856
NAME = "stage11856_einops_python_feasibility"
OUT = ART / NAME
LOGS = OUT / "logs"
SUMMARY = OUT / "einops_python_feasibility.json"
RESULTS = OUT / "einops_python_feasibility_results.jsonl"
REPO = Path("/data/repositories/einops")
SELECTED_TEST = "einops/tests/test_parsing.py::test_elementary_axis_name"
SIBLING_TEST = "einops/tests/test_parsing.py::test_invalid_expressions"
...
def run_pytest(test_id: str, name: str) -> dict[str, Any]:
    ...
    command = ["conda", "run", "-n", "trellis", "pytest", "-q", test_id]
    env = os.environ.copy()
    env.update(
        {
...
def main() -> None:
    selected = run_pytest(SELECTED_TEST, "selected_elementary_axis_name")
    sibling = run_pytest(SIBLING_TEST, "sibling_invalid_expressions")
    ...
    row = {
        ...
        "source_path": "einops/parsing.py",
        "selected_test_path": "einops/tests/test_parsing.py",
        "distractor_source_path": "einops/einops.py",
        "passed": passed,
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "einops_python_feasibility_recorded",
        "passed": passed,
