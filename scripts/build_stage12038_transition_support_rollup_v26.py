#!/usr/bin/env python3
"""Roll Stage12037 verifier-mismatch rows into support v26."""
...
ROOT = Path("runs/local/artifacts/stage12038_transition_support_rollup_v26")
BASE = Path("runs/local/artifacts/stage12036_transition_support_rollup_v25/transition_support_rows_v25.jsonl")
ADDED = Path("runs/local/artifacts/stage12037_verifier_mismatch_not_exercised_rows/verifier_mismatch_not_exercised_rows.jsonl")
ROWS_OUT = ROOT / "transition_support_rows_v26.jsonl"
SUMMARY = ROOT / "transition_support_rollup_v26.json"
SUMMARY_MIRROR = Path("runs/summaries/stage12038_transition_support_rollup_v26.json")
...
def main() -> None:
    ...
    duplicates = 0
    for source, path in [("stage12036", BASE), ("stage12037", ADDED)]:
        for row in read_jsonl(path):
            out = dict(row)
            out["stage12038_source"] = source
            out["split"] = "train"
            out["split_role"] = "stage12038_transition_support_v26_train_support_only"
    ...
    summary = {
        ...
        "verifier_mismatch_not_exercised_added": len(read_jsonl(ADDED)),
        "decision": "support_inventory_v26_not_train_ready" if not train_ready else "support_inventory_v26_floor_ready_requires_anti_cheat_audit",
        "reason": "C/C++ and Rust language floors are closed, but status floors remain unmet."
        if not train_ready
        else "Floor counts are met; run anti-cheat and split audits before training.",
        "claim_boundary": "Support inventory only. No model promotion or Gemma comparison should be claimed from v26.",
        "next_stage_recommendation": {
            "stage": "stage12039_transition_root_250_status_floor_closure",
            "action": "Focus on PASS_TO_PASS, FAIL_TO_PASS, PASS_CURRENT_BUILD_AND_RUN, INSUFFICIENT_EVIDENCE, NOT_EXERCISED, and PASS_CURRENT_BUILD while reducing repo-family concentration.",
        },
    }
