#!/usr/bin/env python3
"""Emit a transition-projection joint semantic+transition head probe."""
...
STAGE = 11939
NAME = "stage11939_transition_joint_candidate_heads_probe_request"
OUT = ART / NAME
SUMMARY = OUT / "transition_joint_candidate_heads_probe_request.json"
COMMAND_JSON = OUT / "transition_joint_candidate_heads_command.json"
MANIFEST = OUT / "transition_joint_candidate_heads_manifest.jsonl"
...
RUN_DIR = ART / "stage11940_transition_joint_candidate_heads_probe"
...
def train_copy(row: dict[str, Any]) -> dict[str, Any]:
    out = dict(row)
    out["row_id"] = f"{row.get('row_id')}::stage11939_transition_joint_heads"
    out["split"] = "train"
    out["package_split"] = "train"
    out["train_support_only"] = True
    out["strict_eval_eligible"] = False
    out["stage11939_replay_kind"] = "transition_projection_joint_candidate_heads"
...
def main() -> None:
    ...
    gates = {
        ...
        "semantic_plus_transition_candidate_head_enabled": "encoder_option_retrieval_semantic_plus_transition_candidate_head" in command,
        ...
        "no_protected_rows_in_train": all(str(row.get("stage11939_replay_kind")) == "transition_projection_joint_candidate_heads" for row in train_rows),
    }
    artifact = {
        "stage": STAGE,
        "stage_name": NAME,
        ...
        "decision": "transition_joint_candidate_heads_probe_request_ready" if all(gates.values()) else "transition_joint_candidate_heads_probe_request_blocked",
        "command": command,
        ...
        "hypothesis": "Stage11924 trails Gemma because semantic candidate scoring and transition metadata need coordinated head updates rather than a frozen semantic head plus residual transition head.",
        "intervention": "Continue from Stage11924, train only semantic and transition candidate heads jointly under the composite scorer, retain transition contrast, and retain same-role plus verifier-value listwise losses.",
        "promotion_gate": {
            "routed_protected_gates_preserved": True,
            "transition_projection_accuracy_above_stage11924_364_of_640": "required",
            "same_manifest_gemma_386_of_640_beaten": "required_for_promotion",
        },
        "source_artifacts": {
        ...
        "claim_boundary": [
            ...
            "This is a transition-projection joint-head diagnostic and candidate only.",
