...
STAGE = 9736
NAME = "stage9736_multilingual_patch_operator_label_aligned_execution_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9735_multilingual_patch_operator_label_aligned_package.json"
BASELINE = ROOT / "runs/local/artifacts/stage9729_probe_smoke_patch_operator_exec/execution_result.json"
RUN_DIR = ROOT / "runs/local/artifacts/stage9736_multilingual_patch_operator_label_aligned_exec"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "multilingual_patch_operator_label_aligned_execution_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "MULTILINGUAL_PATCH_OPERATOR_LABEL_ALIGNED_EXECUTION_AUDIT_STAGE9736.md"
...
def _exact(result: dict[str, Any], split: str) -> float | None:
    ...
    metrics = field_card.get("patch_operator") if isinstance(field_card.get("patch_operator"), dict) else {}
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9735_not_passed")
    if not result:
    ...
    if result.get("train_rows") != 48 or result.get("eval_rows") != 48 or result.get("strict_rows") != 48:
    ...
    next_step = "If the label-aligned package improves over the Stage9729 zero-exact baseline, extend the same label-aligned intervention to verifier_repair and then reevaluate multilingual comparison readiness."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Audited the executed label-aligned multilingual patch-operator run against the Stage9729 zero-exact baseline.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9736 Multilingual Patch Operator Label-Aligned Execution Audit",
        "",
        ...
        "",
        "This stage compares the label-aligned multilingual patch-operator execution against the earlier zero-exact tiny-package baseline.",
        "",
        f"Next: {next_step}",
        "",
    ]), encoding="utf-8")
