...
STAGE = 11234
NAME = "stage11234_grouped_evidence_item_fact_probe_request"
OUT_DIR = ARTIFACTS / NAME
SUMMARY_JSON = OUT_DIR / "grouped_evidence_item_fact_probe_request.json"
COMMAND_JSON = OUT_DIR / "grouped_evidence_item_fact_probe_command.json"
MANIFEST = OUT_DIR / "grouped_evidence_item_fact_probe_manifest.jsonl"
...
ADDED = ARTIFACTS / "stage11233_grouped_evidence_item_fact_package/grouped_evidence_item_fact_train_rows.jsonl"
GROUPED_DIAG = ARTIFACTS / "stage11233_grouped_evidence_item_fact_package/grouped_evidence_item_fact_diagnostic_rows.jsonl"
PACKAGE_SUMMARY = ARTIFACTS / "stage11233_grouped_evidence_item_fact_package/grouped_evidence_item_fact_package.json"
...
RUN_DIR = ARTIFACTS / "stage11234_grouped_evidence_item_fact_probe"
...
def main() -> None:
    ...
    grouped_diag = load_jsonl(GROUPED_DIAG)

    eval_roots = {root_key(row) for row in validation + strict + residual + grouped_diag}
    added_roots = {root_key(row) for row in added}
    overlaps = sorted(added_roots & eval_roots)
    if overlaps:
        raise SystemExit(f"added rows overlap eval/residual/grouped diagnostic roots: {overlaps[:5]}")
    ...
    summary = {
        ...
        "passed": True,
        "decision": "grouped_evidence_item_fact_probe_requested",
        "claim_scope": [
            "Diagnostic-only probe: train on grouped evidence-item fact options with role aliases removed.",
            "Stage11233 has one blocked residual diagnostic row, so this cannot be promoted; it tests whether concrete fact options move the scorer.",
        ],
        "metrics": {
            ...
            "grouped_diag_rows_for_postrun": len(grouped_diag),
            "added_unique_roots": len(added_roots),
            "eval_root_overlaps": overlaps,
            "stage11233_passed": package.get("passed"),
            "stage11233_blocked_rows": ((package.get("counts") or {}).get("blocked_rows")),
        },
        "promotion_gate_for_postrun": {
            "clean_strict_must_remain": "22/22",
            "clean_residual_must_improve": ">5/10",
            "grouped_diag_should_be_nonzero": "diagnostic only, 8 clean rows",
        },
        "source_artifacts": {
