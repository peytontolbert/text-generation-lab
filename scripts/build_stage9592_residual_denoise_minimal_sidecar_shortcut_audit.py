...
STAGE = 9592
NAME = "stage9592_residual_denoise_minimal_sidecar_shortcut_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9591_residual_denoise_minimal_sidecar_manifest.json"
...
DOC = ROOT / "docs" / "RESIDUAL_DENOISE_MINIMAL_SIDECAR_SHORTCUT_AUDIT_STAGE9592.md"
...
FORBIDDEN_FEATURES = [
    ...
    "route",
    "objective_family",
    "model_input.language_family",
    "model_input.repair_route_family",
]
INTENDED_FEATURES = [
    "model_input.verifier_boundary_next_token_miss",
    "model_input.verifier_prefix_only_repair",
]
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9591_not_passed")
    if len(rows) != 96:
    ...
    summary = {
        ...
        "decision": "Audited the minimal sidecar manifest. Non-verifier shortcuts must stay at majority while verifier evidence remains the intended discriminator.",
        "next_best_step": "Run the capped suffix_choice sidecar probe over the minimal verifier-evidence manifest if the shortcut audit passed.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9592 Residual Denoise Minimal Sidecar Shortcut Audit", "", f"Passed: `{audit['passed']}`", f"Majority exact: `{majority}`", f"Strongest forbidden exact: `{strongest_forbidden}`", f"Strongest intended exact: `{strongest_intended}`", "", "The verifier evidence feature is allowed to solve the task; non-verifier shortcuts are not.", ""]), encoding="utf-8")
