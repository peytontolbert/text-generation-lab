...
STAGE = 9183
NAME = "stage9183_trainer_setup_emit_action_instance_design"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
SOURCE_9182 = ROOT / "runs/summaries/stage9182_trainer_setup_emit_action_opening_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "TRAINER_SETUP_EMIT_ACTION_INSTANCE_DESIGN_STAGE9183.md"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
DESIGN = OUT_DIR / "trainer_setup_emit_action_instance_design.json"

REQUIRED_INSTANCE_FIELDS = [
    "source_stage9182_audit_ref",
    "emit_action_opening_design_ref",
    "materializer_entrypoint_ref",
    "materializer_mode_ref",
...
REQUIRED_INSTANCE_GUARDS = [
    "instance_requires_stage9182_passed",
    "instance_requires_exact_materializer_entrypoint",
    "instance_requires_exact_materializer_mode",
...
def registry(latest: int = 9182) -> dict[str, Any]:
...
def build_design(registry_card: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    source = load_json(SOURCE_9182)
    return {
        "stage": STAGE,
        "stage_name": NAME,
        "design_type": "trainer_setup_emit_action_instance_v1",
        "source_stage9182_passed": source.get("passed") is True,
        "registry_frontier_stage9182": int((registry_card.get("metrics") or {}).get("latest_stage", -1)) == 9182,
        ...
        "decision": (
            "Designed the deliberate emit-action instance. The instance may pin the exact local materializer "
            "argv, local output directory, and post-emit hash/cardinality/provenance audits, while keeping "
            "trainer invocation and all model execution paths closed."
        ),
        "next_best_step": "Audit the emit-action instance design; still do not emit outputs.",
    }
...
def validate_design(design: dict[str, Any]) -> list[str]:
    ...
    if design.get("source_stage9182_passed") is not True or metrics.get("source_stage9182_passed") is not True:
        failures.append("source_stage9182_not_passed")
    if design.get("registry_frontier_stage9182") is not True or metrics.get("registry_frontier_stage9182") is not True:
        failures.append("registry_frontier_stage9182")
...
def run_negative_cases() -> dict[str, Any]:
    ...
    for case in NEGATIVE_CASES:
        ...
        if case == "source_stage_missing":
            design["source_stage9182_passed"] = False
            design["metrics"]["source_stage9182_passed"] = False
        elif case == "registry_frontier_bad":
            design["registry_frontier_stage9182"] = False
            design["metrics"]["registry_frontier_stage9182"] = False
        elif case == "missing_instance_field":
            design["required_instance_fields"].remove("materializer_exact_argv_ref")
...
def main() -> None:
    ...
    artifact = {
        ...
        "decision": design["decision"] if passed else "Emit-action instance design failed validation.",
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage9183 Trainer Setup Emit-Action Instance Design",
                "",
                f"Passed: `{summary['passed']}`",
                "",
                "Designs the exact local emit-action instance without emitting outputs.",
                "",
