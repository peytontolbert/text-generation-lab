...
STAGE = 9516
NAME = "stage9516_episode_obs_diag_counterbalance_repair_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9515_episode_obs_diag_counterbalance_repair_preflight_audit.json"
RUN_DIR = ROOT / "runs/local/artifacts/stage9516_episode_obs_diag_counterbalance_repair_target_100m_probe"
...
AUDIT = RUN_DIR / "stage9516_episode_obs_diag_counterbalance_repair_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "EPISODE_OBS_DIAG_COUNTERBALANCE_REPAIR_PROBE_AUDIT_STAGE9516.md"
...
def main() -> None:
    ...
    if source.get("passed") is not True or not source.get("metrics", {}).get("execution_authorized_for_next_stage"):
        safety_failures.append("stage9515_not_authorized")
    ...
    audit = {
        ...
        "source_stage": "stage9515_episode_obs_diag_counterbalance_repair_preflight_audit",
        ...
        "diagnosis": "Safe quality failure. Primitive observation features lowered eval loss but did not fix exactness; the model still overweights surface/language priors on the same eval rows. Next repair should isolate observation-diagnosis into observation-dominant contrast rows.",
    ...
    summary = {
        ...
        "decision": "Counterbalance repair probe failed quality safely; do not broaden execution or reconnect decoder.",
        "next_best_step": "Build Stage9517 observation-dominant diagnosis contrast manifest that removes surface-text priors and trains only from primitive observation facts.",
