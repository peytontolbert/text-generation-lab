...
STAGE = 9816
NAME = "stage9816_web_isolated_gemma_comparison"
MANIFEST = ROOT / "runs/local/artifacts/stage9813_web_isolated_disambiguator_surface/web_isolated_disambiguator_surface.jsonl"
MODEL_ROWS = ROOT / "runs/local/artifacts/stage9815_web_isolated_structured_exec/row_field_logits.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "web_isolated_gemma_comparison.json"
ROWS_OUT = OUT_DIR / "web_isolated_gemma_rows.jsonl"
...
build_prompt = _load_symbol("stage9816_stage9748_runner", ROOT / "scripts/run_stage9748_standalone_gemma_queue_via_ollama.py", "build_prompt")
...
def main() -> None:
    ...
    next_step = "If Stage9816 preserves the four-language Gemma win on the cleaner isolated-field surface, attach it into the review path and prefer it over Stage9807 for future anti-cheat packaging."
    summary = {"stage": STAGE, "stage_name": NAME, "name": NAME, "passed": True, "authority": dict(AUTHORITY_CLOSED), "metrics": {**dict(AUTHORITY_CLOSED), "wins_100m": wins_100m, "wins_gemma": wins_gemma, "ties": ties}, "artifacts": {"audit": str(AUDIT.relative_to(ROOT)), "rows": str(ROWS_OUT.relative_to(ROOT))}, "decision": "Compared deterministic Gemma against the Stage9813 web-isolated structured surface using the same strict-eval rows as the follow-up 100M run.", "next_best_step": next_step, "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}
