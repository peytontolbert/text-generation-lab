...
STAGE = "stage12378_web_fallback_task_specific_selected_test_rerender"
OUT = ROOT / "runs/local/artifacts" / STAGE
SUMMARY = ROOT / "runs/summaries" / f"{STAGE}.json"
SOURCE = ROOT / "runs/local/artifacts/stage12351_web_fallback_selected_test_admissions/web_fallback_selected_test_train_support_rows.jsonl"
TASKS = [
...
def evidence_refs(input_text: str) -> list[str]:
    ...
    try:
        ...
        refs = [raw]
    return [ref for ref in refs if isinstance(ref, str) and not ref.endswith("::None") and "::None" not in ref]
...
def task_specs(task: str, selected_summary: str) -> tuple[list[tuple[str, str]], str, str]:
    if task == "transition_candidate_selection":
        return (
            [
                ("candidate_selected_test_backed", "candidate is supported by compact focused JS/TS verifier evidence"),
                ("candidate_source_surface_only", "candidate relies on Web source hashes without selected-test support"),
    ...
    if task == "transition_next_action":
        return (
            [
                ("action_run_selected_verifier", "run or rely on the focused JS/TS verifier before claiming support"),
    ...
    if task == "transition_continue_or_stop":
        return (
            [
                ...
                ("policy_stop_on_source_surface_only", "stop using Web source hashes alone"),
            ],
    ...
    if task == "transition_verifier_transition":
        return (
            [
                ("status_pass_current_state", "focused local JS/TS verifier passed in the current state"),
    ...
    if task == "transition_evidence_citation":
        return (
            [
                ...
                ("evidence_dependency_or_neighbor", "dependency or neighboring Web artifact is decisive"),
            ],
...
def render_input(row: dict[str, Any], task: str, options: list[dict[str, Any]], refs: list[str]) -> str:
    ...
    source_summary = field(original, "Source hash summary:")
    test_summary = field(original, "Test hash summary:")
    ...
    return (
        ...
        f"Test hash summary: {test_summary}\n"
...
def main() -> None:
    ...
    for source_row in source_rows:
        ...
        refs = evidence_refs(source_row.get("input_text") or "")
        row_blockers = blockers(source_row, refs)
        root_tasks = {row.get("task_family") for row in by_root.get(source_row.get("root_id") or "", [])}
        ...
        rendered = {
            ...
            "source_stage": "stage12351_web_fallback_selected_test_admissions",
            ...
            "admission": {
                ...
                "reason": "Task-specific Web fallback selected-test train-support only under Stage12364 diversity-weighted policy.",
            },
            "blocked_reasons": row_blockers,
        }
        (admitted if not row_blockers else blocked).append(rendered)
    with (OUT / "web_fallback_task_specific_selected_test_rows.jsonl").open("w", encoding="utf-8") as handle:
        for row in admitted:
            handle.write(json.dumps(row, sort_keys=True) + "\n")
    with (OUT / "web_fallback_task_specific_selected_test_blocked_rows.jsonl").open("w", encoding="utf-8") as handle:
        for row in blocked:
            handle.write(json.dumps(row, sort_keys=True) + "\n")
    summary = {
        "stage": STAGE,
        "decision": "web_fallback_task_specific_selected_test_rerender_complete" if admitted else "web_fallback_task_specific_selected_test_rerender_blocked",
        "training_allowed": bool(admitted),
        "claim_boundary": "Train-support-only Web fallback task-specific selected-test re-render. No strict/source-heldout/Level-3/patch/repair claim.",
        ...
        "language_family_counts": dict(Counter(row.get("language_family") for row in admitted)),
        ...
        "stage12364_policy": "MCP/Web rows admitted only as transformed train-support with diversity weighting; no direct source-heldout/product claim.",
    }
    (OUT / "web_fallback_task_specific_selected_test_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
