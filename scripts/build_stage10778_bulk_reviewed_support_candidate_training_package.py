...
STAGE = 10778
NAME = "stage10778_bulk_reviewed_support_candidate_training_package"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
SUMMARY_JSON = OUT_DIR / "bulk_reviewed_support_candidate_training_package.json"
...
GOLD_INDEX = ROOT / "runs/local/artifacts/stage10777_bulk_support_candidate_ai_gold_drafts/bulk_support_candidate_ai_gold_index.jsonl"
...
def build_row(index_row: dict[str, Any], bundle: dict[str, Any], enriched: dict[str, Any], entry: dict[str, Any]) -> dict[str, Any] | None:
    ...
    anti_cheat = {
        ...
        "bulk_stage_support_candidate": True,
    }
    ...
    return {
        ...
        "query_text": f"stage10778::{language_family}::{perspective}",
        "repo_family": str(bundle["repo_family"]),
        "repo_id": str(bundle["repo_family"]),
        "route": "bulk_support_candidate_enrichment",
        "row_id": f"stage10778::{bundle['root_id']}::{perspective}::support_candidate",
        ...
        "standalone_projection_source": {
            ...
            "projection_mode": "stage10778_bulk_support_candidate_competition",
...
def main() -> None:
    ...
    support_rows.sort(key=lambda r: (r["language_family"], r["repo_family"], r["perspective"], r["row_id"]))
    ...
    payload = {
        ...
        "passed": True,
        "decision": "bulk_reviewed_support_candidate_training_package_ready",
        "claim_scope": [
            "Project the 18 enriched C/C++ and Python support candidates into a real support-only multilingual training package.",
        ...
        "headline_findings": [
            "The strongest current C/C++ and Python support candidates are now represented as concrete multilingual train-support rows.",
            "Eval and strict slices remain untouched, so this package preserves the current honest reviewed-v2.7 frontier.",
            "This is the first direct path from the bulk root materialization effort into an executable multilingual support package.",
        ],
        "anti_cheat_contract": [
        ...
        "source_artifacts": {
            ...
            "bulk_support_candidate_gold_index": display(GOLD_INDEX),
        },
        "outputs": {
