#!/usr/bin/env python3
"""Materialize codex-agent-identity Rust selected-verifier support rows."""
...
STAGE = 11821
NAME = "stage11821_codex_agent_identity_rust_support_rows"
OUT = ART / NAME
SUMMARY = OUT / "codex_agent_identity_rust_support_rows.json"
ROWS = OUT / "codex_agent_identity_rust_support_rows.jsonl"
FEASIBILITY = ART / "stage11820_codex_agent_identity_rust_feasibility/codex_agent_identity_rust_feasibility_results.jsonl"
...
def file_evidence(repo: Path, eid: str, kind: str, rel_path: str, summary: str, start: int = 1, end: int = 760) -> dict[str, Any]:
...
def main() -> None:
    ...
    if not probes:
        raise SystemExit("no materializable codex-agent-identity Rust probe found")
    probe = probes[0]
    ...
    root_id = "stage11821::codex_agent_identity::rust::jwks_url_support"
    evidence = [
        file_evidence(repo, "E01", "candidate_change_surface", source_path, "Agent identity source includes JWKS URL construction and inline unit tests for backend-api and codex-api base URLs."),
        file_evidence(repo, "E02", "nearby_source_distractor", distractor_source_path, "Program resolver source is adjacent Rust infrastructure but not the selected JWKS URL verifier target."),
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "fifth_rust_verifier_support_root_materialized",
        "passed": True,
        "row_count": len(rows),
        "root_count": 1,
        "repo_family": repo_family,
        ...
        "claim_boundary": ["Rows are train-support-only.", "This expands Rust verifier support with a non-tokenizers Codex agent-identity repo family."],
    }
