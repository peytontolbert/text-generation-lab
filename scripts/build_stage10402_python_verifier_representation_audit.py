...
NAME = "stage10402_python_verifier_representation_audit"
...
RUNTIME_BUNDLE = ROOT / "runs/local/artifacts/stage10397_multilingual_residual_contrast_probe/runtime_model/runtime_model_bundle.json"
TARGET_ROW_ID = (
...
def _test_focus_text(path_value: str) -> str:
    return f"Selected test under review: {_basename(path_value)}"


def _module_focus_text(path_value: str) -> str:
    return f"Selected test under review: {_parent_basename(path_value)}"


def _repo_focus_text(path_value: str) -> str:
    return f"Selected verifier target under review: {_moduleish(path_value)}"
...
def _option_text(value: str, variant: str) -> str:
    ...
    if variant == "basename_templated":
        return f"Verifier target under review: {_basename(value)}"
    if variant == "stem_templated":
        return f"Verifier target under review: {_stem(value)}"
    if variant == "parent_templated":
        return f"Verifier target under review: {_parent_basename(value)}"
    if variant == "moduleish_templated":
        return f"Verifier target under review: {_moduleish(value)}"
    if variant == "test_focus":
        return _test_focus_text(value)
    if variant == "module_focus":
        return _module_focus_text(value)
    if variant == "repo_focus":
        return _repo_focus_text(value)
...
def main() -> None:
    ...
    out_path = OUT_DIR / "python_verifier_representation_audit.json"
    out_path.write_text(
        json.dumps(
            {
                "stage_name": NAME,
