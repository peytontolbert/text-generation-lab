...
STAGE = 9336
NAME = "stage9336_targeted_repair_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9335_targeted_repair_probe_preexecution.json"
...
RUN_DIR = ROOT / "runs/local/artifacts/stage9336_targeted_anti_insertion_repetition_probe"
AUDIT = RUN_DIR / "stage9336_targeted_repair_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "TARGETED_REPAIR_PROBE_AUDIT_STAGE9336.md"
...
def audit_run() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9335_not_passed")
    ...
    quality_gate_passed = bool(generated == 21 and exact == 21 and target_prefix == 21 and boundary == 21 and contentful == 21 and repetition_rows == 0 and short_rows == 0 and leak_rows == 0)
    return {
        ...
        "diagnosis": "targeted_bad_output_repair_rows_solved_stage9333_residual_insertions_and_repetitions_in_isolation",
        "next_patch_target": "stage9337_merge_targeted_repair_rows_with_routed_ladder_and_reprobe_interference",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "The targeted anti-insertion/repetition repair probe passed exactly in isolation; decoder CE remains closed.",
        "next_best_step": "Merge Stage9334 targeted repair rows back with the Stage9331 routed ladder and rerun an interference probe before any bounded decoder CE step.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9336 Targeted Repair Probe Audit",
        "",
        ...
        "",
        "The repair objective corrected the Stage9333 residual `keeps the`, `verified preserves`, and `operatch` failures in isolation.",
