...
STAGE = 9511
NAME = "stage9511_episode_obs_diag_overlay_wide_eval_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9510_episode_obs_diag_overlay_wide_eval_preflight_audit.json"
RUN_DIR = ROOT / "runs/local/artifacts/stage9511_episode_obs_diag_overlay_wide_eval_target_100m_probe"
...
AUDIT = RUN_DIR / "stage9511_episode_obs_diag_overlay_wide_eval_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "EPISODE_OBS_DIAG_OVERLAY_WIDE_EVAL_PROBE_AUDIT_STAGE9511.md"
...
def main() -> None:
    ...
    if source.get("passed") is not True or not source.get("metrics", {}).get("execution_authorized_for_next_stage"):
        safety_failures.append("stage9510_not_authorized")
    ...
    audit = {
        ...
        "source_stage": "stage9510_episode_obs_diag_overlay_wide_eval_preflight_audit",
        ...
        "diagnosis": "Safe quality failure. Widening eval/strict exposed eval generalization errors hidden by the one-row Stage9507 diagnostic. Failure is concentrated in eval failure_type, repair_outcome, and step_value while strict remains exact.",
    ...
    summary = {
        ...
        "decision": "Widened observation-diagnosis overlay probe failed quality safely; do not broaden execution or reconnect decoder.",
        "next_best_step": "Build Stage9512 eval-error attribution and counterbalanced observation-diagnosis repair rows for the widened eval failures.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    DOC.write_text("\n".join([
        "# Stage9511 Episode Observation Diagnosis Overlay Wide-Eval Probe Audit",
        "",
    ...
    print(json.dumps({"stage": STAGE, "passed": audit["passed"], "safety_passed": audit["safety_passed"], "final_eval": audit["final_eval_joint_proxy_exact"], "final_strict": audit["final_strict_joint_proxy_exact"], "wrong_rows": len(wrong_rows), "failures": failures}, indent=2, sort_keys=True))
