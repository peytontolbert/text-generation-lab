#!/usr/bin/env python3
"""Roll Stage12013 C++/Rust expansion rows into transition support v14."""
...
ROOT = Path("runs/local/artifacts/stage12014_transition_support_rollup_v14")
BASE = Path("runs/local/artifacts/stage12012_transition_support_rollup_v13/transition_support_rows_v13.jsonl")
ADDED = Path("runs/local/artifacts/stage12013_cpp_rust_positive_expansion_rows/cpp_rust_positive_expansion_rows.jsonl")
ROWS_OUT = ROOT / "transition_support_rows_v14.jsonl"
SUMMARY = ROOT / "transition_support_rollup_v14.json"
SUMMARY_MIRROR = Path("runs/summaries/stage12014_transition_support_rollup_v14.json")
...
def main() -> None:
    ...
    duplicates = 0
    for source, path in [("stage12012", BASE), ("stage12013", ADDED)]:
        for row in read_jsonl(path):
            out = dict(row)
            out["stage12014_source"] = source
            out["split"] = "train"
            out["split_role"] = "stage12014_transition_support_v14_train_support_only"
    ...
    summary = {
        "stage": "stage12014_transition_support_rollup_v14",
        ...
        "decision": "support_inventory_v14_not_train_ready",
        "reason": "C++/Rust executable positives improved, but PASS_TO_PASS and NOT_EXERCISED floors remain far short.",
        "claim_boundary": "Support inventory only. No model promotion or Gemma comparison should be claimed from v14.",
        "next_stage_recommendation": {"stage": "stage12015_transition_root_250_not_exercised_and_pass_to_pass_expansion", "action": "Continue mining independent NOT_EXERCISED and PASS_TO_PASS roots; then consider a trainability gate when inventory is materially closer to floors."},
    }
