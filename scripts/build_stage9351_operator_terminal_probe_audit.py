...
STAGE = 9351
NAME = "stage9351_operator_terminal_probe_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9350_operator_terminal_probe_preexecution.json"
...
RUN_DIR = ROOT / "runs/local/artifacts/stage9351_operator_terminal_probe"
AUDIT = RUN_DIR / "stage9351_operator_terminal_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "OPERATOR_TERMINAL_PROBE_AUDIT_STAGE9351.md"
...
def audit_run() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9350_not_passed")
    ...
    quality_gate_passed = bool(generated == 17 and exact == 17 and target_prefix == 17 and boundary == 17 and contentful == 17 and repetition_rows == 0 and short_rows == 0 and leak_rows == 0)
    return {
        ...
        "diagnosis": "operator-terminal anti-repetition rows removed the Stage9348 route_1 `operatch` repetition in isolation.",
        "next_patch_target": "stage9352_full_mixture_with_schema_and_operator_terminal_repairs_manifest",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "The operator-terminal anti-repetition probe passed exactly in isolation; decoder CE remains closed.",
        "next_best_step": "Merge the Stage9346 schema-normalized bridge rows plus Stage9349 operator-terminal rows back into the full mixture and run another closed rejoin probe before decoder CE.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9351 Operator Terminal Probe Audit", "", f"Passed: `{audit['passed']}`", f"Safety gate passed: `{audit['safety_gate_passed']}`", f"Quality gate passed: `{audit['quality_gate_passed']}`", f"Exact match rows: `{audit['exact_match_rows']}` / `{audit['generated_rows']}`", f"Target-prefix rows: `{audit['target_prefix_match_rows']}` / `{audit['generated_rows']}`", f"Boundary next-token rows: `{audit['boundary_next_token_match_rows']}` / `{audit['generated_rows']}`", f"Contentful rows: `{audit['contentful_rows']}` / `{audit['generated_rows']}`", f"Degenerate repetition rows: `{audit['degenerate_repetition_rows']}`", "", "The route_1 `operatch` terminal repetition is solved in isolation. Decoder CE, runtime, harness, Gemma, scoring, promotion, and checkpoint export stayed closed.", ""]), encoding="utf-8")
