...
STAGE = 9498
NAME = "stage9498_boundary_counterbalance_preflight_audit"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9497_boundary_negative_counterbalance_manifest.json"
PREFLIGHT_DIR = ROOT / "runs/local/artifacts/stage9498_boundary_counterbalance_target_100m_contract_preflight"
CONTRACT = PREFLIGHT_DIR / "probe_contract_audit.json"
AUDIT = PREFLIGHT_DIR / "stage9498_preflight_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "BOUNDARY_COUNTERBALANCE_PREFLIGHT_AUDIT_STAGE9498.md"
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9497_not_passed")
    ...
    if contract.get("rows") != 26 or contract.get("split_counts") != {"eval": 4, "other": 0, "strict_eval": 4, "train": 18}:
    ...
    if loss_counts.get("episode_boundary_match_ce") != 26:
    ...
    audit = {
        "passed": not failures,
        "failures": failures,
        "source_stage": "stage9497_boundary_negative_counterbalance_manifest",
    ...
    summary = {
        ...
        "decision": "Authorized only Stage9499 counterbalanced boundary verifier target-100M structured execution if this preflight passes.",
        "next_best_step": "Run Stage9499 counterbalanced boundary verifier probe under trellis, then audit exactness and high-confidence wrong rows.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    DOC.write_text("\n".join([
        "# Stage9498 Boundary Counterbalance Preflight Audit",
        "",
