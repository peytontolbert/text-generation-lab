...
STAGE = 9332
NAME = "stage9332_routed_ladder_probe_preexecution"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9331_routed_ladder_suffix_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9331_routed_ladder_suffix_manifest/routed_ladder_suffix_manifest.jsonl"
OUTPUT_DIR = ROOT / "runs/local/artifacts/stage9333_routed_ladder_suffix_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "routed_ladder_probe_preexecution_audit.json"
COMMAND_JSON = OUT_DIR / "stage9333_routed_ladder_suffix_probe_command.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "ROUTED_LADDER_PROBE_PREEXECUTION_STAGE9332.md"
...
COMMAND = [
    ...
    "--max-train-rows", "31", "--max-eval-rows", "13", "--max-strict-rows", "12",
    ...
    "--enable-generation-audit", "--max-generation-rows", "56", "--max-generation-tokens", "64",
    ...
    "--execution-authorized-for-recovery-probe", "--output-dir", str(OUTPUT_DIR), "--run-id", "stage9333_routed_ladder_suffix_probe",
]
...
def audit() -> dict[str, Any]:
    ...
    if source.get("passed") is not True:
        failures.append("source_stage9331_not_passed")
    if len(rows) != 56:
        failures.append("manifest_row_count_not_56")
    ...
    required_pairs = {
        "--mode": "denoise_repair_probe", "--probe-scale": "target_100m", "--max-train-rows": "31",
        "--max-eval-rows": "13", "--max-strict-rows": "12", "--max-steps": "80",
        "--max-generation-rows": "56", "--decoder-ce-weight": "0.0", "--structured-aux-weight": "0.0",
...
def main() -> None:
    ...
    summary = {
        ...
        "decision": "Prepared the routed ladder 56-row target-100M denoise probe; decoder CE remains closed.",
        "next_best_step": "Execute Stage9333 routed ladder suffix probe under trellis and audit source/phrase/ladder quality.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9332 Routed Ladder Probe Preexecution",
        "",
