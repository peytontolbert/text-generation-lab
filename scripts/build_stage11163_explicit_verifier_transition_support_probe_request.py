...
STAGE = 11163
NAME = "stage11163_explicit_verifier_transition_support_probe_request"
OUT_DIR = ARTIFACTS / NAME
SUMMARY_JSON = OUT_DIR / "explicit_verifier_transition_support_probe_request.json"
COMMAND_JSON = OUT_DIR / "explicit_verifier_transition_support_probe_command.json"
MANIFEST_JSONL = OUT_DIR / "explicit_verifier_transition_support_probe_manifest.jsonl"

PACKAGE_DIR = ARTIFACTS / "stage11162_cleaned_plus_explicit_verifier_transition_support_package"
PACKAGE_JSON = PACKAGE_DIR / "cleaned_plus_explicit_verifier_transition_support_package.json"
...
ADDED_ROWS_JSONL = PACKAGE_DIR / "added_explicit_verifier_transition_rows.jsonl"
...
PROBE_OUT_DIR = ARTIFACTS / "stage11164_explicit_verifier_transition_support_probe" / "bounded_decoder_probe"
RUNTIME_OUT_DIR = ARTIFACTS / "stage11164_explicit_verifier_transition_support_probe" / "runtime_model"
...
def main() -> None:
    ...
    summary = {
        ...
        "passed": True,
        "decision": "explicit_verifier_transition_support_probe_requested",
        "claim_scope": [
            "Run one diagnostic bounded probe with 45 admitted explicit verifier-transition competition support rows.",
            "This is not a promotion run; added rows are train-support-only local source-backed rows.",
        ],
        "source_artifacts": {
        ...
        "metrics": {
            ...
            "explicit_transition_rows_added": len(added_rows),
            "explicit_transition_unique_roots": len({str(row.get("source_root_id") or "") for row in added_rows}),
            "explicit_transition_semantic_targets": package.get("metrics", {}).get("added_by_semantic_target"),
        },
        "required_honesty_gates": [
            "Strict and validation are unchanged from stage11146 and remain canary surfaces.",
            "Added explicit verifier-transition rows are train-support-only and cannot count as heldout improvement.",
            "A useful result must preserve strict >= 21/22 and improve either validation, the Python verifier miss, or reserved residual bank.",
            "If flat, prioritize fresh external/repo-disjoint verifier-transition roots rather than more local support variants.",
        ],
        "next_best_step": "Execute this request, then run a postrun audit against cleaned validation/strict plus reserved residual candidates.",
        "outputs": {
