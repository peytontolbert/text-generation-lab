...
STAGE = 9576
NAME = "stage9576_residual_denoise_boundary_interleaved_capped_probe"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9575_residual_denoise_boundary_interleaved_manifest.json"
MANIFEST = ROOT / "runs/local/artifacts/stage9575_residual_denoise_boundary_interleaved_manifest/residual_denoise_boundary_interleaved_manifest.jsonl"
RUN_ROOT = ROOT / "runs/local/artifacts/stage9576_residual_denoise_boundary_interleaved_capped_probe"
CONTRACT_DIR = RUN_ROOT / "contract_preflight"
RUN_DIR = RUN_ROOT / "denoise_repair_probe"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
AUDIT = OUT_DIR / "residual_denoise_boundary_interleaved_capped_probe_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "RESIDUAL_DENOISE_BOUNDARY_INTERLEAVED_CAPPED_PROBE_STAGE9576.md"
...
def main() -> None:
    ...
    if source.get("passed") is not True:
        failures.append("stage9575_not_passed")
    if (source.get("metrics") or {}).get("first_16_class_counts") != {"BOUNDARY": 8, "PREFIX": 8}:
        failures.append("stage9575_first_generation_window_not_balanced")
    ...
    if not failures:
        contract = subprocess.run(command(output_dir=CONTRACT_DIR, run_id="stage9576_contract_preflight", contract_only=True), cwd=ROOT, text=True, capture_output=True, check=False)
    ...
    if not failures:
        run = subprocess.run(command(output_dir=RUN_DIR, run_id="stage9576_boundary_interleaved_probe", contract_only=False), cwd=ROOT, text=True, capture_output=True, check=False)
        if run.returncode != 0:
    ...
    class_gate_passed = sample_balanced and exact_rate >= 0.85 and boundary_recall is not None and boundary_recall >= 0.85 and prefix_recall is not None and prefix_recall >= 0.85
    ...
    summary = {
        ...
        "decision": "Ran a capped target_100M probe over the interleaved boundary-contrast residual-denoise manifest. Widening remains closed.",
        "next_best_step": "Audit class gate. If boundary and prefix recall pass, build a non-widening integration ticket back into the residual-denoise repair curriculum.",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join(["# Stage9576 Residual Denoise Boundary Interleaved Capped Probe", "", f"Passed: `{audit['passed']}`", f"Sample balanced: `{sample_balanced}`", f"Exact rate: `{exact_rate}`", f"Boundary recall: `{boundary_recall}`", f"Prefix recall: `{prefix_recall}`", f"Class gate passed: `{class_gate_passed}`", "", "This remains a capped diagnostic; widening and external authorities stay closed.", ""]), encoding="utf-8")
    update_registry(summary)
    print(json.dumps({"stage": STAGE, "passed": audit["passed"], "sample_balanced": sample_balanced, "exact_rate": exact_rate, "boundary_recall": boundary_recall, "prefix_recall": prefix_recall, "class_gate_passed": class_gate_passed, "failures": failures}, indent=2, sort_keys=True))
    if not audit["passed"]:
        raise SystemExit(1)
