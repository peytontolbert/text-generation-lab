...
STAGE = 10341
NAME = "stage10341_unresolved_family_focus_execution_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
MANIFEST_JSONL = OUT_DIR / "unresolved_family_focus_manifest.jsonl"
REQUEST_JSON = OUT_DIR / "unresolved_family_focus_execution_request.json"
COMMAND_JSON = OUT_DIR / "unresolved_family_focus_command.json"
...
RUN_ID = "stage10342_unresolved_family_focus_probe"
OUTPUT_DIR = "runs/local/artifacts/stage10342_unresolved_family_focus_probe/bounded_decoder_probe"
RUNTIME_MODEL_DIR = "runs/local/artifacts/stage10342_unresolved_family_focus_probe/runtime_model"
MAX_STEPS = 56
LEARNING_RATE = "8e-6"
BATCH_SIZE = 2
...
def main() -> None:
    ...
    unresolved_base_rows = [
        row
        for row in base_rows
        if row.get("split") == "train"
        and (
            (row.get("language_family") == "c_cpp" and row.get("task_type") == "evidence_citation")
            or (row.get("language_family") == "rust" and row.get("task_type") == "evidence_citation")
            or (row.get("language_family") == "python" and row.get("task_type") == "verifier_outcome")
        )
    ]

    frontloaded_rows = web_support_rows + python_candidate_rows + unresolved_base_rows
    ...
    request = {
        ...
        "passed": True,
        "decision": "unresolved_family_focus_successor_ready",
        ...
        "frontloaded_support_rows": {
            ...
            "unresolved_base_rows": len(unresolved_base_rows),
        },
        "max_steps": MAX_STEPS,
        ...
        "required_honesty_gates": [
            ...
            "only unresolved-family base rows are retained to test whether cyclic label offsets come from train interference",
        ],
        "next_best_step": "run one unresolved-family-focus probe and check whether the remaining cyclic offsets disappear when unrelated train interference is removed",
        "command": command,
