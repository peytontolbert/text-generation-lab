...
STAGE = 9858
NAME = "stage9858_validity_weighted_target_100m_contract_only_preflight"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage9857_v27_validity_weighted_multisurface_compiler_refresh.json"
STRUCTURED = ROOT / "runs/local/artifacts/stage9857_v27_validity_weighted_multisurface_compiler_refresh/compiled/structured_state.jsonl"
BOUNDED = ROOT / "runs/local/artifacts/stage9857_v27_validity_weighted_multisurface_compiler_refresh/compiled/bounded_decoder_ce.jsonl"
...
DOC = ROOT / "docs" / "VALIDITY_WEIGHTED_TARGET_100M_CONTRACT_ONLY_PREFLIGHT_STAGE9858.md"
...
def required_artifacts(*, decoder: bool, contract_only: bool) -> list[str]:
    if contract_only:
        return list(REQUIRED_PREFLIGHT_ARTIFACTS)
    if decoder:
        return REQUIRED_PREFLIGHT_ARTIFACTS + REQUIRED_EXECUTION_ARTIFACTS + [
...
def run_surface(surface: str, manifest: Path) -> dict[str, Any]:
    ...
    missing = [name for name in required_artifacts(decoder=decoder, contract_only=True) if not (out_dir / name).exists()]
    failures: list[str] = []
    if run.returncode != 0:
