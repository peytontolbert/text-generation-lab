#!/usr/bin/env python3
"""Materialize llm-memory-modules-at-scale Python verifier support rows."""
...
STAGE = 11811
NAME = "stage11811_llm_memory_python_support_rows"
OUT = ART / NAME
SUMMARY = OUT / "llm_memory_python_support_rows.json"
ROWS = OUT / "llm_memory_python_support_rows.jsonl"
FEASIBILITY = ART / "stage11810_llm_memory_python_feasibility/llm_memory_python_feasibility_results.jsonl"
...
def main() -> None:
    feasible = [row for row in read_jsonl(FEASIBILITY) if row.get("repo_family") == "llm_memory_modules_at_scale" and row.get("passed") is True]
    if not feasible:
        raise SystemExit("llm-memory feasibility did not pass")
    probe = feasible[0]
    root_id = "stage11811::llm_memory_modules_at_scale::python::product_key_memory_support"
    evidence = [
        file_evidence("E01", "implementation_source", "src/models/memory.py", "ProductKeyMemory projects queries, retrieves top-k memory values, applies optional masks, and returns output plus attention."),
        file_evidence("E02", "selected_test_anchor", "tests/test_memory.py", "Memory tests verify forward output shape, attention shape/sum, and masked attention behavior."),
        file_evidence("E03", "nearby_source_distractor", "src/models/transformer.py", "Transformer model source is adjacent model code but not the selected ProductKeyMemory verifier target."),
        file_evidence("E04", "nearby_source_distractor", "src/training/trainer.py", "Training loop code is adjacent but not the selected memory-layer verifier target."),
        log_evidence("V01", probe["selected_result"], "Selected ProductKeyMemory forward pytest passed."),
        log_evidence("V02", probe["sibling_result"], "Sibling ProductKeyMemory masking pytest passed."),
    ]
    verifier_evidence = {
        "id": "V01",
        "kind": "actual_pytest_pass_log",
        "summary": "Selected and sibling ProductKeyMemory pytest functions passed.",
    ...
    verifier_options = [
        ...
        {"label": "D", "role": "nearby_source_distractor", "value": "src/models/transformer.py", "evidence_ids": ["E03"]},
    ]
    evidence_options = [
        {"label": "A", "role": "candidate_change_surface", "value": "src/models/memory.py", "evidence_ids": ["E01"]},
