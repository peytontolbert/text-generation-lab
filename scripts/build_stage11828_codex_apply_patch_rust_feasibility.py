#!/usr/bin/env python3
"""Capture codex-apply-patch Rust verifier feasibility."""
...
STAGE = 11828
NAME = "stage11828_codex_apply_patch_rust_feasibility"
OUT = ART / NAME
LOGS = OUT / "logs"
SUMMARY = OUT / "codex_apply_patch_rust_feasibility.json"
RESULTS = OUT / "codex_apply_patch_rust_feasibility_results.jsonl"
REPO = Path("/data/agentkernel/other_repos/codex/codex-rs")
SELECTED_TEST = "test_add_file_hunk_creates_file_with_contents"
SIBLING_TEST = "test_update_file_hunk_modifies_content"
...
def run_cargo(test_filter: str, name: str) -> dict[str, Any]:
    ...
    command = ["cargo", "test", "-p", "codex-apply-patch", test_filter, "--", "--nocapture"]
    env = os.environ.copy()
    env.update(
        {
...
def main() -> None:
    selected = run_cargo(SELECTED_TEST, "selected_add_file_hunk")
    sibling = run_cargo(SIBLING_TEST, "sibling_update_file_hunk")
    passed = bool(selected["passed"] and sibling["passed"])
    row = {
        "stage": STAGE,
        "stage_name": NAME,
        "created_at_utc": now(),
        "repo_family": "codex_apply_patch",
        "repo_path": str(REPO),
        ...
        "source_path": "apply-patch/src/lib.rs",
        "distractor_source_path": "apply-patch/src/invocation.rs",
        "passed": passed,
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "codex_apply_patch_rust_feasibility_recorded",
        "passed": passed,
