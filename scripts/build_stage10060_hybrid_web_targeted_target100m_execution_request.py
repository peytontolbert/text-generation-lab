...
STAGE = 10060
NAME = "stage10060_hybrid_web_targeted_target100m_execution_request"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
REQUEST = OUT_DIR / "hybrid_web_targeted_target100m_execution_request.json"
SURFACE_REQUEST = OUT_DIR / "surface_requests" / "edit_localization.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "HYBRID_WEB_TARGETED_TARGET100M_EXECUTION_REQUEST_STAGE10060.md"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
MANIFEST = ROOT / "runs/local/artifacts/stage10059_hybrid_web_targeted_geometry_successor_packet/hybrid_web_targeted_geometry_successor_manifest.jsonl"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage10059_hybrid_web_targeted_geometry_successor_packet.json"
...
FUTURE_RUN_ID = "stage10060_hybrid_web_targeted_target100m_probe"
FUTURE_OUTPUT_DIR = "runs/local/artifacts/stage10060_hybrid_web_targeted_target100m_probe/edit_localization_probe"
...
def build_request() -> dict[str, Any]:
    ...
    stage10059_train_rows = sum(
        1
        for row in rows
        if str(row.get("split") or "") == "train"
        and (
            str(row.get("stage10056_role") or "") == "train"
            or str(row.get("stage10053_role") or "") == "train"
        )
    )
    ...
    metrics = {
        ...
        "stage10059_train_rows": stage10059_train_rows,
    }
    if metrics["rows"] != 124:
        failures.append("rows_not_124")
    ...
    if stage10059_train_rows != 21:
        failures.append("stage10059_train_rows_not_21")
    if heldout_rows != 55:
        failures.append("heldout_compare_rows_not_55")
    if source_summary.get("passed") is not True:
        failures.append("stage10059_not_passed")
    ...
    req = {
        ...
        "required_contract_invariants": {
            "source_stage10059_passed": bool(source_summary.get("passed") is True),
    ...
    return {
        ...
        "metrics": metrics,
        "source_artifacts": {
            "stage10059_packet_summary": display(SOURCE_SUMMARY),
        ...
        "execution_constraints": {
            ...
            "hybrid_web_targeted_packet_required": True,
        },
...
def main() -> None:
    ...
    next_step = "Use this request to run one target-100M edit-localization probe on the hybrid web-targeted successor manifest, then compare it against stage10040 and Gemma on the same 55 heldout rows."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Prepared the execution request for the hybrid web-targeted successor so the next probe can test whether stage10056 rust/cpp recovery can be combined with the stronger stage10053 web behavior without giving back Python.",
    ...
    DOC.write_text(
        "\n".join(
            [
                "# Stage10060 Hybrid Web Targeted Target100M Execution Request",
                "",
