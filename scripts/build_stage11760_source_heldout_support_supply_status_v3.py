#!/usr/bin/env python3
"""Refresh source-heldout failure support supply after Rust verifier admission."""
...
STAGE = 11760
NAME = "stage11760_source_heldout_support_supply_status_v3"
OUT = ART / NAME
SUMMARY = OUT / "source_heldout_support_supply_status_v3.json"
...
RUST_VERIFIER = ART / "stage11759_rust_verifier_support_admission_audit/rust_verifier_support_admission_audit.json"
...
def main() -> None:
    ...
    cpp = read_json(CPP)
    rust_verifier = read_json(RUST_VERIFIER)
    ...
    rust_roots = int(rust_verifier.get("admitted_root_count", 0))
    ...
    support_status = {
        ...
        "rust_verifier_support": {
            ...
            "ready_rows": rust_verifier.get("admitted_rows", 0),
            "required_support_roots": 12,
            "remaining_support_roots": max(0, 12 - rust_roots),
            "strict_analogue_roots_ready": 0,
            "required_strict_analogue_roots": 4,
            "materialized_rows": rust_verifier.get("outputs", {}).get("admitted_rows"),
            "repo_family_warning": "Current true Rust verifier roots are from one repo family; more repo families are needed.",
        },
    ...
    training_probe_ready = python_roots >= 12 and cpp_roots >= 16 and rust_roots >= 12
    artifact: dict[str, Any] = {
        ...
        "delta_since_stage11756": {
            "rust_verifier_ready_roots_added": rust_roots,
            "rust_verifier_ready_rows_added": rust_verifier.get("admitted_rows", 0),
        },
        "next_actions": [
            "Find more non-tokenizers Rust repo families for selected-inline-test verifier support.",
            "Continue Python verifier support expansion beyond 2/12 roots.",
            "Continue C/C++ build/verifier expansion beyond 1/16 roots.",
            "Do not run a promotion training probe until support geometry is materially closer to quota.",
        ],
        "claim_boundary": [
        ...
        "source_artifacts": {
            ...
            "cpp_support": rel(CPP),
            "rust_verifier_support": rel(RUST_VERIFIER),
