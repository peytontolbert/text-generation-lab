...
STAGE = 9216
NAME = "stage9216_repo_local_denoise_one_run_ticket_schema"
REGISTRY = ROOT / "runs/local/artifacts/reconstructed_stage_registry.json"
SOURCE_9215 = ROOT / "runs/summaries/stage9215_repo_local_bounded_decoder_one_run_ticket_audit.json"
SOURCE_8884 = ROOT / "runs/summaries/stage8884_no_execution_denoise_authorization_review.json"
...
DOC = ROOT / "docs" / "REPO_LOCAL_DENOISE_ONE_RUN_TICKET_SCHEMA_STAGE9216.md"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
TICKET = OUT_DIR / "repo_local_denoise_one_run_ticket_inactive.json"
AUDIT = OUT_DIR / "repo_local_denoise_one_run_ticket_schema_audit.json"
...
def denoise_review(matrix: dict[str, Any]) -> dict[str, Any]:
    for item in matrix.get("family_reviews") or []:
        if item.get("mode") == "denoise_repair_probe":
            return item
    return {}
...
def build_ticket(review: dict[str, Any]) -> dict[str, Any]:
    ...
    ticket = {
        "ticket_id": "stage9216_repo_local_denoise_one_run_ticket__inactive",
        "ticket_status": "DESIGN_ONLY_INACTIVE",
        "requested_stage": "future_unassigned",
        "requested_stage_name": "future_repo_local_denoise_repair_one_run",
        "requested_capability": "tiny_denoise_repair_probe",
        "source_stage": 9213,
        "source_manifest_path": manifest_path,
        "future_output_dir": str(ROOT / "runs/local/probes/stage9216_denoise_repair_one_run_candidate"),
...
def audit_ticket(ticket: dict[str, Any], source_9215: dict[str, Any], source_8884: dict[str, Any], matrix: dict[str, Any], review: dict[str, Any]) -> dict[str, Any]:
    failures = audit_diagnostic_ticket_fields(ticket)
    if source_9215.get("passed") is not True:
        failures.append("source_stage9215_not_passed")
    if source_8884.get("passed") is not True:
        failures.append("source_stage8884_not_passed")
    if matrix.get("passed") is not True:
        failures.append("matrix_not_passed")
    if not review:
        failures.append("denoise_review_missing")
    if review.get("mode") != "denoise_repair_probe":
        failures.append("wrong_review_mode")
    if review.get("status") != "requires_dedicated_one_run_ticket_schema":
        failures.append("denoise_review_not_schema_blocked")
    if "dedicated_one_run_ticket_schema_missing" not in (review.get("review_failures") or []):
        failures.append("denoise_schema_missing_failure_absent")
...
def main() -> None:
    ...
    audit = audit_ticket(ticket, source_9215, source_8884, matrix, review)
    ...
    summary = {
        ...
        "decision": (
            "Designed an inactive repo-local denoise one-run ticket schema from the Stage9211 capped denoise manifest. "
            "No command is materialized and no execution, runtime verifier, cleanup, decoder CE, denoise CE execution, "
            "source/body, Gemma, scoring, or mining authority is opened."
        ) if audit["passed"] else "Repo-local denoise one-run ticket schema failed.",
        "next_best_step": "Audit the inactive Stage9216 denoise ticket schema with negative cases. Keep execution closed.",
