...
STAGE = 9921
NAME = "stage9921_hardened_weighted_multilingual_winner_review_packets"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
PACKETS = OUT_DIR / "hardened_weighted_multilingual_winner_review_packets.jsonl"
MANIFEST = OUT_DIR / "hardened_weighted_multilingual_winner_review_packet_manifest.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "HARDENED_WEIGHTED_MULTILINGUAL_WINNER_REVIEW_PACKETS_STAGE9921.md"
...
SHORTCUT_AUDIT = ROOT / "runs/local/artifacts/stage9920_hardened_weighted_edit_localization_shortcut_audit/hardened_weighted_edit_localization_shortcut_audit.json"
EXECUTION = ROOT / "runs/local/artifacts/stage9917_hardened_weighted_structured_execution_review/surface_runs/edit_localization/execution_result.json"
...
def _recommended_subskills(strict_exact: float, gemma_exact: float) -> dict[str, dict[str, Any]]:
    direct_true = {
        "understands_user_intent": "same-surface strict exact is above Gemma on the weighted hardened opaque-choice packet and the task scope is narrow and explicit",
        "retrieves_source_evidence_when_needed": "the weighted packet is evidence-grounded and the model beat Gemma on the same strict rows without a visible label list",
        "binds_symbols_correctly": "the structured localization choice is correct often enough to beat Gemma on the weighted hardened packet",
        "localizes_edit_scope": "the attached weighted hardened multilingual winner localizes the edit target more accurately than Gemma",
        ...
        "avoids_hallucinated_symbols": "the weighted hardened structured outputs stay inside the opaque A-D target set",
        "avoids_internal_tokens": "the prompt still does not print the valid-label list and the outputs remain clean label tokens only",
        "produces_contentful_final_answer": "the model emitted valid opaque choice labels and beat Gemma on the same weighted hardened packet",
    }
    tentative = {
    ...
    for name in RUBRIC_SUBSKILLS:
        if name in direct_true:
            out[name] = {
                ...
                "reviewer_notes": [
                    direct_true[name],
                    f"Weighted hardened same-surface strict exact is {strict_exact} versus Gemma {gemma_exact}.",
                ],
            }
...
def _challenge_recommendations(prompt_leak_free: bool, unique_maps: int, strict_exact: float, gemma_exact: float) -> list[dict[str, Any]]:
    ...
    for challenge in CHALLENGE_FAMILIES:
        ...
        if challenge == "hidden_reference_materialization":
            recommended = True
            confidence = "medium"
            notes = ["Human review should still confirm source lineage, but the weighted hardened prompt surface does not expose a valid-label list."]
        elif challenge == "target_and_teacher_leakage":
            recommended = True if prompt_leak_free else False
            confidence = "high"
            notes = [f"Prompt label exposure buckets: {0 if prompt_leak_free else 1}.", "The weighted hardened comparison uses the same no-label-list prompt for both models."]
            ...
            recommended = None
            confidence = "requires_human_confirmation"
            notes = ["This packet hardens prompt-label leakage, but separate metadata-only baselines are still not attached on the weighted hardened surface."]
            ...
            recommended = True
            confidence = "high"
            notes = [f"Weighted hardened same-surface strict exact is {strict_exact} versus Gemma {gemma_exact}.", "Both models were scored on the same opaque-choice packet and same prompt format."]
...
def build_packets() -> dict[str, Any]:
    ...
    for lang in LANGS:
        ...
        rubric_stub = {
            ...
            "required_human_action": "assign rubric subskill judgments using the weighted hardened Stage9919 same-surface evidence",
            ...
            "supporting_evidence_paths": {
                ...
                "same_surface_rows_100m": str(Path("runs/local/artifacts/stage9917_hardened_weighted_structured_execution_review/surface_runs/edit_localization/row_field_logits.jsonl")),
                "same_surface_rows_gemma12b": str(Path("runs/local/artifacts/stage9919_hardened_weighted_edit_localization_gemma_comparison/hardened_weighted_edit_localization_gemma_rows.jsonl")),
        ...
        rubric_draft = {
            ...
            "reviewer_message": "This is a narrow weighted hardened same-surface edit-localization result. Confirm the attached evidence is sufficient before marking any rubric line passed.",
            "authority": dict(AUTHORITY_CLOSED),
        }
        anti_draft = {
            "cell_key": cell_key,
            ...
            "reviewer_message": "This draft only addresses the weighted hardened opaque-choice surface. It does not generalize to broader v2.7 capability claims.",
            "authority": dict(AUTHORITY_CLOSED),
        }
        ...
        rows.append(
            {
                ...
                "mode": "same_surface_hardened_weighted_opaque_choice",
                "same_surface_eval_exact_100m": eval_exact,
                "same_surface_strict_exact_100m": strict_exact,
                "same_surface_strict_exact_gemma12b": gemma_exact,
                "same_surface_verdict": strict_card.get("verdict"),
    ...
    metrics = {
        ...
        "hardened_weighted_same_surface_win_cells": sum(1 for row in rows if row["same_surface_verdict"] == "100m_better"),
        "prompt_label_exposure_buckets": 0 if prompt_leak_free else 1,
        "unique_permutation_maps": unique_maps,
    }
    if metrics["review_cells"] != 4:
        failures.append("review_cells_not_4")
    if metrics["hardened_weighted_same_surface_win_cells"] != 4:
        failures.append("hardened_weighted_same_surface_win_cells_not_4")
...
def main() -> None:
    ...
    next_step = "Use the weighted hardened review packets as the expert-maintainer and anti-cheat entry point for the current truthful integrated multilingual edit-localization winner, then fill signed human judgments before promoting this beyond a model-vs-model artifact."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Packaged the four weighted hardened multilingual same-surface winner cells into expert-maintainer and anti-cheat review packets using the new integrated Gemma comparison plus the matching shortcut audit.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    DOC.write_text("\n".join([
        "# Stage9921 Hardened Weighted Multilingual Winner Review Packets",
        "",
        f"Passed: `{summary['passed']}`",
        f"Review cells: `{built['metrics']['review_cells']}`",
        f"Weighted multilingual same-surface win cells: `{built['metrics']['hardened_weighted_same_surface_win_cells']}`",
        "",
        summary["decision"],
        "",
        f"Next: {next_step}",
    ...
    print(json.dumps({"stage": STAGE, "passed": summary["passed"], "review_cells": built["metrics"]["review_cells"], "weighted_same_surface_win_cells": built["metrics"]["hardened_weighted_same_surface_win_cells"]}, indent=2, sort_keys=True))
