...
STAGE = 10085
NAME = "stage10085_canonical_label_aligned_source_heldout_same_manifest_gemma_queue"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
QUEUE = OUT_DIR / "canonical_label_aligned_source_heldout_same_manifest_gemma_queue.json"
PACKETS = OUT_DIR / "canonical_label_aligned_source_heldout_same_manifest_gemma_packets.jsonl"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "CANONICAL_LABEL_ALIGNED_SOURCE_HELDOUT_SAME_MANIFEST_GEMMA_QUEUE_STAGE10085.md"
...
OUTPUT = ROOT / "runs/local/artifacts/stage10085_canonical_label_aligned_source_heldout_gemma_execution/same_prompt_surface_gemma12b_outputs.json"
...
def build_queue() -> dict[str, Any]:
    ...
    packet = {
        "cell_key": "canonical_label_aligned_source_heldout_target100m::edit_localization::same_manifest_gemma12b",
        "language_family": "multilingual",
        "skill_area": "edit_localization",
        "review_packet_paths": {
            "same_prompt_surface_gemma12b_outputs": display(OUTPUT),
            "shortcut_validity_audit": "runs/local/artifacts/stage10082_canonical_label_aligned_shortcut_validity_audit/canonical_label_aligned_shortcut_validity_audit.json",
        },
        "same_surface_packet": {
            "row_count": len(rows),
            "row_ids": row_ids,
        ...
        "queue_entries": [
            {
                ...
                "language_family": "multilingual",
                "priority_rank": 1,
                "priority_reason": "same-manifest Gemma comparison for the canonical label-aligned source-heldout frontier",
...
def main() -> None:
    ...
    next_step = "Run the local Ollama Gemma comparator on this canonical source-heldout same-manifest queue, then compare it directly against the matching 100M probe on the same 55 heldout rows."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "decision": "Materialized a same-manifest Gemma queue for the canonical source-heldout multilingual rows so Gemma can be compared fairly on the honest heldout bank after label-semantic alignment.",
    ...
    DOC.write_text("\n".join([
        ...
        f"Heldout rows: `{built['metrics']['rows']}`",
        "",
        summary["decision"],
        "",
        f"Next: {next_step}",
