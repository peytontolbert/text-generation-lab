#!/usr/bin/env python3
"""Emit a transition-projection semantic-head probe with transition-specific contrast loss."""
...
STAGE = 11918
NAME = "stage11918_transition_projection_contrast_head_only_probe_request"
OUT = ART / NAME
SUMMARY = OUT / "transition_projection_contrast_head_only_probe_request.json"
COMMAND_JSON = OUT / "transition_projection_contrast_head_only_command.json"
MANIFEST = OUT / "transition_projection_contrast_head_only_manifest.jsonl"
...
RUN_DIR = ART / "stage11919_transition_projection_contrast_head_only_probe"
...
def train_copy(row: dict[str, Any]) -> dict[str, Any]:
    out = dict(row)
    out["row_id"] = f"{row.get('row_id')}::stage11918_transition_contrast"
    out["split"] = "train"
    out["package_split"] = "train"
    out["train_support_only"] = True
    out["strict_eval_eligible"] = False
    out["stage11918_replay_kind"] = "transition_projection_contrast"
...
def main() -> None:
    ...
    gates = {
        ...
        "contrast_enabled": "--bounded-choice-contrast-weight" in command and "0.5" in command,
        "contrast_margin_enabled": "--bounded-choice-contrast-margin" in command and "0.08" in command,
        ...
        "no_protected_rows_in_train": all(str(row.get("stage11918_replay_kind")) == "transition_projection_contrast" for row in train_rows),
    }
    artifact = {
        "stage": STAGE,
        "stage_name": NAME,
        ...
        "decision": "transition_projection_contrast_head_only_probe_request_ready" if all(gates.values()) else "transition_projection_contrast_head_only_probe_request_blocked",
        "command": command,
        ...
        "hypothesis": "Transition projection loses to Gemma because the semantic candidate head lacks explicit same-row margins for transition next-action, verifier-transition, continue/stop, and candidate-selection distractors.",
        "intervention": "Train only the semantic candidate head on the 640 heldout-safe transition projections with transition-specific contrast specs now applicable to all 640 rows.",
        "promotion_gate": {
            "routed_protected_gates_preserved": True,
            "transition_projection_accuracy_above_stage11912": "required",
