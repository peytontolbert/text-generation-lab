#!/usr/bin/env python3
"""Capture codex-rmcp-client Rust verifier feasibility."""
...
STAGE = 11832
NAME = "stage11832_codex_rmcp_client_rust_feasibility"
OUT = ART / NAME
LOGS = OUT / "logs"
SUMMARY = OUT / "codex_rmcp_client_rust_feasibility.json"
RESULTS = OUT / "codex_rmcp_client_rust_feasibility_results.jsonl"
REPO = Path("/data/agentkernel/other_repos/codex/codex-rs")
SELECTED_TEST = "test_unix_executes_script_without_extension"
SIBLING_TEST = "test_resolved_program_executes_successfully"
...
def run_cargo(test_filter: str, name: str) -> dict[str, Any]:
    ...
    command = ["cargo", "test", "-p", "codex-rmcp-client", test_filter, "--", "--nocapture"]
    env = os.environ.copy()
    env.update(
        {
...
def main() -> None:
    selected = run_cargo(SELECTED_TEST, "selected_unix_executes_script")
    sibling = run_cargo(SIBLING_TEST, "sibling_resolved_program_executes")
    passed = bool(selected["passed"] and sibling["passed"])
    row = {
        "stage": STAGE,
        "stage_name": NAME,
        "created_at_utc": now(),
        "repo_family": "codex_rmcp_client",
        "repo_path": str(REPO),
        ...
        "source_path": "rmcp-client/src/program_resolver.rs",
        "distractor_source_path": "rmcp-client/src/stdio_server_launcher.rs",
        "passed": passed,
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "codex_rmcp_client_rust_feasibility_recorded",
        "passed": passed,
