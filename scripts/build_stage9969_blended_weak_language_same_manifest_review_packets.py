...
STAGE = 9969
NAME = "stage9969_blended_weak_language_same_manifest_review_packets"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
PACKETS = OUT_DIR / "blended_weak_language_same_manifest_review_packets.jsonl"
MANIFEST = OUT_DIR / "blended_weak_language_same_manifest_review_manifest.json"
AUDIT = OUT_DIR / "blended_weak_language_same_manifest_review_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "BLENDED_WEAK_LANGUAGE_SAME_MANIFEST_REVIEW_PACKETS_STAGE9969.md"
...
def packet_dir(language: str) -> Path:
    return OUT_DIR / "review_packets" / f"blended_target100m__{language}__edit_localization"
...
def recommendation_subskills(language: str, compare_count: int) -> dict[str, dict[str, Any]]:
    supported_note = (
        f"This same-manifest weak-language packet exposes evidence-grounded opaque edit-localization choices for `{language}` "
        f"across `{compare_count}` compare rows, so this rubric line becomes scoreable once real Stage9965 and Stage9971 outputs exist."
    )
...
def challenge_rows(language: str, metrics: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "confidence": "medium",
            "reviewer_notes": [
                "Stage9968 binds the future 100M and Gemma runs to the same manifest and same compare-row ids.",
                "Final fairness still requires the real Stage9965 and Stage9971 outputs to land on that exact manifest.",
            ],
        },
        {
...
def build_packets() -> dict[str, Any]:
    ...
    for language in LANGS:
        ...
        rubric_stub = {
            "cell_key": f"blended_target100m::{language}::edit_localization::same_manifest_review",
            "language_family": language,
            "status": "pending_real_outputs_and_human_review",
            "rubric_version": "expert_maintainer_v1",
            "passed": False,
            "requires_real_stage9965_outputs": True,
            "requires_real_stage9971_outputs": True,
            ...
            "supporting_evidence_paths": {
                ...
                "future_stage9965_output_dir": str((handoff.get("handoff_bundle") or handoff).get("hundred_m_execution", {}).get("future_output_dir") or row_contract.get("future_output_dir") or ""),
                "future_stage9971_output_stub": str((handoff.get("handoff_bundle") or handoff).get("gemma_execution", {}).get("future_output_stub") or ""),
            },
        ...
        anti_stub = {
            ...
            "required_human_action": "Confirm the machine-supported anti-cheat signals on the exact same-manifest packet, then recheck them against real Stage9965 and Stage9971 outputs.",
            "supporting_evidence_paths": {
                "same_manifest_request": display(REQUEST),
                "same_manifest_handoff_bundle": display(HANDOFF),
            },
        ...
        rubric_draft = {
            ...
            "pending_requirements": [
                "real_stage9965_100m_outputs",
                "real_stage9971_gemma_outputs",
        ...
        anti_draft = {
            ...
            "summary": (
                ...
                "the real Stage9965 and Stage9971 outputs on this exact manifest."
            ),
...
def main() -> None:
    ...
    next_step = (
        "Use these same-manifest weak-language review packets to keep expert-maintainer and anti-cheat review aligned with the exact Stage9965/Stage9971 comparison surface, then attach real outputs there before making any broader multilingual claim."
    )
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        ...
        "decision": "Materialized reviewer-facing same-manifest weak-language packets that tie expert-maintainer scope and anti-cheat checks to the exact Stage9965/Stage9971 comparison surface without pretending the real model outputs already exist.",
        "next_best_step": next_step,
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    }
    write_json(SUMMARY, summary)
    DOC.write_text("\n".join([
        "# Stage9969 Blended Weak-Language Same-Manifest Review Packets",
        "",
