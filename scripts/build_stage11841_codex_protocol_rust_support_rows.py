#!/usr/bin/env python3
"""Materialize codex-protocol Rust selected-verifier support rows."""
...
STAGE = 11841
NAME = "stage11841_codex_protocol_rust_support_rows"
OUT = ART / NAME
SUMMARY = OUT / "codex_protocol_rust_support_rows.json"
ROWS = OUT / "codex_protocol_rust_support_rows.jsonl"
FEASIBILITY = ART / "stage11840_codex_protocol_rust_feasibility/codex_protocol_rust_feasibility_results.jsonl"
...
def main() -> None:
    ...
    if not probes:
        raise SystemExit("no materializable codex-protocol Rust probe found")
    probe = probes[0]
    ...
    root_id = "stage11841::codex_protocol::rust::agent_path_support"
    evidence = [
        file_evidence(repo, "E01", "candidate_change_surface", source_path, "Codex protocol AgentPath source defines agent path join and resolve behavior covered by focused unit tests."),
        file_evidence(repo, "E02", "nearby_source_distractor", distractor_source_path, "ThreadId protocol source is nearby protocol logic but not the selected AgentPath verifier target."),
    ...
    artifact = {
        ...
        "stage_name": NAME,
        "created_at_utc": now(),
        "decision": "ninth_rust_verifier_support_root_materialized",
        "passed": True,
        "row_count": len(rows),
        "root_count": 1,
        "repo_family": repo_family,
        ...
        "claim_boundary": ["Rows are train-support-only.", "This expands Rust verifier support with a non-tokenizers Codex protocol repo family."],
    }
