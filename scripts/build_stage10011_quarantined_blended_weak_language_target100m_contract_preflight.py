...
STAGE = 10011
NAME = "stage10011_quarantined_blended_weak_language_target100m_contract_preflight"
SOURCE_SUMMARY = ROOT / "runs/summaries/stage10010_quarantined_blended_weak_language_successor_mix.json"
STRUCTURED = ROOT / "runs/local/artifacts/stage10010_quarantined_blended_weak_language_successor_mix/quarantined_blended_weak_language_successor_structured_state.jsonl"
OUT_DIR = ROOT / "runs/local/artifacts" / NAME
MANIFEST_DIR = OUT_DIR / "manifests"
RUNS_DIR = OUT_DIR / "contract_runs"
AUDIT = OUT_DIR / "quarantined_blended_weak_language_target100m_contract_preflight_audit.json"
SUMMARY = ROOT / "runs/summaries" / f"{NAME}.json"
DOC = ROOT / "docs" / "QUARANTINED_BLENDED_WEAK_LANGUAGE_TARGET100M_CONTRACT_PREFLIGHT_STAGE10011.md"
...
def main() -> None:
    ...
    next_step = "Use these preflighted manifests as the next capped target-100M structured run contract so the weak-language replay path inherits the stage10009 quarantine and does not reintroduce unresolved Gemma-advantage rows."
    summary = {
        "stage": STAGE,
        "stage_name": NAME,
        "name": NAME,
        ...
        "metrics": {
            **dict(AUTHORITY_CLOSED),
            ...
            "aggregate_loss_counts": audit["aggregate_loss_counts"],
            "model_execution_attempted_rows": audit["model_execution_attempted_rows"],
            "authority_rows": audit["authority_rows"],
            "unsafe_loss_rows": audit["unsafe_loss_rows"],
            "failures": audit["failures"],
        },
        "artifacts": {"audit": display(AUDIT), "manifest_dir": display(MANIFEST_DIR), "run_dir": display(RUNS_DIR), "doc": display(DOC)},
        "decision": "Ran target-100M contract-only trainer preflights for each surface on the quarantined weak-language successor mix; no model execution or training was authorized.",
    ...
    DOC.write_text("\n".join([
        ...
        f"Model execution attempted rows: `{audit['model_execution_attempted_rows']}`",
    ...
    print(json.dumps({"stage": STAGE, "passed": summary["passed"], "metrics": summary["metrics"], "failures": audit["failures"]}, indent=2, sort_keys=True))
    if audit["failures"]:
        raise SystemExit(1)
