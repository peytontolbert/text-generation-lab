# Stage10130 True Source-Backed First-Wave Scoring Contract

Passed: `True`
Admitted bundles: `0`
Score rows: `0`

Flatten the admitted first-wave multilingual root bundles into perspective-scoring rows only after completed rubric, anti-cheat, and gold adjudication, while preserving root-level solved semantics as the primary maintainer-grade metric.

Primary metric: `root_solved_rate`

Next: Complete the Stage10128 first-wave rubric, anti-cheat, and perspective-gold tasks, rerun Stage10129 to admit bundles, then use these flattened scoring rows for one bounded 100M-versus-Gemma first-wave maintainer comparison.

