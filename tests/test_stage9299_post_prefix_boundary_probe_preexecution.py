...
def _load():
    ...
    path = root / "scripts/build_stage9299_post_prefix_boundary_probe_preexecution.py"
    spec = importlib.util.spec_from_file_location("stage9299", path)
...
def test_stage9299_prepares_post_prefix_boundary_probe_command_with_data_tmp():
    mod = _load()
    card = mod.audit()
    assert card["passed"] is True
    assert card["command_ready"] is True
    assert card["will_execute_now"] is False
    assert card["authorized_next_stage"] == 9300
