...
def _load():
    ...
    path = root / "scripts/build_stage9697_disk_safe_cleanup_execution_readiness_gate.py"
...
def test_future_command_uses_stage9698_and_no_decoder():
    mod = _load()
    cmd = mod.future_command()
    joined = " ".join(cmd)
    assert "stage9698_symbol_binding_target_100m_structured_tiny_probe" in joined
    assert "stage9696_" not in joined
    ...
    assert failures == []
    assert checks["future_command_uses_fresh_stage9698_namespace"] is True
