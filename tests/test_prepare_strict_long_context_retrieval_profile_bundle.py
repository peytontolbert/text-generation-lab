...
from prepare_strict_long_context_retrieval_profile_bundle import prepare_strict_long_context_retrieval_profile_bundle  # noqa: E402


def test_prepare_strict_long_context_retrieval_profile_bundle_builds_default_train_profiles(tmp_path: Path) -> None:
    ...
    assert bundle['selected_profiles'] == ['pure_grounded', 'mixed_grounded_long_join']
    assert bundle['trainer_default_profiles'] == ['pure_grounded', 'mixed_grounded_long_join']
    assert 'heuristic_residual' not in bundle['selected_profiles']
    assert set(bundle['default_sampled_manifests']) == {'pure_grounded', 'mixed_grounded_long_join'}
    for profile in bundle['selected_profiles']:
        assert Path(bundle['default_sampled_manifests'][profile]).is_file()


def test_prepare_strict_long_context_retrieval_profile_bundle_can_resolve_from_bundle_card(tmp_path: Path) -> None:
    dataset_dir = tmp_path / 'retrieval_datasets'
    dataset_dir.mkdir(parents=True)
    rows = {
        'strict_long_context_retriever_rows.jsonl': [
    ...
    result = prepare_strict_long_context_retrieval_profile_bundle(
    ...
    assert result['selected_profiles'] == ['pure_grounded']
