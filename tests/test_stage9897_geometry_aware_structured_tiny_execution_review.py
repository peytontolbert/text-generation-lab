...
def _load():
    ...
    path = root / "scripts/build_stage9897_geometry_aware_structured_tiny_execution_review.py"
    spec = importlib.util.spec_from_file_location("stage9897", path)
...
def test_ticket_stays_inactive_and_requires_followup_contract():
    ...
    assert "fresh_stage9898_capped_contract_only_preflight_passed" in ticket["required_before_execution"]
