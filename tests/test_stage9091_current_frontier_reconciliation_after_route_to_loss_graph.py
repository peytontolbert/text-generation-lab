...
from scripts.build_stage9091_current_frontier_reconciliation_after_route_to_loss_graph import (  # noqa: E402
    AUTHORITY_CLOSED,
...
def registry(latest: int = 9090) -> dict[str, object]:
    return {"metrics": {"latest_stage": latest, "authority_counts": {key: 0 for key in AUTHORITY_CLOSED}}}


def test_stage9091_reconciles_route_to_loss_graph_chain() -> None:
    card = build_card(registry())
    assert card["checks"]["source_stage9088_passed"] is True
    assert card["checks"]["source_stage9089_passed"] is True
    assert card["checks"]["source_stage9090_passed"] is True
    assert card["metrics"]["stage9090_added_nodes"] >= 6
    assert card["metrics"]["stage9090_added_edges"] >= 16
    assert len(RECOVERED_RECENT_CONTROLS) >= 8


def test_stage9091_keeps_translation_model_rows_and_training_closed() -> None:
    card = build_card(registry())
...
def test_stage9091_validation_rejects_open_translation_or_bad_frontier() -> None:
    card = build_card(registry())
