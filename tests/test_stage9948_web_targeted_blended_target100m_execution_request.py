...
def _load():
    ...
    path = root / "scripts/build_stage9948_web_targeted_blended_target100m_execution_request.py"
    spec = importlib.util.spec_from_file_location("stage9948", path)
...
def test_build_request_preserves_web_edit_localization_counts():
    mod = _load()
    ...
    assert edit_row["required_contract_invariants"]["source_stage9946_contract_passed"] is True
    assert edit_row["required_contract_invariants"]["source_stage9946_model_execution_attempted"] is False
...
def test_surface_request_extracts_output_dir_and_run_id():
    ...
    req = mod._surface_request(
        ...
        command=[
            ...
            "--output-dir",
            "runs/local/artifacts/stage9948_symbol_binding/probe",
            "--run-id",
            "stage9948_symbol_binding_probe",
        ],
        stage9946_row={
    ...
    assert req["output_dir"] == "runs/local/artifacts/stage9948_symbol_binding/probe"
    assert req["run_id"] == "stage9948_symbol_binding_probe"
