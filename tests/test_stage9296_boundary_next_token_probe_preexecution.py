...
def _load():
    ...
    path = root / "scripts/build_stage9296_boundary_next_token_probe_preexecution.py"
    spec = importlib.util.spec_from_file_location("stage9296", path)
...
def test_stage9296_prepares_boundary_probe_command():
    mod = _load()
    card = mod.audit()
    assert card["passed"] is True
    assert card["command_ready"] is True
    assert card["will_execute_now"] is False
    assert card["authorized_next_stage"] == 9297
