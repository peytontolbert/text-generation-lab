...
from scripts.build_stage9187_trainer_setup_no_trainer_emit_execution_authorization_design import (  # noqa: E402
    NEGATIVE_CASES,
    build_design,
    registry,
    run_negative_cases,
...
def test_stage9187_design_passes() -> None:
    design = build_design(registry())
    assert validate_design(design) == []


def test_stage9187_rejects_negative_cases() -> None:
    negatives = run_negative_cases()
    assert set(NEGATIVE_CASES) == set(negatives)
    assert all(item["rejected"] for item in negatives.values())


def test_stage9187_keeps_execution_closed() -> None:
    design = build_design(registry())
    metrics = design["metrics"]
    assert metrics["authorization_open_now"] is False
