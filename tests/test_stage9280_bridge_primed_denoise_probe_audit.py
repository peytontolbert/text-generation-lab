...
def _load():
    ...
    path = root / "scripts/build_stage9280_bridge_primed_denoise_probe_audit.py"
    spec = importlib.util.spec_from_file_location("stage9280", path)
...
def test_stage9280_audit_records_bridge_priming_regression():
    mod = _load()
    audit = mod.audit_run()
...
def test_stage9280_keeps_forbidden_surfaces_closed():
    mod = _load()
    audit = mod.audit_run()
