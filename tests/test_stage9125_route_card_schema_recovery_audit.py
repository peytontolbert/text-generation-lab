...
from scripts.build_stage9125_route_card_schema_recovery_audit import (  # noqa: E402
    build_audit,
    run_negative_cases,
)


def test_stage9125_base_audit_passes() -> None:
    audit = build_audit()
    ...
    assert audit["checks"]["required_route_fields_recorded"] is True
    assert audit["checks"]["route_enum_recorded"] is True
    assert audit["checks"]["no_route_materialization"] is True
    assert audit["checks"]["authority_closed"] is True


def test_stage9125_rejects_negative_cases() -> None:
    ...
    assert "missing_route_field:loss_mask_ref" in negatives["missing_loss_mask_ref"]["failures"]
    assert "missing_route_enum:KEEP_BOUNDED_DECODER" in negatives["missing_keep_bounded_decoder_route"]["failures"]
...
def test_stage9125_does_not_materialize_or_execute() -> None:
    audit = build_audit()
