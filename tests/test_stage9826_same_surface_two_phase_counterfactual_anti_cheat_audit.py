...
def _load():
    ...
    path = root / "scripts/build_stage9826_same_surface_two_phase_counterfactual_anti_cheat_audit.py"
    spec = importlib.util.spec_from_file_location("stage9826", path)
...
def test_stage9826_builds_same_surface_two_phase_cards_with_stubbed_probes():
    mod = _load()
    ...
    assert built["metrics"]["wins_100m"] == 4

    python_card = next(row for row in built["records"] if row["language_family"] == "python")
    assert python_card["comparison_summary"]["phase2_strict_exact_100m"] == 0.4
    ...
    assert python_card["continuation_in_memory"] is True
