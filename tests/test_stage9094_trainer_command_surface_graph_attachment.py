...
from scripts.build_stage9094_trainer_command_surface_graph_attachment import (  # noqa: E402
    AUTHORITY_CLOSED,
    NEW_EDGES,
    NEW_NODES,
    build_card,
...
def registry(latest: int = 9093) -> dict[str, object]:
    return {"metrics": {"latest_stage": latest, "authority_counts": {key: 0 for key in AUTHORITY_CLOSED}}}


def test_stage9094_adds_trainer_command_graph_controls() -> None:
    ...
    assert "contract:route_to_trainer_loss_translation_no_data_v1" in node_ids


def test_stage9094_keeps_trainer_invocation_closed() -> None:
    card = build_card(registry())
...
def test_stage9094_validation_rejects_open_trainer_or_bad_frontier() -> None:
    card = build_card(registry())
    no_graph = dict(card)
    no_graph.pop("graph")
